// ==================== 纯时间驱动对战系统【修复版：修复GS初始化 + 阶段判定错乱 + 修复弹窗被覆盖 + 防止刷新回弹 + 商店数据同步】 ====================
window.YYCardBattle = (function() {
    ...
    // ====== 修复：弹窗打开时不刷新UI + 商店刷新时不覆盖 ======
    function safeRefreshUI() {
        // ★ 商店正在异步刷新时，跳过 UI 刷新，防止旧数据覆盖乐观更新
    ...
    async function refreshGameState() {
        ...
        if (newState) {
            ...
            if (currentUserId && newState.players[currentUserId] && gameState?.players?.[currentUserId]) {
                ...
                myOld.shopCards = myNew.shopCards;   // ★ 同步商店数据
    ...
    // ★ 修复：添加 shopCards、board、hand 的合并
        ...
        for (const [pid, newPlayer] of Object.entries(newPlayers)) {
            if (pid === currentUserId) {
                result[pid] = {
                    ...
                    shopCards: newPlayer.shopCards,   // ★ 加上
                    board: newPlayer.board,           // ★ 加上
                    hand: newPlayer.hand,             // ★ 加上
...
console.log('✅ battle.js 已修复：弹窗打开时不再被UI刷新覆盖');
