...
window.YYCardMatchmaking = (function() {
    ...
    // ✅ 【双缓冲v3 + 装备槽初始化】初始化游戏，生成两组商店卡牌（每组6张）
    async function initializeGame(roomId, players) {
        log('🎮 开始初始化游戏状态（双缓冲商店 v3 + 装备槽）...');
        try {
            ...
            for (const p of players) {
                ...
                // 【核心修改】：初始化board时强制给每张角色卡添加 weapon/item1/item2
                const rawBoard = deck.slice(0, 3).concat(new Array(3).fill(null)).slice(0, 6);
                const boardWithEquipment = rawBoard.map(card => {
                    if (card && (card.cardId || card.card_id)) {
                        return {
                            ...card,
                            weapon: null,
                            item1: null,
...
console.log('✅ matchmaking.js 加载完成（终极修复版 + 双缓冲商店v3初始化 + 装备槽初始化）');
