...
window.YYCardCheckin = (function() {
  ...
  // ★ 核心修改：使用 user.id 查询签到状态，不再依赖 telegram_id
  async function fetchStatus() {
    const user = auth.currentUser;
    if (!user) {
      state = { canCheckin: false, totalPoints: 0, waitHours: 0, lastTime: null };
      updateBtn();
      return;
    }

    // 优先通过 user_id 查询，否则回退到电报或谷歌邮箱
    ...
    if (!checkin) {
      // 回退：电报用户可能还没有 user_id 填充
      const profile = auth.currentProfile;
      if (profile?.telegram_id) {
        ({ data: checkin } = await supabase
          .from('user_checkins')
          .select('last_checkin_time, total_points')
  ...
  function updateBtn() {
    ...
    const isLoggedIn = !!auth.currentUser;  // 只要有登录用户即可
  ...
  function openModal() {
    ...
    if (!isLoggedIn) {
      // 理论上不会进入这里，因为按钮只对登录用户显示，但保留保护
      modal.innerHTML = `
        <div class="checkin-box notice-box">
          <p>${t('modal_close')}</p>
          <button id="modal-close" class="secondary-btn">${t('modal_close')}</button>
        </div>
      `;
