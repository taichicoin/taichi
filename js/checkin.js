...
window.YYCardCheckin = (function() {
  ...
  function injectStyles() {
    ...
    s.textContent = `
      ...
      .bronze-frame {
        position: relative; width: 120px; height: 74px; overflow: hidden;
        background: linear-gradient(180deg, #665239, #3f3223 45%, #241d16);
        border: 2px solid #9a7642;
        clip-path: polygon(10% 0, 90% 0, 100% 12%, 100% 88%, 90% 100%, 10% 100%, 0 88%, 0 12%);
        box-shadow: inset 0 2px rgba(255,255,255,.18), inset 0 -2px rgba(0,0,0,.45), 0 6px 14px rgba(0,0,0,.45);
        transition: .18s;
      }
      .yy-bronze-checkin:hover .bronze-frame {
        transform: translateY(-2px);
      }
      .yy-bronze-checkin:active .bronze-frame {
        transform: scale(.96);
      }
      .bronze-top {
        height: 10px;
        background: linear-gradient(90deg, #6d5639, #d0a866, #6d5639);
      }
      .bronze-title {
        margin-top: 8px; text-align: center; color: #f3df9a; font-size: 13px; font-weight: 700;
        letter-spacing: 1px; text-shadow: 0 1px 2px #000;
      }
      .bronze-status {
        margin-top: 6px; text-align: center; color: white; font-size: 18px; font-weight: 800;
        text-shadow: 0 2px 5px black;
      }
      .bronze-glow {
        position: absolute; left: 50%; top: 50%; width: 120px; height: 120px;
        transform: translate(-50%, -50%);
        background: radial-gradient(rgba(255,210,90,.25), transparent 70%);
        animation: bronzeGlow 2.5s infinite; pointer-events: none;
      }
      .bronze-frame.can::after {
        content: ""; position: absolute; right: 8px; top: 8px;
        width: 11px; height: 11px; border-radius: 50%; background: #ff3b30;
        box-shadow: 0 0 10px #ff3b30; animation: redPulse 1s infinite;
      }
      @keyframes bronzeGlow {
        0% { opacity: .4; transform: translate(-50%, -50%) scale(.8); }
        50% { opacity: 1; transform: translate(-50%, -50%) scale(1.15); }
        100% { opacity: .4; transform: translate(-50%, -50%) scale(.8); }
      }
      @keyframes redPulse {
        0% { transform: scale(1); }
  ...
  function createUI() {
    ...
    const tryAppend = () => {
      ...
      if (container && !checkinBtn) {
        ...
        checkinBtn.className = 'yy-bronze-checkin';
        checkinBtn.innerHTML = `
          <div class="bronze-frame">
            <div class="bronze-top"></div>
            <div class="bronze-title"> ☀ 每日签到 </div>
            <div class="bronze-status"> 可领取 </div>
  ...
  function updateBtn() {
    ...
    const status = checkinBtn.querySelector(".bronze-status");
    const frame = checkinBtn.querySelector(".bronze-frame");
    if (!status || !frame) return;

    frame.classList.remove("can");
    if (!hasTelegram) {
      status.textContent = "签到";
    } else if (state.canCheckin) {
      status.textContent = "领取";
      frame.classList.add("can");
