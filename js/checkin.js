...
window.YYCardCheckin = (function() {
  ...
  const dict = {
    // 按钮
    btn_title:      { zh: '每日签到', en: 'Check-in' },
    btn_status_default: { zh: '签到', en: 'Check-in' },
    btn_status_can: { zh: '领取', en: 'Claim' },
    btn_status_done:{ zh: '完成', en: 'Done' },
    btn_status_h:   { zh: 'h', en: 'h' },  // 小时缩写，中英通用

    // 弹窗 - 无 Telegram 提示
    modal_no_tg_title: { zh: '签到', en: 'Check-in' },
    modal_no_tg_desc:  { zh: '请前往 Telegram 中启动并使用此功能', en: 'Please open in Telegram to use this feature' },
    modal_no_tg_close: { zh: '关闭', en: 'Close' },

    // 弹窗 - 正常
    modal_title:    { zh: '每日签到', en: 'Daily Check-in' },
    modal_status_label: { zh: '签到状态', en: 'Status' },
    modal_status_can:   { zh: '可签到', en: 'Available' },
    modal_status_cd:    { zh: '冷却中', en: 'Cooldown' },
    modal_last_label:   { zh: '上次签到', en: 'Last Check-in' },
    modal_btn_can:      { zh: '立即签到', en: 'Check-in Now' },
    modal_btn_done:     { zh: '已签到', en: 'Checked In' },
    modal_close:    { zh: '关闭', en: 'Close' },

    // 操作提示
    err_login_expired:  { zh: '登录过期', en: 'Login expired' },
    err_checkin_fail:   { zh: '签到失败', en: 'Check-in failed' },
    err_network:        { zh: '网络错误：', en: 'Network error: ' },
    checking_in:        { zh: '签到中...', en: 'Checking in...' },
  };
      ...
      if (container && !checkinBtn) {
        ...
        checkinBtn.innerHTML = `
          ...
          <div class="token-content">
            <div class="token-title">${t('btn_title')}</div>
            <div class="token-status">${t('btn_status_default')}</div>
          </div>
        `;
  ...
  function updateBtn() {
    ...
    const statusEl = checkinBtn.querySelector(".token-status");
    const titleEl = checkinBtn.querySelector(".token-title");
    if (titleEl) titleEl.textContent = t('btn_title');
    checkinBtn.classList.remove("can", "done", "cooldown");

    if (!hasTelegram) {
      statusEl.textContent = t('btn_status_default');
    } else if (state.canCheckin) {
      statusEl.textContent = t('btn_status_can');
      checkinBtn.classList.add("can");
    } else if (state.waitHours > 0) {
      statusEl.textContent = state.waitHours + t('btn_status_h');
      checkinBtn.classList.add("cooldown");
    } else {
      statusEl.textContent = t('btn_status_done');
  ...
  function openModal() {
    ...
    if (!hasTelegram) {
      modal.innerHTML = `
        <div class="checkin-box notice-box">
          <h3 style="color:#fff; letter-spacing:1px; margin:0 0 16px;">📅 ${t('modal_no_tg_title')}</h3>
          <p>${t('modal_no_tg_desc')}</p>
          <button id="modal-close" class="secondary-btn">${t('modal_no_tg_close')}</button>
        </div>
      `;
    ...
    const lastStr = state.lastTime ? new Date(state.lastTime).toLocaleString(getLang() === 'en' ? 'en-US' : 'zh-CN', { hour12: false }) : '--';
    const statusText = state.canCheckin ? t('modal_status_can') : t('modal_status_cd');
    const statusClass = state.canCheckin ? 'status-green' : 'status-gray';
    const btnText = state.canCheckin ? t('modal_btn_can') : t('modal_btn_done');
    ...
    if (state.canCheckin) {
      document.getElementById('modal-do').onclick = async () => {
        ...
        if (!session) return alert(t('err_login_expired'));
        const btn = document.getElementById('modal-do');
        btn.disabled = true;
        btn.textContent = t('checking_in');
        try {
          ...
          if (data.success) {
            ...
            alert(data.message || t('err_checkin_fail'));
            btn.disabled = false;
            btn.textContent = t('modal_btn_can');
          }
        } catch (err) {
