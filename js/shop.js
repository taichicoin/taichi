...
window.YYCardShop = (function() {
    ...
    async function handleHandToBoard(handIdx, boardIdx) {
        ...
        if (result.data.updatedPlayer) {
            const upd = result.data.updatedPlayer;
            if (upd.gold !== undefined) my.gold = upd.gold;
            if (upd.exp !== undefined) my.exp = upd.exp;
            if (upd.shopLevel !== undefined) my.shopLevel = upd.shopLevel;
            if (upd.health !== undefined) my.health = upd.health;
            updateBuyExpButtonState();
            document.getElementById('my-gold').textContent = my.gold;
            document.getElementById('my-health').textContent = my.health;
        }
    ...
    async function handleBoardToHand(boardIdx) {
        ...
        if (moveResult.data.updatedPlayer) {
            const upd = moveResult.data.updatedPlayer;
            if (upd.gold !== undefined) my.gold = upd.gold;
            if (upd.exp !== undefined) my.exp = upd.exp;
            if (upd.shopLevel !== undefined) my.shopLevel = upd.shopLevel;
            if (upd.health !== undefined) my.health = upd.health;
        ...
        if (type === 'board') {
            ...
            if (!sellResult.success) {
                ...
                if (sellResult.data.updatedPlayer) {
                    const upd = sellResult.data.updatedPlayer;
                    if (upd.gold !== undefined) my.gold = upd.gold;
                    if (upd.exp !== undefined) my.exp = upd.exp;
                    if (upd.shopLevel !== undefined) my.shopLevel = upd.shopLevel;
                    if (upd.health !== undefined) my.health = upd.health;
    ...
    return {
        ...
        get isDragging() { return isDragging; }   // ★ 暴露拖拽状态
    };
})();
