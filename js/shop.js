// ==================== 商店与交互系统（手动模拟拖拽版：触碰即拿起，支持商店→手牌 + 缓冲期） ====================
window.YYCardShop = (function() {
    ...
    function renderEnemyBoard() {
        ...
        if (gameState.phase === 'battle' && gameState.battlePairs) {
            ...
            if (oppId) {
                console.log(`[renderEnemyBoard] 从 battlePairs 找到对手: ${oppId}`);
            } else {
                console.warn(`[renderEnemyBoard] battlePairs 存在但未找到当前玩家的对手`);
            }
        }
        ...
        if (!oppId) {
            ...
            if (aliveHumans.length > 0) {
                oppId = aliveHumans[0][0];
                console.log(`[renderEnemyBoard] 回退到第一个存活真人: ${oppId}`);
            }
        }
        ...
        if (!oppId) {
            ...
            if (aliveAny) {
                oppId = aliveAny[0];
                console.log(`[renderEnemyBoard] 回退到第一个存活玩家（含人机）: ${oppId}`);
            }
        }
        ...
        if (!oppId) {
            ...
            console.log(`[renderEnemyBoard] 最终回退到第一个非己玩家: ${oppId}`);
        }
        
        if (oppId && gameState.players[oppId]) {
            renderBoard('enemy-board', gameState.players[oppId].board, false);
            // 调试日志：打印对手棋盘摘要
            const boardSummary = gameState.players[oppId].board.map((c, i) => c ? `${c.name}(${i+1})` : `空(${i+1})`).join(' ');
            console.log(`[renderEnemyBoard] 对手 ${oppId} 棋盘: ${boardSummary}`);
        } else {
            console.error(`[renderEnemyBoard] 无法找到有效对手`);
        }
    }
    ...
    function init() {
        ...
        bindUIEvents();
        refreshAllUI();
        log('✅ 商店交互模块已启动（手动模拟拖拽版，支持商店→手牌 + 缓冲期）');
    }
...
console.log('✅ shop.js 加载完成（手动模拟拖拽版 + 战斗对手精确匹配 + 缓冲期）');
