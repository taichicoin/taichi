...
window.YYCardShop = (function() {
    ...
    function toast(message, isError = false, duration = 2000) {
        ...
        toastTimer = setTimeout(() => { toastEl.remove(); toastTimer = null; }, duration);
    }
    ...
    function renderShop() {
        ...
        container.innerHTML = hasCards ? '' : '<div style="color:#aaa;">商店暂无卡牌</div>';
        if (hasCards) container.appendChild(fragment);
    }
    ...
    async function handleUnequip(boardIdx, slotKey) {
        ...
        if (emptyIdx === -1) { toast('手牌已满，无法卸下', true); return; }
    ...
    async function unequipAllIfNeeded(boardIdx) {
        ...
        for (const slot of slotsToRemove) await handleUnequip(boardIdx, slot);
        return true;
    }
    ...
    async function handleEquipFromHand(handIdx, boardIdx) {
        ...
        const userId = getCurrentUserId();
        const roomId = getCurrentRoomId();
        const my = getGameState()?.players[userId];
        if (!my) return;
        ...
        if (!targetCard || targetCard.type === 'weapon' || targetCard.type === 'item') {
            toast('只能装备到角色身上', true); return;
        }

        if (equipCard.type === 'weapon' && targetCard.weapon) { toast('该角色已装备武器', true); return; }
        if (equipCard.type === 'item' && targetCard.item1 && targetCard.item2) { toast('该角色已装备2个道具', true); return; }

        const slotKey = equipCard.type === 'weapon' ? 'weapon' : (!targetCard.item1 ? 'item1' : 'item2');
        const equipData = {
            card_id: equipCard.card_id || equipCard.cardId,
            name: equipCard.name, type: equipCard.type,
            atk: equipCard.base_atk ?? equipCard.atk ?? 0,
            hp: equipCard.base_hp ?? equipCard.hp ?? 0,
            image: equipCard.image, rarity: equipCard.rarity || 'Common'
        };
    ...
    async function handleEquipFromShop(card, shopIdx, boardIdx) {
        ...
        if (card.type === 'weapon' && targetCard.weapon) { toast('该角色已装备武器', true); return; }
        if (card.type === 'item' && targetCard.item1 && targetCard.item2) { toast('该角色已装备2个道具', true); return; }

        const slotKey = card.type === 'weapon' ? 'weapon' : (!targetCard.item1 ? 'item1' : 'item2');
        const equipData = {
            card_id: card.card_id || card.cardId, name: card.name, type: card.type,
            atk: card.base_atk ?? card.atk ?? 0, hp: card.base_hp ?? card.hp ?? 0,
            image: card.image, rarity: card.rarity || 'Common'
        };
        ...
        targetCard[slotKey] = equipData;
        const shop = my.shopCards;
        shop.buffer[shop.active ?? 0][shopIdx] = null;
        renderMyBoard(); renderShop();

        const result = await invokeFunction(FUNCTION_NAME_MAP.EQUIP_ITEM, {
            roomId, userId, shopIndex: shopIdx, boardIndex: boardIdx, slotKey, equipData, price
        });
        if (!result.success) {
            my.gold = oldGold; my.board = oldBoard; my.shopCards = oldShopRaw;
            renderMyBoard(); renderShop();
            return;
        }
        if (result.data.updatedPlayer) {
    ...
    function onDragStart(e, type, card, index, element) {
        ...
        dragState = { active: true, type, card, index, sourceElement: element, cloneElement: clone,
    ...
    function onDragEnd(e) {
        ...
        if (dropResult) executeDropAction(type, index, card, dropResult);
    }
        ...
        while (el && el !== document.body) {
            if (el.classList.contains('card-slot')) {
                ...
                if (board?.id === 'my-board') {
                    ...
                    if (slotIndex !== null) {
                        const gameState = getGameState();
                        const boardCard = gameState?.players[getCurrentUserId()]?.board[parseInt(slotIndex)];
    ...
    async function executeDropAction(type, index, card, dropResult) {
        ...
        if (type === 'hand' && (card.type === 'weapon' || card.type === 'item')) {
            if (dropResult.zone === 'board' && dropResult.hasCharacter) await handleEquipFromHand(index, dropResult.index);
            else toast('武器/道具只能装备到角色身上', true);
            return;
        }
        if (type === 'shop' && (card.type === 'weapon' || card.type === 'item')) {
            if (dropResult.zone === 'board' && dropResult.hasCharacter) await handleEquipFromShop(card, index, dropResult.index);
            else if (dropResult.zone === 'hand') await handleShopToHand(card, index);
            else toast('武器/道具可拖到手牌购买或直接装备到角色', true);
            return;
        }
    ...
    async function handleBoardToHand(boardIdx) {
        ...
        if (!my) return;
        if (!await unequipAllIfNeeded(boardIdx)) return;
        ...
        if (!result.success) { my.board = oldBoard; my.hand = oldHand; renderMyBoard(); renderHand(); return; }
        if (result.data.updatedPlayer) { mergeUpdatedPlayer(my, result.data.updatedPlayer); updateUIAfterSuccess(result.data.updatedPlayer); }
    }
    ...
    async function handleSell(type, index) {
        ...
        const my = gameState?.players[userId];
        if (!my) return;
        if (type === 'board' && !await unequipAllIfNeeded(index)) return;
        ...
        if (!result.success) {
            ...
            renderMyBoard(); renderHand(); return;
        }
        if (result.data.updatedPlayer) { mergeUpdatedPlayer(my, result.data.updatedPlayer); updateUIAfterSuccess(result.data.updatedPlayer); }
    }
    ...
    async function handleShopToBoard(card, shopIdx, boardIdx) {
        ...
        const oldShopRaw = JSON.parse(JSON.stringify(my.shopCards));
        const oldGold = my.gold, oldBoard = [...my.board];
        const targetCard = my.board[boardIdx];
        if (isValidCard(targetCard) && getValidHandCount(my.hand) >= 15) { toast('手牌已满', true); return; }

        my.gold -= price;
        const newCard = { ...card, instanceId: Date.now() + '-' + Math.random(),
            cardId: card.cardId || card.card_id || '', card_id: card.card_id || card.cardId || '',
            faction: card.faction || '', weapon: null, item1: null, item2: null };
        my.board[boardIdx] = newCard;
        if (isValidCard(targetCard)) { const idx = getFirstAvailableHandSlot(my.hand); if (idx !== -1) my.hand[idx] = targetCard; }
        my.shopCards.buffer[my.shopCards.active ?? 0][shopIdx] = null;
        renderMyBoard(); renderHand(); renderShop();

        const result = await invokeFunction(FUNCTION_NAME_MAP.BUY_CARD, { roomId, userId, shopIndex: shopIdx, targetBoardIndex: boardIdx });
        if (!result.success) { my.gold = oldGold; my.board = oldBoard; my.shopCards = oldShopRaw; renderMyBoard(); renderHand(); renderShop(); return; }
        if (result.data.updatedPlayer) { mergeUpdatedPlayer(my, result.data.updatedPlayer); updateUIAfterSuccess(result.data.updatedPlayer); }
    }
    ...
    async function handleShopToHand(card, shopIdx) {
        ...
        const result = await invokeFunction(FUNCTION_NAME_MAP.BUY_CARD, { roomId, userId, shopIndex: shopIdx });
        if (!result.success) { my.gold = oldGold; my.shopCards = oldShopRaw; my.hand = oldHand; renderHand(); renderShop(); return; }
        if (result.data.updatedPlayer) { mergeUpdatedPlayer(my, result.data.updatedPlayer); updateUIAfterSuccess(result.data.updatedPlayer); }
    }
    ...
    async function buyExpAction() {
        ...
        const roomId = getCurrentRoomId();
        const my = getGameState()?.players[userId];
        if (!my || my.shopLevel >= (config.MAX_SHOP_LEVEL || 5) || my.gold < 1) return;
        const oldGold = my.gold;
        my.gold -= 1;
    ...
    function updateBuyExpButtonState() {
        ...
        const shouldDisable = !canOperate() || isMaxLevel;
        let expNeeded = 0;
        if (!isMaxLevel) {
            const exp = my.exp;
    ...
    function init() {
        ...
        bindUIEvents();
        refreshAllUI();
        console.log('✅ 商店系统 (含装备/卸下 + 计时器) 已启动');
    }
