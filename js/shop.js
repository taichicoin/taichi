...
window.YYCardShop = (function() {
    ...
    function renderMyBoard() {
        ...
        const slots = document.querySelectorAll('#my-board .card-slot');
        slots.forEach((slot, index) => {
            ...
            // ★ 已移除 cloneNode 和 replaceChild，直接给当前生成的 el 绑定事件！
            el.addEventListener('pointerdown', (e) => {
                // ★ 恢复音效触发 (业务层负责)
                if (card.type === 'loot') {
                    try { new Audio('/assets/mp3/use.mp3').volume = 0.8; new Audio('/assets/mp3/use.mp3').play(); } catch(e){}
                } else {
                    try { new Audio('/assets/mp3/grab.mp3').volume = 0.6; new Audio('/assets/mp3/grab.mp3').play(); } catch(e){}
                }
                window.YYCardDrag.onDragStart(
                    e, 'board', card, index, el,
                    canOperate, executeDropAction
                );
            });
        });
    }
    ...
    function renderHand() {
        ...
        const cardEls = container.querySelectorAll('.card[data-hand-index]');
        cardEls.forEach(el => {
            const index = parseInt(el.getAttribute('data-hand-index'));
    ...
    function renderShop() {
        ...
        const cardEls = container.querySelectorAll('.card[data-shop-index]');
        cardEls.forEach(el => {
            const index = parseInt(el.getAttribute('data-shop-index'));
            const active = my.shopCards?.active ?? 0;
            const card = my.shopCards?.buffer?.[active]?.[index];
            if (!card || card.type === 'consumable' || card.isConsumable) return;
    ...
    async function init() {
        ...
        console.log('✅ 商店系统 (核心) 已启动，渲染/交互已剥离，事件已重新挂载');
    }
