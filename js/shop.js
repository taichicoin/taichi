...
window.YYCardShop = (function() {
    ...
    function mergeUpdatedPlayer(target, updatedPlayer) {
        ...
        // ★ 关键修改：fields 数组加入 'shopBonus'
        const fields = ['gold', 'exp', 'shopLevel', 'health', 'shopCards', 'shopBonus', 'isBot', 'isEliminated', 'isReady', 'hand', 'board'];
        fields.forEach(key => {
    ...
    function updateUIAfterSuccess(updatedPlayer) {
        ...
        if (updatedPlayer.exp !== undefined || updatedPlayer.shopLevel !== undefined) {
            render.updateBuyExpButtonState();
        }

        if (!drag.isDragging && !window._consumableDragging) {
            render.renderShop();
            render.renderHand();
            render.renderMyBoard();
        }
    ...
    async function useLootOnCharacter(handIdx, boardIdx) {
        ...
        try {
            ...
            render.renderHand();
            render.renderMyBoard();
            const result = await callUseLootFunction({ roomId, userId, boardIndex: boardIdx, handIndex: handIdx });
            if (!result.success) {
                my.hand = oldHand;
                my.board = oldBoard;
                render.renderHand();
    ...
    const handleSell = window.YYCardSellCard?.sellCard || function(){};
    ...
    async function init() {
        ...
        if (window.YYCardShopRefresh) {
            window.YYCardShopRefresh.init({
                ...
                renderShop: render.renderShop,
                renderHand: render.renderHand,
                renderMyBoard: render.renderMyBoard,
                updateBuyExpButtonState: render.updateBuyExpButtonState,
                getGameState,
                getCurrentUserId,
                getCurrentRoomId,
                toast,
        ...
        console.log('✅ 商店系统 (核心) 已启动，卖卡/升级功能已独立');
    }

    return {
        init,
        refreshAllUI: () => { render.refreshAllUI(); },
        renderMyBoard: () => { render.renderMyBoard(); },
        renderHand: () => { render.renderHand(); },
        handleUnequip,
        toast,
