...
window.YYCardShop = (function() {
    ...
    async function unequipAllIfNeeded(boardIdx) {
        ...
        if (emptySlots < slots.length) {
            toast(`手牌空间不足，无法卸下装备（需${slots.length}空位）`, true);
            return false;
        }
        for (const s of slots) await handleUnequip(boardIdx, s);
        return true;
    }
    ...
    async function executeDropAction(type, index, card, dropResult) {
        ...
        if (type === 'hand' && (card.type === 'weapon' || card.type === 'item')) {
            ...
            else toast('武器/道具只能装备到角色身上，或拖到商店出售', true);
            return;
        }
        // 商店武器/道具
    ...
    async function handleBoardToHand(boardIdx) {
        ...
        const emptyIdx = getFirstAvailableHandSlot(my.hand);
        if (emptyIdx !== -1) my.hand[emptyIdx] = card; else my.hand.push(card);
        renderMyBoard(); renderHand();
    ...
    async function handleSell(type, index) {
        ...
        const my = gameState?.players[userId];
        if (!my) return;
        if (type === 'board') { if (!await unequipAllIfNeeded(index)) return; }
        ...
        if (type === 'hand') my.hand[index] = null; else my.board[index] = null;
        my.gold += price;
        renderMyBoard(); renderHand();
        const result = await invokeFunction(FUNCTION_NAME_MAP.SELL_CARD, { roomId, userId, type, index });
        if (!result.success) { my.gold = oldGold; my.hand = oldHand; my.board = oldBoard; renderMyBoard(); renderHand(); return; }
        if (result.data.updatedPlayer) { mergeUpdatedPlayer(my, result.data.updatedPlayer); updateUIAfterSuccess(result.data.updatedPlayer); }
    }
    ...
    async function handleShopToBoard(card, shopIdx, boardIdx) {
        ...
        const shop = my.shopCards, group = shop.buffer[shop.active ?? 0];
        group[shopIdx] = null;
    ...
    function init() {
        ...
        refreshAllUI();
        console.log('✅ 商店系统 (原版刷新+装备/拖拽) 已启动');
    }
