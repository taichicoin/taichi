...
window.YYCardShop = (function() {
    ...
    function initDebugPanel() {
        ...
        p.style.cssText = `
            ...
            color:#0f0; font-size:11px; padding:4px 8px; z-index:100000;
    ...
    function renderEnemyBoard() {
        ...
        if (!gameState) return;
        const userId = getCurrentUserId();
        // 找到对手ID（房间中除自己外的第一个真人或人机）
        const oppId = Object.keys(gameState.players).find(id => id !== userId);
        if (oppId) {
            const enemy = gameState.players[oppId];
            // 直接使用敌人当前的 board 渲染
            renderBoard('enemy-board', enemy.board, false);
        }
    }
    ...
    function renderHand() {
        ...
        container.addEventListener('drop', (e) => {
            ...
            if (dragState.type === 'board') {
                handleBoardToHand(dragState.index);
            }
        });
    }
    ...
    function renderShop() {
        ...
        container.addEventListener('dragover', (e) => {
            ...
            if (dragState.active && (dragState.type === 'hand' || dragState.type === 'board')) {
                const shopArea = container.closest('.shop-area');
                if (shopArea) shopArea.classList.add('drop-target');
            }
        });
    ...
    function refreshAllUI() {
        renderMyBoard();
        renderEnemyBoard();  // 现在会强制从最新 gameState 读取对手棋盘
        renderHand();
        renderShop();
        const gameState = getGameState();
        if (gameState) {
            ...
            if (isSelf) {
                ...
                slot.addEventListener('drop', (e) => {
                    ...
                    if (dragState.type === 'hand') {
                        handleHandToBoard(dragState.index, targetIndex);
                    } else if (dragState.type === 'shop') {
                        handleShopToBoard(dragState.card, dragState.index, targetIndex);
                    } else if (dragState.type === 'board' && isSelf) {
                        handleBoardSwap(dragState.index, targetIndex);
                    }
                });
            }
    ...
    function onDragStart(e, type, card, index, element) {
        ...
        clone.style.cssText = `
            position:fixed; z-index:99999; left:${clientX - element.offsetWidth/2}px; top:${clientY - element.offsetHeight/2}px;
            width:${element.offsetWidth}px; height:${element.offsetHeight}px; opacity:0.85; transform:scale(1.05);
            box-shadow:0 8px 20px rgba(0,0,0,0.5); pointer-events:none; transition:none;
        `;
    ...
    function getDropTarget(element) {
        ...
        while (el && el !== document.body) {
            if (el.classList.contains('card-slot')) {
                ...
                if (boardId === 'my-board' && slotIndex !== null) return { zone: 'board', index: parseInt(slotIndex) };
            }
    ...
    async function handleHandToBoard(handIdx, boardIdx) {
        ...
        if (success) refreshAllUI(); else toast('放置失败', true);
    }
    ...
    function setPhase(phase) { currentPhase = phase; }
