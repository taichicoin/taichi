// ==================== 商店与交互系统（最终版：toast 提示 + 准备按钮调用 playerReadyAction） ====================
window.YYCardShop = (function() {
    ...
    function bindUIEvents() {
        ...
        // ===== 修改：准备按钮调用 playerReadyAction =====
        const endPrepareBtn = document.getElementById('end-prepare-btn');
        if (endPrepareBtn) {
            endPrepareBtn.addEventListener('click', async () => {
                if (window.YYCardBattle?.playerReadyAction) {
                    await window.YYCardBattle.playerReadyAction();
                } else {
                    toast('准备功能不可用', true);
                }
            });
...
console.log('✅ shop.js 加载完成（toast 提示版 + playerReadyAction）');
