...
window.YYCardShop = (function() {
    ...
    async function handleBoardToHand(boardIdx) {
        ...
        if (card.weapon) equipmentSlots.push({ slot: 'weapon', equip: card.weapon });
        if (card.item1) equipmentSlots.push({ slot: 'item1', equip: card.item1 });
        if (card.item2) equipmentSlots.push({ slot: 'item2', equip: card.item2 });

        const neededSlots = equipmentSlots.length + 1;
        const emptyHandSlots = 15 - getValidHandCount(my.hand);
        if (neededSlots > emptyHandSlots) {
        ...
        for (const eq of equipmentSlots) {
            my.hand[handIdx] = {
        ...
        // 剥离装备字段后将纯角色卡放入手牌
        const pureCard = { ...card };
        delete pureCard.weapon;
        delete pureCard.item1;
        delete pureCard.item2;
        my.board[boardIdx] = null;
        my.hand[handIdx] = pureCard;

        renderMyBoard();
        renderHand();

        // 并发调用后端卸下和移动
        const unequipPromises = equipmentSlots.map(eq => callUnequipFunction({
            roomId, userId, boardIndex: boardIdx, slotKey: eq.slot
        }));
        const movePromise = invokeFunction(FUNCTION_NAME_MAP.BOARD_TO_HAND, { roomId, userId, boardIndex: boardIdx });
        const results = await Promise.all([...unequipPromises, movePromise]);

        let hasFailure = false;
        for (const res of results) {
            if (res && !res.success) { hasFailure = true; break; }
        }
        if (hasFailure) {
            my.board = oldBoard;
            my.hand = oldHand;
        ...
        const moveResult = results[results.length - 1];
    ...
    async function handleSell(type, index) {
        ...
        if (type === 'board') {
            ...
            const neededSlots = equipmentSlots.length;
            const emptyHandSlots = 15 - getValidHandCount(my.hand);
            if (neededSlots > emptyHandSlots) {
            ...
            // 并发调用后端：卸下每个装备 + 出售角色卡
            const unequipPromises = equipmentSlots.map(eq => callUnequipFunction({
                roomId, userId, boardIndex: index, slotKey: eq.slot
            }));
            const sellPromise = invokeFunction(FUNCTION_NAME_MAP.SELL_CARD, { roomId, userId, type: 'board', index });
            const results = await Promise.all([...unequipPromises, sellPromise]);
            ...
            const sellResult = results[results.length - 1];
