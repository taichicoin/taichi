...
window.YYCardShop = (function() {
    ...
    async function refreshShopAction() {
        ...
        if (shopContainer && !shopContainer.querySelector('.refresh-loading-hint')) {
            ...
            loadingHint.style.cssText = 'position:absolute; top:50%; left:50%; transform:translate(-50%,-50%); background:rgba(0,0,0,0.7); color:white; padding:4px 12px; border-radius:20px; font-size:12px; z-index:10; pointer-events:none; white-space:nowrap;';
        ...
        try {
            ...
            // 后端返回成功后，用返回的数据更新本地状态
            if (data.updatedPlayer) {
                // 直接覆盖金币、商店卡牌等（后端已经扣除了金币并生成了新卡牌）
                if (data.updatedPlayer.gold !== undefined) my.gold = data.updatedPlayer.gold;
                if (data.updatedPlayer.shopCards !== undefined) my.shopCards = data.updatedPlayer.shopCards;
                if (data.updatedPlayer.exp !== undefined) my.exp = data.updatedPlayer.exp;
                if (data.updatedPlayer.shopLevel !== undefined) my.shopLevel = data.updatedPlayer.shopLevel;
                if (data.updatedPlayer.health !== undefined) my.health = data.updatedPlayer.health;
            } else if (data.shopCards) {
                // 兼容旧格式
                my.shopCards = data.shopCards;
                if (data.gold !== undefined) my.gold = data.gold;
            } else {
                toast('刷新成功但未获得卡牌数据', true);
                return;
            }
