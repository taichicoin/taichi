...
window.YYCardShop = (function() {
    ...
    // mergeUpdatedPlayer 保持原样，但乐观回调中不用它
    ...
    async function refreshShopAction() {
        ...
        if (canFlip) {
            // ★ 乐观秒切：不加任何锁，直接翻页，后台异步发请求，完全不碰商店数据
            my.gold -= 1;
            document.getElementById('my-gold').textContent = my.gold;
            ...
            // 仍然没有，加锁请求后端，设置 1.5 秒超时解锁（此时才允许更新商店）
