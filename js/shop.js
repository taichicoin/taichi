// ==================== 商店与交互系统（纯拖拽版：出售区=商店区域，修复商店卡牌误判） ====================
window.YYCardShop = (function() {
    ...
    function renderShop() {
        ...
        // 商店区作为出售目标（仅接收手牌或棋盘卡牌）
        container.addEventListener('dragover', (e) => {
            e.preventDefault();
            e.dataTransfer.dropEffect = 'move';
            // 只有手牌或棋盘卡牌拖过时才显示出售高亮
            if (draggedCard && (draggedCard.type === 'hand' || draggedCard.type === 'board')) {
    ...
    function injectDisableStyles() {
        ...
        style.textContent = `
            ...
            .card, .card-slot, .hand-area, .shop-area, .board-area {
                ...
                touch-action: pan-y; /* 保留垂直滚动，但减少点击延迟 */
            }
            /* 加速拖拽响应 */
            .card[draggable="true"] {
                touch-action: none;
