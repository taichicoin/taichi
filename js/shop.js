...
window.YYCardShop = (function() {
    ...
    let toastTimer = null;
    const domCache = {};
    let isRefreshingShop = false;        // 网络请求进行中，但不阻止本地翻页
    let isDragging = false;
    let isOperating = false;             // 任何会修改数据的操作进行中
    ...
    async function refreshShopAction() {
        ...
        // 如果已有网络请求进行中，只做本地翻页（不扣金币），保证按钮永远有反应
        if (isRefreshingShop) {
            performLocalFlip();
            return;
        }
