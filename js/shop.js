...
window.YYCardShop = (function() {
    ...
    function createCardElement(card, cardType = 'board', isBoard = false) {
        ...
        if (card.type === 'consumable' || card.isConsumable) {
            ...
            descEl.textContent = card.name || '消耗牌';   // name 里面存的就是 description
    ...
    function onDragStart(e, type, card, index, element) {
        ...
        // ★ 移除沉重的 card-frame
        const heavyFrame = clone.querySelector('.card-frame');
        if (heavyFrame) heavyFrame.remove();

        // 强制清除所有可能拖慢性能的样式，同时加上简易金色边框
        clone.style.filter = 'none';
        clone.style.clipPath = 'none';
        clone.style.mask = 'none';
        clone.style.webkitMask = 'none';
        clone.style.border = '3px solid #ffd83d';
        clone.style.boxShadow = '0 0 8px rgba(255,210,80,0.6)';
        clone.style.borderRadius = '1.5vmin';
        clone.style.overflow = 'hidden';
        clone.style.background = '#141a2a';
        ...
        clone.style.transform = `translate(${clientX - rect.width / 2}px, ${clientY - rect.height / 2}px)`;
    ...
    function onDragMove(e) {
        ...
        rafId = requestAnimationFrame(() => {
            ...
            if (!clone) return;
            // 直接设置 transform，只触发合成
            clone.style.transform = `translate(${clientX - clone.offsetWidth / 2}px, ${clientY - clone.offsetHeight / 2}px)`;
    ...
    function injectStyles() {
        ...
        style.textContent = `
            ...
            .card-drag-clone { 
                ...
                /* 不设置 will-change，因为移动使用 transform，已自动创建合成层 */
            }
