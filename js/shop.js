...
window.YYCardShop = (function() {
    ...
    async function unequipAllIfNeeded(boardIdx) {
        ...
        if (emptySlots < slotsToRemove.length) {
            toast(`手牌空间不足，无法自动卸下装备（需${slotsToRemove.length}空位）`, true);
            return false;
        }
        ...
        try {
            ...
            if (result.data) {
                if (result.data.updatedPlayer) {
                    ...
                    // 兼容旧版接口：直接用 shopCards 和 gold 回写
                    if (result.data.shopCards !== undefined) {
                        my.shopCards = result.data.shopCards;
                    }
                    if (result.data.gold !== undefined) {
                        my.gold = result.data.gold;
                    }
                    updateUIAfterSuccess({
                        shopCards: result.data.shopCards,
    ...
    function init() {
        ...
        refreshAllUI();
        console.log('✅ 商店系统 (刷新修复 + 装备/拖拽) 已启动');
    }
