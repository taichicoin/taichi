// ==================== 商店与交互系统（极速无锁 + 纯乐观音效 + 拿起/放下/卸下完整版 + 合成保护） ====================
window.YYCardShop = (function() {
    ...
    async function equipFromHand(handIdx, boardIdx) {
        ...
        // 检查手牌中的装备卡是否受保护（一般装备卡不会是合成材料，但以防万一）
        if (isCardInMerge(equipCard)) {
            toast('该卡牌正在参与合成，无法使用', true);
            return false;
        }

        const targetCheck = canEquipTo(boardIdx);   // 已经包含目标卡合并检查
        if (!targetCheck.ok) {
    ...
    async function equipFromShop(shopIdx, boardIdx) {
        ...
        // 商店卡不是合成材料，不检查 isCardInMerge

        const targetCheck = canEquipTo(boardIdx);   // 包含目标检查
        if (!targetCheck.ok) {
            toast(targetCheck.reason, true);
            return false;
        }
    ...
    async function executeDropAction(type, index, dropResult) {
        // 注意：拖拽的卡牌在 dragState.card 中，已经在 onDragStart 中检查了，但还需要检查目标位置卡牌是否受保护
        if (type === 'hand') {
            const gameState = getGameState();
            const userId = getCurrentUserId();
    ...
    async function handleHandToBoard(handIdx, boardIdx) {
        ...
        // 手牌卡合成保护（已在 onDragStart 检查，但仍然保留）
        if (isCardInMerge(card)) {
