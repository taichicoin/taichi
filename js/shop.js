// ==================== 商店与交互系统（最终稳定版，仅 isRefreshing 锁） ====================
window.YYCardShop = (function() {
    ...
    let refreshSeq = 0; // 请求序号，用于丢弃过期的刷新结果
    ...
    async function refreshShopAction() {
        ...
        try {
            ...
            if (result.data.updatedPlayer) {
                const updated = result.data.updatedPlayer;
                // 更新基础属性
                if (updated.gold !== undefined) my.gold = updated.gold;
                if (updated.exp !== undefined) my.exp = updated.exp;
                if (updated.shopLevel !== undefined) my.shopLevel = updated.shopLevel;
                if (updated.health !== undefined) my.health = updated.health;

                // 更新商店数据，但绝不覆盖 local active/subIndex
                if (updated.shopCards) {
                    const s = updated.shopCards;
                    if (s.buffer) my.shopCards.buffer = s.buffer;
                    if (s.next !== undefined) my.shopCards.next = s.next;
                    // 保持 local active/subIndex 不变
                }
            ...
            updateUIAfterSuccess({ gold: oldGold });
            renderShop();
            toast('网络异常，刷新失败', true);
        } finally {
            // 只有最新请求才能解锁，避免旧请求错误释放锁
            if (mySeq === refreshSeq) {
