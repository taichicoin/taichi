...
window.YYCardShop = (function() {
    ...
    function onDragStart(e, type, card, index, element) {
        const gameState = getGameState();
        // ✅ 修复：使用 gameState.phase 而不是模块内部的 currentPhase（避免不同步）
        if (!gameState || gameState.phase !== 'prepare' || isRefreshingShop) {
            toast('现在不能操作', true);
            return;
        }
    ...
    async function buyExpAction() {
        const gameState = getGameState();
        if (gameState?.phase !== 'prepare' || isRefreshingShop) {
            toast('现在不能操作', true);
            return;
        }
        const userId = getCurrentUserId();
        ...
        const my = gameState?.players[userId];
        if (!my || my.shopLevel >= (config.MAX_SHOP_LEVEL || 5)) { toast('已满级', true); return; }
