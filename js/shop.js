...
window.YYCardShop = (function() {
    ...
    // ★ 实时阶段：永远用时间公式，不依赖 gameState.phase
    function getCurrentPhaseFromBattle() {
        const battle = window.YYCardBattle;
        if (battle?.getCurrentPhaseInfo) {
            return battle.getCurrentPhaseInfo().phase;
        }
        // 降级：自己用 gameStartTime 算
        const gs = getGameState();
        if (gs?.gameStartTime) {
            const now = Math.floor(Date.now() / 1000);
            const start = Math.floor(new Date(gs.gameStartTime).getTime() / 1000);
            const elapsed = now - start;
            let round = 1;
            const getPrep = (r) => 27 + (r-1)*10;
            const buf = 3;
            const getBat = (r) => 30 + (r-1)*5;
            while (true) {
                const total = getPrep(round) + buf + getBat(round);
                if (elapsed >= total) {
                    elapsed -= total;
                    round++;
                } else {
                    if (elapsed < getPrep(round)) return 'prepare';
                    else if (elapsed < getPrep(round) + buf) return 'buffering';
                    else return 'battle';
                }
            }
        }
    ...
    function onDragStart(e, type, card, index, element) {
        ...
        clone.style.cssText = `
            ...
            opacity: 0.85;
            transform: translate(${clientX - element.offsetWidth / 2}px, ${clientY - element.offsetHeight / 2}px) scale(1.05);
        ...
        element.onpointermove = (ev) => {
            ev.preventDefault();
            clone.style.transform = `translate(${ev.clientX - clone.offsetWidth/2}px, ${ev.clientY - clone.offsetHeight/2}px) scale(1.05)`;
            ...
            if (type === 'hand' || type === 'board') {
                ...
                if (shopContainer) {
                    ...
                    if (shopArea) {
                        ...
                        shopArea.classList.toggle('drop-target', over);
                    }
                }
            }
        };
        ...
        element.onpointerup = (ev) => {
            ...
            window.__yyIsDragging = false;

            const cloneEl = dragState.cloneElement;
    ...
    async function handleShopToBoard(card, shopIdx, boardIdx) {
        ...
        my.board[boardIdx] = { ...card, instanceId: Date.now() + '-' + Math.random() };
        if (targetCard) {
    ...
    async function handleShopToHand(card, shopIdx) {
        ...
        my.gold -= price;
        my.shopCards.splice(shopIdx, 1);
        my.hand[emptyIdx] = { ...card, instanceId: Date.now() + '-' + Math.random() };
        refreshAllUI();
    ...
    async function buyExpAction() {
        const phase = getCurrentPhaseFromBattle();
        if (phase !== 'prepare') { toast('只能在准备阶段操作', true); return; }
        const userId = getCurrentUserId();
        const roomId = getCurrentRoomId();
        if (!roomId || !userId) { toast('房间信息缺失', true); return; }
    ...
    async function refreshShopAction() {
        if (isRefreshingShop) { toast('正在刷新中，请稍候', true); return; }
        const phase = getCurrentPhaseFromBattle();
        if (phase !== 'prepare') { toast('只能在准备阶段刷新', true); return; }
        ...
        try {
            ...
            const data = await response.json();
            if (!response.ok || !data.success) { toast(data.error || '刷新失败', true); return; }
            ...
            if (data.updatedPlayer) {
                finalUpdatedData = data.updatedPlayer;
            } else {
                finalUpdatedData = {
                    shopCards: data.shopCards || latestMy.shopCards,
                    gold: data.gold !== undefined ? data.gold : latestMy.gold
                };
            }
    ...
    function injectStyles() {
        ...
        style.textContent = `
            ...
            .card-drag-clone { pointer-events: none !important; will-change: transform; transform: translateZ(0); }
