...
window.YYCardShop = (function() {
    ...
    let refreshSeq = 0; // 请求序号，丢弃过期的刷新结果
    ...
    // 注意：不再使用 isRefreshingShop 阻止新点击，每次点击都真实发请求
    async function refreshShopAction() {
        ...
        // 保存翻页前状态
        const oldGold = my.gold;
        const oldShopCardsCopy = JSON.parse(JSON.stringify(my.shopCards));
        const shop = my.shopCards;
        const oldActive = shop.active ?? 0;
        const oldSubIndex = shop.subIndex ?? 0;
        ...
        const newActive = shop.active ?? 0;
        const newSubIndex = shop.subIndex ?? 0;

        // 请求序号（丢弃过期的响应）
        const mySeq = ++refreshSeq;
        isRefreshingShop = true;      // 通知 battle.js 暂停 UI 全量刷新
        updateBuyExpButtonState();
        ...
        try {
            const result = await invokeFunction(FUNCTION_NAME_MAP.REFRESH_SHOP, {
                roomId,
                userId,
                currentActive: newActive,
                currentSubIndex: newSubIndex,
                oldActive: oldActive,
                oldSubIndex: oldSubIndex
            });

            if (mySeq !== refreshSeq) return; // 已在途中发起新的刷新，丢弃本结果

            if (!result.success) {
                // 失败回滚
                my.gold = oldGold;
                my.shopCards = oldShopCardsCopy;
                updateUIAfterSuccess({ gold: oldGold, shopCards: oldShopCardsCopy });
                renderShop();
                toast(result.error || '刷新失败', true);
                return;
            }

            // 成功：用服务器数据覆盖基础属性和 buffer，但保持前端页码不变
            if (result.data.updatedPlayer) {
                ...
                if (up.shopCards) {
                    if (up.shopCards.buffer) my.shopCards.buffer = up.shopCards.buffer;
                    if (up.shopCards.next !== undefined) my.shopCards.next = up.shopCards.next;
                    // active / subIndex 完全由前端保留
                }

                updateUIAfterSuccess({
                    gold: my.gold,
                    exp: my.exp,
                    shopLevel: my.shopLevel,
                    health: my.health,
                    shopCards: my.shopCards,
                    hand: up.hand,
                    board: up.board
                });
            }
