...
window.YYCardShop = (function() {
    ...
    function renderMyBoard() {
        if (isDragging) return;  // 🚫 拖拽中禁止重绘自己棋盘
        const gameState = getGameState();
        if (!gameState) return;
        const userId = getCurrentUserId();
        const my = gameState.players[userId];
    ...
    function renderHand() {
        if (isDragging) return;  // 🚫 拖拽中禁止重绘手牌
        const gameState = getGameState();
        if (!gameState) return;
        const userId = getCurrentUserId();
        const my = gameState.players[userId];
    ...
    function init() {
        ...
        bindUIEvents();
        refreshAllUI();
        console.log('✅ 全接口免JWT·RPC适配版 初始化完成');
    }
