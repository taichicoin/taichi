...
window.YYCardShop = (function() {
        ...
        fields.forEach(key => {
            if (updatedPlayer[key] !== undefined) {
                if (key === 'shopCards' && target.shopCards && updatedPlayer.shopCards) {
                    // 只合并 buffer，保留本地 active/subIndex，不再处理 next
    ...
    function isLocalFlipSafe() {
        ...
        // 切组：检查目标组（另一组）是否已有有效卡牌（由后端切组时自动生成）
    ...
    function init() {
        ...
        bindUIEvents();
        refreshAllUI();
        console.log('✅ 商店系统 (完美匹配新SQL，永无卡顿) 已启动');
    }
