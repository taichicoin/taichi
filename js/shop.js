// ==================== 商店与交互系统【全接口免JWT·RPC终极适配版 + 双缓冲商店】 ====================
window.YYCardShop = (function() {
    ...
    let isDragging = false;  // 拖拽锁，防止拖拽期间 UI 重绘干扰
    ...
    // ✅ 将双缓冲商店转为简单数组（当前激活组），用于购买时的乐观更新
    function flattenShopForBuy(player) {
        const shop = player.shopCards;
        if (shop && shop.buffer) {
            const active = shop.active || 0;
            const group = shop.buffer[active] || [];
            // 转为纯数组（包含 null 占位，保持索引）
            return [...group];
        }
    ...
    function renderShop() {
        ...
        const shopCards = getShopDisplayCards(my);  // 当前应显示的卡牌数组
    ...
    async function handleShopToBoard(card, shopIdx, boardIdx) {
        ...
        // 将双缓冲商店转成简单数组用于乐观更新
        const oldShopRaw = my.shopCards; // 保存原始对象或数组
        const currentGroup = flattenShopForBuy(my);
        ...
        // 从当前组中删除购买的卡
        currentGroup.splice(shopIdx, 1);
        // 将 player.shopCards 临时设置为当前组数组，确保 UI 渲染正确
        my.shopCards = currentGroup;
        ...
        if (!result.success) {
            ...
            my.board = oldBoard;
            my.hand = oldHand;
            my.shopCards = oldShopRaw; // 恢复原始对象
            renderMyBoard();
            renderHand();
            renderShop();
            toast(result.error, true);
        ...
        // 成功：用后端返回的数据覆盖，可能 shopCards 已变成数组（或仍为对象）
        if (result.data.updatedPlayer) {
            mergeUpdatedPlayer(my, result.data.updatedPlayer);
            updateUIAfterSuccess(result.data.updatedPlayer);
        }
    ...
    function init() {
        ...
        bindUIEvents();
        refreshAllUI();
        console.log('✅ 全接口免JWT·RPC适配版 + 双缓冲商店 初始化完成');
    }
