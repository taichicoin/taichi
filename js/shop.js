...
window.YYCardShop = (function() {
    ...
    let lastRefreshTime = 0;           // 防连点时间戳（刷新专用）
        ...
        fields.forEach(key => {
            if (updatedPlayer[key] !== undefined) {
                if (key === 'shopCards' && target.shopCards && updatedPlayer.shopCards) {
                    // 只合并 buffer 和 next，保留本地 active/subIndex
    ...
    function init() {
        ...
        bindUIEvents();
        refreshAllUI();
        console.log('✅ 商店系统 (极速无锁+防重复绑定+防连点+超时保护) 已启动');
    }
