...
window.YYCardShop = (function() {
    ...
    let toastTimer = null;
    const domCache = {};
    let isRefreshingShop = false;      // 刷新锁（乐观与等待共用）
    ...
    function isLocalFlipSafe() {
        ...
        return next && Array.isArray(next) && next.length === 9;
    }
    ...
    async function refreshShopAction() {
        ...
        if (isRefreshingShop) return;   // 已在进行中，防止连点
        ...
        if (canFlip) {
            // ========== 乐观秒切（本地有牌）==========
            isRefreshingShop = true;            // 上锁，阻止 refreshAllUI 干扰
            ...
            try {
                ...
                if (result.success && result.data.updatedPlayer) {
                    ...
                    // 只更新 buffer / next，不碰 active/subIndex
                    if (up.shopCards) {
    ...
    function init() {
        ...
        bindUIEvents();
        refreshAllUI();
        console.log('✅ 商店系统 (最终完整版 + 购买修复) 已启动');
    }
