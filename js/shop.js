...
window.YYCardShop = (function() {
    ...
    // ✅ 双缓冲v3：获取当前应显示的商店卡牌数组（根据 active 和 subIndex 从6张组中切出3张）
    ...
    // ✅ 获取当前显示卡在大组内的真实起始索引（用于购买时知道删哪个元素）
    function getCurrentGroupBaseIndex(player) {
        const shop = player.shopCards;
        if (shop && shop.buffer) {
            const sub = shop.subIndex ?? 0;
            return sub === 0 ? 0 : 3;
    ...
    async function handleShopToBoard(card, shopIdx, boardIdx) {
        ...
        // 保存原始商店对象和当前大组和子组信息
        const oldShopRaw = my.shopCards;
        const currentGroupIndex = oldShopRaw.active ?? 0;
        const currentSub = oldShopRaw.subIndex ?? 0;
        const baseIdx = getCurrentGroupBaseIndex(my); // 0 或 3
        const realIndexInGroup = baseIdx + shopIdx; // 在大组（6张）中的真实位置
        ...
        // 从当前大组中删除购买的卡（splice在6张数组上操作）
        const group = oldShopRaw.buffer[currentGroupIndex];
        group.splice(realIndexInGroup, 1);
        // 可选的：若组变短了不足6张，暂时不管，刷新时会恢复
        ...
        const result = await invokeFunction(FUNCTION_NAME_MAP.BUY_CARD, { 
            roomId, userId, shopIndex: realIndexInGroup, targetBoardIndex: boardIdx 
        });
    ...
    async function handleShopToHand(card, shopIdx) {
        ...
        const oldShopRaw = my.shopCards;
        const currentGroupIndex = oldShopRaw.active ?? 0;
        const baseIdx = getCurrentGroupBaseIndex(my);
        const realIndexInGroup = baseIdx + shopIdx;
        ...
        const tempInstanceId = Date.now() + '-' + Math.random();
        const emptyIdx = getFirstAvailableHandSlot(my.hand);
        if (emptyIdx !== -1) my.hand[emptyIdx] = { ...card, instanceId: tempInstanceId };
        else my.hand.push({ ...card, instanceId: tempInstanceId });
        ...
        const result = await invokeFunction(FUNCTION_NAME_MAP.BUY_CARD, { 
            roomId, userId, shopIndex: realIndexInGroup 
        });
    ...
    function init() {
        ...
        bindUIEvents();
        refreshAllUI();
        console.log('✅ 全接口免JWT·RPC适配版 + 双缓冲商店v3 初始化完成');
    }
