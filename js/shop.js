window.YYCardShop = {
  ...
  async loadTemplates() {
    try {
      // 【唯一修改】路径从 /y/data/characters.json 改为 /yycard/data/characters.json
  ...
  createCardInstance(template) {
    return {
      ...
      // 路径改为 /yycard/assets/...
      icon: template.icon || template.image || '/yycard/assets/default-avatar.png'
    };
  },
  ...
  renderShop() {
    ...
    this.currentShopCards.forEach(card => {
      ...
      cardEl.innerHTML = `
        <div class="card-icon">
          <img src="${card.icon}" alt="${card.name}" onerror="this.src='/yycard/assets/default-avatar.png'">
        </div>
