// ==================== 商店与交互系统（最终极速版：250ms防连点 + 金币不回弹 + 购买防冲突 + 音效事件） ====================
window.YYCardShop = (function() {
    ...
    let lastRefreshTime = 0;           // 防连点时间戳（250ms，实际代码内已改为300ms）
        ...
        fields.forEach(key => {
            if (updatedPlayer[key] !== undefined) {
                if (key === 'shopCards' && target.shopCards && updatedPlayer.shopCards) {
                    // 只合并 buffer，不处理 next，保留本地 active/subIndex
    ...
    // ==================== ★ 商店刷新（最终版：250ms防连点 + 仅更新buffer，金币不回弹） ====================
    function isLocalFlipSafe() {
        ...
        if (!shop?.buffer) return false;
        const sub = shop.subIndex ?? 0;
        if (sub < 4) return true;   // 组内翻页，一定有牌

        // 切组：检查目标组（另一组）是否已有有效卡牌
            ...
            invokeFunction(FUNCTION_NAME_MAP.REFRESH_SHOP, { roomId, userId })
                .then(result => {
                    ...
                    if (result.success && result.data.updatedPlayer) {
                        ...
                        // ★ 只更新 buffer，保留 active/subIndex
                        if (up.shopCards && up.shopCards.buffer) {
                            my.shopCards.buffer = up.shopCards.buffer;
                        }
                        if (!isDragging) {
