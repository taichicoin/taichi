...
window.YYCardShop = (function() {
    ...
    let toastTimer = null;
    const domCache = {};
    let isRefreshingShop = false;        // 刷新操作进行中（用于防连点）
    let isDragging = false;
    let isOperating = false;             // ★ 任何前后端交互的操作进行中
    ...
    function renderShop() {
        ...
        if (!Array.isArray(group) || group.length < 9) {
    ...
    function refreshAllUI() {
        // ★ 有操作正在进行时，不响应外部刷新（防止覆盖乐观数据）
        if (isOperating || isRefreshingShop) return;
    ...
    function updateBuyExpButtonState() {
        ...
        const shouldDisable = (gameState.phase !== 'prepare') || isMaxLevel;
        let expNeeded = 0;
        if (!isMaxLevel) {
            const exp = my.exp;
    ...
    async function handleUnequip(boardIdx, slotKey) {
        ...
        card[slotKey] = null;
        my.hand[emptyIdx] = {
            card_id: equip.card_id, cardId: equip.card_id, name: equip.name,
            type: equip.type, rarity: equip.rarity || 'Common',
    ...
    async function handleHandToBoard(handIdx, boardIdx) {
        ...
        if (isValidCard(oldTarget) && getValidHandCount(my.hand) >= 15) { toast('手牌已满，无法交换', true); isOperating = false; return; }
    ...
    async function handleShopToBoard(card, shopIdx, boardIdx) {
        ...
        const my = gameState?.players[userId];
        if (!my) return;
        if (card.type === 'weapon' || card.type === 'item') { toast('武器/道具只能装备到角色身上，请拖到角色格子上', true); return; }
        const price = (config.ECONOMY?.CARD_PRICE?.[card.rarity]?.buy) || 1;
        ...
        const oldBoard = [...my.board];
        const targetCard = my.board[boardIdx];
        if (isValidCard(targetCard) && getValidHandCount(my.hand) >= 15) { toast('手牌已满，无法交换', true); isOperating = false; return; }
        ...
        try {
            const result = await invokeFunction(FUNCTION_NAME_MAP.BUY_CARD, { roomId, userId, shopIndex: realIndex, targetBoardIndex: boardIdx });
            if (!result.success) {
    ...
    async function handleShopToHand(card, shopIdx) {
        ...
        try {
            const result = await invokeFunction(FUNCTION_NAME_MAP.BUY_CARD, { roomId, userId, shopIndex: realIndex });
            if (!result.success) {
    ...
    async function handleBoardToHand(boardIdx) {
        ...
        for (const eq of equipment) {
            my.hand[handIdx] = {
                card_id: eq.equip.card_id, cardId: eq.equip.card_id, name: eq.equip.name,
                type: eq.equip.type, rarity: eq.equip.rarity || 'Common',
    ...
    async function handleSell(type, index) {
        ...
        if (type === 'board') {
            ...
            my.board[index] = null;
            my.gold += (config.ECONOMY?.CARD_PRICE?.[card.rarity]?.sell || 1);
            renderMyBoard(); renderHand();
    ...
    function performLocalFlip() {
        ...
        if (subIndex < 2) {
            ...
            if (next && Array.isArray(next) && next.length === 9) {
                buffer[oldActive] = next;
                shop.next = null;
            }
        }
        shop.active = active;
    ...
    async function refreshShopAction() {
        ...
        if (isOperating || isRefreshingShop) return;   // 操作进行中，不重复触发
        ...
        const oldGold = my.gold;
        const oldShopCards = JSON.parse(JSON.stringify(my.shopCards));
        my.gold -= 1;
        document.getElementById('my-gold').textContent = my.gold;
        performLocalFlip();
        ...
        try {
            ...
            if (!result.success) {
                ...
                updateUIAfterSuccess({ gold: oldGold });
                renderShop();
                return;
            }
            if (result.data.updatedPlayer) {
                // 保留本地最新的子索引
                const myCurrentShop = my.shopCards;
                if (result.data.updatedPlayer.shopCards) {
                    result.data.updatedPlayer.shopCards.active = myCurrentShop.active;
                    result.data.updatedPlayer.shopCards.subIndex = myCurrentShop.subIndex;
                    result.data.updatedPlayer.shopCards.next = myCurrentShop.next;
                }
                mergeUpdatedPlayer(my, result.data.updatedPlayer);
                updateUIAfterSuccess(result.data.updatedPlayer);
            }
        } catch (err) {
            console.error('刷新请求异常:', err);
            my.gold = oldGold;
            my.shopCards = oldShopCards;
            updateUIAfterSuccess({ gold: oldGold });
            renderShop();
    ...
    return {
        ...
        get isOperating() { return isOperating; }
    };
})();
