...
window.YYCardShop = (function() {
    ...
    function renderShop() {
        ...
        for (let i = start; i < start + 3; i++) {
            ...
            if (isValidCard(card)) {
                hasCards = true;
                const el = createCardElement(card);
                el.setAttribute('data-shop-index', i);   // ✅ 绝对索引 0~5
    ...
    // 商店→棋盘购买（已修正索引：直接使用绝对索引，置null替代splice）
    ...
    // 商店→手牌购买（已修正索引：直接使用绝对索引，置null替代splice）
    ...
    function init() {
        ...
        bindUIEvents();
        refreshAllUI();
        console.log('✅ 全接口免JWT·RPC适配版 + 双缓冲商店v3 初始化完成 (拖拽优化)');
    }
