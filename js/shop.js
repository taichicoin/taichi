...
window.YYCardShop = (function() {
    ...
    let toastTimer = null;
    const domCache = {};
    let isRefreshingShop = false;      // 只在极少数必须等后端时才上锁
    let isDragging = false;
    let refreshSeq = 0;
    let lastRefreshTime = 0;           // 防连点时间戳
    ...
    // mergeUpdatedPlayer 保留，仅在必须等后端时才用
        ...
        fields.forEach(key => {
            if (updatedPlayer[key] !== undefined) {
                if (key === 'shopCards' && target.shopCards && updatedPlayer.shopCards) {
                    // 合并 buffer、next、active、subIndex
                    if (updatedPlayer.shopCards.buffer) {
                        target.shopCards.buffer = updatedPlayer.shopCards.buffer;
                    }
                    if (updatedPlayer.shopCards.next !== undefined) {
                        target.shopCards.next = updatedPlayer.shopCards.next;
                    }
                    // 新增：同步翻页位置，解决只扣钱不切牌的问题
                    if (updatedPlayer.shopCards.active !== undefined) {
                        target.shopCards.active = updatedPlayer.shopCards.active;
                    }
                    if (updatedPlayer.shopCards.subIndex !== undefined) {
                        target.shopCards.subIndex = updatedPlayer.shopCards.subIndex;
                    }
    ...
    // ★ 记录金币被本地修改（用于 battle.js 判断是否跳过同步）
    function touchGold() {
    ...
    function isLocalFlipSafe() {
        ...
        if (!shop?.buffer) return false;
        const sub = shop.subIndex ?? 0;
        if (sub < 4) return true;      // 组内翻页，本地有牌
        const next = shop.next;
        return next && Array.isArray(next) && next.length === 15; // 切组且有 next
    }
    ...
    async function refreshShopAction() {
        ...
        if (canFlip) {
            // ★ 乐观秒切：不加任何锁，直接翻页，后台异步发请求，绝不更新商店数据
            my.gold -= 1;
            document.getElementById('my-gold').textContent = my.gold;
            ...
            // 仍然没有，加锁请求后端，设置 1 秒超时解锁（此时才允许更新商店）
    ...
    function init() {
        ...
        bindUIEvents();
        refreshAllUI();
        console.log('✅ 商店系统 (绝对稳定 纯乐观 无覆盖) 已启动');
    }
