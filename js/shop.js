// ==================== 商店与交互系统（手动模拟拖拽版：触碰即拿起，支持商店→手牌 + 缓冲期 + 正确镜像敌方棋盘 + 灵敏度优化） ====================
window.YYCardShop = (function() {
    ...
    // ===== 战斗阶段根据 battlePairs 精确找到对手，并按总纲规则显示（上下颠倒，左右不变） =====
    function renderEnemyBoard() {
        ...
        if (oppId && gameState.players[oppId]) {
            ...
            // 构建敌方显示数组：
            // 第一行（远离分界线）：敌方后排（索引3,4,5），左右顺序不变
            // 第二行（靠近分界线）：敌方前排（索引0,1,2），左右顺序不变
            const enemyDisplayBoard = [
                originalBoard[3], originalBoard[4], originalBoard[5], // 后排
                originalBoard[0], originalBoard[1], originalBoard[2]  // 前排
            ];
            ...
            console.log(`[renderEnemyBoard] 敌方显示棋盘: ${enemyDisplayBoard.map(c=>c?c.name:'空').join(',')}`);
        }
    }
    ...
    function onDragStart(e, type, card, index, element) {
        ...
        // 使用 requestAnimationFrame 节流移动事件
        let rafId = null;
        const onMove = (e) => {
            if (rafId) cancelAnimationFrame(rafId);
            rafId = requestAnimationFrame(() => onDragMove(e));
        };
    ...
    function onDragEnd(e) {
        ...
        // 移除所有监听器（注意：我们之前用了具名函数，但这里为了简洁直接移除全部，实际项目中应保存引用）
    ...
    function getDropTarget(element) {
        ...
        while (el && el !== document.body) {
            ...
            // 手牌区：直接判断容器
            if (el.id === 'hand-container' || el.classList.contains('hand') || el.closest('#hand-container')) {
                return { zone: 'hand' };
            }
            // 商店区：直接判断容器
            if (el.id === 'shop-container' || el.classList.contains('shop-cards') || el.closest('#shop-container')) {
                return { zone: 'shop' };
            }
            el = el.parentElement;
        }
...
console.log('✅ shop.js 加载完成（灵敏度优化版）');
