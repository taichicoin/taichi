...
window.YYCardShop = (function() {
    ...
    // ✅ 双缓冲v3：获取当前应显示的商店卡牌数组（根据 active 和 subIndex 从6张组中切出3张，并过滤掉null）
    ...
    // ✅ 获取当前显示卡在大组内的真实起始索引（用于购买时计算真实位置）
    ...
    // 商店→棋盘购买（已修复：不再 splice，改为置 null）
    async function handleShopToBoard(card, shopIdx, boardIdx) {
        ...
        const realIndex = baseIdx + shopIdx;
        ...
        // ✅ 关键修复：将购买位置置 null，而不是 splice，保持数组长度不变
    ...
    // 商店→手牌购买（已修复：不再 splice，改为置 null）
    ...
    function init() {
        ...
        bindUIEvents();
        refreshAllUI();
        console.log('✅ 全接口免JWT·RPC适配版 + 双缓冲商店v3 初始化完成（购买已修复）');
    }
