...
window.YYCardShop = (function() {
    ...
    async function refreshShopAction() {
        ...
        if (shopContainer && !shopContainer.querySelector('.refresh-loading-hint')) {
            ...
            loadingHint.style.cssText = 'position:absolute; top:8px; right:8px; background:rgba(0,0,0,0.5); color:#ffd966; border-radius:20px; padding:2px 8px; font-size:14px; z-index:10; pointer-events:none;';
            shopContainer.style.position = 'relative';
            shopContainer.appendChild(loadingHint);
        }

        try {
            // 3. 直接发起请求，无任何前置修改、无锁、无频率限制
            const accessToken = await getAccessToken();
            ...
            if (data.updatedPlayer) {
                ...
                my.gold = data.gold !== undefined ? data.gold : my.gold - 1;
                // 同步更新金币显示
                const goldEl = document.getElementById('my-gold');
