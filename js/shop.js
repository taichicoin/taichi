...
window.YYCardShop = (function() {
    ...
    // ✅ 渲染商店：每张卡携带绝对索引，并强制追加可见价格标签（内联样式，无视任何CSS覆盖）
    function renderShop() {
        const gameState = getGameState();
        if (!gameState) return;
        const userId = getCurrentUserId();
        ...
        for (let i = start; i < start + 3; i++) {
            ...
            if (isValidCard(card)) {
                ...
                priceTag.style.cssText = `
                    ...
                    color: #fff !important;
                    text-shadow: 0 0 6px #000, 0 0 8px #000, 0 0 12px #000, 0 0 2px #000;
                    padding: 0 !important;
                    margin: 0 !important;
                    pointer-events: none;
                `;
    ...
    function renderBoard(containerId, cards, isSelf) {
        ...
        for (let i = 0; i < 6; i++) {
            ...
            if (isValidCard(c)) {
                const el = createCardElement(c, true);  // ✅ 棋盘卡牌使用纯数字
                if (isSelf) {
    ...
    function createCardElement(card, isBoard = false) {
    ...
    function init() {
        ...
        bindUIEvents();
        refreshAllUI();
        console.log('✅ 全接口免JWT·RPC适配版 + 双缓冲商店v3 初始化完成 (棋盘纯数字+价格强制显示透明背景)');
    }
