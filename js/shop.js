...
window.YYCardShop = (function() {
    ...
    async function refreshShopAction() {
        if (!canOperate()) return;                     // 权限检查（含 isRefreshingShop 锁）
        const userId = getCurrentUserId();
        const roomId = getCurrentRoomId();
        const gameState = getGameState();
        const my = gameState?.players[userId];
        if (!my) return;
        if (my.gold < 1) return;                      // 金币不足，直接阻止
    ...
    function bindUIEvents() {
        // ★ 刷新按钮：手指一碰立刻触发，不再需要抬起
        const refreshBtn = document.getElementById('refresh-shop-btn');
        if (refreshBtn) {
            refreshBtn.addEventListener('pointerdown', (e) => {
                e.preventDefault();               // 防止触发移动端的默认事件
                if (isRefreshingShop) return;     // 正在刷新中，忽略连点
                refreshShopAction();
            });
        }

        // 其他按钮保持原来事件（或根据需要也改成 pointerdown）
    ...
    function init() {
        ...
        bindUIEvents();
        refreshAllUI();
        console.log('✅ 商店系统 (安全装备 + 秒切刷新) 已启动');
    }
