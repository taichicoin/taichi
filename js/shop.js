...
window.YYCardShop = (function() {
    ...
    async function syncFromBackend() {
        if (window.YYCardBattle?.forceRefreshState) {
            ...
            // 降级：手动调用 updateGameState 会拉取最新并合并本地修改（不完美，但可用）
    ...
    // 但为了完整性，这里仍使用原有的 battle.js 方法（因为交换只是本地状态，不需要复杂校验）
        ...
        try {
            ...
            const response = await fetch(BUY_CARD_FUNCTION_URL, {
                ...
                body: JSON.stringify({
                    ...
                    userId,
                    shopIndex: shopIdx,
                    targetBoardIndex: boardIdx   // 指定要放置的棋盘格子
                })
            });
    ...
    // 出售（仍可使用原有 battle.js 方法，因为只增加金币，但为了安全也可以后端，先保留原有）
...
console.log('✅ shop.js 加载完成（后端刷新商店 + 后端购买/交换 + 弹窗调试）');
