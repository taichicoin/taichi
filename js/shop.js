...
window.YYCardShop = (function() {
    ...
    function getFirstAvailableHandSlot(hand) {
        for (let i = 0; i < hand.length; i++) { if (!isValidCard(hand[i])) return i; }
        return -1;
    }
    function getValidHandCount(hand) { return hand.filter(isValidCard).length; }
    function canOperate() {
        const gs = getGameState();
        return !!(gs && gs.phase === 'prepare' && !gs.players?.[getCurrentUserId()]?.isBot);
    }
    ...
    const FUNCTION_NAME_MAP = {
        REFRESH_SHOP: 'refresh-shop', BUY_CARD: 'buy-card', SWAP_BOARD: 'swap-board',
        SELL_CARD: 'sell-card', PLACE_CARD: 'place-card', BOARD_TO_HAND: 'board-to-hand',
        BUY_EXP: 'buy-exp'
    };
        ...
        fields.forEach(key => {
            if (updatedPlayer[key] !== undefined) {
                if (key === 'shopCards' && target.shopCards && updatedPlayer.shopCards) {
                    ...
                    if (updatedPlayer.shopCards.next !== undefined) target.shopCards.next = updatedPlayer.shopCards.next;
                ...
                } else { target[key] = updatedPlayer[key]; }
            }
        });
    }
    ...
    function updateUIAfterSuccess(updatedPlayer) {
        if (!updatedPlayer) return;
        const gs = getGameState(), uid = getCurrentUserId();
    ...
    function toast(msg, isErr = false, dur = 2000) {
        ...
        toastTimer = setTimeout(() => { if (toastEl.parentNode) toastEl.remove(); toastTimer = null; }, dur);
    }
    ...
    function renderMyBoard() {
        ...
        document.getElementById('my-board').setAttribute('data-player-id', uid);
    }
    ...
    function renderShop() {
        ...
        if (!hasCards) container.innerHTML = '<div style="color:#aaa;padding:10px;text-align:center;width:100%;">商店暂无卡牌</div>';
        else { container.innerHTML = ''; container.appendChild(fragment); }
    }

    function getShopDisplayCards(player) {
        const shop = player.shopCards;
        if (shop?.buffer && Array.isArray(shop.buffer)) {
            const active = shop.active ?? 0, sub = shop.subIndex ?? 0;
            const group = shop.buffer[active];
            if (Array.isArray(group) && group.length >= 15) {
                return group.slice(sub * 3, sub * 3 + 3).filter(isValidCard);
            }
        }
    ...
    function refreshAllUI() {
        ...
        const gs = getGameState();
        if (gs) {
            const uid = getCurrentUserId(), my = gs.players[uid];
            if (my) {
    ...
    function canEquipTo(boardIdx) {
        const gs = getGameState();
        const my = gs?.players[getCurrentUserId()];
    ...
    async function equipFromHand(handIdx, boardIdx) {
        ...
        const result = await callEquipFunction({
            roomId: getCurrentRoomId(), userId: getCurrentUserId(),
            boardIndex: boardIdx, slotKey, handIndex: handIdx
        });
        if (!result.success) {
            my.hand = oldHand;
            renderHand();
    ...
    async function equipFromShop(shopIdx, boardIdx) {
        ...
        const result = await callEquipFunction({
            roomId: getCurrentRoomId(), userId: getCurrentUserId(),
            boardIndex: boardIdx, slotKey, shopIndex: shopIdx
        });
        if (!result.success) {
            my.gold = oldGold;
    ...
    async function handleUnequip(boardIdx, slotKey) {
        ...
        card[slotKey] = null;
        my.hand[emptyIdx] = {
            card_id: equip.card_id, cardId: equip.card_id,
            name: equip.name, type: equip.type, rarity: equip.rarity || 'Common',
            base_atk: equip.atk || 0, base_hp: equip.hp || 0,
            atk: equip.atk || 0, hp: equip.hp || 0,
            image: equip.image || '', faction: '', star: equip.star || 0
        };
        renderMyBoard();
        renderHand();
        ...
        while (el && el !== document.body) {
            if (el.classList.contains('card-slot')) {
                ...
                if (board?.id === 'my-board') {
                    ...
                    if (slotIndex !== null) {
                        const gs = getGameState();
                        const boardCard = gs?.players?.[getCurrentUserId()]?.board?.[parseInt(slotIndex)];
    ...
    async function executeDropAction(type, index, dropResult) {
        if (type === 'hand') {
            const gs = getGameState();
            const card = gs?.players[getCurrentUserId()]?.hand[index];
        ...
        if (type === 'shop') {
            const gs = getGameState();
            const shop = gs?.players[getCurrentUserId()]?.shopCards;
            const active = shop?.active ?? 0;
    ...
    async function handleHandToBoard(handIdx, boardIdx) {
        ...
        if (card.type === 'weapon' || card.type === 'item') { toast('武器/道具不能直接放到棋盘', true); return; }
    ...
    async function handleShopToBoard(card, shopIdx, boardIdx) {
        ...
        const my = gs?.players[userId];
        if (!my) return;
        if (card.type === 'weapon' || card.type === 'item') { toast('武器/道具只能装备到角色身上', true); return; }
        const price = (config.ECONOMY?.CARD_PRICE?.[card.rarity]?.buy) || 1;
        ...
        const newCard = {
            ...
            faction: card.faction || '', weapon: null, item1: null, item2: null
        };
        ...
        const result = await invokeFunction(FUNCTION_NAME_MAP.BUY_CARD, {
            roomId, userId, shopIndex: realIndex, targetBoardIndex: boardIdx, active
        });
        if (!result.success) {
            my.gold = oldGold; my.board = oldBoard; my.hand = oldHand; my.shopCards = oldShopRaw;
            renderMyBoard(); renderHand(); renderShop();
            return;
        }
        if (result.data.updatedPlayer) {
    ...
    async function handleShopToHand(card, shopIdx) {
        ...
        const result = await invokeFunction(FUNCTION_NAME_MAP.BUY_CARD, { roomId, userId, shopIndex: realIndex, active });
        if (!result.success) {
            my.gold = oldGold; my.shopCards = oldShopRaw; my.hand = oldHand;
            renderHand(); renderShop();
            return;
        }
        if (result.data.updatedPlayer) {
    ...
    async function handleBoardToHand(boardIdx) {
        ...
        for (const eq of equipment) {
            my.hand[handIdx] = {
                card_id: eq.equip.card_id, cardId: eq.equip.card_id,
                name: eq.equip.name, type: eq.equip.type, rarity: eq.equip.rarity || 'Common',
                base_atk: eq.equip.atk || 0, base_hp: eq.equip.hp || 0,
                atk: eq.equip.atk || 0, hp: eq.equip.hp || 0,
                image: eq.equip.image || '', faction: '', star: eq.equip.star || 0
            };
            card[eq.slot] = null;
    ...
    async function handleSell(type, index) {
        ...
        if (type === 'board') {
            ...
            const emptySlots = 15 - getValidHandCount(my.hand);
            if (neededSlots > emptySlots) { toast(`手牌空间不足，无法出售（需${neededSlots}空位）`, true); return; }
        ...
        if (!result.success) {
            my.gold = oldGold; my.hand = oldHand;
            renderHand();
            return;
        }
        if (result.data.updatedPlayer) {
    ...
    async function buyExpAction() {
        ...
        if (!result.success) { my.gold = oldGold; return; }
        if (result.data.updatedPlayer) {
            mergeUpdatedPlayer(my, result.data.updatedPlayer);
            updateUIAfterSuccess(result.data.updatedPlayer);
        }
    ...
    function isLocalFlipSafe() {
        ...
        return next && Array.isArray(next) && next.length === 15;
    }
    ...
    function performLocalFlip() {
        ...
        let active = shop.active ?? 0, subIndex = shop.subIndex ?? 0;
        const buffer = shop.buffer, oldActive = active;

        if (subIndex < 4) { subIndex += 1; }
        else {
            subIndex = 0;
            active = 1 - active;
            buffer[oldActive] = shop.next;
            shop.next = null;
        }
        shop.active = active; shop.subIndex = subIndex;
        renderShop();
    }

    function forceUnlockRefresh() {
        if (isRefreshingShop) {
            console.warn('刷新锁超时，强制释放');
            isRefreshingShop = false;
            if (refreshLockTimer) { clearTimeout(refreshLockTimer); refreshLockTimer = null; }
        }
    }
    ...
    async function refreshShopAction() {
        ...
        if (canFlip) {
            ...
            if (state) state.players[getCurrentUserId()].gold = my.gold;
            touchGold();
            performLocalFlip();
            ...
            invokeFunction(FUNCTION_NAME_MAP.REFRESH_SHOP, { roomId, userId })
                .then(result => {
                    ...
                    } else if (!result.success) toast(result.error || '刷新失败', true);
                })
                .catch(() => {});
            updateBuyExpButtonState();
        } else {
            ...
            refreshLockTimer = setTimeout(() => { forceUnlockRefresh(); updateBuyExpButtonState(); }, 1500);
            ...
            try {
                ...
                if (mySeq !== refreshSeq) return;
                if (!result.success) { toast(result.error || '刷新失败', true); return; }
                if (result.data.updatedPlayer) {
                    mergeUpdatedPlayer(my, result.data.updatedPlayer);
                    updateUIAfterSuccess(result.data.updatedPlayer);
                }
            } catch (err) { if (mySeq === refreshSeq) toast('网络异常', true); }
            finally {
                if (mySeq === refreshSeq) {
                    isRefreshingShop = false;
                    if (refreshLockTimer) { clearTimeout(refreshLockTimer); refreshLockTimer = null; }
