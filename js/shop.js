...
window.YYCardShop = (function() {
    ...
    async function renderShop() {
        ...
        if (!shopMeta) {
            container.innerHTML = '<div style="color:#aaa;padding:10px;text-align:center;width:100%;">商店数据加载中...</div>';
            return;
        }
        ...
        if (!roomId || !userId) {
            container.innerHTML = '<div style="color:#f5d76e;padding:10px;text-align:center;width:100%;">⏳ 等待房间同步...</div>';
            return;
        }
        ...
        try {
            ...
            if (error) throw new Error(error.message || '查询商店失败');

            // ★ 如果 query 成功，但 data 是空的
            if (!data || data.length === 0) {
                container.innerHTML = '<div style="color:#aaa;padding:10px;text-align:center;width:100%;">该页暂无卡牌</div>';
                return;
            }
            ...
            // ★ 强制补全为 3 张卡，防止买空越界
            let groupCards = _shopCardCache[cacheKey];
            while (groupCards.length < 3) {
                groupCards.push(null);
            }
            ...
            console.error('读取商店表失败:', err);
            // ★ 去掉了花哨的报错渲染，只显示简单的失败提示 ★
            container.innerHTML = '<div style="color:#aaa;padding:10px;text-align:center;width:100%;font-size:14px;">⚠️ 商店加载失败，请稍后刷新</div>';
        }
    }
    ...
    async function equipFromShop(shopIdx, boardIdx) {
        ...
        try {
            ...
            // ★★★ 核心修正：不依赖后端覆盖棋盘手牌，只同步金库与页码，保持乐观 ★★★
            const up = result.updatedPlayer;
            if (up) {
                ...
                if (up.shopCards) {
                    if (up.shopCards.active !== undefined) my.shopCards.active = up.shopCards.active;
                    if (up.shopCards.subIndex !== undefined) my.shopCards.subIndex = up.shopCards.subIndex;
                }
                // 绝不使用 mergeUpdatedPlayer 覆盖 my.board 和 my.hand！
            }
    ...
    async function handleShopToBoard(card, shopIdx, boardIdx) {
        ...
        try {
            ...
            if (!result.success) {
                ...
                // ★★★ 核心修正：拒绝后端全覆盖，只同步必要信息 ★★★
                const up = result.data?.updatedPlayer;
                if (up) {
                    // 1. 强制同步金币（防止小数点/后端扣费差值）
                    if (up.gold !== undefined) {
                        my.gold = up.gold;
                        document.getElementById('my-gold').textContent = up.gold;
                        touchGold();
                    }
                    // 2. 如果换组翻页导致页码变更，仅更新页码（绝对页码）
                    if (up.shopCards) {
                        if (up.shopCards.active !== undefined) my.shopCards.active = up.shopCards.active;
                        if (up.shopCards.subIndex !== undefined) my.shopCards.subIndex = up.shopCards.subIndex;
                    }
                    // 3. 绝不执行 mergeUpdatedPlayer 和 updateUIAfterSuccess！
                    // 因为前端已经在购买前执行了乐观更新（_purchasedIndices.add / renderShop）
                    // 棋盘和手牌的变更在前端本地已经做到了极致视觉同步。
                }
                
                // 仅做成功提示
                toast(`${card.name} 已购买${p_target_board_index !== null ? '并装备' : '并入手牌'}`);
            }
        } finally {
            isBusy = false;
            operationLock = false;
        ...
        try {
            ...
            if (!result.success) {
                ...
                if (up) {
                    ...
                    // 绝不调用 mergeUpdatedPlayer 和 updateUIAfterSuccess！
                }
                toast(`${card.name} 已购买`);
            }
        } finally {
