...
window.YYCardShop = (function() {
    ...
    const BUY_EXP_FUNCTION_URL = 'https://sznjaotjoljaiawbvfro.supabase.co/functions/v1/buy-exp';   // 新增升级函数
    ...
    function onDragStart(e, type, card, index, element) {
        ...
        element.style.transform = 'scale(0.95)'; // 轻微缩小，更跟手
    ...
    // 放置手牌到棋盘（调用后端 place-card 函数，支持交换）
    ...
    // 棋盘卡牌移回手牌（调用后端 board-to-hand 函数）
    ...
    function init() {
        ...
        bindUIEvents();
        refreshAllUI();
        log('✅ 商店交互模块已启动（后端全部操作 + 升级调用后端 + 流畅优化版）');
    }
...
console.log('✅ shop.js 加载完成（流畅优化版 + 已移除刷新弹窗）');
