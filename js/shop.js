...
window.YYCardShop = (function() {
    ...
    // ★ 修复后的合并函数：彻底禁止后端覆盖前端的 active / subIndex
    function mergeUpdatedPlayer(target, updatedPlayer) {
        if (!updatedPlayer) return;
        // 注意：主动排除 active / subIndex，完全由前端本地控制，后端不许覆盖
        ...
        fields.forEach(key => {
            if (updatedPlayer[key] !== undefined) {
                if (key === 'shopCards' && target.shopCards && updatedPlayer.shopCards) {
                    ...
                    // 关键点：完全忽略后端的 active、subIndex，不做任何覆盖！
                    return;
                } 
    ...
    async function refreshShopAction() {
        ...
        if (canFlip) {
            // ===== 乐观秒切：不加锁，直接翻页，后台异步请求 =====
            my.gold -= 1;
            document.getElementById('my-gold').textContent = my.gold;
            // ★ 同步到本地 gameState 防止 refreshAllUI 回弹
            ...
            invokeFunction(FUNCTION_NAME_MAP.REFRESH_SHOP, { roomId, userId })
                .then(result => {
                    ...
                    if (result.success && result.data.updatedPlayer) {
                        ...
                        if (up.shopCards) {
                            // ★ 只同步 buffer 和 next，坚决不碰 active / subIndex
    ...
    function init() {
        ...
        bindUIEvents();
        refreshAllUI();
        console.log('✅ 商店系统 (极速无锁+防重复绑定+防连点+金币不回弹) 已启动');
    }
