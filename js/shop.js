...
window.YYCardShop = (function() {
    ...
    let toastTimer = null;
    const domCache = {};
    let isRefreshingShop = false;   // 保留变量，但不再用于锁点击
    ...
    function refreshAllUI() {
        // 移除 isRefreshingShop 检查，让 UI 可以随时更新
    ...
    async function refreshShopAction() {
        ...
        // 记录翻页前的页码（用于失败回滚）
        const oldActive = my.shopCards.active ?? 0;
        const oldSubIndex = my.shopCards.subIndex ?? 0;
        ...
        // 2. 异步发送请求，不阻塞操作
        invokeFunction(FUNCTION_NAME_MAP.REFRESH_SHOP, {
            roomId,
            userId,
            oldActive,
            oldSubIndex
            ...
            if (!result.success) {
                // 失败：恢复翻页前的页码
                my.shopCards.active = oldActive;
                my.shopCards.subIndex = oldSubIndex;
                renderShop();
    ...
    function init() {
        ...
        bindUIEvents();
        refreshAllUI();
        console.log('✅ 商店系统 (最终版：无锁秒切，后端扣钱) 已启动');
    }
