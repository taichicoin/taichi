...
window.YYCardShop = (function() {
    ...
    function isLocalFlipSafe() {
        ...
        if (!shop?.buffer) return false;
        const sub = shop.subIndex ?? 0;
        if (sub < 2) return true;            // 组内翻页，buffer 中已有后面 6 张（当前组已生成）
        // subIndex == 2，需要切换到另一组，检查 next 是否已存在
        const next = shop.next;
        return next && Array.isArray(next) && next.length === 9;
    }

    // 只在 isLocalFlipSafe() 为 true 时调用
    function performLocalFlip() {
        ...
        if (subIndex < 2) {
            ...
            subIndex = 0;
            active = 1 - active;
            // next 一定存在，因为 isLocalFlipSafe 已检查
    ...
    async function refreshShopAction() {
        ...
        if (canFlip) {
            ...
            my.gold -= 1;
            document.getElementById('my-gold').textContent = my.gold;
            performLocalFlip();   // 已经渲染新三张

            // 后台静默请求，只更新数据库，不重新渲染商店（关键：避免闪回）
            invokeFunction(FUNCTION_NAME_MAP.REFRESH_SHOP, { roomId, userId })
                .then(result => {
                    if (result.success && result.data.updatedPlayer) {
                        ...
                        // 只更新 buffer 和 next，不覆盖 active/subIndex，也不重新渲染商店
                        if (up.shopCards) {
            ...
            try {
                ...
                if (mySeq === refreshSeq) toast('网络异常，刷新失败', true);
            } finally {
                if (mySeq === refreshSeq) {
