...
window.YYCardShop = (function() {
    ...
    function refreshAllUI() {
        ...
        if (gameState) {
            ...
            // 不再更新回合显示，交给 battle.js 的 tick() 使用玩家自己的 playerRound
            updateBuyExpButtonState();
        }
    ...
    function createCardElement(card, cardType = 'board', isBoard = false) {
        ...
        if (card.type === 'consumable' || card.isConsumable) {
            ...
            nameEl.textContent = card.name || '消耗牌';
            ...
            descEl.textContent = formatConsumableEffect(card);
            descEl.style.cssText = 'color:rgba(255,255,255,0.9); font-size:clamp(0.5rem,1.5vw,0.7rem); text-align:center;';
