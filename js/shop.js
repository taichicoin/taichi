...
window.YYCardShop = (function() {
    ...
    async function equipFromHand(handIdx, boardIdx) {
        ...
        // handIdx 可能因为前面插入旧武器而偏移，重新定位新装备卡在 my.hand 中的位置
        ...
        const result = await callEquipFunction({
            roomId, userId,
            handIndex: currentHandIdx,  // 现在手牌中的位置（已经是null，但后端会忽略，直接写入槽位）
            boardIndex: boardIdx,
            slotKey,
            equipData
        });
    ...
    function init() {
        ...
        bindUIEvents();
        refreshAllUI();
        console.log('✅ 商店系统 (武器替换乐观更新修复) 已启动');
    }
