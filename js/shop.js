// ==================== 商店与交互系统（无刷新/升级，由 refresh.js 接管，适配30张10页） ====================
window.YYCardShop = (function() {
    ...
    function renderShop() {
        ...
        if (!Array.isArray(group) || group.length < 30) {   // ★ 改为 30
    ...
    function isLocalFlipSafe() {
        ...
        if (!shop?.buffer) return false;
        const sub = shop.subIndex ?? 0;
        if (sub < 9) return true;                         // ★ 9 页内安全
        const nextActive = 1 - (shop.active ?? 0);
        const targetGroup = shop.buffer[nextActive];
        if (!Array.isArray(targetGroup) || targetGroup.length < 30) return false;   // ★ 改为 30
