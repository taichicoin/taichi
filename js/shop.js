...
window.YYCardShop = (function() {
    ...
    // 专用装备/卸下调用（直接 fetch，绕过可能出问题的 invoke）
    async function callEquipFunction(body) {
        try {
            ...
            const resp = await fetch(
                `${Deno.env.get('SUPABASE_URL')}/functions/v1/equip-item`,
                {
                    method: 'POST',
                    headers: {
    ...
    async function callUnequipFunction(body) {
        try {
            ...
            if (!session) throw new Error('未登录');
            const resp = await fetch(
                `${Deno.env.get('SUPABASE_URL')}/functions/v1/unequip-item`,
                {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
