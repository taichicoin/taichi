...
window.YYCardShop = (function() {
    ...
    // ========== 已修复：彻底禁止后端覆盖商店active/subIndex ==========
    function mergeUpdatedPlayer(target, updatedPlayer) {
        if (!updatedPlayer) return;
        const fields = ['gold', 'exp', 'shopLevel', 'health', 'isBot', 'isEliminated', 'isReady', 'hand', 'board'];
        fields.forEach(key => {
            if (updatedPlayer[key] !== undefined) {
                if (key === 'shopCards' && target.shopCards && updatedPlayer.shopCards) {
                    // 只同步buffer和next，**坚决不碰active、subIndex**
                    if (updatedPlayer.shopCards.buffer) {
                        target.shopCards.buffer = JSON.parse(JSON.stringify(updatedPlayer.shopCards.buffer));
                    }
                    if (updatedPlayer.shopCards.next !== undefined) {
                        target.shopCards.next = JSON.parse(JSON.stringify(updatedPlayer.shopCards.next));
                    }
                    // 直接return，不再执行后面赋值，保护本地页码
                    return;
    ...
    // ==================== ★ 商店刷新（已修复 乐观秒切 不被后端覆盖） ====================
    // 已放宽判断：不用严格15长度，只要是数组就允许切组
    ...
    async function refreshShopAction() {
        ...
        // ========== 防连点改成150毫秒 ==========
        if (Date.now() - lastRefreshTime < 150) return;
        ...
        if (canFlip) {
            ...
            if (gs) {
                gs.players[getCurrentUserId()].gold = my.gold;
                // 同步商店状态到全局，防止渲染读旧数据
                gs.players[getCurrentUserId()].shopCards = JSON.parse(JSON.stringify(my.shopCards));
            }
            touchGold();
            performLocalFlip();
            updateBuyExpButtonState();
