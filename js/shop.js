...
window.YYCardShop = (function() {
    ...
    let pendingNetworkRequest = false;  // 是否有尚未返回的后端请求
    ...
    async function refreshShopAction() {
        ...
        // 已经有网络请求在进行中，只做本地翻页，不扣金币
        if (pendingNetworkRequest) {
            performLocalFlip();
            return;
        }
    ...
    function init() {
        ...
        bindUIEvents();
        refreshAllUI();
        console.log('✅ 商店系统 (安全装备 + 乐观刷新 + 防连点) 已启动');
    }
