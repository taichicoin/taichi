...
window.YYCardShop = (function() {
    ...
    // ★ 新增：本地商店卡牌缓存与乐观更新机制
    let _shopCardCache = {}; // key: `${roomId}-${userId}-${activeGroup}-${subIndex}` => [cards...]
    let _purchasedIndices = new Set(); // 当前分页已被玩家点买的卡牌索引 (乐观更新用)
    let _currentShopGroup = null; // 记录当前渲染的组索引
    let _currentShopSub = 0;      // 记录当前渲染的子索引
        ...
        fields.forEach(key => {
            if (updatedPlayer[key] !== undefined) {
                if (key === 'shopCards' && target.shopCards && updatedPlayer.shopCards) {
                    if (updatedPlayer.shopCards.active !== undefined) target.shopCards.active = updatedPlayer.shopCards.active;
                    if (updatedPlayer.shopCards.subIndex !== undefined) target.shopCards.subIndex = updatedPlayer.shopCards.subIndex;
    ...
    async function renderShop() {
        ...
        const shopMeta = my.shopCards;
        if (!shopMeta) {
            container.innerHTML = '<div style="color:#aaa;padding:10px;text-align:center;width:100%;">商店元数据为空</div>';
            return;
        }

        const active = shopMeta.active ?? 0;
        const sub = shopMeta.subIndex ?? 0;
        _currentShopGroup = active;
        _currentShopSub = sub;

        const cacheKey = `${roomId}-${userId}-${active}-${sub}`;
        if (!_shopCardCache[cacheKey]) {
            _purchasedIndices.clear();
        }

        // ★ 前置验证：防止拉取时 roomId 或 userId 为空导致白查询
        if (!roomId || !userId) {
            container.innerHTML = '<div style="color:#f5d76e;padding:10px;text-align:center;width:100%;">⏳ 等待获取房间或用户ID...</div>';
            return;
        }
        ...
        try {
            ...
            const { data, error } = await supabaseClient
                ...
                .eq('room_id', roomId)
                .eq('user_id', userId)
                .eq('group_index', active)
                .order('card_index')
                .range(sub * 3, sub * 3 + 2);

            // ★ 遇到任何 Supabase 错误，直接停止并打印详细错误，而不是只显示“加载失败”！
            if (error) {
                let errorMsg = `<b>❌ Supabase 拒绝请求 (Error ${error.code || '未知'})</b><br><br>`;
                errorMsg += `<b>详细信息：</b><span style="color:#ffa500">${error.message || error}</span><br>`;
                if (error.details) errorMsg += `<b>详情：</b>${error.details}<br>`;
                
                // 检测到权限被拒的专门提示
                if (error.code === '42501' || error.message?.includes('permission denied')) {
                    errorMsg += `<br><b style="color:red;">🚨 真实原因：RLS 权限拦截！</b><br>`;
                    errorMsg += `请立刻去 Supabase 的 SQL 面板执行：<br>`;
                    errorMsg += `<div style="background:#222;color:#d4d4d4;padding:8px;margin-top:6px;font-family:monospace;border-radius:4px;text-align:left;overflow-x:auto;font-size:12px;">`;
                    errorMsg += `ALTER TABLE shop_cards ENABLE ROW LEVEL SECURITY;<br>`;
                    errorMsg += `CREATE POLICY "允许玩家读自己的商店牌" ON shop_cards FOR SELECT USING (auth.uid() = user_id);`;
                    errorMsg += `</div>`;
                }

                // 抛出全新的错误信息，直接让下面的 catch 捕获并在画面上渲染
                throw new Error(errorMsg);
            }

            // ★ 如果 query 成功，但 data 是空的
            if (!data || data.length === 0) {
                container.innerHTML = '<div style="color:#aaa;padding:10px;text-align:center;width:100%;">⚠️ 当前页没有卡牌数据（可能 RLS 策略还在生效）</div>';
                return;
            }

            _shopCardCache[cacheKey] = data.map(item => item.card_data);

            const groupCards = _shopCardCache[cacheKey];
            container.innerHTML = '';
            const fragment = document.createDocumentFragment();
            ...
            for (let i = 0; i < 3; i++) {
                ...
                if (isValidCard(card) && !_purchasedIndices.has(i)) {
                    hasAnyCard = true;
            ...
            if (!hasAnyCard) {
                container.innerHTML = '<div style="color:#aaa;padding:10px;text-align:center;width:100%;">该页商店暂无卡牌</div>';
            } else {
                container.appendChild(fragment);
            }
        } catch (err) {
            console.error('读取商店表彻底崩溃:', err);
            // ★ 直接将报错拍死在屏幕的商店区域
            container.innerHTML = `<div style="color:#ff6b6b;padding:15px;text-align:left;width:100%;font-size:13px;background:rgba(255,0,0,0.1);border:1px solid #ff6b6b;border-radius:8px;line-height:1.6;word-break:break-all;overflow:auto;">${err.message}</div>`;
        }
    }
    ...
    async function equipFromShop(shopIdx, boardIdx) {
        ...
        try {
            ...
            // ★ 从本地缓存读取卡牌，而不是从 buffer
            const cacheKey = `${getCurrentRoomId()}-${getCurrentUserId()}-${_currentShopGroup}-${_currentShopSub}`;
            const groupCards = _shopCardCache[cacheKey] || [];
            const shopCard = groupCards[shopIdx];
            ...
            if (!result.success) {
                my.gold = oldGold;
                _purchasedIndices.delete(shopIdx);
                renderShop();
