            ...
            try {
                ...
                if (session) {
                    ...
                    if (result?.success && result.updatedPlayer) {
                        // ★ 直接应用返回的玩家数据到本地 gameState
                        const myId = auth.currentUser.id;
                        if (gameState?.players?.[myId]) {
                            const my = gameState.players[myId];
                            if (result.updatedPlayer.gold !== undefined) my.gold = result.updatedPlayer.gold;
                            if (result.updatedPlayer.exp !== undefined) my.exp = result.updatedPlayer.exp;
                            if (result.updatedPlayer.freeRefresh !== undefined) my.freeRefresh = result.updatedPlayer.freeRefresh;
                            if (result.updatedPlayer.isReady !== undefined) my.isReady = result.updatedPlayer.isReady;
                            if (result.playerRound !== undefined) my.playerRound = result.playerRound;
                        }
                        // 刷新 UI
