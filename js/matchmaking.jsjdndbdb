// ==================== 匹配系统【终极修复版 + 棋盘纯角色 + 手牌限一武器 + 商店每组15张 + 增益初始化 + 免费刷新字段】 ====================
window.YYCardMatchmaking = (function() {
    ...
    // ✅ 【双缓冲v3 + 棋盘纯角色 + 手牌限一武器 + 每组15张 + 增益字段 + freeRefresh】初始化游戏
    async function initializeGame(roomId, players) {
        log('🎮 开始初始化游戏状态（双缓冲商店 v3 + 棋盘纯角色 + 手牌限一武器 + 每组15张 + freeRefresh）...');
        try {
            ...
            log('🎉 游戏初始化完成（含 freeRefresh），进入对战！');
            window.YYCardBattle?.enterBattle?.(roomId);
            resetUI();
...
console.log('✅ matchmaking.js 加载完成（终极修复版 + 双缓冲商店v3初始化 + 装备槽 + 棋盘纯角色 + 每组15张 + 增益初始化 + freeRefresh）');
