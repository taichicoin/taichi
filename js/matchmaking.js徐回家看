...
window.YYCardMatchmaking = (function() {
    ...
    // 强力清理：直接删除当前用户的所有 room_players 记录
    async function cancel() {
        ...
        if (deleteError) {
            log(`❌ 删除失败: ${deleteError.message}`, true);
        } else {
            log(`✅ 已从所有房间移除用户 ${uid}`);
        }
    ...
    async function cleanEmptyRooms() {
        const { data: rooms } = await supabase.from('rooms').select('id').eq('status', 'waiting');
        if (!rooms) return;
        for (const r of rooms) {
            const { data: players } = await supabase.from('room_players').select('*').eq('room_id', r.id).eq('is_bot', false);
            if (!players || players.length === 0) {
                await supabase.from('game_states').delete().eq('room_id', r.id);
