...
window.YYCardInspector = (function() {
    ...
    function showCardDetail(card, boardIndex, element, isOwn) {
        ...
        // 生成每个槽的HTML —— 垂直高度再缩小20%，虚线分隔，文字提亮
        function renderSlot(slotKey, equip, label) {
            if (equip) {
                return `
                    <div class="equip-row" style="display:flex; justify-content:space-between; align-items:center; background:rgba(255,255,255,0.1); padding:2px 4px; border-radius:4px; margin:1px 0; border-bottom:1px dashed rgba(255,255,255,0.15);">
                ...
                return `
                    <div class="equip-row empty-slot" style="background:rgba(255,255,255,0.05); padding:2px 4px; border-radius:4px; margin:1px 0; color:#ddd; font-size:10px; border-bottom:1px dashed rgba(255,255,255,0.15);">
                        ${label}待装配
                    </div>
                `;
            }
