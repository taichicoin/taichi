...
window.YYCardInspector = (function() {
    ...
    function getSkillFromCard(card) {
        ...
        if (card.skill) {
            try {
                ...
                if (s && (s.skillName || s.name)) {
                    return { name: s.skillName || s.name, desc: s.skill_describe || s.desc || '' };
                }
            } catch(e) {}
        }
        if (card.abilities && Array.isArray(card.abilities) && card.abilities.length > 0) {
            ...
            if (ab) {
                return { name: ab.skillName || ab.name || '技能', desc: ab.skill_describe || ab.desc || '' };
            }
        }
    ...
    function showCardDetail(card, boardIndex, element, isOwn) {
        ...
        // ★ 修改后的卡牌 HTML 生成，图片使用 contain 居中，不放大不裁切
        ...
        Object.assign(popupEl.style, {
            ...
            borderRadius: '0 0 16px 16px', padding: '10% 10%',
