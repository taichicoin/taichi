...
window.YYCardInspector = (function() {
    ...
    function showCardDetail(card, boardIndex, element, isOwn) {
        ...
        // 生成每个槽的HTML —— 高度统一，虚线分隔，文字亮度适中
        function renderSlot(slotKey, equip, label) {
            const baseStyle = 'display:flex; justify-content:space-between; align-items:center; background:rgba(255,255,255,0.1); padding:4px 6px; border-radius:4px; margin:1px 0; min-height:33px; line-height:1.3; border-bottom:1px dashed rgba(255,255,255,0.2);';
                ...
                return `
                    <div class="equip-row" style="${baseStyle}">
                        <div style="display:flex; align-items:center; gap:5px;">
                            ...
                            <div>
                                <div style="font-weight:bold; font-size:12px;">${equip.name}</div>
                                <div style="font-size:10px; color:#ccc;">${equip.type === 'weapon' ? '⚔️' : '💊'} +${equip.atk||0} / +${equip.hp||0}</div>
                            </div>
                        </div>
                        ${isOwn ? `<button class="unequip-btn" data-slot="${slotKey}" style="background:#c44; color:#fff; border:none; border-radius:3px; padding:2px 8px; font-size:10px; cursor:pointer;">✕</button>` : ''}
                    </div>
                `;
            } else {
                return `
                    <div class="equip-row empty-slot" style="${baseStyle} color:#ddd; font-size:11px;">
                        <div style="display:flex; align-items:center; gap:5px;">
                            <div style="width:24px; height:24px; border-radius:4px; background:rgba(255,255,255,0.1); flex-shrink:0;"></div>
                            <span>${label}待装配</span>
                        </div>
                    </div>
                `;
        ...
        // ---------- 弹窗样式：左右拉满，内部内容左右内边距10% ----------
        Object.assign(popupEl.style, {
            position: 'fixed',
            top: '0',                    // 顶部对齐
            left: '0',                   // 左拉满
            right: '0',                  // 右拉满
            maxHeight: '90vh',           // 高度缩小10%
            overflowY: 'auto',
            background: 'rgba(20,30,50,0.95)',
            color: '#fff',
            borderRadius: '0 0 16px 16px',
            padding: '12px 10%',         // 上下12px，左右10%内边距
            zIndex: '100002',
