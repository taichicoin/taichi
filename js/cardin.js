...
window.YYCardInspector = (function() {
    ...
    function showCardDetail(card, boardIndex, element, isOwn) {
        ...
        // 渲染装备槽：已装备有背景块，空槽极简文字行
        function renderSlot(slotKey, equip, label) {
            if (equip) {
                return `
                    <div class="equip-row" style="display:flex; justify-content:space-between; align-items:center; background:rgba(255,255,255,0.1); padding:2px 4px; border-radius:4px; margin:1px 0; border-bottom:1px dashed rgba(255,255,255,0.25);">
                ...
                // 空槽：无背景块，极简高度
                return `
                    <div class="equip-row empty-slot" style="padding:1px 0; margin:0; color:#eee; font-size:10px; border-bottom:1px dashed rgba(255,255,255,0.25);">
                        ${label}待装配
                    </div>
                `;
            }
