// ==================== 卡牌查看系统（名字/图片从 /data/image.json 获取，技能从卡牌自身读取） ====================
window.YYCardInspector = (function() {
    ...
    function getSkillFromCard(card) {
        ...
        if (!skillObj && card.abilities) {
            const ab = Array.isArray(card.abilities) ? card.abilities[0] : card.abilities;
            if (ab) skillObj = ab;
        }
        if (!skillObj) return null;
        return {
            name: skillObj.skillName || skillObj.name || '未知技能',
            desc: skillObj.skill_describe || skillObj.desc || '暂无描述'
        };
    }
    ...
    function showCardDetail(card, boardIndex, element, isOwn) {
        ...
        Object.assign(popupEl.style, {
            ...
            borderRadius: '0 0 16px 16px', padding: '12px 5%',
    ...
    async function init() {
        ...
        console.log('✅ 卡牌查看系统已启动（名字/图片从 image.json，技能从卡牌自身）');
    }
