...
window.YYCardInspector = (function() {
    ...
    function showCardDetail(card, boardIndex, element, isOwn) {
        ...
        function renderSlot(slotKey, equip, label) {
            if (equip) {
                return `
                    <div class="equip-row" style="display:flex; justify-content:space-between; align-items:center; background:rgba(255,255,255,0.1); padding:6px 8px; border-radius:8px; margin:4px 0;">
                        ...
                        ${isOwn ? `<button class="unequip-btn" data-slot="${slotKey}" style="background:#c44; color:#fff; border:none; border-radius:4px; padding:2px 10px; font-size:12px; cursor:pointer;">✕</button>` : ''}
                    </div>
                `;
            } else {
                return `
                    <div class="equip-row empty-slot" style="background:rgba(255,255,255,0.05); padding:6px 8px; border-radius:8px; margin:4px 0; color:#999; font-size:12px;">
                        ${label}待装配
                    </div>
                `;
            }
        ...
        popupEl.innerHTML = `
            ...
            <div class="inspect-equipment" style="margin-top:10px; border-top:1px solid #444; padding-top:8px;">
        ...
        // ---------- 修改样式：顶部对齐、左右拉满、高度缩小10% ----------
        Object.assign(popupEl.style, {
            position: 'fixed',
            top: '0',                    // 最顶部对齐
            left: '0',
            right: '0',                  // 左右拉满
            maxHeight: '90vh',           // 高度缩小10%（原100%视口高度的90%）
            overflowY: 'auto',           // 内容过多时可滚动
            background: 'rgba(20,30,50,0.95)',
            color: '#fff',
            borderRadius: '0 0 16px 16px', // 保留下方圆角，上方贴边平直
            padding: '20px',
            zIndex: '100002',
