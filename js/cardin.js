...
window.YYCardInspector = (function() {
    ...
    function showCardDetail(card, boardIndex, element, isOwn) {
        ...
        // 生成每个槽的HTML —— 高度进一步缩小，空槽文字提亮，装备/空槽高度一致
        function renderSlot(slotKey, equip, label) {
            if (equip) {
                return `
                    <div class="equip-row" style="display:flex; justify-content:space-between; align-items:center; background:rgba(255,255,255,0.1); padding:2px 4px; border-radius:4px; margin:1px 0;">
                ...
                return `
                    <div class="equip-row empty-slot" style="background:rgba(255,255,255,0.05); padding:2px 4px; border-radius:4px; margin:1px 0; color:#ccc; font-size:11px;">
                        ${label}待装配
                    </div>
                `;
            }
