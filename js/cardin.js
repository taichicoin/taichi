...
window.YYCardInspector = (function() {
    ...
    function showCardDetail(card, boardIndex, element, isOwn) {
        ...
        // 生成每个槽的HTML —— 高度完全统一，虚线分隔，空槽文字提亮
        function renderSlot(slotKey, equip, label) {
            // 统一基础样式，固定最小高度，确保装备/空槽高度一致
            const baseStyle = 'display:flex; justify-content:space-between; align-items:center; background:rgba(255,255,255,0.1); padding:2px 4px; border-radius:4px; margin:1px 0; min-height:22px; line-height:1.2; border-bottom:1px dashed rgba(255,255,255,0.15);';
            
            if (equip) {
                return `
                    <div class="equip-row" style="${baseStyle}">
                        <div style="display:flex; align-items:center; gap:4px;">
                            <img src="${equip.image || '/assets/default-avatar.png'}" style="width:18px; height:18px; border-radius:3px;" onerror="this.src='/assets/default-avatar.png'">
                            <div>
                                <div style="font-weight:bold; font-size:11px;">${equip.name}</div>
                                <div style="font-size:9px; color:#ccc;">${equip.type === 'weapon' ? '⚔️' : '💊'} +${equip.atk||0} / +${equip.hp||0}</div>
                            </div>
                        </div>
                        ${isOwn ? `<button class="unequip-btn" data-slot="${slotKey}" style="background:#c44; color:#fff; border:none; border-radius:3px; padding:1px 6px; font-size:10px; cursor:pointer;">✕</button>` : ''}
                    </div>
                `;
            } else {
                return `
                    <div class="equip-row empty-slot" style="${baseStyle} color:#ddd; font-size:10px;">
                        <div style="display:flex; align-items:center; gap:4px;">
                            <!-- 占位块，保持与装备图标相同尺寸，确保高度一致 -->
                            <div style="width:18px; height:18px; border-radius:3px; background:rgba(255,255,255,0.1); flex-shrink:0;"></div>
                            <span>${label}待装配</span>
                        </div>
                    </div>
                `;
