// ==================== 商店与交互系统【修复版：解决操作锁死+接口解析报错+阶段判定错乱】 ====================
window.YYCardShop = (function() {
    ...
    // ========== 新增：专门用于刷新商店的请求函数（无JWT验证） ==========
    async function requestRefreshShop(roomId, userId) {
        try {
            const controller = new AbortController();
            const timeoutId = setTimeout(() => controller.abort(), 10000);
            ...
            if (!contentType || !contentType.includes('application/json')) {
                const errorText = await response.text();
                console.error('刷新接口返回非JSON内容：', errorText);
                throw new Error('服务器接口异常，请检查函数部署状态');
            }
    ...
    async function refreshShopAction() {
        ...
        // ========== 改用无JWT的刷新接口 ==========
        const result = await requestRefreshShop(roomId, userId);
