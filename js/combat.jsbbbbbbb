...
window.YYCardCombat = (function() {
    ...
    // 直接通过棋盘 ID 和索引定位卡牌
    function getCardElement(boardId, position) {
        const board = document.getElementById(boardId);
        if (!board) return null;
        const slots = board.querySelectorAll('.card-slot');
        if (position >= slots.length) return null;
        const slot = slots[position];
        // 查找卡牌元素（可能是 .card 或 img 或其他）
    ...
    function fightWithLog(board1, board2, owner1, owner2) {
        ...
        while (units1.length > 0 && units2.length > 0 && turn < maxTurns) {
            ...
            combatLog.push({
                attacker: { side: attacker.side, pos: aPos, atk: attacker.atk },
                defender: { side: target.side, pos: dPos, hpAfter: target.hp },
    ...
    function playAttackAnim(action) {
        return new Promise(resolve => {
            ...
            if (!$attacker || !$defender) {
                panelLog(`⚠️ 跳过攻击 (${aBoard}[${action.attacker.pos}] → ${dBoard}[${action.defender.pos}])`, true);
                resolve();
                return;
            }

            panelLog(`⚔️ ${aBoard}[${action.attacker.pos}] → ${dBoard}[${action.defender.pos}]`);
