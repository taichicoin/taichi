// ==================== 商店与交互系统（后端刷新商店 + 后端购买/交换 + 后端出售 + 后端放置 + 调试弹窗） ====================
window.YYCardShop = (function() {
    ...
    const PLACE_CARD_FUNCTION_URL = 'https://sznjaotjoljaiawbvfro.supabase.co/functions/v1/place-card';  // 新增放置函数
    ...
    function init() {
        ...
        bindUIEvents();
        refreshAllUI();
        log('✅ 商店交互模块已启动（后端刷新 + 购买/交换 + 出售 + 放置 + 弹窗调试）');
    }
...
console.log('✅ shop.js 加载完成（后端刷新商店 + 后端购买/交换 + 后端出售 + 后端放置 + 弹窗调试）');
