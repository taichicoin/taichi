...
window.YYCardMatchmaking = (function() {
    ...
    async function handleTimeout() {
        ...
        log(`✅ 成功添加 ${availableBots.length} 个人机，现在主动检查房间满员`);
        // 关键修复：插入人机后立即检查并开局
        await checkRoomFull(currentRoom.id);
    }
    ...
    async function checkRoomFull(roomId) {
        log(`🔎 检查房间 ${roomId.slice(0, 8)} 是否满员...`);
        ...
        if (error) {
            log(`❌ 查询房间玩家失败: ${error.message}`, true);
            return;
        }

        const count = players?.length || 0;
        updateStatus(`匹配中... ${count}/${config.MAX_PLAYERS_PER_ROOM}`);
        log(`👥 当前房间人数: ${count}/${config.MAX_PLAYERS_PER_ROOM}`);
        ...
        if (count >= config.MAX_PLAYERS_PER_ROOM) {
            ...
            if (room.status === 'waiting') {
                ...
                if (updateError) {
                    log(`❌ 更新房间状态失败: ${updateError.message}`, true);
                    return;
                }
...
console.log('✅ matchmaking.js 加载完成（强制开局修复版）');
