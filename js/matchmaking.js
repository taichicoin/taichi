// ==================== 匹配系统【终极修复版 + 棋盘纯角色 + 手牌限一武器 + 商店每组15张 + 增益初始化 + 脏数据修复】 ====================
window.YYCardMatchmaking = (function() {
    ...
    // ✅ 【双缓冲v3 + 棋盘纯角色 + 手牌限一武器 + 每组15张 + 增益字段 + 脏数据修复】初始化游戏
    async function initializeGame(roomId, players) {
        log('🎮 开始初始化游戏状态（双缓冲商店 v3 + 棋盘纯角色 + 手牌限一武器 + 每组15张）...');
        try {
            // 幂等性校验：游戏状态已存在且有效，直接进入对战
            const { data: existing } = await supabase
                .from('game_states')
                .select('state')
                .eq('room_id', roomId)
                .maybeSingle();
            // ★ 修复脏数据：只有 existing 存在且 state 不为 null 才跳过初始化
            ...
            function buildShopGroup(shopLevel) {
                ...
                while (group.length < 15) group.push(null);   // ★ 改为 15
                return group;
            }
            ...
            for (const p of players) {
                ...
                // 剩余角色卡放入手牌
                const remainingCharacters = characters.slice(3).map(c => ({...c, star: c.star || 0}));
                const handCards = [...remainingCharacters];
                ...
                try {
                    ...
                    log(`⚠️ 玩家${p.player_id.slice(0,8)}商店生成失败: ${e.message}，使用空组`, true);
                    // 空组改为 15 个 null
                    const nulls15 = new Array(15).fill(null);
                    shopData = {
                        buffer: [nulls15, nulls15],
                        active: 0,
                        subIndex: 0,
                        next: null
                    };
                ...
                state.players[p.player_id] = {
                    ...
                    hand: finalHand,
                    shopCards: shopData,          // ✅ 双缓冲 v3 对象 (每组15张)
                    isBot: isBot,
                    isReady: false,
                    isEliminated: false,
                    activeBuff: null,            // ★ 增益效果初始为空
                    buffRefreshCount: 0,         // ★ 增益刷新计数初始为0
                    totalRefreshes: 0            // ★ 全局刷新次数（姜子牙技能等需要）
                };
            }
            ...
            log('🎉 游戏初始化完成（v3双缓冲商店 + 棋盘纯角色 + 手牌限一武器 + 每组15张 + 增益字段 + 脏数据修复），进入对战！');
            window.YYCardBattle?.enterBattle?.(roomId);
            resetUI();
...
console.log('✅ matchmaking.js 加载完成（终极修复版 + 双缓冲商店v3初始化 + 装备槽 + 棋盘纯角色 + 每组15张 + 增益初始化 + 脏数据修复）');
