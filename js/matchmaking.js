...
window.YYCardMatchmaking = (function() {
    ...
    async function cancel() {
        if (!currentRoom) { resetUI(); return; }
        log('🛑 取消匹配...');
        cleanup();
        await supabase.from('room_players').delete().eq('room_id', currentRoom.id).eq('player_id', auth.currentUser.id);
        const { data: remaining } = await supabase.from('room_players').select('*').eq('room_id', currentRoom.id).eq('is_bot', false);
        if (!remaining?.length) {
    ...
    // 修复：正确处理 existingIds 数组，不手动拼接 SQL
    async function handleTimeout() {
        ...
        // 2. 构建查询：从 profiles 表选取 is_bot = true 且不在 existingIds 中的账号
        let query = supabase
            .from('profiles')
            .select('id')
            .eq('is_bot', true)
        ...
        // 关键修复：只有当 existingIds 不为空时才添加 not.in 条件
        // 直接传入数组，Supabase 会自动格式化为正确语法
        if (existingIds.length > 0) {
            query = query.not('id', 'in', `(${existingIds.map(id => `'${id}'`).join(',')})`);
        }

        const { data: availableBots, error: botError } = await query;

        if (botError) {
            log(`❌ 查询人机失败: ${botError.message}`, true);
            return;
        }

        if (!availableBots || availableBots.length < neededBots) {
            log(`❌ 可用人机不足，需要 ${neededBots}，实际 ${availableBots?.length || 0}`, true);
            return;
        }
        ...
        for (const bot of availableBots) {
            ...
            if (insertError) {
                log(`❌ 插入人机失败: ${insertError.message}`, true);
            }
        }
