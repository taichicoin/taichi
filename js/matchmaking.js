...
window.YYCardMatchmaking = (function() {
    ...
    async function start() {
        ...
        try {
            ...
            if (!room) {
                const { data: newRoom } = await supabase.from('rooms').insert({ status: 'waiting', max_players: config.MAX_PLAYERS_PER_ROOM }).select('*').single();
                room = newRoom;
                log(`✅ 创建新房间: ${room.id}`);
            } else {
                log(`✅ 加入现有房间: ${room.id}`);
...
console.log('✅ matchmaking.js 加载完成（仅修复人机填充后开局）');
