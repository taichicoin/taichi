...
window.YYCardMatchmaking = (function() {
    ...
    // ✅ 【双缓冲v3 + 棋盘纯角色 + 手牌限一武器 + 每组9张】初始化游戏
    async function initializeGame(roomId, players) {
        log('🎮 开始初始化游戏状态（双缓冲商店 v3 + 棋盘纯角色 + 手牌限一武器 + 每组9张）...');
        try {
            ...
            function buildShopGroup(shopLevel) {
                ...
                while (group.length < 9) group.push(null);
                return group;
            }
            ...
            for (const p of players) {
                ...
                state.players[p.player_id] = {
                    ...
                    hand: finalHand,
                    shopCards: shopData,          // ✅ 双缓冲 v3 对象 (每组9张)
                    isBot: isBot,
                    isReady: false,
                    isEliminated: false
                };
            ...
            log('🎉 游戏初始化完成（v3双缓冲商店 + 棋盘纯角色 + 手牌限一武器 + 每组9张），进入对战！');
            window.YYCardBattle?.enterBattle?.(roomId);
            resetUI();
...
console.log('✅ matchmaking.js 加载完成（终极修复版 + 双缓冲商店v3初始化 + 装备槽 + 棋盘纯角色）');
