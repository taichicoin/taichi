...
window.mergeService = (function() {
    ...
    function getRarityColors(rarity) {
        switch(rarity) {
            case 'Legendary': return { glow: 'rgba(255,0,0,0.9)',   inner: 'rgba(255,0,0,0.2)',   particle: '#ff3333' };
            case 'Epic':      return { glow: 'rgba(255,136,0,0.9)', inner: 'rgba(255,136,0,0.2)', particle: '#ff8800' };
            case 'Rare':      return { glow: 'rgba(0,128,255,0.9)', inner: 'rgba(0,128,255,0.2)', particle: '#0088ff' };
            default:          return { glow: 'rgba(0,255,0,0.9)',   inner: 'rgba(0,255,0,0.2)',   particle: '#00ff00' };
        }
    }
    ...
    function updateMergeGlow() {
        ...
        document.querySelectorAll(selectors.join(',')).forEach(el => {
            ...
            if (card && canMerge(card)) {
                ...
                // 外发光 (明显) + 内发光 (铺满内部, 透明度20%)
                el.style.boxShadow = `0 0 20px 5px ${colors.glow}, inset 0 0 40px ${colors.inner}`;
            } else {
                el.style.boxShadow = '';
            }
        });
