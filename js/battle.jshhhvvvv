...
window.YYCardBattle = (function() {
    ...
    async function enterBattle(roomId) {
        ...
        const waitForState = setInterval(() => {
            ...
            // 关键修复：如果第一回合状态不是 prepare，强制修正
            if (round === 1 && phase !== 'prepare') {
                log(`⚠️ 检测到第1回合状态异常 (${phase})，强制改为 prepare`, true);
                phase = 'prepare';
                gameState.phase = 'prepare';
                updateGameState();
            }
    ...
    function refreshAllShops() {
        for (const pid in gameState.players) {
            gameState.players[pid].shopCards = utils.generateShopCards(gameState.players[pid].shopLevel);
        }
    }
...
console.log('✅ battle.js 加载完成（强制准备阶段版）');
