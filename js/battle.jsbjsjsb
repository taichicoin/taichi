// ==================== 对战系统【纯时间驱动版 + 状态机根治锁 + 强制同步UI + 轮询保底 + 缓冲期 + 淘汰名次 + 重连修复】 ====================
window.YYCardBattle = (function() {
    ...
    // 修改：增加 persistent 参数，true 表示 60 秒后自动移除
    ...
    // ===== 记录淘汰顺序，输出名次日志（仅此处使用 persistent） =====
    ...
    async function fastForwardAndResume() {
        ...
        // ===== 修复：重连时不刷新商店，使用数据库中的现有数据 =====
        // await refreshAllShops();   // <--- 这行已删除
        await updateGameState();
...
console.log('✅ battle.js 加载完成（重连商店修复 + 仅保留淘汰名次日志）');
