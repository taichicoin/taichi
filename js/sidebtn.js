...
(function () {
    ...
    if (!document.getElementById('sidebtn-common-style')) {
        ...
        style.textContent = `
            ...
            .genshin-side-btn {
                ...
                user-select: none;
                transition: 0.25s;
                position: relative; /* 不再是 fixed，由容器定位 */
            }
    ...
    window.YYCardSideBtn = {
         ...
         * @param {string} options.iconSvg - SVG 图标字符串
         * @param {string} options.text - 按钮文字
         * @param {Function} options.onClick - 点击处理
         * @param {HTMLElement} [options.appendTo] - 挂载容器，默认为 body
         ...
         * @param {HTMLElement} [options.appendTo] - 按钮挂载容器
         */
        createWithPanel: function(options) {
            const { iconSvg, text, title, contentHtml, appendTo } = options;
