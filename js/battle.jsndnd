...
window.YYCardBattle = (function() {
    ...
    // ★ 修改后的 launchBattleAnimation，加上 battleAnimationLock
    async function launchBattleAnimation(isReconnect = false) {
        // 双锁：动画播放中 或 已发起动画请求，直接返回
    ...
    function forceStopAnimation() {
        ...
        battleAnimationLock = false;   // 强制解锁
    }
        ...
        for (let i = 0; i < 20; i++) {
            ...
            if (state) {
                ...
                if (currentPhase === 'battle' && !myPlayer?.isEliminated) {
                    ...
                    // ★ 重连也走 launchBattleAnimation，它会自己加锁
