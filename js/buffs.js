...
window.YYCardBuff = (function() {
    ...
    // ★ 修改：改用玩家自己的 playerRound 判断，不再依赖全局回合
    function tryShowBuffSelection(round, phase) {
        ...
        // 只检查自己的回合（跳过战斗后会推进 playerRound）
        const playerRound = my.playerRound || gameState.round || 1;
        if (playerRound < 6) return;          // 自己的回合还没到6
        if (phase !== 'prepare') return;      // 必须处于准备阶段
        if (my.activeBuff) return;            // 已经选过增益
