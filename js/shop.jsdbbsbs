...
window.YYCardShop = (function() {
    ...
    async function equipFromShop(shopIdx, boardIdx) {
        ...
        // 1. 获取商店中的武器/道具（直接从 gameState 里拿，避免拖拽对象过期）
        const shop = my.shopCards;
    ...
    async function unequipAllIfNeeded(boardIdx) {
        ...
        if (emptySlots < slotsToRemove.length) {
            toast(`手牌空间不足（需${slotsToRemove.length}空位）`, true);
            return false;
        }
    ...
    function onDragEnd(e) {
        ...
        if (dropResult) {
            executeDropAction(type, index, dropResult, currentX, currentY);
        }
        dragState.active = false;
    }
    ...
    function init() {
        ...
        bindUIEvents();
        refreshAllUI();
        console.log('✅ 商店系统 (独立装备函数 + 原版刷新) 已启动');
    }
