// ==================== 自动合成模块 (合成锁 + 伪元素内光 + 外发光 + 颜色飞行 + 卡牌锁定 + 卸下修复 + 目标位置装备保留 + 手牌武器合成 + 战斗阶段只显示光效不合成) ====================
window.mergeService = (function() {
    ...
    let isMerging = false;
    let mergingCards = [];
    let mergeLock = false;  // ★ 合并锁：为 true 时禁止外部刷新覆盖本地数据
    ...
    function findMergePlan(my) {
        ...
        for (const key in characterGroups) {
            ...
            if (group.length >= 3) {
                ...
                const targetCard = my.board[targetIndex];
                const newCard = {
                    ...mergeTargets[0].card,
    ...
    // ★ 修改：允许强制准备模式下合成，且只在准备阶段（或强制准备）执行
