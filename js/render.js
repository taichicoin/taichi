// ==================== 渲染模块：负责 UI 绘制（已移除价格渲染、护盾/道具外显独立） ====================
window.YYCardRender = (function() {
    ...
    // ----- 卡牌元素创建（图片 108%，不含护盾、不含价格、不含效果图标） -----
    function createCardElement(card, cardType = 'board', isBoard = false) {
        ...
        // ★ 护盾和效果图标已移至 YYCardItemRender.applyEffects 统一处理
        return d;
    }
        ...
        if (total <= 4) {
            my.hand.forEach((card, i) => {
                if (isValidCard(card)) {
                    ...
                    // ★ 应用效果（手牌只加护盾，不加图标）
                    if (window.YYCardItemRender) {
                        window.YYCardItemRender.applyEffects(el, card, { includeIcons: false });
                    }
    ...
    function renderBoard(containerId, cards, isSelf) {
        ...
        for (let i = 0; i < 6; i++) {
            ...
            if (isValidCard(c)) {
                ...
                // ★ 棋盘渲染：应用全部效果（护盾 + 嘲讽/跳过行动图标）
                if (window.YYCardItemRender && isSelf) {
                    window.YYCardItemRender.applyEffects(el, c, { includeIcons: true });
                }
            } else {
