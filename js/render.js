...
window.YYCardRender = (function() {
    ...
    function renderHand() {
        ...
        // ========== 统一强制使用 Flex 布局，确保底层拖拽库100%可靠 ==========
        container.style.display = 'flex';
        container.style.flexWrap = 'nowrap';
        container.style.alignItems = 'flex-end';
        container.style.paddingLeft = PADDING_LEFT_VW + 'vw';  // 固定左边界
        container.style.paddingRight = PADDING_RIGHT_VW + 'vw'; // 固定右边界
        ...
        // 2. 扇形圆环弧度参数
        const arcHeightVW = 4.2;       // 中间拱起的最大高度（微调）
        ...
        validCards.forEach((card, i) => {
            ...
            el.style.width = CARD_WIDTH_VW + 'vw';
            el.style.flex = '0 0 ' + CARD_WIDTH_VW + 'vw';
            el.style.position = 'relative'; // 脱离 absolute 魔咒
            el.style.margin = '0';

            // 核心：通过负 MarginRight 让它们紧密重叠叠放，最后一张边距为 0
            const marginRight = i === n - 1 ? 0 : -(CARD_WIDTH_VW - stepX);
            el.style.marginRight = marginRight + 'vw';

            // 扇形倾斜与弧形高度（Transform 不影响文档流布局，可完美拖拽）
