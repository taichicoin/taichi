...
window.YYCardRender = (function() {
    ...
    function renderHand() {
        ...
        // 情况 B：5张牌及以上，渲染成扇形圆环（扑克牌重叠）排列
        else {
            const containerWidthVW = 98; // 容器宽度vw
            const cardWidthVW = 23;      // 卡牌宽度vw
            const paddingLeftVW = 1.5;   // 左侧留出固定距离
            const paddingRightVW = 1.5;  // 右侧留出固定距离

            // 首牌左边缘位置 = 左边距，尾牌左边缘位置 = 容器宽度 - 右边距 - 卡牌宽
            const startLeftVW = paddingLeftVW;
            const endLeftVW = containerWidthVW - paddingRightVW - cardWidthVW;
            const rangeX = endLeftVW - startLeftVW; 

            // 垂直方向的下沉参数，模拟图的“顶部向右下方走”的弧度
            const topBasePx = 15;    // 第一张牌的高度
            const topOffsetPx = 45;  // 最后一张牌下沉的差值

            // 设置容器属性
            container.style.position = 'relative';
            container.style.overflow = 'visible';
            container.style.minHeight = (topBasePx + topOffsetPx + 100) + 'px'; // 给手牌区留下足够高度
            ...
            for (const card of my.hand) {
                ...
                // 计算 0 ~ 1 的进度比例
                const t = i / (total - 1);
                // 核心：使用二次方缓动 t^2，实现"前面压得很紧，最后一张完全露出"的扇形展开效果
                const easedT = t * t; 

                // 水平位置：左边界坐标 (vw)
                const leftPos = startLeftVW + rangeX * easedT;
                el.style.position = 'absolute';
                el.style.left = leftPos + 'vw';

                // 垂直位置：向下延伸 (px)
                const topPos = topBasePx + topOffsetPx * easedT;
