...
window.YYCardRender = (function() {
    ...
    // ===== 核心修复：改用了 inline-block + 负边距布局，完美兼容器物拖拽 =====
    function renderHand() {
        if (isDragging()) return;
        const gameState = getGameState();
        if (!gameState) return;
        ...
        // ========== 分支2：5张及以上，改为“内联块 + 负边距”完美兼容拖动 ==========
        // 使用内联块（inline-block）来替代绝对定位，直接保留 offsetLeft 属性，彻底修复拖不动的问题
        const n = total;
        const availableX = CONTAINER_WIDTH_VW - PADDING_LEFT_VW - PADDING_RIGHT_VW - CARD_WIDTH_VW;
        const stepX = availableX / (n - 1);
        
        // 计算重叠的负边距（牌越多重叠越多，牌越少重叠越少）
        const overlapX = CARD_WIDTH_VW - stepX;

        // 扇形参数：稍微降低一点拱高，防止5~6张牌时拱出容器触摸范围
        const arcHeightVW = 2.8;       
        const maxRotateAngle = 10;     

        // 配置父容器：利用 padding 实现第一张和最后一张的左右固定距离
        container.style.cssText = `
            position: relative;
            display: block;
            padding-left: ${PADDING_LEFT_VW}vw;
            padding-right: ${PADDING_RIGHT_VW}vw;
        ...
        validCards.forEach((card, i) => {
            ...
            // ⚠️ 绝妙改动：变成 inline-block 并保留 position: relative！
            // 游戏拖拽库依赖它，绝对不冲突了
            el.style.cssText += `
            ...
            // 第一张贴左边，后面的牌用负 margin-left 向左挤压，达成重叠效果
            if (i > 0) {
                el.style.marginLeft = -overlapX + 'vw';
            } else {
                el.style.marginLeft = '0vw';
            }

            // Y轴：利用负 margin-top 实现拱起（中间高，两边低）
            const t = i / (n - 1);
            const yOffset = Math.sin(t * Math.PI) * arcHeightVW;
            el.style.marginTop = -yOffset + 'vw';

            // Z轴层级：自然文档流覆盖，加大 Z-index 确保右边压左边
            el.style.zIndex = i + 1;
