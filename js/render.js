...
window.YYCardRender = (function() {
    ...
    function renderHand() {
        ...
        // ========== 分支2：5张及以上，绝对定位 + 半圆环曲线（无倾斜旋转） ==========
        // 开启相对定位，让内部的牌可以用绝对定位自由摆放
        ...
        validCards.forEach((card, i) => {
            ...
            // X轴：第一张在左(2vw)，最后一张在右，中间紧贴叠压
            el.style.left = (PADDING_LEFT_VW + i * stepX) + 'vw';
            
            // Y轴：正弦波曲线。中间高，两边低，形成半圆环（完美保留扇形视觉效果）
            ...
            // ★ 重大改动：移除了 transform: rotate。
            // 这样做是为了防止和你系统的拖拽模块 YYCardDrag 抢夺 transform 属性控制权（避免拖不动）。
            // 只要我们不旋转它，拖拽库就可以完美操作这个元素！
