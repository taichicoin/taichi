...
window.YYCardRender = (function() {
    ...
    function renderHand() {
        ...
        // ⚠️ 这里必须提前过滤出有效卡片，后续遍历用 validCards.forEach 确保索引 i 是连续的 (0, 1, 2...)，绝对避免数组空位导致的索引跳跃！
        ...
        // ========== 分支2：5张及以上，使用扇形/半圆环交错叠压排列 ==========
        // 开启相对定位，让内部的牌可以用绝对定位自由摆放（如扑克牌扇子）
        ...
        // 每个牌之间的步进距离（牌越多，步进越小，叠压越紧凑）
        const stepX = availableX / (n - 1);

        // 2. 扇形圆环弧度参数
        const arcHeightVW = 3.5;       // 中间拱起的最大高度
        const maxRotateAngle = 12;     // 每张牌最大的旋转角度

        // ✅ 现在遍历 validCards，确保 i 永远是 0 到 n-1 的连续数字，防止卖牌后跳位导致的飞出屏幕！
        validCards.forEach((card, i) => {
            const el = createCardElement(card, 'hand');
            el.setAttribute('data-hand-index', i);
            el.setAttribute('data-card-type', 'hand');
            ...
            // 强制使用绝对定位脱离文档流，便于独立控制叠放和倾斜
            el.style.position = 'absolute';
            el.style.margin = '0';
            
            // X轴：第一张在左，最后一张在右，中间紧贴叠压
            el.style.left = (PADDING_LEFT_VW + i * stepX) + 'vw';
            
            // Y轴：正弦波曲线。中间高，两边低，形成半圆环
            const t = i / (n - 1); // 0 ~ 1 区间
            const yOffset = Math.sin(t * Math.PI) * arcHeightVW;
            el.style.bottom = yOffset + 'vw';

            // Z轴：越靠右的牌层级越高，压在左边的牌上面
            el.style.zIndex = i + 1;

            // 设置倾斜旋转轴心为底部中心，像实体扑克牌一样呈扇形打开
