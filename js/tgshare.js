...
window.YYCardTgShare = (function() {
  ...
  // ---------- 修改：允许所有登录用户使用邀请功能 ----------
  function isLoggedIn() {
    return !!auth.currentUser;  // 只要有当前用户，即已登录
  }

  // 获取 user_checkins 记录（通过 user_id 查询）
  async function fetchUserCheckin() {
    ...
    const { data, error } = await supabase
      ...
      .eq('user_id', user.id)   // 改为用 user_id 查询
      .maybeSingle();
    if (error) {
