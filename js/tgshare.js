// ========== 邀请分享模块（仅 Telegram 可操作，普通浏览器提示跳转） ==========
window.YYCardTgShare = (function() {
  ...
  function shareMyLink() {
    // 如果不是 Telegram 环境，提示用户
    if (!isTelegramEnv()) {
      alert('请前往 Telegram 中使用邀请功能');
      return;
    }
  ...
  // 处理 Telegram start_param（仅 TG 环境）
  ...
  async function init() {
    ...
    // 无论是否 TG 环境，都获取一下用户数据（如果有 TG 用户登录也适用）
    const data = await fetchUserCheckin();
    if (data) {
      currentSelfCode = data.self_code;
      currentInviterCode = data.inviter_code;
