...
(function() {
  ...
  // 查询某笔哈希是否已经上过账（返回记录或 null）
  async function checkAlreadyProcessed(txHash) {
    try {
      // 通过 supabase 客户端（带 RLS）
      if (window.supabase) {
        const { data, error } = await window.supabase
          .from('deposit_events')
          .select('created_at')
        ...
        return data; // { created_at: '...' } 或 null
      }
      // 回退 fetch
      const token = window.YYCardAuth?.currentSession?.access_token || '';
      const resp = await fetch(`${SUPABASE_URL}/rest/v1/deposit_events?tx_hash=eq.${encodeURIComponent(txHash)}&select=created_at`, {
        headers: {
          'apikey': SUPABASE_ANON_KEY,
          'Authorization': `Bearer ${token}`
        }
    ...
    overlay.querySelector('#submit-self-btn').onclick = async () => {
      ...
      for (const hash of hashes) {
        ...
        try {
          ...
          if (already) {
            const timeStr = formatTime(already.created_at);
            successCount++;
            results.push(`✅ ${hash.slice(0, 10)}... 已于 ${timeStr} 到账`);
            continue;
          }
          ...
          if (contentType.includes('application/json')) {
            ...
            if (res.ok && json.success) {
              successCount++;
              results.push(`✅ ${hash.slice(0, 10)}... ${json.message || '上账成功'}`);
            } else {
              failCount++;
