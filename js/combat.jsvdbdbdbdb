    ...
    async function resolveBattles(gameState, log, updateCallback) {
        ...
        for (const [p1, p2] of pairs) {
            ...
            const mapped = combatLog.map(e => ({
                attackerBoard: e.attackerOwnerId === myId ? 'my-board' : 'enemy-board',
                defenderBoard: e.defenderOwnerId === myId ? 'my-board' : 'enemy-board',
