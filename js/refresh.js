...
window.YYCardShopRefresh = (function() {
    ...
    async function applySingleBuff(cardElement, newAtk, newHp, atkBuff, hpBuff) {
        ...
        if (atkEl) doShrink(atkEl);
        if (hpEl) doShrink(hpEl);
    ...
    async function processCardBuffQueue(boardIndex, buffs) {
        ...
        // 确保最终数值与状态一致（applySingleBuff 已设置）
    }
    ...
    function applyUpdateAndRerender(updatedPlayer) {
        ...
        // 注意：此时不调用 _renderMyBoard，让动画自己管理 DOM 值
        // 但我们仍需要更新 hand 等其他部分（如果有）
        if (updatedPlayer.hand) {
            my.hand = updatedPlayer.hand;
            _renderHand?.();
        }
        // 商店等正常更新
        if (updatedPlayer.shopCards) {
            // 在切组回调中已经处理，这里可忽略
        }
    }

    // 动画结束后确保棋盘最终显示正确（以防万一）
    ...
    function init(deps) {
        ...
        bindEvents();
        console.log('✅ refresh.js 已启动（排队动画）');
    }
