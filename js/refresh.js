...
window.YYCardShopRefresh = (function() {
    ...
    function playBuffOnCard(cardElement, atkBuff, hpBuff) {
        ...
        cardElement.style.transform = 'scale(1.10)';
        setTimeout(() => {
            cardElement.style.transition = 'transform 0.15s ease-in';
        ...
        // 3. 逐个添加飘字
        if (atkBuff > 0) createFloatText(floatContainer, `⚔️ +${atkBuff}`, '#4caf50');
        if (hpBuff > 0) createFloatText(floatContainer, `🛡️ +${hpBuff}`, '#4caf50');
    ...
    function triggerBuffAnimations(boardBuffs) {
        ...
        boardBuffs.forEach(({ boardIndex, atkBuff, hpBuff }) => {
            const cardEl = document.querySelector(`#my-board [data-slot-index="${boardIndex}"]`);
            if (cardEl) {
    ...
    async function refreshShopAction() {
        ...
        invokeFunction('refresh-shop', { roomId, userId })
            .then(result => {
                if (isSwitchingGroup) {
                    // ★ 切组回调：同步后端状态，只更新非活跃组 buffer，并校正 active/subIndex
                    ...
                    // 普通翻页：只同步金币/免费次数/经验等，不更新 buffer
                    if (mySeq !== refreshSeq) return;
                    if (result.success && result.data.updatedPlayer) {
                        const up = result.data.updatedPlayer;
                        if (up.gold !== undefined) {
    ...
    function init(deps) {
        setDeps(deps);
        injectBuffAnimationStyles();  // 注入动画关键帧
        bindEvents();
