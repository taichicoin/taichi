...
window.YYCardShopRefresh = (function() {
        ...
        if (!isLocalFlipSafe()) {
            ...
            try {
                ...
                if (result.data.updatedPlayer) {
                    ...
                    // 强制更新棋盘（不用 updateUIAfterSuccess，避免条件拦截）
                    forceUpdateBoard(result.data.updatedPlayer);
                    _updateBuyExpButtonState?.();
                    _emit('refresh');
    ...
    function init(deps) {
        ...
        bindEvents();
        console.log('✅ refresh.js 已启动（数值实时更新 + 动画）');
    }
