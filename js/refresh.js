...
window.YYCardShopRefresh = (function() {
    ...
    async function refreshShopAction() {
        ...
        // 乐观更新本地页码显示（后端返回后会覆盖修正）
        const newActive = Math.floor(pageToSend / 10);
        const newSub = pageToSend % 10;
        my.shopCards.active = newActive;
        my.shopCards.subIndex = newSub;
        _renderShop?.();
        updateRefreshButtonDisplay();
        _emit('refresh');

        const mySeq = ++refreshSeq;
        invokeFunction('refresh-shop', { roomId, userId, page: pageToSend })
            .then(result => {
                ...
                if (up) {
                    ...
                    if (result.data.page !== undefined) {
                        ...
                        savePage(currentShopPage);  // ★ 持久化校正后的页码
                    }
                }
    ...
    async function buyExpAction() {
        ...
        if (result.data.updatedPlayer) {
            // ★ 直接从后端数据获取新等级（不依赖 _mergeUpdatedPlayer 的合并行为）
            ...
            // 🔍 临时调试：显示等级变化（测试通过后删除这两行）
            alert('升级检测：旧等级=' + oldLevel + '，新等级=' + newLevel);

            // ★ 用新等级判断是否升级
            if (newLevel > oldLevel) {
                alert('即将弹出消耗牌面板。 consumable 模块：' + (window.YYCardConsumable ? '已加载' : '未加载'));
                if (window.YYCardConsumable?.showSelectionPanel) {
                    window.YYCardConsumable.showSelectionPanel();
                } else {
                    alert('错误：consumable 模块不存在或没有 showSelectionPanel 方法');
                }
            }
    ...
    function init(deps) {
        ...
        // ★ 优先从 sessionStorage 恢复页码
        const savedPage = restorePage();
        if (savedPage !== null) {
            currentShopPage = savedPage;
            console.log('📄 从 sessionStorage 恢复页码:', currentShopPage);
        } else {
            // 首次进入，从 gameState 初始化
            ...
            if (my?.shopCards) {
                ...
                savePage(currentShopPage);
                console.log('📄 首次初始化页码:', currentShopPage);
            }
        }
        
        injectBuffAnimationStyles();
        bindEvents();
        console.log('✅ refresh.js 已启动（绝对页码驱动 + 持久化保护）');
    }
