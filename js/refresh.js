...
window.YYCardShopRefresh = (function() {
    ...
    /**
     * 播放单张卡牌的 buff 动画
     * @param {HTMLElement} cardElement 卡牌 DOM
     * @param {number} atkBuff 攻击加成
     * @param {number} hpBuff  生命加成
     * @param {number} newAtk  最新攻击值（从状态对象取）
     * @param {number} newHp   最新生命值（从状态对象取）
     */
    function playBuffOnCard(cardElement, atkBuff, hpBuff, newAtk, newHp) {
        if (!cardElement) return;

        const atkEl = cardElement.querySelector('.card-atk');
        const hpEl = cardElement.querySelector('.card-hp');

        // 1. 立刻同步数值文字（直接从状态对象写入，不等 DOM）
        if (atkEl && newAtk !== undefined) atkEl.textContent = newAtk;
        if (hpEl && newHp !== undefined) hpEl.textContent = newHp;

        // 2. 底部数值弹跳（使用 Web Animations API，每次独立动画）
        const doBounce = (el) => {
            if (!el) return;
            el.animate([
                { transform: 'scale(1)' },
        ...
        const createSpan = (text) => {
            ...
            span.style.cssText = `
                ...
                animation: buffFloat 1.1s ease-out forwards;
                white-space: nowrap;
                font-family: inherit;
            `;
            return span;
        };
        if (atkBuff > 0) floatContainer.appendChild(createSpan(`+${atkBuff}`));
        if (hpBuff > 0) floatContainer.appendChild(createSpan(`+${hpBuff}`));
    ...
    /**
     * 触发一批卡牌的 buff 动画
     * @param {Array} boardBuffs  [{ boardIndex, atkBuff, hpBuff }]
     */
    function triggerBuffAnimations(boardBuffs) {
        if (!boardBuffs || boardBuffs.length === 0) return;

        // 直接从游戏状态对象获取最新棋盘数据，不等 DOM 刷新
        const gameState = getGameState();
        const userId = getCurrentUserId();
        const my = gameState?.players?.[userId];
        if (!my || !my.board) return;

        // 等一小帧确保棋盘 DOM 存在（renderMyBoard 已完成）
        requestAnimationFrame(() => {
            boardBuffs.forEach(({ boardIndex, atkBuff, hpBuff }) => {
                ...
                // 从状态对象取最新数值（一定是后端返回的最新值）
                const cardData = my.board[boardIndex];
                const newAtk = cardData?.atk;
                const newHp = cardData?.hp;

                playBuffOnCard(cardEl, atkBuff || 0, hpBuff || 0, newAtk, newHp);
            });
        });
    }
    ...
    function init(deps) {
        ...
        bindEvents();
        console.log('✅ refresh.js 已启动（状态同步 + 独立动画）');
    }
