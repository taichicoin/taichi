...
window.YYCardShopRefresh = (function() {
    ...
    function triggerBuffAnimations(boardBuffs) {
        ...
        setTimeout(() => {
            boardBuffs.forEach(({ boardIndex, atkBuff, hpBuff }) => {
                ...
                if (cardEl) {
                    // 从当前 DOM 里读取最新数值（此时已由 shop.js 更新过）
                    const atkEl = cardEl.querySelector('.card-atk');
                    const hpEl = cardEl.querySelector('.card-hp');
                    const newAtk = atkEl ? parseInt(atkEl.textContent) || 0 : undefined;
                    const newHp = hpEl ? parseInt(hpEl.textContent) || 0 : undefined;
    ...
    function init(deps) {
        ...
        bindEvents();
        console.log('✅ refresh.js 已启动（数值同步 + 重叠动画）');
    }
