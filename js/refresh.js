...
window.YYCardShopRefresh = (function() {
    ...
    async function processCardBuffQueue(boardIndex, buffs) {
        ...
        let curAtk = baseAtk, curHp = baseHp;
    ...
    async function runAnimQueue() {
        ...
        while (animQueue.length > 0) {
            ...
            try { await playAllBuffAnimations(buffs); } catch (e) { console.warn('动画异常', e); }
        }
        isAnimating = false;
        finalizeBoard();
    }
    ...
    function applyUpdateAndRerender(updatedPlayer) {
        ...
        if (updatedPlayer.hand) { my.hand = updatedPlayer.hand; _renderHand?.(); }
    }
    ...
    async function refreshShopAction() {
        ...
        invokeFunction('refresh-shop', { roomId, userId })
            .then(result => {
                ...
                if (!result.data.updatedPlayer) return;
                const up = result.data.updatedPlayer;

                if (isSwitchingGroup) {
                    // 切组回调：更新非活跃组 buffer，并校正 active/subIndex
                    if (up.shopCards) {
    ...
    function init(deps) {
        ...
        bindEvents();
        console.log('✅ refresh.js 已启动（切组秒切+动画队列）');
    }
