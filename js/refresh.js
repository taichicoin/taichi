...
window.YYCardShopRefresh = (function() {
    ...
    async function playAllBuffAnimations(boardBuffs) {
        ...
        boardBuffs.forEach(b => {
            if (!groups[b.boardIndex]) groups[b.boardIndex] = [];
            groups[b.boardIndex].push(b);
        });
        const tasks = Object.entries(groups).map(([idx, buffs]) => processCardBuffQueue(parseInt(idx), buffs));
    ...
    async function refreshShopAction() {
        ...
        if (isLocalFlipSafe()) {
            ...
            isRefreshingShop = true; // 无法本地翻页时加锁
        }
        ...
        invokeFunction('refresh-shop', { roomId, userId })
            .then(result => {
                ...
                if (result.data.updatedPlayer) {
                    ...
                    // ★ 切组时，更新非活跃组的buffer（后端已补新组）
                    if (isSwitchingGroup && up.shopCards) {
                    ...
                    // 切组后重绘商店（确保显示新补的组）
                    if (isSwitchingGroup) _renderShop?.();
            ...
            .catch(() => {})
            .finally(() => {
                if (isRefreshingShop && mySeq === refreshSeq) {
    ...
    function init(deps) {
        ...
        bindEvents();
        console.log('✅ refresh.js 已启动（原版逻辑+动画）');
    }
