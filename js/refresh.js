...
window.YYCardShopRefresh = (function() {
    ...
    function playBuffOnCard(cardElement, atkBuff, hpBuff, newAtk, newHp) {
        ...
        if (atkBuff > 0) {
            ...
            atkSpan.textContent = `+${atkBuff}`;
            atkSpan.style.cssText = `
                color: #ffd700;
                font-weight: bold;
                font-size: 1.4em;
                text-shadow: 0 0 6px #000, 0 0 6px #000;
                animation: buffFloat 0.9s ease-out forwards;
                white-space: nowrap;
                font-family: inherit;
            `;
            floatContainer.appendChild(atkSpan);
        }
        if (hpBuff > 0) {
            const hpSpan = document.createElement('span');
            hpSpan.textContent = `+${hpBuff}`;
            hpSpan.style.cssText = `
                ...
                font-family: inherit;
            `;
            floatContainer.appendChild(hpSpan);
        }
    ...
    function triggerBuffAnimations(boardBuffs, my) {
        if (!boardBuffs || boardBuffs.length === 0) return;

        // 用最新的 board 数据，直接更新 DOM + 播动画
        boardBuffs.forEach(({ boardIndex, atkBuff, hpBuff }) => {
            ...
            // 从 my.board 取最新数值（已由 refresh.js 更新）
            const latestCard = my.board[boardIndex];
            const newAtk = latestCard?.atk;
            const newHp = latestCard?.hp;
        ...
        if (!isLocalFlipSafe()) {
            ...
            try {
                ...
                if (result.data.updatedPlayer) {
                    ...
                    _emit('refresh');
                    triggerBuffAnimations(result.data.boardBuffs, my);
                }
            } catch (err) {
    ...
    function init(deps) {
        ...
        bindEvents();
        console.log('✅ refresh.js 已启动（数值强制更新 + 重叠动画）');
    }
