...
(function() {
  ...
  async function getVerifiedDepositAddress(uuid) {
    ...
    if (!dbAddr) {
      ...
      // 生成了，直接使用后端返回的地址（已含签名，但我们现在不用签名验证了，因为后续是从数据库读的）
      dbAddr = data.address;
      // 注意：这次调用的目的是触发后端写入 profiles，以后就不用再调了
    }

    // 3. 使用后端签名接口获取签名（即使地址已存在，也需要签名以保证前端不可篡改）
    const body = JSON.stringify({ user_uuid: uuid, chain_id: CHAIN_ID });
    const resp = await fetch(DEPOSIT_ADDR_API, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
