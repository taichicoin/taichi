...
(function() {
  ...
  const MAIN_ADDR = '0x0ACd8d4977f7aE68BCa6B19702aD808D6391F24b'; // 新合约
  ...
  // 从 profiles 表读取充值地址
  async function getDepositAddressFromDB(uuid) {
  ...
  async function computeAddressLocally(uuid) {
    const provider = new ethers.providers.JsonRpcProvider(
      window.YYCardConfig?.RPC_URL || 'https://rpc.ankr.com/bsc_testnet_chapel'
    );
  ...
  async function getDepositAddress(uuid) {
    // 1. 尝试从数据库读取
    const dbAddr = await getDepositAddressFromDB(uuid);
