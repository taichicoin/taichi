...
window.YYCardShop = (function() {
    ...
    async function equipFromHand(handIdx, boardIdx) {
        ...
        // 确定槽位（道具优先填 item1，否则 item2；武器填 weapon）
        ...
        // 乐观更新：只从手牌移除该卡，目标卡状态不做任何修改（后端会处理）
        my.hand[handIdx] = null;
        renderMyBoard();
        renderHand();

        // 调用后端（后端RPC会自动卸下旧装备并移入手牌，再装上新的）
    ...
    function init() {
        ...
        bindUIEvents();
        refreshAllUI();
        console.log('✅ 商店系统 (RPC自动装备 + 合成) 已启动');
    }
