...
window.YYCardShop = (function() {
    ...
    function renderHand() {
        ...
        // 手牌区作为放置目标（接收棋盘返回的单位）
        container.addEventListener('dragover', (e) => e.preventDefault()); // 保留以兼容
    }
    ...
    function onDragEnd(e) {
        ...
        log(`👆 拖拽结束，落点: ${dropResult?.zone || 'none'}`);
    ...
    function injectStyles() {
        ...
        style.textContent = `
            .card {
                touch-action: none;  /* 禁止浏览器滚动/缩放，让 pointer 事件立即响应 */
                user-select: none;
    ...
    function init() {
        ...
        bindUIEvents();
        refreshAllUI();
        log('✅ 商店交互模块已启动（手动模拟拖拽版，触碰即拿起，实时跟随）');
    }
...
console.log('✅ shop.js 加载完成（手动模拟拖拽版）');
