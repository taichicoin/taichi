...
window.YYCardShop = (function() {
    ...
    let dragOverSellZone = false;     // 当前是否悬停在出售区
    ...
    function handleDragStart(e, type, card, index) {
        ...
        const onDragOver = (ev) => {
            ...
            if (ev.clientY > screenMiddle && ev.clientY < rect.top) {
                if (!dragOverSellZone) {
                    dragOverSellZone = true;
                    showSellZone();
                }
            } else {
                if (dragOverSellZone) {
                    dragOverSellZone = false;
                    hideSellZone();
                }
            }
        };
        ...
        e.target.addEventListener('dragend', onDragEndCleanup, { once: true });
    }
    ...
    function handleDragEnd(e) {
        ...
        hideSellZone();
        dragOverSellZone = false;
        // 不在这里清除 draggedCard，因为 drop 事件可能还未触发，在 drop 中清除
    }
...
console.log('✅ shop.js 加载完成（拖拽终极修复版）');
