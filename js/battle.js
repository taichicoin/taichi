...
window.YYCardBattle = (function() {
    ...
    function fastForwardToCurrentRound() {
        ...
        while (elapsed > 0) {
            ...
            if (elapsed >= total) {
                ...
                if (elapsed < prep) { currentPhase = 'prepare'; remaining = prep - elapsed; }
                else if (elapsed < prep + bat) { currentPhase = 'battle'; remaining = prep + bat - elapsed; }
                else { currentPhase = 'settle'; remaining = total - elapsed; }
                break;
            }
        }
    ...
    // ===== 供 shop 调用的方法（修改：准备按钮调用 playerReady） =====
...
console.log('✅ battle.js 加载完成【双方准备提前结束 + 阶段切换修复】');
