...
window.YYCardBattle = (function() {
    ...
    // ★ 战斗动画启动（正常/重连）—— 先渲染初始棋盘，再调用后端，播完应用结果
    ...
    function renderPlayerStatus() {
        ...
        // 1. 从 battlePairs 中找出当前玩家的对手和先后手信息
        const pairs = gameState.battlePairs || [];
        const myPair = pairs.find(p => p.p1 === myId || p.p2 === myId);
        let opponentId = null;
        ...
        orderedIds.forEach(pid => {
            ...
            if (pid === myId || pid === opponentId) {
                ...
                const markBg = isFirst ? '#e94560' : '#3b82f6'; // 红先蓝后
                inner += `<span style="
                    position: absolute;
                    top: 0.1vh;
                    right: 0.1vw;
