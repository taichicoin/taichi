    ...
    async function refreshGameState() {
        ...
        if (newState) {
            ...
            if (currentUserId && newState.players[currentUserId]) {
                ...
                // 只更新数值字段，保护 board、hand、shopCards
    ...
    function mergePlayersData(currentPlayers, newPlayers, currentUserId) {
        ...
        for (const [pid, newPlayer] of Object.entries(newPlayers)) {
            if (pid === currentUserId) {
                // 当前玩家：只更新数值字段，保留 board、hand、shopCards
                result[pid] = {
                    ...result[pid],
                    gold: newPlayer.gold,
                    exp: newPlayer.exp,
...
console.log('✅ battle.js 加载完成（轮询保护当前玩家的 board/hand/shopCards）');
