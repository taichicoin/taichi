...
window.YYCardBattle = (function() {
    ...
    async function onPhaseEnd(phase) {
        ...
        try {
            if (phase === 'prepare') {
                ...
                if (over.isOver) {
                    // 游戏结束，必须在释放锁之前调用 endGame，但 endGame 内部会清理一切
                    // 注意：endGame 中会设置 isInPhaseTransition = false
                    endGame(over.winner);
                    clearTimeout(lockTimeout);
                    return;  // endGame 已释放锁，直接返回
                }
                gameState.round++;
    ...
    async function simulateBattle() {
        try {
            ...
            await updateGameState();  // 即使出错也同步一次状态
        }
    }
...
console.log('✅ battle.js 加载完成（锁修复版）');
