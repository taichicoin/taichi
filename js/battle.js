...
window.YYCardBattle = (function() {
    ...
    async function onPhaseEnd(phase) {
        ...
        try {
            if (phase === 'prepare') {
                ...
                log(`🛒 进入第 ${gameState.round} 回合准备阶段`);
            }
        } catch (e) {
    ...
    async function enterBattle(roomId) {
        ...
        const waitForState = setInterval(() => {
            ...
            // 🔥 关键修复：如果初始阶段不是 prepare，强制修正
            if (phase !== 'prepare') {
                log(`⚠️ 检测到异常阶段: ${phase}，强制修正为 prepare`, true);
                gameState.phase = 'prepare';
                updateGameState();
                phase = 'prepare';
            }
    ...
    function subscribeToGame(roomId) {
        ...
        supabase.from('game_states').select('state').eq('room_id', roomId).single().then(({ data }) => {
            if (data) { gameState = data.state; safeRenderBattleUI(); }
        });
    }
    ...
    function renderPlayerStatus() {
        ...
        for (const [pid, p] of Object.entries(players)) {
            ...
            html += `<div class="player-status-item"><img src="${avatarUrl}" onerror="this.src='${config.DEFAULT_AVATAR}'"><div style="flex:1;"><div style="font-size:0.6rem;">${displayName}</div><div class="hp-bar"><div class="hp-fill" style="width:${Math.max(0, (p.health/(config.INITIAL_HEALTH||100))*100)}%;"></div></div></div></div>`;
        }
    ...
    function renderBattleUI() {
        ...
        renderPlayerStatus();
        document.body.classList.toggle('battle-view-mode', gameState.phase !== 'prepare');
    }
    ...
    function refreshAllShops() {
        for (const pid in gameState.players) gameState.players[pid].shopCards = utils.generateShopCards(gameState.players[pid].shopLevel);
    }
    ...
    function bindBattleEvents() {
        ...
        document.getElementById('settings-btn')?.addEventListener('click', () => alert('设置功能开发中...'));
    }

    return { enterBattle, getState: () => gameState };
})();
