// ==================== 纯时间驱动对战系统【完整版·精简结算 · 全局回合显示 · 重连锁UI彻底修复】 ====================
window.YYCardBattle = (function() {
    ...
    async function launchBattleAnimation(isReconnect = false) {
        // ★ 重连且已结算：强制跳过动画并解锁UI
        if (isReconnect && gameState.lastSettledRound >= gameState.round) {
            console.log('⚡ 重连时发现已结算，跳过动画');
            forceStopAnimation(); // 清除任何残留动画锁
            hasCalledSettleBattle = true;
            // 同步全局状态（确保 gameState.phase 被设为 'prepare'）
