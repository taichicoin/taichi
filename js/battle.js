// ==================== 纯时间驱动对战系统（动画锁死/重复触发终极修复版 + 倒计时/动画修复） ====================
window.YYCardBattle = (function() {
    ...
    async function fastForwardAndResume() {
        ...
        // 重新计算 phaseStartTime 和 phaseEndTime
        const nowServer = await getServerTimeMs();
        const phaseDuration = getPhaseDuration(phase, round);
        gameState.phaseStartTime = new Date(nowServer - (phaseDuration - remaining) * 1000).toISOString();
        gameState.phaseEndTime = new Date(nowServer + remaining * 1000).toISOString();
        await updatePhaseToDB();
...
console.log('✅ battle.js 加载完成（倒计时/动画修复版）');
