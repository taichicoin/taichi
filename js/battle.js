...
window.YYCardBattle = (function() {
    ...
    let gameState = null;               // 包含 players, gameStartTime, 以及动态 phase/round
    let gameSubscription = null;
    let pollingInterval = null;
    let mainTimer = null;
    let enterGuard = false;
    let isSettling = false;             // 防止并发结算
    let lastBattleTriggered = false;    // 防止同一场战斗重复触发
    ...
    // 核心计算：根据 gameStartTime 独立计算当前回合、阶段、剩余秒数
    ...
    async function callSettlement() {
        ...
        try {
            ...
            // 结算成功后，重新拉取完整游戏状态（包括可能更新的 gameStartTime 和玩家数据）
            const newState = await fetchGameState();
            if (newState) {
                gameState.players = newState.players;
                gameState.gameStartTime = newState.gameStartTime;
    ...
    async function tick() {
        ...
        // 更新 UI 模式（准备阶段显示商店，战斗阶段隐藏）
        if (currentPhase !== phase) {
            applyUIMode(phase === 'prepare');
            currentPhase = phase;
        }

        // 结算触发：当前是战斗阶段且剩余时间 <= 0，且尚未触发过本次战斗结算
        if (phase === 'battle' && remaining <= 0 && !isSettling && !lastBattleTriggered) {
            log("⚡ 战斗阶段自然结束，触发结算");
            lastBattleTriggered = true;
            await callSettlement();
            // 注意：结算后 gameStartTime 可能已更新，下一轮 tick 会重新计算
        }
    }
        ...
        gameSubscription = supabase.channel(`game:${currentRoomId}`)
            .on('postgres_changes', {
                ...
                if (newState && newState.players) {
                    // 只更新玩家数据，不覆盖 gameStartTime 和 phase
                    gameState.players = newState.players;
                    if (window.YYCardShop?.refreshAllUI) window.YYCardShop.refreshAllUI();
                }
            })
    ...
    async function enterBattle(roomId) {
        ...
        enterGuard = false;
        log("✅ 战斗界面加载完成，纯时间驱动模式（支持商店操作+自动结算）");
    }
...
console.log('✅ battle.js 加载完成（修复结算触发，商店操作正常）');
