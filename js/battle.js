...
window.YYCardBattle = (function() {
    ...
    function getCurrentPhaseInfo(gameStartTime, currentRound) {
        ...
        while (true) {
            ...
            if (round > currentRound + 20) break; // 安全限制
        }
    ...
    function startPhaseTimerFromTheory() {
        ...
        timerInterval = setInterval(() => {
            ...
            // 如果理论阶段与当前存储的阶段不同，需要切换（由 Realtime 触发，此处不主动切换）
        }, 200);
        // 设置到期定时器
        phaseTimer = setTimeout(async () => {
            ...
            timerInterval = null;
            phaseTimer = null;
            // 到期后触发阶段结束处理（注意：可能因为理论阶段已变化，这里直接调用 onPhaseEnd 传入当前理论阶段）
        ...
        safetyTimer = setTimeout(() => {
            if (phaseTimer) {
                log(`⚠️ 阶段 ${info.phase} 超时未响应，强制结束`, true);
                clearTimeout(phaseTimer);
                phaseTimer = null;
                clearInterval(timerInterval);
                timerInterval = null;
                onPhaseEnd(info.phase);
            }
        }, (remaining + 2) * 1000);
    }
    ...
    async function startBuffering() {
        ...
        if (window.YYCardShop?.setPhase) window.YYCardShop.setPhase('battle');
    }

    // 结算重试函数：不断调用 settlement 直到成功或阶段不再是 settle
    async function callSettlementUntilSuccess() {
        const maxAttempts = 30; // 最多尝试 30 次（15 秒）
    ...
    async function onPhaseEnd(phase) {
        ...
        try {
            if (phase === 'prepare') {
                ...
                // 结算阶段结束 → 调用后端函数发放奖励
                await callSettlementUntilSuccess();
                // 刷新后检查游戏是否结束
                ...
                // 后端已将 phase 改为 prepare，直接进入准备阶段
    ...
    async function simulateBattle() {
        try {
            ...
            if (!window.YYCardCombat) {
                log('❌ combat 模块未加载，无法模拟战斗', true);
                return;
            }
            // 战斗模拟会修改 gameState 中的血量等数据，并调用 updateGameState 写入数据库
    ...
    async function fastForwardAndResume() {
        ...
        // 注意：不修改 phaseStartTime，因为理论计算不依赖它
        await updateGameState();
    ...
    // 已删除所有玩家操作方法（由 shop.js 通过 Edge Functions 实现）
...
console.log('✅ battle.js 加载完成（绝对时间驱动 + 结算重试）');
