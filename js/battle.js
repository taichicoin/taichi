...
window.YYCardBattle = (function() {
    ...
    function fastForwardToCurrentRound() {
        ...
        if (currentRound > oldRound) {
            ...
            const expNeeded = config.ECONOMY.SHOP_LEVEL_EXP;
            for (let r = oldRound; r < currentRound; r++) {
                ...
                for (const pid in gameState.players) {
                    ...
                    p.exp += expAdd;
                    // 修复：使用下一级所需经验判断升级
                    while (p.shopLevel < (config.MAX_SHOP_LEVEL || 5)) {
                        const nextLevelExp = expNeeded[p.shopLevel + 1];
                        if (p.exp >= nextLevelExp) {
                            p.shopLevel++;
    ...
    async function buyExp() {
        ...
        const expNeeded = config.ECONOMY?.SHOP_LEVEL_EXP || {1:0,2:4,3:8,4:14,5:20};
        while (my.shopLevel < (config.MAX_SHOP_LEVEL||5)) {
            const nextLevelExp = expNeeded[my.shopLevel + 1];
            if (my.exp >= nextLevelExp) {
                my.shopLevel++;
    ...
    function startBotAutoPlay() {
        ...
        autoBotTimer = setInterval(async () => {
            ...
            if (my.gold >= 1 && my.shopLevel < (config.MAX_SHOP_LEVEL || 5)) {
                // 检查是否经验已够升级，如果够了就不浪费金币
                const nextLevelExp = expNeeded[my.shopLevel + 1];
                if (my.exp >= nextLevelExp) {
                    // 已经可以升级，直接升级，不消耗金币
                    while (my.shopLevel < (config.MAX_SHOP_LEVEL || 5)) {
                        const nextExp = expNeeded[my.shopLevel + 1];
                        if (my.exp >= nextExp) {
                            my.shopLevel++;
                            log(`🤖 人机商店升级到 Lv.${my.shopLevel} (经验自动)`);
                        } else {
                            break;
                        }
                    }
...
console.log('✅ battle.js 加载完成【修复商店升级 + 全局时间戳重连】');
