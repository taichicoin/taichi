...
window.YYCardBattle = (function() {
    ...
    let eliminationShown = false;   // 是否已经弹出淘汰提示
    ...
    function checkElimination() {
        ...
        // 显示淘汰弹窗
        const rank = myPlayer.eliminationRank || '?';
        const overlay = document.createElement('div');
        overlay.className = 'elimination-overlay';
        overlay.innerHTML = `
            <div class="elimination-dialog" style="
                background: rgba(20,30,50,0.95); color: #fff; border-radius: 16px; padding: 24px;
                text-align: center; max-width: 300px; border: 2px solid #ffd966; box-shadow: 0 0 20px rgba(0,0,0,0.8);
            ">
                <h2 style="margin: 0 0 12px;">💀 你已被淘汰</h2>
                <p style="font-size: 18px;">最终排名：第 <strong style="color:#ffd966;">${rank}</strong> 名</p>
                <button id="elimination-play-again" style="
                    margin-top: 16px; padding: 12px 24px; font-size: 16px; font-weight: bold;
                    background: #ffd966; color: #000; border: none; border-radius: 8px; cursor: pointer;
                ">🔄 再来一局</button>
            </div>
        `;
        overlay.style.cssText = `position:fixed; z-index:100000; top:0; left:0; right:0; bottom:0; 
            background: rgba(0,0,0,0.6); display:flex; align-items:center; justify-content:center;`;
        document.body.appendChild(overlay);

        document.getElementById('elimination-play-again').onclick = () => {
            overlay.remove();
            ...
            eliminationShown = false; // 重置，以备后续可能的再次进入
        };
    }
    ...
    async function tick() {
        ...
        // ★ 首先检查是否已淘汰，如果淘汰则直接锁定并弹窗，不再执行后续阶段逻辑
        checkElimination();
        if (eliminationShown) return; // 弹出后停止 tick
    ...
    async function enterBattle(roomId) {
        ...
        // ★ 重连时检查是否已被淘汰
        const myId = auth?.currentUser?.id;
        if (myId && gameState.players[myId]?.isEliminated) {
            toast("你已被淘汰，无法重连", true);
            enterGuard = false;
            document.getElementById('battle-view').style.display = 'none';
            document.getElementById('lobby-view').style.display = 'block';
            return;
...
console.log('✅ battle.js 淘汰锁UI+再来一局版');
