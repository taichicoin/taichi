...
window.YYCardBattle = (function() {
    ...
    // ★ 刷新状态时，始终以后端数据为准，不保护当前玩家的 round
    ...
    async function tick() {
        ...
        if (forcePrepare) {
            ...
            if (gameState.round !== forcePrepareRound) {
                ...
                // 计算本回合剩余总时间作为准备阶段倒计时
                const prep = getPrepareDuration(forcePrepareRound);
                const buf = BUFFER_DURATION;
                const bat = getBattleDuration(forcePrepareRound);
                const totalRound = prep + buf + bat;
                // 计算从游戏开始到现在经过的秒数
                const elapsed = nowSec - startSec;
                // 本回合开始的秒数
                let roundStartSec = 0;
                for (let r = 1; r < forcePrepareRound; r++) {
                    roundStartSec += getPrepareDuration(r) + BUFFER_DURATION + getBattleDuration(r);
                }
                const elapsedInRound = nowSec - (startSec + roundStartSec);
                const remaining = Math.max(0, totalRound - elapsedInRound);

                gameState.phase = 'prepare';
                gameState.round = forcePrepareRound;
                ...
                if (roundEl) roundEl.textContent = forcePrepareRound;
