// ==================== 纯时间驱动对战系统【适配玩家独立行 · 跳过战斗 · 动画保护 · 动画后立即结算】 ====================
window.YYCardBattle = (function() {
    ...
    let manualPrepareFlag = false;          // 动画完成后强制准备模式标志
        ...
        try {
            ...
            if (myPair) {
                ...
                if (oppId && gameState.players[oppId]?.board) {
                    if (window.YYCardCombat?.renderEnemyBoardFromData) {
            ...
            const onAnimationComplete = async () => {
                ...
                // 5秒后重置标志，但保留 forcePrepareMode 直到真正的 prepare 阶段到来
                setTimeout(() => {
                    manualPrepareFlag = false;
                }, 5000);
    ...
    async function tick() {
        ...
        // ★ 确定实际显示给玩家的阶段（强准备模式下显示 prepare）
        let effectivePhase = phase;
        const isForcePrepare = window.YYCardShop?.getForcePrepareMode?.() || false;
        if (manualPrepareFlag || isForcePrepare) {
        ...
        if (window.YYCardShop?.updateTimerDisplay) {
            window.YYCardShop.updateTimerDisplay(remaining, effectivePhase);
        }
        if (window.YYCardShop?.setPhase) window.YYCardShop.setPhase(effectivePhase);
        ...
        // ★ 战斗相关 UI 仅在真实战斗阶段且没有被强制准备时显示
        const showBattleUI = (phase === 'battle' && !manualPrepareFlag && !isForcePrepare);
        if (showBattleUI) {
        ...
        // 阶段切换逻辑（仅当真实阶段变化且非强制准备时处理）
        if (lastPhase !== phase && !isForcePrepare && !manualPrepareFlag) {
        ...
        // 应用 UI 模式（准备模式显示商店等，战斗模式隐藏）
        const shouldPrepare = (effectivePhase === 'prepare');
        applyUIMode(shouldPrepare);

        if (window.YYCardBuff?.tryShowBuffSelection) {
            window.YYCardBuff.tryShowBuffSelection(round, effectivePhase);
        }
