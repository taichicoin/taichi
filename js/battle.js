...
window.YYCardBattle = (function() {
    ...
    // 强制解锁动画状态（用于异常恢复）
    function forceUnlockAnimation() {
        if (isPlayingAnimation) {
            console.warn('[Battle] 强制解锁动画状态');
    ...
    async function tick() {
        ...
        if (currentPhase !== phase) {
            ...
            // ★ 如果从非准备阶段切换到准备阶段，强制解锁动画状态
            if (phase === 'prepare' && isPlayingAnimation) {
                log('⚠️ 检测到准备阶段但动画标志未清除，强制重置');
                forceUnlockAnimation();
            }
            ...
            if (phase === 'battle' && !isPlayingAnimation && !isSettling) {
                ...
                if (window.YYCardCombat?.resolveBattles) {
                    try {
                        ...
                        isPlayingAnimation = false; // ★ 即使出错也解锁
                        safeRefreshUI();
                    }
                } else {
                    log('❌ 战斗动画模块未加载', true);
                    isPlayingAnimation = false; // ★ 立即解锁
                }
            }
...
console.log('✅ battle.js 修复版 (阶段死锁修复 + 动画强制重置)');
