...
window.YYCardBattle = (function() {
    ...
    // ========== 【修复版】动画核心逻辑：三重锁+兜底释放，彻底解决死锁/重复调用 ==========
    async function simulateBattleWithAnimation() {
        // 前置三重锁，只要有一个业务在执行，直接拦截，绝对不重复执行
        if (isAnimating || isInPhaseTransition || isSettling) {
            log(`⚠️ 动画被锁，当前状态：动画中=${isAnimating} 阶段切换中=${isInPhaseTransition} 结算中=${isSettling}`);
            return;
        }
        // 先加锁，再执行逻辑，避免执行过程中被二次调用
        isAnimating = true;
        log("⚔️ 【唯一触发】进入战斗阶段，开始播放战斗动画");
    ...
    // ========== 【修复版】阶段切换核心：双重锁+唯一动画触发点，彻底解决重复执行 ==========
    async function onPhaseEnd(phase) {
        // 双重锁：阶段切换中直接拦截，绝对不重复执行
        if (isInPhaseTransition) {
            log(`⚠️ 阶段切换被锁，当前阶段: ${phase} 已在执行中`, true);
            return;
        }
        ...
        }, 15000); // 超时时间拉长到15s，给动画足够执行时间
        log(`🔄 阶段结束: ${phase}`);

        try {
            if (phase === 'prepare') {
                // 严格执行正确流程：缓冲→动画→切换阶段→启动计时器，全程加锁不被打断
                await startBuffering('battle');
                // 全局唯一的动画调用点，全程在阶段切换锁内，不会被重复触发
    ...
    // ========== 【修复版】tick函数：100%纯UI显示，彻底清掉业务逻辑，杜绝重复触发 ==========
    async function tick() {
...
console.log('✅ battle.js 加载完成（动画锁死/重复调用修复完整版）');
