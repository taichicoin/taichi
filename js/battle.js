...
window.YYCardBattle = (function() {
    ...
    function safeRefreshUI() {
        ...
        if (isAnimPlaying || (window.YYCardCombat?.isAnimating && window.YYCardCombat.isAnimating())) {
            return;
        }
        if (window.YYCardShop?.refreshAllUI) window.YYCardShop.refreshAllUI();
    }
    ...
    async function tick() {
        ...
        // 立即更新 gameState，让 shop.js 能读到最新阶段
        ...
        if (phase !== 'battle') {
            safeRefreshUI();   // 准备阶段主动刷新UI，确保操作可用
        }
    ...
    function startGlobalTimer() {
        ...
        tick();                                 // 立刻执行一次
        mainTimer = setInterval(tick, 1000);    // 之后每秒
    }
