...
window.YYCardBattle = (function() {
    ...
    // ★★★ applyPSettlement：增加 activeBuff 处理
    async function applyPSettlement() {
        ...
        try {
            ...
            if (result?.success && result.updatedPlayer) {
                ...
                if (result.updatedPlayer.pendingConsumables !== undefined) {
                    p.pendingConsumables = result.updatedPlayer.pendingConsumables;
                    window.YYCardConsumable?.updateRewardBadge?.();
                }
                // ★ 新增：同步 activeBuff
                if (result.updatedPlayer.activeBuff !== undefined) {
                    p.activeBuff = result.updatedPlayer.activeBuff;
                }
                if (result.updatedPlayer.buffRefreshCount !== undefined) {
                    p.buffRefreshCount = result.updatedPlayer.buffRefreshCount;
                }
    ...
    // ★★★ fetchGameState：增加 pendingConsumables 和 activeBuff
    async function fetchGameState(roomId) {
        ...
        // 2. 自己数据：增加 pendingConsumables 和 activeBuff
        ...
        players[myId] = {
            ...
            _checkedActiveBuff: true  // ★ 标记已检查过，防止 buffs.js 重复查询数据库
        };
    ...
    async function refreshGameState() {
        ...
        if (currentUserId && newState.players[currentUserId]) {
            ...
            // ★ 保留标记字段，防止 buffs.js 重复查询
            if (myOld._checkedActiveBuff === undefined) {
    ...
    // ★★★ launchBattleAnimation：同步 activeBuff
    ...
    // ★★★ subscribeGameState：同步 pendingConsumables 和 activeBuff
    ...
    // ★★★ enterBattle：加载完成后刷新徽章和buff检查标记
