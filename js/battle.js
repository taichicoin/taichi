...
window.YYCardBattle = (function() {
    ...
    // ★★★ 核心动画启动器：直接查数据库 lastSettledRound，不信任本地缓存 ★★★
    async function launchBattleAnimation() {
        ...
        try {
            console.log('🔍 直接查询数据库全局行...');
            const { data: freshGlobal, error: freshError } = await supabase
                .from('game_states')
                .select('state')
            ...
            if (freshError || !freshGlobal?.state) {
                console.error('❌ 查询全局行失败，放弃动画');
                isAnimPlaying = false;
                return;
            }

            const freshState = freshGlobal.state;
            const currentRound = gameState.round;
            const lastSettled = freshState.lastSettledRound || 0;

            console.log(`📊 当前回合: ${currentRound}, 数据库 lastSettledRound: ${lastSettled}`);

            if (lastSettled >= currentRound) {
                console.log('⚡ 数据库显示本回合已结算，跳过动画，进入准备阶段');
                isAnimPlaying = false;
                gameState.lastSettledRound = lastSettled;
                await applyPSettlement();
                if (window.YYCardShop?.setForcePrepareMode) {
                    window.YYCardShop.setForcePrepareMode(true);
                }
            ...
            console.log('📡 首次结算，调用 settle-battle...');
