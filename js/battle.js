...
window.YYCardBattle = (function() {
    ...
    // ===== 【核心新增】快速推进到当前回合（基于 gameStartTime） =====
        ...
        if (currentRound > oldRound) {
            ...
            for (let r = oldRound; r < currentRound; r++) {
                ...
                for (const pid in gameState.players) {
                    ...
                    p.gold += goldAdd;
                    p.exp += expAdd;
                    while (p.shopLevel < (config.MAX_SHOP_LEVEL||5) && p.exp >= expNeeded[p.shopLevel]) {
                        p.shopLevel++;
    ...
    async function enterBattle(roomId) {
        ...
        const startGameWithState = () => {
            ...
            if (fastForwarded) return; // fastForwardToCurrentRound 内部已处理 UI 和计时器
    ...
    function startBotAutoPlay() {
        ...
        autoBotTimer = setInterval(async () => {
            ...
            const expNeeded = config.ECONOMY?.SHOP_LEVEL_EXP || {1:0,2:4,3:8,4:14,5:20};
            const canUpgrade = my.shopLevel < (config.MAX_SHOP_LEVEL||5) && my.exp >= expNeeded[my.shopLevel];
            if (my.gold >= 1 && my.shopLevel < (config.MAX_SHOP_LEVEL||5) && !canUpgrade) {
                my.gold--;
                my.exp++;
...
console.log('✅ battle.js 加载完成【全局时间戳重连版】');
