...
window.YYCardBattle = (function() {
    ...
    let hasCalledSettlement = false;      // 标记全局结算是否已调用过（防止同一回合多次调用，每回合重置）
    ...
    function resetRoundFlags(startSec) {
        ...
        hasCalledSettlement = false;        // ★ 新回合重置，允许再次调用全局结算
    ...
    async function callSettlement() {
        ...
        try {
            ...
            const result = await resp.json();
            if (result?.success) {
                hasCalledSettlement = true;      // 标记已调用过，但回合更新后会被重置
                await refreshGameState();
            } else {
    ...
    async function enterBattle(roomId) {
        ...
        // ★ 初始化 lastPhase 为当前阶段，避免重连后阶段变化检测失效
        const nowSec = await getServerTime();
        const startSec = Math.floor(new Date(gameState.gameStartTime).getTime() / 1000);
