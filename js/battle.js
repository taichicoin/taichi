...
window.YYCardBattle = (function() {
    ...
    let lastBattleTriggerRound = 0;   // 记录已触发结算的回合，防止重复触发
    ...
    async function tick() {
        ...
        // 结算触发：战斗阶段且剩余时间 <= 0，并且当前回合还没有触发过结算
        if (phase === 'battle' && remaining <= 0 && lastBattleTriggerRound !== round && !isSettling) {
            log(`⚡ 战斗阶段结束 (回合${round})，触发结算`);
            lastBattleTriggerRound = round;
            // 暂停计时器，避免重复触发
...
console.log('✅ battle.js 加载完成（纯时间驱动 + 可靠结算 + 商店操作支持）');
