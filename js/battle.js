// ==================== 纯时间驱动对战系统【最终修复版：仅用 isRefreshing 保护】 ====================
window.YYCardBattle = (function() {
    ...
    function safeRefreshUI() {
        // ★ 商店正在刷新时禁止任何 UI 刷新，防止旧数据覆盖
        if (window.YYCardShop?.isRefreshing) return;
    ...
    async function fetchGameState() {
        ...
        return { 
            ...
            gameStartTime: data.state.gameStartTime || new Date().toISOString()
        };
    }
    ...
    async function refreshGameState() {
        ...
        if (newState) {
            ...
            if (currentUserId && newState.players[currentUserId] && gameState?.players?.[currentUserId]) {
                ...
                // shopCards 只更新 buffer/next，不覆盖页码
                if (myNew.shopCards) {
                    if (myOld.shopCards) {
                        myOld.shopCards.buffer = myNew.shopCards.buffer;
                        myOld.shopCards.next = myNew.shopCards.next;
                    } else {
                        myOld.shopCards = myNew.shopCards;
                    }
                }
    ...
    // ★ 合并数据：自己保留 shopCards 本地页码，轮询/推送绝不覆盖
    function mergePlayersData(currentPlayers, newPlayers, currentUserId) {
        ...
        for (const [pid, newPlayer] of Object.entries(newPlayers)) {
            if (pid === currentUserId && window.YYCardShop?.isRefreshing) {
                continue;   // 正在刷新时完全跳过
            }
            if (pid === currentUserId) {
                // 自己：除 shopCards 外全部合并
                result[pid] = {
                    ...result[pid],
                    gold: newPlayer.gold,
                    exp: newPlayer.exp,
...
console.log('✅ battle.js 已修复：轮询/推送不再覆盖商店页码');
