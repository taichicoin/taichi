// ==================== 纯时间驱动对战系统【适配玩家独立行 · 跳过战斗 · 淘汰延迟显示 · 个人时间补偿 · 稳定版】 ====================
window.YYCardBattle = (function() {
            ...
            try {
                ...
                if (session) {
                    ...
                    if (result?.success) {
                        ...
                        // ★ 更新本地玩家的 timeOffset
                        if (gameState && gameState.players[auth.currentUser.id]) {
                            gameState.players[auth.currentUser.id].timeOffset = result.timeOffset || 0;
                        }

                        // ★ 强制重新计算阶段
                        const nowSec = await getServerTime();
                        const startSec = Math.floor(new Date(gameState.gameStartTime).getTime() / 1000);
                        const myTimeOffset = gameState.players[auth.currentUser.id]?.timeOffset || 0;
                        const { phase } = calculatePhaseInfo(startSec, nowSec, myTimeOffset);
                        
                        // ★ 同步全局状态
                        gameState.phase = phase;
                        lastPhase = phase;       // ★ 关键：让 tick 不再触发阶段变化
                        thisBattleRound = 0;      // ★ 重置，防止 tick 再次启动动画
    ...
    // ★★★ 加 timeOffset 参数 ★★★
    function calculatePhaseInfo(gameStartSec, nowSec, timeOffset = 0) {
        const effectiveStartSec = gameStartSec + timeOffset;
        if (!effectiveStartSec || isNaN(effectiveStartSec) || effectiveStartSec > nowSec) {
            return { round: 1, phase: 'prepare', remaining: getPrepareDuration(1), total: getPrepareDuration(1) };
        }
        let elapsed = nowSec - effectiveStartSec;
    ...
    function getCurrentPhaseInfo() {
        ...
        const myTimeOffset = myId ? (gameState.players[myId]?.timeOffset || 0) : 0;
        return calculatePhaseInfo(startSec, nowSec, myTimeOffset);
    }
    ...
    async function refreshGameState() {
        ...
        if (currentUserId && newState.players[currentUserId] && gameState?.players?.[currentUserId]) {
            ...
            // ★ 同步 timeOffset
            if (myNew.timeOffset !== undefined) {
                myOld.timeOffset = myNew.timeOffset;
            }
        } else {
    ...
    async function tick() {
        ...
        const myTimeOffset = myPlayer?.timeOffset || 0;
        ...
        const nowSec = await getServerTime();
        const startSec = Math.floor(new Date(gameState.gameStartTime).getTime() / 1000);
        const { round, phase, remaining, total } = calculatePhaseInfo(startSec, nowSec, myTimeOffset);
        gameState.phase = phase;
