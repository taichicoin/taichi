// ==================== 纯时间驱动对战系统（终极修复：非阻塞动画 + 时间驱动阶段 + 安全结算） ====================
window.YYCardBattle = (function() {
    ...
    let mainTimer = null;                 // setInterval ID
    let enterGuard = false;
    let isSettling = false;               // 结算中
    let isPlayingAnimation = false;       // 动画播放中
    ...
    // 安全刷新 UI（动画期间跳过，且只在准备或缓冲阶段才允许刷新棋盘）
    ...
    async function tick() {
        ...
        if (lastPhase !== phase) {
            ...
            // 离开战斗阶段时，调用结算（如果尚未调用）
            if (lastPhase === 'battle' && phase !== 'battle' && !isSettling) {
                log("⚡ 战斗阶段结束，触发结算");
                callSettlement();
            }
...
console.log('✅ battle.js 终极修复版 (纯时间驱动 + 非阻塞动画 + 阶段结算)');
