...
window.YYCardBattle = (function() {
    ...
    async function onPhaseEnd(phase) {
        ...
        try {
            if (phase === 'prepare') {
                gameState.phase = 'battle'; gameState.phaseStartTime = new Date().toISOString();
                await updateGameState(); applyUIMode(false);
                startPhaseTimer('battle', getBattleDuration(gameState.round));
                await simulateBattle();
            } else if (phase === 'battle') {
                gameState.phase = 'settle'; gameState.phaseStartTime = new Date().toISOString();
                await updateGameState(); applyUIMode(false);
                startPhaseTimer('settle', SETTLE_DURATION);
            } else if (phase === 'settle') {
                await distributeRoundRewards();
                const over = checkGameOver();
                if (over.isOver) { endGame(over.winner); isInPhaseTransition = false; return; }
                gameState.round++;
                gameState.phase = 'prepare'; gameState.phaseStartTime = new Date().toISOString();
                await updateGameState(); applyUIMode(true);
                await refreshAllShops();
                startPhaseTimer('prepare', getPrepareDuration(gameState.round));
            }
        } catch (e) { log(`❌ onPhaseEnd 出错: ${e.message}`, true); }
        finally { isInPhaseTransition = false; }
    }
    ...
    async function simulateBattle() {
        ...
        for (let i = 0; i < players.length; i += 2) {
            ...
            const p1 = gameState.players[players[i]], p2 = gameState.players[players[i+1]];
            const u1 = p1.board.filter(c => c).length, u2 = p2.board.filter(c => c).length;
    ...
    async function fastForwardAndResume() {
        ...
        gameState.round = round; gameState.phase = phase;
        gameState.phaseStartTime = new Date(now - (getPhaseDuration(phase, round) - remaining) * 1000).toISOString();
        await refreshAllShops(); await updateGameState();
        applyUIMode(phase === 'prepare');
        if (window.YYCardShop?.refreshAllUI) window.YYCardShop.refreshAllUI();
        clearAllTimers(); startPhaseTimer(phase, remaining);
        return true;
    }

    async function enterBattle(roomId) {
        if (enterGuard) return; enterGuard = true; currentRoomId = roomId;
        document.getElementById('lobby-view').style.display = 'none';
        document.getElementById('battle-view').style.display = 'block';
        initDebugPanel(); if (window.YYCardShop) window.YYCardShop.init();
        subscribeToGame(roomId); bindBattleEvents(); startBotAutoPlay();
        ...
        const wait = async () => {
            ...
            if (attempts < MAX) { attempts++; log(`⏳ 等待状态 (${attempts}/${MAX})`);
    ...
    function startBotAutoPlay() {
        ...
        autoBotTimer = setInterval(async () => {
            ...
            const uid = auth.currentUser?.id; const my = gameState.players[uid];
            if (!my || !my.isBot) return;
            if (bought[uid] === gameState.round) return;
            if (my.shopLevel >= 5) return;
            if (my.gold >= 1) { my.gold--; my.exp++;
                const lvl = getShopLevelByExp(my.exp); if (lvl > my.shopLevel) my.shopLevel = lvl;
                await updateGameState(); bought[uid] = gameState.round; }
        }, 2000);
    }
    ...
    function bindBattleEvents() {
        document.getElementById('leave-battle-btn')?.addEventListener('click', async () => {
            ...
            clearAllTimers(); if (autoBotTimer) clearInterval(autoBotTimer);
    ...
    // ---------- 核心购买逻辑（修复版）----------
    async function buyCardAction(card, shopIndex) {
        if (!gameState || gameState.phase !== 'prepare') return false;
        const uid = auth.currentUser?.id;
        const my = gameState.players[uid];
        if (!my || my.isBot) return false;

        const price = getCardBuyPrice(card);
        if (my.gold < price) { log(`❌ 金币不足: 需要${price}, 现有${my.gold}`, true); toast('金币不足', true); return false; }

        const handCount = my.hand.filter(c => c !== null).length;
        if (handCount >= (config.HAND_MAX_COUNT || 15)) { log(`❌ 手牌已满`, true); toast('手牌已满', true); return false; }
        ...
        else my.hand.push(newCard); // 理论上不会执行，但兜底

        // 从商店移除
        if (shopIndex !== undefined && my.shopCards) my.shopCards.splice(shopIndex, 1);

        await updateGameState();
        log(`✅ 购买成功: ${card.name}，剩余金币 ${my.gold}`);
        return true;
    }
    ...
    async function buyExpAction() {
        ...
        my.gold--; my.exp++;
        const lvl = getShopLevelByExp(my.exp); if (lvl > my.shopLevel) my.shopLevel = lvl;
        await updateGameState(); return true;
    }
    ...
    async function placeCardAction(handIdx, boardIdx) {
        if (!gameState || gameState.phase !== 'prepare') return false;
        const uid = auth.currentUser?.id; const my = gameState.players[uid];
        const card = my.hand[handIdx]; if (!card) return false;
        const old = my.board[boardIdx];
        my.board[boardIdx] = card; my.hand[handIdx] = old || null;
        await updateGameState(); return true;
    }
    ...
    async function sellCardAction(type, index) {
        ...
        const price = getCardSellPrice(card); my.gold += price;
        await updateGameState(); return true;
    }
    ...
    async function swapBoardAction(idxA, idxB) {
        if (!gameState || gameState.phase !== 'prepare') return false;
        const uid = auth.currentUser?.id; const my = gameState.players[uid];
        if (my.isBot) return false;
        const temp = my.board[idxA]; my.board[idxA] = my.board[idxB]; my.board[idxB] = temp;
        await updateGameState(); return true;
    }

    async function boardToHandAction(boardIdx) {
        if (!gameState || gameState.phase !== 'prepare') return false;
        const uid = auth.currentUser?.id; const my = gameState.players[uid];
        if (my.isBot) return false;
        const card = my.board[boardIdx]; if (!card) return false;
        const handCount = my.hand.filter(c => c !== null).length;
        if (handCount >= (config.HAND_MAX_COUNT || 15)) { toast('手牌已满', true); return false; }
        const emptyIndex = my.hand.findIndex(c => c === null);
        if (emptyIndex === -1) { toast('手牌已满', true); return false; }
        my.hand[emptyIndex] = card; my.board[boardIdx] = null;
        await updateGameState(); return true;
    }
...
console.log('✅ battle.js 加载完成【购买修复版】');
