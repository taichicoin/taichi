...
window.YYCardBattle = (function() {
    ...
    let manualPrepareFlag = false;  // ★ 新增：动画完成后强制进入准备模式标志
    ...
    function safeRefreshUI() {
        if (window._consumableDragging) return; // 消耗牌拖拽中，禁止任何 UI 刷新
    ...
    async function launchBattleAnimation(isReconnect = false) {
        ...
        try {
            ...
            const onAnimationComplete = async () => {
                ...
                await refreshGameState();
                await applyPSettlement();
                await callSettlement();                // ★ 立即全局结算
                await refreshGameState();              // 再次刷新状态
