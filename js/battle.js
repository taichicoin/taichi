...
window.YYCardBattle = (function() {
    ...
    let phaseTransitionTimer = null; // 锁超时保护
    ...
    function initDebugPanel() {
        ...
        p.style.cssText = `
            ...
            font-family:monospace; text-shadow:0 0 4px black;
            background: rgba(0,0,0,0.7); border: 1px solid #f5d76e;
            border-radius: 8px;
            pointer-events: auto;  /* 允许鼠标交互，可滑动 */
    ...
    async function onPhaseEnd(phase) {
        if (isInPhaseTransition) { 
            log(`⚠️ 阶段切换被锁拦截: ${phase}，强制重置锁`, true); 
    ...
    async function buyCardAction(card, idx) {
        ...
        if (empty !== -1) my.hand[empty] = newCard; else my.hand.push(newCard);
    ...
    async function placeCardAction(handIdx, boardIdx) {
        ...
        const old = my.board[boardIdx]; my.board[boardIdx] = card; my.hand[handIdx] = old || null;
        await updateGameState(); return true;
    }
    ...
    async function buyAndPlaceAction(card, shopIndex, boardIndex) {
        ...
        my.gold -= price;
        my.board[boardIndex] = { ...card, instanceId: generateInstanceId() };
    ...
    async function swapBoardAction(indexA, indexB) {
        ...
        const uid = auth.currentUser?.id; const my = gameState.players[uid];
        if (my.isBot) return false;
        [my.board[indexA], my.board[indexB]] = [my.board[indexB], my.board[indexA]];
        await updateGameState(); return true;
    }
    ...
    async function boardToHandAction(boardIndex) {
        ...
        const uid = auth.currentUser?.id; const my = gameState.players[uid];
        if (my.isBot) return false;
        const card = my.board[boardIndex]; if (!card) return false;
        if (my.hand.filter(c => c).length >= (config.HAND_MAX_COUNT || 15)) { toast('手牌已满', true); return false; }
        const empty = my.hand.findIndex(c => c === null);
        if (empty === -1) { toast('手牌已满', true); return false; }
        my.hand[empty] = card; my.board[boardIndex] = null;
        await updateGameState(); return true;
    }
