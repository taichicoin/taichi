...
window.YYCardBattle = (function() {
    ...
    async function onPhaseEnd(phase) {
        ...
        try {
            if (phase === 'prepare') {
                ...
                // 调用后端 settlement 函数处理回合结束（奖励、刷新商店、推进回合）
                ...
                if (!accessToken) {
                    ...
                    try {
                        ...
                        if (!data.success) {
                            log(`❌ 回合结束处理失败: ${data.error}`, true);
                        } else {
                            log(`✅ 回合结束处理成功，新回合: ${data.round}`);
                        }
                    } catch (err) {
                        log(`❌ 调用 settlement 函数异常: ${err.message}`, true);
                    }
                }
                // 强制刷新最新状态（包括血量、回合、商店等）
                ...
                // 后端已经将 phase 改为 'prepare' 并更新了 phaseStartTime，直接进入准备阶段
    ...
    async function fastForwardAndResume() {
        ...
        if (round > oldRound) {
            // 注意：这里不再手动补发奖励，因为 settlement 函数会处理，但重连时可能需要补发？
            // 简单处理：只更新回合数，不补发奖励（后续调用 settlement 时会正常发放）
    ...
    // 已删除所有玩家操作方法（buyExpAction, buyCardAction, placeCardAction, sellCardAction, buyAndPlaceAction, swapBoardAction, boardToHandAction）
    ...
    return {
        ...
        getCurrentRoomId,
        forceRefreshState,
        updateGameState,   // 仍用于后台逻辑和战斗结果写入，后续可迁移
    };
})();

console.log('✅ battle.js 加载完成（已移除玩家操作，回合结算调用 settlement 函数）');
