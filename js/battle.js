...
window.YYCardBattle = (function() {
    ...
    // ★ 合并其他玩家（只更新轻量字段）
    function mergeOtherPlayers(newState) {
    ...
    async function refreshGameState() {
        ...
        // ── 关键保护：如果商店正在进行操作（拖拽/购买/刷新等），不更新自己，只更新其他玩家 ──
        if (window.YYCardShop?.operationLock || window.YYCardShopRefresh?.isRefreshing) {
            mergeOtherPlayers(newState);
            renderPlayerStatus();
            return;
        }
