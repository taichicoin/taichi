...
window.YYCardBattle = (function() {
    ...
    function initDebugPanel() {
        ...
        p.style.cssText = `
            ...
            color:#7bffb1; font-size:11px; padding:4px 8px;
            z-index:100000;
    ...
    async function onPhaseEnd(phase) {
        ...
        try {
            if (phase === 'prepare') {
                ...
                // ===== 新增：为下回合战斗生成随机配对 =====
                if (window.YYCardCombat && window.YYCardCombat.pairPlayers) {
                    gameState.battlePairs = window.YYCardCombat.pairPlayers(gameState.players);
                    log(`🔄 下回合配对: ${JSON.stringify(gameState.battlePairs)}`);
                } else {
                    log('⚠️ 战斗模块未加载，无法配对', true);
                }
...
console.log('✅ battle.js 加载完成【模块化战斗版 + 配对修复】');
