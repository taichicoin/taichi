...
window.YYCardBattle = (function() {
    ...
    function safeRefreshUI() {
        ...
        if (document.getElementById('eliminated-overlay')) return;  // 淘汰后禁止刷新
    ...
    // ★ 淘汰界面全屏弹窗
    function showEliminatedOverlay(rank) {
        // 防止重复创建
        ...
        `;
        overlay.innerHTML = `
            <div style="font-size: 56px; margin-bottom: 20px;">💀 你已被淘汰</div>
    ...
    async function tick() {
        ...
        const myId = auth?.currentUser?.id;
        const myPlayer = myId ? gameState?.players?.[myId] : null;
        if (myPlayer?.isEliminated && !document.getElementById('eliminated-overlay')) {
            showEliminatedOverlay(myPlayer.rank || '?');
        }
...
console.log('✅ battle.js 淘汰弹窗 + 重连拦截完整版');
