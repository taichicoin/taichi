...
window.YYCardBattle = (function() {
    ...
    // ★ 修改后的战斗动画启动函数
    async function launchBattleAnimation(forceReplay = false) {
        if (isAnimPlaying && !forceReplay) return;
        const myPlayer = gameState?.players?.[auth?.currentUser?.id];
        if (myPlayer?.isEliminated) return;
        isAnimPlaying = true;

        // 调用 settle-battle，检查是否已结算
        const { data: { session } } = await supabase.auth.getSession();
        ...
        // 如果后端已经结算过，直接更新玩家状态并跳过动画
        if (settleData?.alreadySettled) {
            console.log('⚡ 战斗已结算过，跳过动画，直接更新玩家状态');
            // 更新玩家状态
    ...
    async function enterBattle(roomId) {
        ...
        for (let i = 0; i < 20; i++) {
            ...
            if (state) {
                ...
                // 如果处于战斗阶段，尝试启动动画（自动判断是否已结算）
                if (currentPhase === 'battle' && !myPlayer?.isEliminated) {
                    console.log('⚔️ 重连时处于战斗阶段，准备启动战斗动画');
