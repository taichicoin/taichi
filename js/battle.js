            ...
            if (!enemyBoard) {
                ...
                if (battleView) {
                    ...
                    if (myBoard && myBoard.parentNode) {
                        myBoard.parentNode.insertBefore(enemyBoard, myBoard.nextSibling);
                    } else {
                        battleView.appendChild(enemyBoard);
                    }
                } else {
            ...
            if (myPair) {
                ...
                if (gameState.players[oppId]?.board && window.YYCardCombat?.renderEnemyBoardFromData) {
                    ...
                    if (window.YYCardCombat?.waitForEnemyBoard) {
                        const ready = await window.YYCardCombat.waitForEnemyBoard(oppId);
                        if (!ready) {
                            debugLog('⚠️ 等待敌方棋盘超时，但继续尝试播放');
                        }
                    }
                }
            }
    ...
    async function enterBattle(roomId) {
        ...
        for (let i = 0; i < 20; i++) {
            ...
            if (state) {
                ...
                if (currentPhase === 'battle' && gameState.lastSettledRound >= calculatedRound) {
                    console.log('重连发现已结算，直接进准备');
                    applyUIMode(true);
                    loaded = true;
                    break;
                }
