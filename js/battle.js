...
window.YYCardBattle = (function() {
    ...
    let gameState = null;               // 只存储 players, gameStartTime, round
    ...
    // ========== 核心：根据 gameStartTime 和 round 实时计算当前阶段和剩余秒数 ==========
    function calculatePhase(gameStartSec, round, nowSec) {
        ...
        // 跳过已经完成的完整回合
        while (elapsed >= totalRoundSec && round < 100) {
    ...
    // ========== 从数据库拉取必要数据（只读 players, round, gameStartTime） ==========
    ...
    // 更新本地游戏状态（仅 players 和 round，不涉及阶段）
    ...
    async function tick() {
        ...
        let { round, phase, remaining } = calculatePhase(startSec, gameState.round, nowSec);

        // 如果计算出的回合大于本地存储，说明后端已推进，同步本地 round
        ...
        // 检测战斗阶段结束（上一轮是战斗，且剩余时间<=0或当前阶段不是战斗）
    ...
    function subscribeGameState() {
        ...
        gameSubscription = supabase.channel(`game:${currentRoomId}`)
            .on('postgres_changes', {
                ...
                if (newState) {
                    // 只同步 players 和 round，不覆盖 gameStartTime
    ...
    return {
        ...
        // 以下两个函数供 shop.js 同步数据
        forceRefreshState: refreshGameState,
        fetchGameState
    };
})();

console.log('✅ battle.js 加载完成（纯时间驱动，无前端商店操作，战斗结束自动结算）');
