...
window.YYCardBattle = (function() {
    ...
    function startPhaseTimer(phase, duration) {
        ...
        if (!duration || duration <= 0) {
            log(`⚠️ 阶段 ${phase} 时长为 ${duration}，强制立即结束`, true);
            onPhaseEnd(phase);
            return;
        }
        ...
        safetyTimer = setTimeout(() => {
            if (phaseTimer) {
                log(`⚠️ 阶段 ${phase} 超时未结束，强制触发`, true);
    ...
    async function onPhaseEnd(phase) {
        ...
        try {
            if (phase === 'prepare') {
                ...
                await simulateBattle();  // 战斗模拟异步进行，不影响计时器
            }
                ...
                startPhaseTimer('prepare', prepareDur);
            }
        } catch (err) {
            log(`❌ onPhaseEnd 异常: ${err.message}`, true);
            // 异常恢复：回到准备阶段
            if (gameState) {
                gameState.phase = 'prepare';
                await updateGameState();
    ...
    async function refreshAllShops() {
        if (!gameState || gameState.phase !== 'prepare') {
            log(`⚠️ 刷新商店跳过，当前阶段 ${gameState?.phase}`);
            return;
        }
        for (const pid in gameState.players) {
    ...
    async function fastForwardAndResume() {
        ...
        // 计算阶段开始时间（用于计时器剩余时间）
        let phaseDuration = 0;
        if (phase === 'prepare') phaseDuration = getPrepareDuration(round);
        else if (phase === 'buffering') phaseDuration = BUFFER_DURATION;
        else if (phase === 'battle') phaseDuration = getBattleDuration(round);
        else phaseDuration = SETTLE_DURATION;
...
console.log('✅ battle.js 加载完成（递增回合 + 缓冲期2s + 结算期3s + 根治卡00）');
