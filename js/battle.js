...
window.YYCardBattle = (function() {
    ...
    // ★★★ 核心修复：正确处理 alreadySettled，已结算的回合绝不播放动画 ★★★
    async function launchBattleAnimation() {
        ...
        try {
            ...
            if (settleData.alreadySettled) {
                console.log('⚡ 战斗已结算过，跳过动画，直接进入准备阶段');
                isAnimPlaying = false;
                safeRefreshUI();
                // 仍然需要补发个人奖励
                const myId = auth?.currentUser?.id;
            ...
            if (window.YYCardCombat?.resolveBattles) {
                // 将 buffEvents 和 combatResults 注入到 gameState 中，供动画模块使用
    ...
    async function enterBattle(roomId) {
        ...
        for (let i = 0; i < 20; i++) {
            ...
            if (state) {
                ...
                // 无论什么阶段，立即根据计算结果设置 UI
                applyUIMode(currentPhase === 'prepare');

                // 如果在战斗阶段，调用 launchBattleAnimation，它自己会判断要不要播动画
                if (currentPhase === 'battle' && !myPlayer?.isEliminated) {
                    console.log('⚔️ 重连时处于战斗阶段，启动动画检查');
                    ...
                    launchBattleAnimation(); // 它会根据 alreadySettled 决定是否跳过
                }
