...
window.YYCardBattle = (function() {
    ...
    async function onPhaseEnd(phase) {
        ...
        if (!gameState || !currentRoomId) {
            log('❌ gameState 或 currentRoomId 为空', true);
            return;
        }
    ...
    async function enterBattle(roomId) {
        ...
        const waitForState = setInterval(() => {
            ...
            if (phase === 'prepare') {
                startPhaseTimer('prepare', getPrepareDuration(round));
            } else if (phase === 'battle') {
    ...
    function renderPlayerStatus() {
        const container = safeGetElement('player-status-list');
    ...
    function renderBattleUI() {
        ...
        const phaseInfo = safeGetElement('phase-info');
        if (phaseInfo) {
            phaseInfo.textContent = gameState.phase === 'prepare' ? '🛒 准备阶段' : 
        ...
        const endBtn = safeGetElement('end-prepare-btn');
        const refreshBtn = safeGetElement('refresh-shop-btn');
        const buyExpBtn = safeGetElement('buy-exp-btn');
    ...
    function renderHand(cards) {
        const container = safeGetElement('hand-container');
        if (!container) return;
        container.innerHTML = '';
    ...
    function renderShop(cards) {
        const container = safeGetElement('shop-container');
        if (!container) return;
        container.innerHTML = '';
        cards.forEach((card, index) => {
            if (!card) return;
    ...
    function bindBattleEvents() {
        safeGetElement('refresh-shop-btn')?.addEventListener('click', refreshShop);
        safeGetElement('refresh-shop-btn-bottom')?.addEventListener('click', refreshShop);
        safeGetElement('buy-exp-btn')?.addEventListener('click', buyExp);
        safeGetElement('buy-exp-btn-bottom')?.addEventListener('click', buyExp);
        safeGetElement('end-prepare-btn')?.addEventListener('click', endPreparePhase);
        safeGetElement('leave-battle-btn')?.addEventListener('click', async () => {
        ...
        safeGetElement('settings-btn')?.addEventListener('click', () => alert('设置功能开发中...'));
    }
...
console.log('✅ battle.js 加载完成（终极DOM加固版）');
