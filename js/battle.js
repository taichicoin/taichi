...
window.YYCardBattle = (function() {
    ...
    async function launchBattleAnimation(isReconnect = false) {
        ...
        try {
            // 重连检测：用前端计算的回合与 lastSettledRound 对比
            const nowSec = getNowServerSec();
            const startSec = Math.floor(new Date(gameState.gameStartTime).getTime() / 1000);
            const { round: calculatedRound } = calculatePhaseInfo(startSec, nowSec);

            if (isReconnect && (gameState.lastSettledRound || 0) >= calculatedRound) {
                console.log('⚡ 重连时发现本回合已结算，跳过动画');
                isAnimPlaying = false;
                ...
                if (settleData.updatedPlayers) {
                    for (const pid in settleData.updatedPlayers) {
                        if (gameState.players[pid]) {
                            ...
                            if (u.gold !== undefined) gameState.players[pid].gold = u.gold;
                            if (u.exp !== undefined) gameState.players[pid].exp = u.exp;
                            if (u.shopLevel !== undefined) gameState.players[pid].shopLevel = u.shopLevel;
                        }
                    }
                }
                isAnimPlaying = false;
    ...
    async function tick() {
        ...
        const nowSec = getNowServerSec();
        const startSec = Math.floor(new Date(gameState.gameStartTime).getTime() / 1000);
        const { round: calculatedRound, phase, remaining, total } = calculatePhaseInfo(startSec, nowSec);

        gameState.phase = phase;
        gameState.round = calculatedRound;
        ...
        if (roundEl) {
            roundEl.textContent = gameState.round || calculatedRound;
        }
        ...
        if (lastPhase !== phase) {
            ...
            if (phase === 'battle' && thisBattleRound !== calculatedRound && !battleAnimationLock && !isAnimPlaying) {
                thisBattleRound = calculatedRound;
        ...
        if (window.YYCardBuff?.tryShowBuffSelection) {
            window.YYCardBuff.tryShowBuffSelection(calculatedRound, phase);
        }
        ...
        gameSubscription = supabase.channel(`game:${currentRoomId}`)
            .on('postgres_changes', {
                ...
                if (changedUserId === auth?.currentUser?.id) {
                    ...
                    if (my && newState) {
                        ['gold','exp','health','shopLevel','playerRound','freeRefresh','isEliminated','rank'].forEach(k => {
    ...
    async function enterBattle(roomId) {
        ...
        for (let i = 0; i < 20; i++) {
            ...
            if (state) {
                ...
                // 重连检测：前端算的回合如果已经结算，直接进准备页
                if ((gameState.lastSettledRound || 0) >= calculatedRound) {
                    applyUIMode(true);
                    loaded = true;
                    break;
                }
