...
window.YYCardBattle = (function() {
    ...
    function initMobileDebugPanel() {
        ...
        p.style.cssText = 'position:fixed;bottom:0;left:0;right:0;max-height:120px;overflow-y:auto;background:rgba(0,0,0,0.8);color:#0f0;font-size:10px;padding:6px 10px;z-index:9999;border-top:1px solid #f5d76e;font-family:monospace;pointer-events:none;';
    ...
    function logToScreen(msg, isError) {
        ...
        if (p.children.length > 30) p.removeChild(p.firstChild);
    }
    ...
    function renderBoard(containerId, cards, isSelf) {
        ...
        for (let i=0; i<6; i++) {
            ...
            if (c) {
                slot.innerHTML = `<div class="card" data-rarity="${c.rarity}"><div class="card-name">${c.name}</div><div class="card-stats">⚔️${c.atk} 🛡️${c.hp}</div>${c.star>0?'<div class="card-star">★</div>':''}</div>`;
            } else {
                slot.innerHTML = `<div class="card empty-slot">⬤</div>`;
            }
            if (isSelf && gameState.phase==='prepare') slot.addEventListener('click', ()=>handleBoardSlotClick(i));
    ...
    function renderHand(cards) {
        ...
        if (!cont) return;
        cont.innerHTML = '';
        cards.forEach((c,i) => { if(c) { const el=createCardElement(c); el.addEventListener('click', ()=>handleHandCardClick(c,i)); cont.appendChild(el); } });
    }
    ...
    function renderShop(cards) {
        ...
        if (!cont) return;
        cont.innerHTML = '';
        cards.forEach((c,i) => { if(c) { const el=createCardElement(c); el.addEventListener('click', ()=>handleShopCardClick(c,i)); cont.appendChild(el); } });
    }
    ...
    function createCardElement(card) {
        ...
        d.innerHTML = `<div class="card-name">${card.name}</div><div class="card-stats">⚔️${card.atk} 🛡️${card.hp}</div><div class="card-price">💰${getCardPrice(card)}</div>${card.star>0?'<div class="card-star">★</div>':''}`;
        return d;
    }
...
console.log('✅ battle.js 加载完成（终极稳定版）');
