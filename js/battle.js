...
window.YYCardBattle = (function() {
    ...
    // ★ 强制准备模式下，只更新金币/经验，不更新 phase/round
    async function refreshGameState() {
        ...
        if (forcePrepare) {
            ...
            // 检测后端回合是否已推进到下一回合的 prepare
            if (newState.round > forcePrepareRound && newState.phase === 'prepare') {
                forcePrepare = false;
                gameState.round = newState.round;
    ...
    async function tick() {
        ...
        if (forcePrepare) {
            if (gameState.round !== forcePrepareRound) {
                // 如果 refreshGameState 已经解除了锁定，这里就不强制覆盖
                if (forcePrepare) phase = 'prepare';
            } else {
                phase = 'prepare';
            }
        }
