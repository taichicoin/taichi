...
window.YYCardBattle = (function() {
    ...
    // ========== 【终极修复】动画核心逻辑：移除错误锁拦截，仅防动画自身重复调用 ==========
    async function simulateBattleWithAnimation() {
        // 仅拦截动画自身重复执行、结算中，不再拦截阶段切换中（动画就在阶段切换内执行）
        if (isAnimating || isSettling) {
            log(`⚠️ 动画被锁，当前状态：动画中=${isAnimating} 结算中=${isSettling}`);
            return;
        }
        isAnimating = true;
        log("⚔️ 【动画启动】进入战斗阶段，开始播放战斗动画");
        ...
        try {
            ...
            if (window.YYCardShop?.refreshAllUI) window.YYCardShop.refreshAllUI();
            log("✅ 【动画完成】战斗动画播放完毕，血量已更新");
        } catch (e) {
            log(`❌ 动画执行出错: ${e.message}`, true);
        } finally {
            isAnimating = false;
            log("🔓 动画锁已释放");
        }
    ...
    async function startPhaseTimerFromEndTime(phaseEndTimeMs) {
        ...
        if (remaining <= 0) {
            log(`⏰ 阶段 ${phase} 剩余时间≤0，直接触发结束`);
            onPhaseEnd(phase);
            return;
        }

        currentPhaseEndServerTime = phaseEndTimeMs;
        log(`⏰ 启动阶段 ${phase} 计时器，剩余 ${remaining}s`);
        ...
        phaseTimer = setTimeout(() => {
            log(`⏰ 阶段 ${phase} 计时器到期，触发结束`);
            clearAllTimers();
            onPhaseEnd(phase);
        }, remaining * 1000);

        // 启动安全兜底定时器，仅在主定时器失效时触发
    ...
    // ========== 【终极修复】阶段切换核心：锁逻辑优化，杜绝重入，唯一动画触发点 ==========
    async function onPhaseEnd(phase) {
        // 阶段切换中直接拦截，100%杜绝重复执行
        if (isInPhaseTransition) {
            log(`⚠️ 阶段切换被锁，当前阶段: ${phase} 已在执行中，直接跳过`, true);
            return;
        }
        ...
        log(`🔄 【阶段切换启动】阶段结束: ${phase}`);

        try {
            // 锁超时兜底：动画+阶段总时长最多15s，超时强制释放
            lockTimeout = setTimeout(() => {
                if (isInPhaseTransition) { 
                    log(`⚠️ 阶段切换锁超时强制释放`, true); 
                    isInPhaseTransition = false; 
                }
            }, 15000);

            if (phase === 'prepare') {
                // 严格流程：缓冲→动画→切换阶段→启动计时器，全程锁保护
                await startBuffering('battle');
                // 全局唯一动画触发点，锁内执行，不会被打断
            ...
            log(`🔄 【阶段切换完成】${phase} 阶段处理完毕`);
        } catch (e) {
            log(`❌ onPhaseEnd 执行出错: ${e.message}`, true);
        } finally {
            // 正常执行完毕，先清超时定时器，再释放锁
            if (lockTimeout) clearTimeout(lockTimeout);
            isInPhaseTransition = false;
    ...
    // ========== tick函数：100%纯UI显示，无任何业务逻辑 ==========
    async function tick() {
        if (!gameState || !gameState.gameStartTime) return;
        // 任何业务进行中，仅更新UI，不做任何其他操作
...
console.log('✅ battle.js 加载完成（动画锁死/重复触发终极修复版）');
