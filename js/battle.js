...
window.YYCardBattle = (function() {
    ...
    async function fetchGameState(roomId) {
        ...
        for (const row of coreRows) {
            if (row.user_id === GLOBAL_USER_ID) {
                ...
                players[row.user_id] = row.state;   // 自己的完整状态
            }
        }
        ...
        const { data: otherRows, error: otherError } = await supabase
            .from('game_states')
            .select('user_id, state->health, state->isEliminated, state->rank, state->playerRound')
            .eq('room_id', roomId)
        ...
        if (otherError) {
            ...
            for (const row of otherRows) {
                players[row.user_id] = {
                    ...
                    isBot: false,
                    freeRefresh: 0,
                    avatar: null,  // 如果有头像需求，可后续从其他地方获取
                };
            }
        }
    ...
    async function refreshGameState() {
        ...
        // 处理自己和其他玩家的数据合并
        const mergeOtherPlayers = (onlyLight = false) => {
            for (const pid of Object.keys(newState.players)) {
                ...
                // 如果需要更新 shopLevel，可酌情添加
                // oldP.shopLevel = newP.shopLevel !== undefined ? newP.shopLevel : oldP.shopLevel;
                // 注意：board/hand 保留不变，除非明确需要覆盖
            }
        };
    ...
    function renderPlayerStatus() {
        ...
        orderedIds.forEach(pid => {
            ...
            const level = p.shopLevel || 1;  // 有默认值1
