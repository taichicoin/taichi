    ...
    async function enterBattle(roomId) {
        ...
        for (let i = 0; i < 20; i++) {
            ...
            if (state) {
                ...
                // ★ 如果当前处于战斗阶段，立即启动战斗动画（自动调用 settleBattle）
                if (currentPhase === 'battle' && !myPlayer?.isEliminated) {
                    console.log('⚔️ 重连时处于战斗阶段，初始化战斗动画');
                    hasCalledSettleBattle = false;
                    // 设置 battleStartTime 为战斗阶段开始时间，用于进度条和跳过按钮
                    const battleStartSec = startSec + getPrepareDuration(calculatedRound) + BUFFER_DURATION;
                    battleStartTime = battleStartSec;
                    // 启动动画（会内部调用 settleBattle）
                    launchBattleAnimation();
                }
