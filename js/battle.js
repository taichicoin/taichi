...
window.YYCardBattle = (function() {
                ...
                if (session) {
                    ...
                    if (gameState) {
                        ...
                        if (window.YYCardShop?.updateTimerDisplay) {
                            window.YYCardShop.updateTimerDisplay(0, 'prepare');
                        }
                        // 6. 刷新界面
                        safeRefreshUI();
    ...
    function calculatePhaseInfo(gameStartSec, nowSec, forcePrepare = false) {
        ...
        while (true) {
            ...
            if (elapsed >= totalRound) {
                ...
                if (forcePrepare && elapsed >= prep) {
                    const remainingTotal = totalRound - elapsed;
                    return { round, phase: 'prepare', remaining: remainingTotal, total: prep };
                }
    ...
    async function tick() {
        ...
        let { round, phase, remaining, total } = calculatePhaseInfo(startSec, nowSec, forcePrepare);

        // 3. 强制准备模式：锁定状态，禁止任何阶段切换
        if (forcePrepare) {
            // 如果回合数已经变了，说明自然进入了下一回合，解除强制准备
            if (gameState.round !== forcePrepareRound) {
                forcePrepare = false;
            } else {
                phase = 'prepare'; // 强制保持准备阶段
                // 直接更新计时器和回合数显示，但跳过所有阶段变更逻辑
                gameState.phase = 'prepare';
                gameState.round = round;
                if (window.YYCardShop?.updateTimerDisplay) window.YYCardShop.updateTimerDisplay(remaining, 'prepare');
                if (window.YYCardShop?.setPhase) window.YYCardShop.setPhase('prepare');
