...
window.YYCardBattle = (function() {
    ...
    async function tick() {
        ...
        // ★ 绿色通道保护：如果已开启，强制保持 prepare 显示，直到时间走到真正的 prepare
        if (window.YYCardShop?.getForcePrepareMode && window.YYCardShop.getForcePrepareMode()) {
            // 强制当前阶段为 prepare，确保 UI 不跳回战斗
            gameState.phase = 'prepare';
            // 当时间公式也计算出 prepare 时，关闭绿色通道，恢复正常
            if (phase === 'prepare') {
                window.YYCardShop.setForcePrepareMode(false);
                gameState.round = round; // 同步回合
            }
        } else {
        ...
        // 更新 lastPhase，绿色通道开启时视为 prepare
