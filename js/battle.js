...
window.YYCardBattle = (function() {
    ...
    function ensureSkipUI() {
        ...
        bar.style.cssText = `
            ...
            width: 100%;
            height: 100%;
            background: linear-gradient(90deg, #ff8800, #f5d76e);
            border-radius: 0.4vh;
    ...
    async function launchBattleAnimation(isReconnect = false) {
        ...
        try {
            ...
            const onAnimationComplete = async () => {
                ...
                safeRefreshUI();
                await applyPSettlement();
                callSettlement(); // 推进全局回合
                checkAndShowElimination();
            };

            if (window.YYCardCombat?.resolveBattles) {
                window.YYCardCombat.resolveBattles(gameState, onAnimationComplete);
            } else {
    ...
    // ★ 渲染左侧玩家状态列表（头像方形、等级左上角、血量数字右下角三角形）
    ...
    async function tick() {
        ...
        if (phase === 'battle') {
            ...
            if (progressBar) {
                ...
                // 改用 scaleX 动画，从右往左收缩，不改变布局宽度，彻底杜绝溢出
                progressBar.style.transform = `scaleX(${remainingPct / 100})`;
                progressBar.style.width = '100%';        // 保持宽度不变
                progressBar.style.clipPath = 'none';     // 废弃 clip-path
            }
            if (skipBtn) {
    ...
    function subscribeGameState() {
        ...
        gameSubscription = supabase.channel(`game:${currentRoomId}`)
            .on('postgres_changes', {
                ...
                await refreshGameState();
                renderPlayerStatus(); // 数据库推送后立即刷新列表
            })
            .subscribe();
    }
        ...
        for (let i = 0; i < 20; i++) {
            ...
            if (state) {
                ...
                if (currentPhase === 'battle' && !myPlayer?.isEliminated) {
                    ...
                    launchBattleAnimation(true); // 重连模式
                }
