// ==================== 对战系统【纯时间驱动版 + 状态机根治锁 + 强制同步UI + 轮询保底 + 缓冲期 + 淘汰名次】 ====================
window.YYCardBattle = (function() {
    ...
    // ===== 新增：缓冲期，等待数据同步，禁止操作 =====
    async function startBuffering(seconds, targetPhase) {
        log(`⏳ 进入缓冲期 ${seconds} 秒，准备切换到 ${targetPhase} 阶段`);
        ...
        if (window.YYCardShop?.updateTimerDisplay) {
            window.YYCardShop.updateTimerDisplay(seconds, 'buffering');
        }
        
        await new Promise(resolve => setTimeout(resolve, seconds * 1000));
        
        if (window.YYCardShop?.setPhase) {
            window.YYCardShop.setPhase(targetPhase);
        }
    }
        ...
        if (!isPrepare) {
            ...
            if (!error && fresh?.state) {
                ...
                if (oppId) {
                    ...
                    const boardStr = oppBoard.map((c, i) => c ? `[${i+1}]${c.name}` : `[${i+1}]空`).join(' ');
                    log(`📋 对手 ${oppId.slice(0,8)} 棋盘: ${boardStr}`);
                } else {
                    log(`⚠️ 未找到对手ID`);
                }
            }
    ...
    async function simulateBattle() {
        try {
            ...
            if (error) {
                log(`❌ 获取最新状态失败: ${error.message}`, true);
                return;
            }
    ...
    function checkGameOver() {
        ...
        Object.entries(players).forEach(([id, p]) => {
            if (p.health <= 0 && !p.isEliminated) {
                ...
                if (!eliminationOrder.includes(id)) {
                    ...
                    log(`☠️ 玩家 ${id.slice(0,8)} 被淘汰，获得第 ${rank} 名`);
                }
            }
        });
        ...
        if (alive.length <= 1) {
            ...
            if (alive[0] && !eliminationOrder.includes(winner)) {
                ...
                log(`🏆 玩家 ${winner.slice(0,8)} 获得第 1 名`);
            }
    ...
    async function enterBattle(roomId) {
        ...
        if (!shopReady) {
            log('⚠️ shop 模块未在 3 秒内就绪，尝试强制初始化');
    ...
    function startPolling() {
        ...
        pollingInterval = setInterval(async () => {
            ...
            if (data?.state) {
                ...
                const oldBoard = gameState?.players[auth.currentUser?.id]?.board;
                const newBoard = newState?.players[auth.currentUser?.id]?.board;
...
console.log('✅ battle.js 加载完成（缓冲期 + 淘汰名次日志）');
