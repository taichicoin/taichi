...
window.YYCardBattle = (function() {
    ...
    let isSettling = false;          // 旧 settlement 调用中
    let isPlayingAnimation = false;  // 战斗动画播放中
    let lastPhase = null;
    let thisBattleRound = 0;         // 当前战斗阶段的回合号，用于防重复
    ...
    // 启动战斗动画（调用 combat 模块，由 combat 负责调用后端 settle-battle）
        ...
        if (window.YYCardCombat?.resolveBattles) {
            try {
                await window.YYCardCombat.resolveBattles(gameState, log, async () => {
                    // 动画完成回调：用后端返回的最新数据更新 gameState
                    // （resolveBattles 内部已经更新了 gameState.players）
    ...
    async function tick() {
        ...
        if (currentPhase !== phase) {
            ...
            // 离开战斗阶段 → 调用旧 settlement 结算经验、金币
            if (currentPhase === 'battle' && phase !== 'battle') {
                log("⚡ 战斗阶段结束，触发旧结算");
                callSettlement();
                thisBattleRound = 0;  // 允许下次战斗阶段重新触发动画
            }
