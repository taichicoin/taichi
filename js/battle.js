    ...
    function updateBodyPhaseClass(phase) {
        ...
        switch (phase) {
            case 'prepare':
                ...
                log(`📌 页面切换为【准备模式】，商店/手牌已显示`);
                break;
            case 'buffering':
                // 缓冲期：禁用操作，不隐藏
                document.body.classList.add('buffering-mode');
                log(`📌 页面切换为【缓冲模式】，操作已禁用`);
                break;
            case 'battle':
            case 'settle':
                // 战斗/结算阶段：强制加战斗模式类，隐藏商店/手牌
                document.body.classList.add('battle-view-mode');
                log(`📌 页面切换为【战斗模式】，商店/手牌已强制隐藏`);
                break;
            default:
                document.body.classList.add('prepare-mode');
                break;
...
console.log('✅ battle.js 加载完成（修复完整版：战斗阶段隐藏100%生效）');
