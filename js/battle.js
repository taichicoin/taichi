...
window.YYCardBattle = (function() {
    ...
    async function fastForwardAndResume() {
        ...
        if (round > gameState.round) {
            ...
            if (finalRemaining <= 0) { onPhaseEnd(gameState.phase); return true; }
        ...
        if (remaining <= 0) { onPhaseEnd(phase); return true; }
    ...
    async function onPhaseEnd(phase) {
        ...
        try {
            if (phase === 'prepare') {
                ...
                // 如果后端已经更新了 phase 为 prepare，直接进入准备阶段
                if (gameState.phase === 'prepare') {
                    ...
                    // 后端没有更新（结算失败），强制切换到准备阶段（本地推进）
                    log(`⚠️ 结算接口未更新状态，强制进入下一回合准备阶段`, true);
                    gameState.round++;
                    gameState.phase = 'prepare';
                    const nowServer = await getServerTimeMs();
                    const prepareDur = getPrepareDuration(gameState.round);
                    gameState.phaseEndTime = new Date(nowServer + prepareDur * 1000).toISOString();
                    gameState.phaseStartTime = new Date(nowServer).toISOString();
                    await updatePhaseToDB();
                    await applyUIMode(true);
                    await startPhaseTimerFromEndTime(new Date(gameState.phaseEndTime).getTime());
                    if (window.YYCardShop?.toast) window.YYCardShop.toast('结算异常，已强制进入下一回合', true);
                }
            }
        } catch (e) {
