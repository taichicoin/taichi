...
window.YYCardBattle = (function() {
    ...
    async function refreshGameState() {
        ...
        if (newState) {
            // 全量刷新时，也需要保护当前玩家的 board/hand（因为可能尚未提交）
    ...
    // ========== 辅助函数：合并玩家数据，保护当前玩家的 board/hand ==========
        ...
        for (const [pid, newPlayer] of Object.entries(newPlayers)) {
            if (pid === currentUserId) {
                ...
                result[pid] = {
                    ...
                    // board 和 hand 保持原样
                    board: result[pid].board,
                    hand: result[pid].hand,
                };
            } else {
                // 其他玩家：完全替换
...
console.log('✅ battle.js 加载完成（纯时间驱动，轮询保护乐观更新）');
