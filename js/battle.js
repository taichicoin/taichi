    ...
    async function enterBattle(roomId) {
        ...
        for (let i = 0; i < 20; i++) {
            ...
            if (state) {
                ...
                const nowSec = await getServerTime();
                const startSec = Math.floor(new Date(gameState.gameStartTime).getTime() / 1000);
                const { round: calculatedRound, phase: currentPhase, remaining, total } = calculatePhaseInfo(startSec, nowSec);
                ...
                // 重连补发个人奖励（仅在战斗阶段且 playerRound 落后时）
                ...
                // 如果处于战斗阶段，启动战斗动画（需要确保对手数据完整）
                if (currentPhase === 'battle' && !myPlayer?.isEliminated) {
                    console.log('⚔️ 重连时处于战斗阶段，准备启动战斗动画');
                    // 计算战斗开始时间，用于进度条和跳过按钮
                    battleStartTime = nowSec - (total - remaining);
                    // 允许重新调用 settleBattle（如果之前没调用过，则调用；如果已调用，后端应幂等）
                    hasCalledSettleBattle = false;
                    // 启动动画（会内部调用 settleBattle 和播放动画）
                    launchBattleAnimation();
                }
