    ...
    function ensureSkipUI() {
        ...
        skipBtn.onclick = async () => {
            ...
            try {
                ...
                // ① 先查一次全局 phase，避免无意义的 400
                const globalRow = await supabase
                    .from('game_states')
                    .select('state')
                ...
                const currentPhase = globalRow?.data?.state?.phase || 'battle';
                console.log('🔍 跳过前全局 phase:', currentPhase);
                ...
                if (currentPhase === 'battle') {
                    ...
                    console.log('📦 psettlement 返回:', result);
                    ...
                    console.warn('⚠️ 全局已非战斗阶段，直接切换 UI');
                }
