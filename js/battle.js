...
window.YYCardBattle = (function() {
    ...
    // ★★★ fetchGameState：包含 pendingConsumables 和 activeBuff
    ...
    // 移除了 callSettlement 和 applyPSettlement
    // 战斗结算已内置在 settle-battle 中，奖励发放也由 settle-battle 完成
    ...
    async function launchBattleAnimation(isReconnect = false) {
        ...
        try {
            ...
            const onAnimationComplete = async () => {
                ...
                // settle-battle 已经发放了奖励、推进了回合，直接切换到准备阶段
    ...
    async function tick() {
        ...
        // 移除了原来的全局回合推进检测和 callSettlement 调用
    ...
    async function enterBattle(roomId) {
        ...
        for (let i = 0; i < 20; i++) {
            ...
            if (state) {
                ...
                // 不再需要补发奖励，settle-battle 在战斗结算时已发放
