...
window.YYCardBattle = (function() {
    ...
    function ensureSkipUI() {
        ...
        skipBtn.onclick = async () => {
            // 必须已经调用过 settle-battle（即已经开始结算流程）
            if (!hasCalledSettleBattle) {
                alert('战斗尚未结算完成，请稍候');
                return;
            }
            // 如果动画正在播放，直接终止；若还未播放，也强制终止等待状态
            forceStopAnimation();
            if (window.YYCardShop?.setForcePrepareMode) {
                window.YYCardShop.setForcePrepareMode(true);
            }
            // 确保本地 lastSettledRound 已经更新（因为 settle-battle 请求可能已经完成）
            // 这里通过 gameState.round - 1 保守赋值，避免重连时误判
            if (gameState) {
    ...
    async function enterBattle(roomId) {
        ...
        for (let i = 0; i < 20; i++) {
            ...
            if (state) {
                ...
                // 如果在战斗阶段且尚未结算，启动战斗动画（可能已结算也会被 alreadySettled 拦截）
