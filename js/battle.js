// ==================== 纯时间驱动对战系统【最终修复版：操作锁保护，不再覆盖正在操作的玩家数据】 ====================
window.YYCardBattle = (function() {
    ...
    function safeRefreshUI() {
        // ★ 商店正在操作中，跳过 UI 刷新
        if (window.YYCardShop?.isOperating || window.YYCardShop?.isRefreshing) return;
    ...
    // ★ 核心修改：合并数据时，如果当前用户正在操作，完全保留其本地数据
    function mergePlayersData(currentPlayers, newPlayers, currentUserId) {
        ...
        for (const [pid, newPlayer] of Object.entries(newPlayers)) {
            if (pid === currentUserId && window.YYCardShop?.isOperating) {
                // 当前用户正在操作，跳过合并，保留本地数据
                continue;
            }
...
console.log('✅ battle.js 已修复：操作期间不再覆盖本地数据');
