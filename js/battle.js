// ==================== 对战系统（状态机 + 后台逻辑，玩家操作已移至 Edge Functions） ====================
    ...
    async function onPhaseEnd(phase) {
        ...
        try {
            if (phase === 'prepare') {
                ...
                // ========== 详细日志：结算阶段开始 ==========
                log(`📞 [结算] 阶段开始，准备调用 settlement 函数`, false, true);
                const userId = auth.currentUser?.id;
                const roomId = currentRoomId;
                log(`🔑 当前用户: ${userId}`, false, true);
                log(`🏠 房间ID: ${roomId}`, false, true);
                const { data: { session } } = await supabase.auth.getSession();
                const accessToken = session?.access_token;
                if (!accessToken) {
                    log('❌ 无法获取 accessToken，跳过回合结束处理', true, true);
                } else {
                    try {
                        const url = 'https://sznjaotjoljaiawbvfro.supabase.co/functions/v1/settlement';
                        log(`🌐 请求URL: ${url}`, false, true);
                        const requestBody = JSON.stringify({ roomId });
                        log(`📦 请求体: ${requestBody}`, false, true);
                        ...
                        log(`📡 响应状态码: ${response.status}`, false, true);
                        log(`📦 响应数据: ${JSON.stringify(data)}`, false, true);
                        if (!response.ok || !data.success) {
                            log(`❌ 回合结束处理失败: ${data.error || '未知错误'}`, true, true);
                        } else {
                            log(`✅ 回合结束处理成功，新回合: ${data.round}`, false, true);
                        }
                    } catch (err) {
                        log(`❌ 调用 settlement 函数异常: ${err.message}`, true, true);
                    }
                }
                // 强制刷新最新状态
                log(`🔄 强制刷新游戏状态...`, false, true);
                await forceRefreshState();
                log(`🔄 刷新后 round=${gameState.round}, phase=${gameState.phase}`, false, true);
                const over = checkGameOver();
                if (over.isOver) {
                    log(`🏁 游戏结束，胜者: ${over.winner}`, false, true);
                    endGame(over.winner);
                    clearTimeout(lockTimeout);
                    return;
                }
                // 后端应该已经将 phase 改为 'prepare' 并更新 phaseStartTime
            ...
            log(`❌ onPhaseEnd 出错: ${e.message}`, true, true);
        } finally {
            clearTimeout(lockTimeout);
...
console.log('✅ battle.js 加载完成（已移除玩家操作，回合结算调用 settlement 函数，添加详细日志）');
