// ==================== 纯时间驱动对战系统【适配玩家独立行 · 跳过战斗 · 淘汰延迟显示 · 跳过可操作修复】 ====================
window.YYCardBattle = (function() {
    ...
    async function tick() {
        ...
        if (lastPhase !== phase) {
            ...
            // ★ 修复：applyUIMode 直接操作 DOM，彻底清除战斗阶段限制
    ...
    // ★★★ 核心修复：applyUIMode 直接操作 DOM，而不是只切 CSS 类 ★★★
    function applyUIMode(isPrepare) {
        const battleView = document.getElementById('battle-view');
        if (!battleView) return;

        if (isPrepare) {
            // 准备阶段：显示商店、手牌，隐藏敌方棋盘和分隔线
