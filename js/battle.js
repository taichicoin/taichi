...
window.YYCardBattle = (function() {
    ...
    async function launchBattleAnimation(isReconnect = false) {
        ...
        try {
            // 重连检测：lastSettledRound >= round 表示本回合已结算
            const nowSec = getNowServerSec();
            const startSec = Math.floor(new Date(gameState.gameStartTime).getTime() / 1000);
            const { round: calculatedRound } = calculatePhaseInfo(startSec, nowSec);

            if (isReconnect && gameState.lastSettledRound >= calculatedRound) {
    ...
    // ★★★ 主循环 tick（不再调用 gsettlement / psettlement） ★★★
    async function tick() {
        if (!gameState) return;
        const myId = auth?.currentUser?.id;
    ...
    async function enterBattle(roomId) {
        ...
        for (let i = 0; i < 20; i++) {
            ...
            if (state) {
                ...
                // 重连检测：如果 lastSettledRound >= 当前计算回合，代表已结算
                if (gameState.lastSettledRound >= calculatedRound) {
                    // 已经结算过，直接进准备页
                    applyUIMode(true);
                    loaded = true;
                    break;
