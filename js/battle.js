...
window.YYCardBattle = (function() {
    ...
    async function refreshGameState() {
        ...
        gameState.gameStartTime = newState.gameStartTime;
        gameState.lastSettledRound = newState.lastSettledRound;  // ★ 同步
        safeRefreshUI();
    }
    ...
    // ★★★ 终极修复：用 lastSettledRound 判断是否已结算，已结算就切准备页，不播动画 ★★★
    async function launchBattleAnimation() {
        if (isAnimPlaying) return;
        const myPlayer = gameState?.players?.[auth?.currentUser?.id];
        if (myPlayer?.isEliminated) return;

        const currentRound = gameState.round;
        const lastSettled = gameState.lastSettledRound || 0;

        console.log(`🔍 本回合: ${currentRound}, lastSettledRound: ${lastSettled}`);

        // 如果本回合已经结算过，直接切换到准备界面，绝不播放动画
        if (lastSettled >= currentRound) {
            console.log('⚡ 本回合已结算，跳过动画，直接进入准备阶段');
            isAnimPlaying = false;
            if (window.YYCardShop?.setForcePrepareMode) {
                window.YYCardShop.setForcePrepareMode(true);
            }
        ...
        // 未结算，正常请求 settle-battle 并播放动画
        isAnimPlaying = true;
        try {
            const { data: { session } } = await supabase.auth.getSession();
            if (!session) { isAnimPlaying = false; return; }
            ...
            if (settleData.updatedPlayers) {
                for (const pid in settleData.updatedPlayers) {
                    if (gameState.players[pid]) {
                        const updated = settleData.updatedPlayers[pid];
                        if (updated.health !== undefined) gameState.players[pid].health = updated.health;
                        if (updated.board !== undefined) gameState.players[pid].board = updated.board;
                        if (updated.hand !== undefined) gameState.players[pid].hand = updated.hand;
                        if (updated.isEliminated !== undefined) gameState.players[pid].isEliminated = updated.isEliminated;
                        if (updated.rank !== undefined) gameState.players[pid].rank = updated.rank;
                    }
                }
            }

            // 后端双重保险：如果 alreadySettled 为真，也不播放
            if (settleData.alreadySettled) {
                console.log('⚡ 后端确认已结算，跳过动画');
                isAnimPlaying = false;
                if (window.YYCardShop?.setForcePrepareMode) {
                    window.YYCardShop.setForcePrepareMode(true);
                }
                ...
                if (myId) {
                    try {
                        const resp2 = await fetch(PSETTLEMENT_URL, {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                Authorization: `Bearer ${session.access_token}`
                        ...
                        if (result2?.success && result2.updatedPlayer && gameState?.players?.[myId]) {
                            const p = gameState.players[myId];
                            if (result2.updatedPlayer.gold !== undefined) p.gold = result2.updatedPlayer.gold;
                            if (result2.updatedPlayer.exp !== undefined) p.exp = result2.updatedPlayer.exp;
                            if (result2.updatedPlayer.freeRefresh !== undefined) p.freeRefresh = result2.updatedPlayer.freeRefresh;
                            if (result2.playerRound !== undefined) {
                                p.playerRound = result2.playerRound;
                                lastDisplayedRound = result2.playerRound;
                            }
                            document.getElementById('round-num').textContent = lastDisplayedRound;
                            if (window.YYCardShop?.refreshAllUI) window.YYCardShop.refreshAllUI();
                        }
            ...
            if (settleData.buffEvents) gameState._buffEvents = settleData.buffEvents;
            if (settleData.combatResults) gameState._combatResults = settleData.combatResults;
    ...
    async function tick() {
        ...
        if (round > gameState.round && !isSettling) {
            console.warn(`⚠️ 全局回合落后，调用 gsettlement`);
            callSettlement();
        }
    ...
    async function enterBattle(roomId) {
        ...
        for (let i = 0; i < 20; i++) {
            ...
            if (state) {
                ...
                if (myPlayer && myPlayer.playerRound < gameState.round && currentPhase === 'battle') {
                    try {
                        const { data: { session } } = await supabase.auth.getSession();
                        if (session) {
                            const resp = await fetch(PSETTLEMENT_URL, {
                ...
                if (currentPhase === 'battle' && !myPlayer?.isEliminated) {
                    ...
                    launchBattleAnimation(); // 自动识别是否已结算
                }
