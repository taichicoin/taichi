...
window.YYCardBattle = (function() {
    ...
    let gameState = null;               // 包含 players, gameStartTime, 以及动态 phase
    ...
    async function tick() {
        ...
        // 同步 phase 到 gameState，供 shop.js 使用
        gameState.phase = phase;
        gameState.round = round;   // 可选，用于显示回合数
        ...
        // 战斗阶段结束检测：上一轮是战斗，且剩余时间 <= 0
        if (currentPhase === 'battle' && remaining <= 0 && !isSettling) {
            log("⚡ 战斗阶段自然结束，触发结算");
            await callSettlement();
            // 结算后，下一轮 tick 会自然进入新回合（因为 elapsed 已超过总时长）
        }
    }
...
console.log('✅ battle.js 加载完成（纯时间驱动，支持 shop.js 操作，战斗结束自动结算）');
