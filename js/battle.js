...
window.YYCardBattle = (function() {
    ...
    function getCurrentPhaseInfo() {
        ...
        const nowSec = getNowServerSec(); // 使用本地时钟
    ...
    // ★★★ 核心优化：轻量拉取游戏状态，绝不拉取完整state ★★★
    async function fetchGameState(roomId) {
        if (!roomId) return null;
        const myId = auth?.currentUser?.id;
        if (!myId) return null;

        // 1. 全局数据：只拉必要的数字字段，不拉 battle_logs
    ...
    async function callSettlement() {
        ...
        try {
            ...
            if (result?.success) {
                ...
                await refreshGameStateLight(); // 结算后只做轻量同步
            }
        } catch (e) {
    ...
    async function launchBattleAnimation(isReconnect = false) {
        ...
        try {
            // 重连检测：利用内存中 lastSettledRound 判断，不再查询数据库
            if (isReconnect) {
            ...
            const onAnimationComplete = async () => {
                ...
                await refreshGameStateLight();   // 动画结束后轻量同步
                await applyPSettlement();
                if (window.YYCardShop?.setForcePrepareMode) {
                    window.YYCardShop.setForcePrepareMode(true);
                }
    ...
    async function tick() {
        ...
        const nowSec = getNowServerSec();  // ★ 不再调用RPC
        const startSec = Math.floor(new Date(gameState.gameStartTime).getTime() / 1000);
    ...
    // ★★★ Realtime 订阅：只处理变化字段，绝不重新拉取完整 state ★★★
    ...
    async function enterBattle(roomId) {
        ...
        subscribeGameState();    // ★ 只用 Realtime 监听
        startGlobalTimer();
        bindLeaveButton();
