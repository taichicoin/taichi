...
window.YYCardBattle = (function() {
    ...
    // 自动注入战斗动画CSS（防止样式缺失导致看不见动画）
    function injectCombatCss() {
        if (document.querySelector('#combat-animation-style')) return;
        const style = document.createElement('style');
        style.id = 'combat-animation-style';
        style.textContent = `
            .card.attacking{animation:attack 0.2s ease-in-out;}
            @keyframes attack{0%{transform:translateX(0);}50%{transform:translateX(8px);}100%{transform:translateX(0);}}
            .card.dying{animation:die 0.6s forwards;}
            @keyframes die{to{opacity:0;transform:scale(0.7);}}
            .damage-number{position:absolute;top:-12px;left:50%;transform:translateX(-50%);color:#ff3333;font-weight:bold;font-size:14px;animation:damage 0.8s forwards;z-index:99;}
            @keyframes damage{0%{opacity:1;transform:translate(-50%,0);}100%{opacity:0;transform:translate(-50%,-20px);}}
        `;
    ...
    async function simulateBattleWithAnimation() {
        // 三重锁：动画播放中/阶段切换中/结算中，直接拦截，绝对不重复执行
        if (isAnimating || isInPhaseTransition || isSettling) {
            log("⚠️ 动画被锁，跳过重复调用");
            return;
        }
        isAnimating = true;
        injectCombatCss(); // 保证样式一定注入，动画能正常显示
        log("⚔️ 进入战斗阶段，播放战斗动画");

        try {
            if (!window.YYCardCombat) {
                log('❌ 战斗模块未加载，跳过动画', true);
                return;
            }
            // 动画播放前强制刷新最新游戏状态，保证有完整战斗数据
            await fetchGameState();
            const combatEvents = await window.YYCardCombat.resolveBattlesWithEvents(gameState);
            
            // 兜底判断：没有战斗事件也正常走完流程，不卡死逻辑
            if (!combatEvents || combatEvents.length === 0 || !combatEvents.some(p => p.events && p.events.length > 0)) {
                log("⚠️ 没有战斗事件，跳过动画播放");
                return;
            }

            // 播放动画，等待动画完全执行完毕再走后续逻辑
            await playCombatAnimations(combatEvents);
            // 动画播放完成后，再应用战斗结果，更新玩家数据
            window.YYCardCombat.applyCombatResult(gameState, combatEvents);
            await updatePlayersToDB();
            if (window.YYCardShop?.refreshAllUI) window.YYCardShop.refreshAllUI();
            log("✅ 战斗动画播放完成，血量已更新");
        } catch (e) {
            log(`❌ 战斗动画执行出错: ${e.message}`, true);
        } finally {
            // 最终一定会释放锁，不会出现死锁卡死
            isAnimating = false;
        }
    ...
    async function onPhaseEnd(phase) {
        ...
        try {
            if (phase === 'prepare') {
                ...
                // 2. 缓冲结束，阶段切换的瞬间，只播放一次动画，等待动画完全播完
                await simulateBattleWithAnimation();
                // 3. 动画播放完成后，再正式切换到战斗阶段，更新数据库状态
    ...
    async function fastForwardAndResume() {
        ...
        if (window.YYCardShop?.refreshAllUI) window.YYCardShop.refreshAllUI();
        clearAllTimers();
        startPhaseTimerFromEndTime(new Date(gameState.phaseStartTime).getTime() + getPhaseDuration(phase, round) * 1000);
        return true;
    }
    ...
    // 修复版：纯UI显示，彻底移除动画和业务逻辑，避免和onPhaseEnd打架
    async function tick() {
        if (!gameState || !gameState.gameStartTime) return;
        // 动画/阶段切换/结算进行中，只更新UI，不跑任何业务逻辑
        if (isAnimating || isInPhaseTransition || isSettling) return;

        // 仅做时间计算和UI显示，绝对不修改gameState，不触发任何业务逻辑
...
console.log('✅ battle.js 加载完成（动画修复完整版）');
