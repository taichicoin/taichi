    ...
    function launchBattleAnimation() {
        ...
        if (window.YYCardCombat?.resolveBattles) {
            window.YYCardCombat.resolveBattles(gameState, log, () => {
                log('🎬 动画自然播放完成');
                isAnimPlaying = false;
                // 动画完成时主动刷新一次 UI，但此时一般已离开战斗阶段，safeRefreshUI 会放行
                safeRefreshUI();
            });
        } else {
            log('❌ 战斗动画模块未加载', true);
    ...
    async function tick() {
        ...
        if (lastPhase !== phase) {
            ...
            if (lastPhase === 'battle' && phase !== 'battle') {
                ...
                forceStopAnimation();
                callSettlement();
                thisBattleRound = 0;  // 重置，下一回合允许再次触发动画
            }

            // 进入战斗阶段 → 如果该回合还未触发过动画，则启动
...
console.log('✅ battle.js 每回合动画修复版');
