...
window.YYCardBattle = (function() {
    ...
    function applyUIMode(isPrepare) {
        ...
        // 确保 buffering 模式样式正确（变暗，非闪白）
        // shop.js 的 setPhase 已经处理了 buffering-mode 类，这里无需额外操作
    }

    // ========== 辅助函数：合并玩家数据，保护当前玩家的 board/hand/shopCards ==========
    ...
    // 强制覆盖缓冲期样式为变暗（解决可能的闪白问题）
    (function ensureBufferingStyle() {
        if (document.getElementById('buffering-style-override')) return;
        const style = document.createElement('style');
        style.id = 'buffering-style-override';
        style.textContent = `
            .buffering-mode .card,
            .buffering-mode .btn,
            .buffering-mode .shop-area,
            .buffering-mode .hand-area,
            .buffering-mode .board,
            .buffering-mode .card-slot {
...
console.log('✅ battle.js 加载完成（轮询保护 + 缓冲期变暗特效）');
