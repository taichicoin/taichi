...
window.YYCardInspector = (function() {
    ...
    function showCardDetail(card, boardIndex, element, isOwn) {
        ...
        // 生成左侧卡牌 HTML（使用与游戏中相同的结构，样式由全局 CSS 控制）
        function buildCardHTML(currentCard) {
            ...
            const star = currentCard.star || 0;
            return `
                <div class="card" data-rarity="${currentCard.rarity || 'Common'}" style="width:23.6vw; height:29.1vw; flex-shrink:0; margin:0; position:relative;">
                    ...
                    ${star > 0 ? '<div class="card-star">★</div>' : ''}
        ...
        // 生成右侧详情 HTML
        function buildDetailHTML(currentCard, showEquip) {
            ...
            function renderSlot(slotKey, equip, label) {
                const baseStyle = 'display:flex; justify-content:space-between; align-items:center; background:rgba(255,255,255,0.1); padding:5px 7px; border-radius:4px; margin:2px 0; min-height:40px; line-height:1.4;';
                if (equip) {
                    return `
                        <div class="equip-row" style="${baseStyle}">
                            <div style="display:flex; align-items:center; gap:5px;">
                                <img src="${equip.image || '/assets/default-avatar.png'}" style="width:24px; height:24px; border-radius:4px;" onerror="this.src='/assets/default-avatar.png'">
                                <div>
                    ...
                    return `
                        <div class="equip-row empty-slot" style="${baseStyle} color:#aaa; font-size:12px;">
                            <div style="display:flex; align-items:center; gap:5px;">
                                <div style="width:24px; height:24px; border-radius:4px; background:rgba(255,255,255,0.1);"></div>
                                <span>${label}待装配</span>
                            </div>
                        </div>
                    `;
            ...
            return `
                <div style="margin-bottom:8px;">
                    <div style="display:flex; gap:8px; align-items:center; margin-bottom:6px;">
                        <span class="rarity-tag rarity-${rarity}" style="background:#444; padding:2px 8px; border-radius:8px;">${rarity}</span>
                        <span class="faction-tag" style="background:#444; padding:2px 8px; border-radius:8px;">${faction}</span>
                        ${shield > 0 ? `<span style="display:inline-block; border:1px solid #ff8800; border-radius:10px; padding:0 6px; font-size:11px; color:#ff8800;">🛡️${shield}</span>` : ''}
                    ...
                    <div style="font-size:14px; margin-bottom:4px;">⚔️ 攻击 ${atk}  ❤️ 生命 ${hp}</div>
                    ${!isOwn ? '<div style="color:#ffd966; font-size:12px;">👁️ 敌方卡牌</div>' : ''}
                </div>
                ${skillInfo ? `
                <div class="inspect-skill" style="background:rgba(255,255,255,0.08); border-radius:8px; padding:8px; margin-bottom:8px;">
                    <div style="font-weight:bold; color:#ffd966;">✨ ${skillInfo.name}</div>
                    <div style="font-size:13px; color:#ddd;">${skillInfo.desc}</div>
                </div>` : '<div style="color:#aaa; margin-bottom:8px;">无技能</div>'}
        ...
        detailContainer.style.minWidth = '0';
        detailContainer.innerHTML = buildDetailHTML(card, showEquipment);

        popupEl.appendChild(cardContainer);
        popupEl.appendChild(detailContainer);
                ...
                if (unequipBtn) {
                    ...
                    if (window.YYCardShop && typeof window.YYCardShop.handleUnequip === 'function') {
                        ...
                        if (updatedCard) {
                            ...
                            detailContainer.innerHTML = buildDetailHTML(updatedCard, showEquipment);
                        } else {
                            popupEl.remove();
                        }
                    }
    ...
    function init() {
        ...
        console.log('✅ 卡牌查看系统已启动（横向布局，技能在右）');
    }
