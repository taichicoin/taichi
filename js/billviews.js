...
(function() {
  ...
  function mergeAndSort(deposits, withdraws) {
    ...
    return merged.slice(0, 50); // 最多返回 50 条
  }
  ...
  function buildBillModal(records) {
    ...
    if (!records || records.length === 0) {
      listHtml = '<div style="text-align:center;padding:20px;color:#888;">暂无记录</div>';
    } else {
      listHtml = records.map(r => {
        ...
        if (r.type === 'deposit') {
          ...
          return `
            <div style="background:#f8fafc; border-left:4px solid #16a34a; border-radius:8px; padding:12px; margin-bottom:8px; font-size:13px;">
              <div style="display:flex; justify-content:space-between; align-items:center;">
                <span style="font-weight:bold; color:#16a34a;">充值</span>
          ...
          const txLink = r.tx_hash ? ` <a href="https://testnet.bscscan.com/tx/${r.tx_hash}" target="_blank" style="color:#3b82f6;">查看</a>` : '';
          return `
            <div style="background:#f8fafc; border-left:4px solid #dc2626; border-radius:8px; padding:12px; margin-bottom:8px; font-size:13px;">
              <div style="display:flex; justify-content:space-between; align-items:center;">
                <span style="font-weight:bold; color:#dc2626;">提现</span>
                <span style="font-weight:bold;">-${amount} ${symbol}</span>
              </div>
              <div style="color:#666; font-size:12px;">${time}</div>
              <div style="color:#999; font-family:monospace; font-size:12px;">至 ${toAddr}${txLink}</div>
            </div>
          `;
        }
      }).join('');
