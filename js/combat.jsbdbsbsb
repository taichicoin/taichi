...
window.YYCardCombat = (function() {
    ...
    async function resolveBattles(gameState, log, onComplete) {
            ...
            if (!document.querySelector(`.board[data-player-id="${oppId}"]`)) {
                ...
                if (oppPlayer && oppPlayer.board) {
                    ...
                    if (myBoard) {
                        ...
                        clone.id = 'enemy-board-' + oppId.slice(0,8);
                        clone.setAttribute('data-player-id', oppId);
                        const parent = myBoard.parentNode;
                        if (parent && !document.querySelector(`.board[data-player-id="${oppId}"]`)) {
                            ...
                            debugLog(`🔧 强制渲染对手 ${oppId.slice(0,8)} 棋盘`);
                        }
                    }
                }
            }
        ...
        // ===== 清理临时创建的敌方棋盘 =====
        document.querySelectorAll('.board[id^="enemy-board-"]').forEach(el => el.remove());
        debugLog('🧹 清理临时渲染的敌方棋盘');
