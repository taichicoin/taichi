...
window.YYCardCombat = (function() {
    ...
    async function waitForBoardElements() {
        ...
        for (let i = 0; i < maxAttempts; i++) {
            ...
            if (mySlots.length === 6 && enemySlots.length === 6) {
                // 检查每个卡槽内是否有 .card 元素
                const myCards = Array.from(mySlots).some(s => s.querySelector('.card'));
                const enemyCards = Array.from(enemySlots).some(s => s.querySelector('.card'));
                if (myCards || enemyCards) {
                    return true;
                }
            }
    ...
    function fightWithLog(board1, board2, owner1, owner2) {
        ...
        while (units1.length > 0 && units2.length > 0 && turn < maxTurns) {
            ...
            if (attackerPos < 0 || attackerPos > 5 || defenderPos < 0 || defenderPos > 5) {
                currentSide = currentSide === 1 ? 2 : 1; turn++; continue;
            }

            combatLog.push({
                attacker: { ownerId: attacker.ownerId, pos: attackerPos, instanceId: attacker.instanceId, atk: attacker.atk },
                defender: { ownerId: target.ownerId, pos: defenderPos, instanceId: target.instanceId, hpAfter: target.hp },
    ...
    function findCardElement(ownerId, position) {
        ...
        if (!slot) return null;
        return slot.querySelector('.card') || slot.firstElementChild;
    }
    ...
    async function resolveBattlesWithAnimation(gameState, log, updateCallback) {
        ...
        const ready = await waitForBoardElements();
        if (!ready) {
            panelLog('⚠️ 等待棋盘元素超时，可能无法播放动画', true);
        } else {
            panelLog('✅ 棋盘元素已就绪');
        }
        ...
        for (const [p1Id, p2Id] of pairs) {
            ...
            try {
                ...
                const validLog = combatLog.filter(a => a.attacker.pos >= 0 && a.attacker.pos <= 5 && a.defender.pos >= 0 && a.defender.pos <= 5);
