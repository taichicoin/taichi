    ...
    async function playSteps(steps) {
        ...
        for (let i = 0; i < steps.length; i++) {
            ...
            if (step.type === 'buff') {
                ...
                if (isMyEvent) {
                    // ★ 检测 AOE：使用后端标记或连续攻击
                    if (step.isAoe || (i + 1 < steps.length && steps[i+1].type === 'attack' && steps[i+1].attackerOwnerId === step.attackerOwnerId && steps[i+1].attackerPos === step.attackerPos)) {
                        const aoeGroup = [step];
                        let j = i + 1;
