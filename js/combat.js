...
window.YYCardCombat = (function() {
    ...
    function getCardElement(playerId, dataPos, isEnemy) {
        ...
        // 3. 敌方额外尝试视觉翻转位置（因为敌方棋盘渲染时交换了前后排）
        if (isEnemy) {
            const visualPos = ENEMY_DATA_TO_VISUAL[dataPos];
    ...
    function renderEnemyBoardFromData(oppId, oppBoardData) {
        ...
        for (let i = 0; i < 6; i++) {
            ...
            // ★ 重要：data-board-index 设为数据层位置（0~5），前端查找时直接用
