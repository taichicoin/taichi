...
window.YYCardCombat = (function() {
    ...
    function spawnWeaponOnCard(cardEl) {
        ...
        // 朝向可以先随机或朝向目标，这里暂时用 lookAt 但无目标，先给个初始旋转
        weapon.rotateZ(Math.PI / 2);   // 让剑身宽面朝向屏幕
        _scene.add(weapon);
        return weapon;
    }

    // 执行武器飞行并移除（返回 Promise）
    function flyWeaponToTarget(weapon, startPos, defEl) {
        ...
        return new Promise(resolve => {
            ...
            weapon.rotateZ(Math.PI / 2); // 维持宽面朝向
            ...
            function fly(now) {
                ...
                weapon.rotateZ(rotateSpeed * dt);   // 旋转
                if (t < 1) {
    ...
    function attackAnim(a) {
        return new Promise(async resolve => {
            ...
            if (_3DReady) {
                // 1. 提前创建武器并放在攻击者卡牌上
                const startPos = domToWorld(attEl); // 保存起点，因为卡牌放大不影响中心坐标
