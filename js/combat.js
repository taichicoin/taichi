...
window.YYCardCombat = (function() {
    ...
    async function init3D() {
        ...
        try {
            ...
            _swordGLB.scale.set(0.8, 0.8, 0.8);   // 新模型大小，可根据需要调整
    ...
    // 构建旋转矩阵：使剑尖指向 tipDir，宽面指向 faceDir（相机方向）
    function makeSwordRotation(tipDir, faceDir) {
        ...
        // 假设模型剑尖是 Y 轴，宽面是 Z 轴 → 映射：X=right, Y=tip, Z=face
        // 如果方向反了，改为 (right, face, tip) 试试
        return new THREE.Matrix4().makeBasis(right, tip, face);
    }

    // 在卡牌上生成武器（竖直平放，剑尖朝上/下，宽面朝屏幕）
    function spawnWeaponOnCard(cardEl, isEnemy) {
        ...
        const tipDir = new THREE.Vector3(0, isEnemy ? -1 : 1, 0);   // 剑尖方向
        const faceDir = new THREE.Vector3(0, 0, 1);                 // 宽面朝向相机
    ...
    function flyWeaponToTarget(weapon, startPos, defEl) {
        ...
        return new Promise(resolve => {
            ...
            const faceDir = new THREE.Vector3(0, 0, 1);   // 尽量保持宽面朝向相机
            const rotMat = makeSwordRotation(flyDir, faceDir);
            weapon.quaternion.setFromRotationMatrix(rotMat);

            const duration = 300;   // 飞行时长 0.3 秒
