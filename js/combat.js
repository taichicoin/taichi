// ==================== 战斗模拟模块（3D 武器攻击版 · 受击音效 · 遗言飞刀 · 刀尖朝向修复 · 曹操全体护盾优化 · 遗言金币处理 · 共工减半debuff动画 · 连续buff支持） ====================
window.YYCardCombat = (function() {
    ...
    async function playSteps(steps, myId, oppId) {
        ...
        for (let i = 0; i < steps.length; i++) {
            ...
            // ★★★ 连续 buff 处理 ★★★
            if (step.type === 'buff' && step.continuous) {
                const batchId = step.batchId;
                const batch = [];
                let j = i;
                // 收集相同 batchId 的连续 buff
                ...
                for (const bstep of batch) {
                    ...
                    await new Promise(r => setTimeout(r, 150));  // 0.15 秒间隔
                }
                i = j - 1;  // 跳过已处理的步
                continue;
            }
