...
window.YYCardCombat = (function() {
    ...
    panelLog('✅ combat.js (攻击动画修复+技能联动)');
    ...
    function applyBattleStartSkills(gameState) {
        ...
        for (const playerId in gameState.players) {
            ...
            if (!board) continue;
            panelLog(`🔍 玩家 ${playerId.slice(0,6)} 技能触发`);
    ...
    function processEnlightenment(card, skill, playerId, pos, board, buffEvents, gameState) {
        ...
        if (isJingwei) {
            ...
            gainAtk = base + card.enlightenLevel * bonus;
            gainHp = gainAtk;
            panelLog(`  ✨ 精卫 Lv.${card.enlightenLevel} +${gainAtk}/+${gainAtk}`);
        } else {
            gainAtk = skill.effect.baseValue || 1;
            gainHp = gainAtk;
            panelLog(`  🌊 ${card.name} 悟道 +${gainAtk}/+${gainHp}`);
        }
        ...
        targets.forEach(({ card: targetCard, pos: targetPos }) => {
            ...
            buffEvents.push({ type: 'buff', playerId, position: targetPos, atkGain: gainAtk, hpGain: gainHp, sourceCard: card.name });
            panelLog(`    ↳ ${targetCard.name} +${gainAtk}/+${gainHp}`);
        });
    ...
    function triggerReactionSkills(gameState, playerId, sourceCard, sourcePos, board, buffEvents) {
        panelLog(`  ⚡ 反应技能 (因 ${sourceCard.name} 悟道)`);
        for (let i = 0; i < 6; i++) {
            const card = board[i];
            if (!card) continue;
            const skill = parseSkill(card);
            ...
            if (skill.skillId === 'skill_jinglei') {
                ...
                card.atk += gainAtk;
                card.hp += gainHp;
                buffEvents.push({ type: 'buff', playerId, position: i, atkGain: gainAtk, hpGain: gainHp, sourceCard: card.name });
                panelLog(`    ↳ 白泽 ${card.name} +${gainAtk}/+${gainHp}`);
                ...
                if (sourceCard.instanceId !== card.instanceId) {
                    sourceCard.atk += gainAtk;
                    sourceCard.hp += gainHp;
                    buffEvents.push({ type: 'buff', playerId, position: sourcePos, atkGain: gainAtk, hpGain: gainHp, sourceCard: card.name });
                }
                panelLog(`    ↳ 穷奇 ${card.name} 惩善扬恶 +${gainAtk}/+${gainHp} 给自身和${sourceCard.name}`);
            }
        }
    }
    ...
    function pairPlayers(players) {
        ...
        for (let i = 0; i < humanIds.length; i += 2) {
            if (i + 1 < humanIds.length) pairs.push([humanIds[i], humanIds[i+1]]);
            else pairs.push([humanIds[i], botIds.shift() || null]);
        }
    ...
    function fightWithLog(board1, board2, owner1, owner2) {
        const units1 = [], units2 = [];
        for (let i = 0; i < 6; i++) {
            if (board1[i] && board1[i].hp > 0) { const c = cloneCard(board1[i], i); if (c) { c.ownerId = owner1; c.side = 1; units1.push(c); } }
            if (board2[i] && board2[i].hp > 0) { const c = cloneCard(board2[i], i); if (c) { c.ownerId = owner2; c.side = 2; units2.push(c); } }
        }
        const combatLog = [];
        ...
        let curSide = side1First ? 1 : 2, turn = 0, idx1 = 0, idx2 = 0;
        ...
        while (units1.length > 0 && units2.length > 0 && turn < 200) {
            ...
            if (curSide === 1) {
                ...
                for (let i = 0; i < units1.length; i++) {
                    ...
                    if (units1[checkIdx].hp > 0) { attacker = units1[checkIdx]; idx1 = (checkIdx + 1) % units1.length; break; }
                }
            } else {
                let startIdx = idx2 % units2.length;
                for (let i = 0; i < units2.length; i++) {
                    const checkIdx = (startIdx + i) % units2.length;
                    if (units2[checkIdx].hp > 0) { attacker = units2[checkIdx]; idx2 = (checkIdx + 1) % units2.length; break; }
                }
            }
            if (!attacker) break;
            ...
            if (target.hp <= 0) {
                ...
                if (deadIdx !== -1) {
                    ...
                    if (curSide === 1) { if (deadIdx <= idx2) idx2 = Math.max(0, idx2 - 1); }
                    else { if (deadIdx <= idx1) idx1 = Math.max(0, idx1 - 1); }
                }
            }
            curSide = curSide === 1 ? 2 : 1;
            turn++;
            if (turn % 10 === 0) panelLog(`🔄 回合${turn}  side1剩余:${units1.length}  side2剩余:${units2.length}`);
        }
    ...
    function playBuffAnim(buff) {
        return new Promise(resolve => {
            ...
            if (!cardEl) { panelLog(`⏭️ 跳过增益动画 pos=${buff.position}`, true); return resolve(); }
    ...
    function playAttackAnim(a) {
        return new Promise(resolve => {
            ...
            if (!$att || !$def) {
                panelLog(`⏭️ 跳过攻击动画 ${a.attackerPos}→${a.defenderPos}`, true);
                return resolve();
            }
