...
window.YYCardCombat = (function() {
    ...
    async function getCardElementRetry(playerId, dataPos, isEnemy, maxRetries = 5) {
        for (let i = 0; i < maxRetries; i++) {
            ...
            if (el) return el;
            if (i < maxRetries - 1) await new Promise(r => setTimeout(r, 150));
        }
        return null;
    }
    ...
    async function resolveBattles(gameState, onComplete) {
        ...
        // 如果是镜像，oppId 保持 myId，自己的棋盘就是对手
        if (!oppId && combatResults.length > 0) {
            // 可能镜像，尝试取任意对手
            const cr = combatResults[0];
            if (cr.p1 !== myId) oppId = cr.p1;
            else if (cr.p2 !== myId) oppId = cr.p2;
        }
