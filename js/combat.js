// ==================== 战斗模拟模块（最终稳定版：增益动画双索引映射 + 攻击动画保留） ====================
window.YYCardCombat = (function() {
    let isAnimating = false;
    const AVG_STEP_MS = 480;

    // 敌方数据索引 → 视觉索引的映射（与 shop.js 中 renderEnemyBoard 一致）
    ...
    function getCardElement(playerId, dataPos, isEnemy) {
        ...
        // 最后兜底，遍历所有槽位按 data-board-index 查找
        const slots = board.querySelectorAll('.card-slot');
        for (const s of slots) {
            const idx = s.getAttribute('data-board-index');
            if (idx == dataPos) return s.querySelector('.card:not(.empty-slot)');
        }
        return null;
    }
