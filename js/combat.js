...
window.YYCardCombat = (function() {
    ...
    async function resolveBattles(gameState, log, onComplete) {
        ...
        // ===== 确保对手棋盘已渲染（使用原有逻辑，不创建临时棋盘） =====
        let myOpponentId = null;
        for (const cr of combatResults) {
            if (cr.p1 === myId && cr.p2 !== myId) {
                myOpponentId = cr.p2;
                break;
            } else if (cr.p2 === myId && cr.p1 !== myId) {
                myOpponentId = cr.p1;
                break;
            } else if (cr.p1 === myId && cr.p2 === myId) {
                // 镜像战，无对手
                break;
            }
        }

        // 如果已经确定了对手，且对手棋盘还没渲染，则手动触发 renderEnemyBoard
        if (myOpponentId) {
            // 保证 gameState 里 battlePairs 指向正确的对手
            if (gameState.battlePairs) {
                // 更新 battlePairs 中与 myId 相关的对
                const newPairs = gameState.battlePairs.map(([p1, p2]) => {
                    if (p1 === myId) return [myId, myOpponentId];
                    if (p2 === myId) return [myOpponentId, myId];
                    return [p1, p2];
                });
                gameState.battlePairs = newPairs;
            } else {
                gameState.battlePairs = [[myId, myOpponentId]];
            }

            // 调用 shop.js 暴露出的渲染敌人棋盘函数
            if (window.YYCardShop?.renderEnemyBoard) {
                window.YYCardShop.renderEnemyBoard();
                debugLog(`🔧 已调用 renderEnemyBoard，对手ID: ${myOpponentId.slice(0,8)}`);
            } else {
                debugLog('⚠️ renderEnemyBoard 不可用，尝试强制渲染');
                // 如果 shop.js 没有暴露该函数，则直接操作（一般会有）
            }
        }
        // ===== 对手棋盘准备完毕 =====
