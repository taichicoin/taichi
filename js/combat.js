...
window.YYCardCombat = (function() {
    ...
    function instantKillAnim(step) {
        return new Promise(resolve => {
            ...
            const startX = ar.left + ar.width/2;
            const startY = ar.top + ar.height/2;
            ...
            function animate(now) {
                ...
                const progress = Math.min(elapsed / flyDuration, 1.0);

                const curX = startX + (endX - startX) * progress;
                const curY = startY + (endY - startY) * progress;

                orb.style.left = (curX - 8) + 'px';
                orb.style.top = (curY - 8) + 'px';

                const dx = curX - startX;
                const dy = curY - startY;
                const length = Math.sqrt(dx * dx + dy * dy);
                const angle = Math.atan2(dy, dx) * 180 / Math.PI;
                lightLine.style.width = length + 'px';
                lightLine.style.transform = `rotate(${angle}deg)`;
