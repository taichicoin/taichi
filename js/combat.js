// ==================== 战斗模拟模块（3D 武器攻击版 · 受击音效 · 遗言飞刀 · 刀尖朝向修复 · 曹操全体护盾优化 · 遗言金币处理 · 共工减半debuff动画） ====================
window.YYCardCombat = (function() {
    ...
    // ★★★ 新增：共工debuff动画 ★★★
    async function debuffAnim(step) {
        const myId = window.YYCardAuth?.currentUser?.id;
        const isEnemy = step.playerId !== myId; // debuff目标所属玩家
        ...
        const atkDiff = step.oldAtk - step.newAtk; // 减少的攻击力
        const hpDiff = step.oldHp - step.newHp;    // 减少的生命值

        // 更新卡牌显示的攻击和生命（直接设置为新值，保证同步）
        const atkEl = el.querySelector('.card-atk');
        ...
        if (atkDiff > 0 || hpDiff > 0) {
            ...
            floatingText(el, text, '#ffffff', 500);
        }
            ...
            // ★★★ debuff 分支 ★★★
                ...
                if (step.isRanged) {
                    ...
                    while (j < steps.length && steps[j].type === 'attack' && steps[j].isRanged) {
                        ...
                        if (curKey !== firstKey) break;   // 攻击者变了，立即断开分组
    ...
    // ★★★ 渲染敌方棋盘（使用 image.json 获取图片，边框根据稀有度/星级） ★★★
