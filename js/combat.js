...
window.YYCardCombat = (function() {
    ...
    async function playSteps(steps, myId, oppId) {
        ...
        for (let i = 0; i < steps.length; i++) {
            ...
            // 群体增益：合并连续同源的所有 mass_buff（不比较数值）
            else if (step.type === 'mass_buff') {
                const group = [step];
                let j = i + 1;
                while (j < steps.length && steps[j].type === 'mass_buff' && steps[j].sourceCard === step.sourceCard) {
                    group.push(steps[j]);
                    j++;
                }
                if (group.length > 1) {
                ...
                if (myId && step.playerId === myId) {
                    ...
                    if (step.generator === 'spirit_loot') {
                        ...
                        const newCard = {
                            ...
                            cardId: step.card_id, card_id: step.card_id,
                            name: step.cardName, type: 'character',
                            rarity: step.rarity || 'Common', faction: '中立',
                            atk: step.atk || 0, hp: step.hp || 0,
                            baseAtk: step.atk || 0, baseHp: step.hp || 0,
                            star: 0, shield: 0, chi: 0,
