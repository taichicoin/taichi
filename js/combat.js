...
window.YYCardCombat = (function() {
    ...
    async function init3D() {
        ...
        try {
            ...
            _swordGLB.scale.set(0.05, 0.06, 0.05);    // 可调整大小
            _swordGLB.rotation.set(0, 0, 0);        // 朝向待定
    ...
    function spawnWeaponOnCard(cardEl, isEnemy) {
        ...
        // 我方（屏幕下方）剑尖朝上（+Y），敌方（屏幕上方）剑尖朝下（-Y）
        const direction = isEnemy ? -1 : 1;
        const upTarget = pos.clone().add(new THREE.Vector3(0, direction, 0));
        weapon.lookAt(upTarget);
        weapon.rotateZ(Math.PI / 2);   // 宽面朝向相机
    ...
    function flyWeaponToTarget(weapon, startPos, defEl) {
        ...
        return new Promise(resolve => {
            ...
            weapon.rotateZ(Math.PI / 2); // 维持宽面朝向 + 后续自转基准

            const duration = 400;        // 飞行时长 0.4 秒
            const startTime = performance.now();
            const rotateSpeed = 0.01;    // 旋转速度（弧度/ms）
