...
window.YYCardCombat = (function() {
    ...
    function fight(board1, board2, logCallback) {
        const units1 = [], units2 = [];
        for (let i = 0; i < 6; i++) {
            if (board1[i] && board1[i].hp > 0) units1.push({ ...board1[i], position: i, instanceId: board1[i].instanceId || Math.random().toString() });
            if (board2[i] && board2[i].hp > 0) units2.push({ ...board2[i], position: i, instanceId: board2[i].instanceId || Math.random().toString() });
        }
    ...
    async function resolveBattles(gameState, log, updateCallback) {
        ...
        log(`🔄 本回合配对: ${JSON.stringify(gameState.battlePairs)}`);

        for (const [p1Id, p2Id] of gameState.battlePairs) {
            ...
            log(`💥 伤害=${level}+${bonus}=${damage}，${loserPlayer===p1?p1Id:p2Id}剩余${loserPlayer.health}`);
