...
window.YYCardCombat = (function() {
    ...
    // ★★★ 新攻击动画：翻转0.2s → 显示变大武器 0.6s (第0.3s插入0.3s) → 翻转回角色0.2s → 缩小0.1s → 伤害
    function attackAnim(a) {
        return new Promise(async resolve => {
            ...
            if (_3DReady) {
                ...
                // 2. 显示变大武器 0.6s，第 0.3s 开始插入长矛（插入耗时 0.3s）
                const insertDelay = 300;   // 0.3秒后开始插入
                const displayDuration = 600; // 武器显示总时长改为 0.6 秒
                const insertPromise = (async () => {
                    await new Promise(r => setTimeout(r, insertDelay));
                    await insertWeaponIntoCard(defEl, isEnemyAttacker); // 0.3 秒插入
                })();
                // 保证武器图显示至少 0.6 秒，插入也在此期间完成
    ...
    function multiHitAnim(stepsList) {
        return new Promise(async resolve => {
            ...
            if (_3DReady) {
                ...
                // 2. 显示武器 0.6s，第 0.3s 开始插入（插入 0.3s）
                const insertDelay = 300;          // 0.3 秒后开始
                await new Promise(r => setTimeout(r, insertDelay));
                await insertWeaponIntoCard(defEl, isEnemyAttacker); // 插入 0.3 秒
                // 由于插入耗时 0.3 秒，总时间已达到 0.6 秒，无需额外等待
