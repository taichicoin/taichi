...
window.YYCardCombat = (function() {
    ...
    function buffAnim(buff) {
        return new Promise(resolve => {
            ...
            console.log('[Combat] buffAnim 查找 DOM:', buff.playerId.slice(0,8), 'pos:', buff.position, el ? '✅找到' : '❌未找到');
    ...
    async function playSteps(steps) {
        ...
        for (const step of steps) {
            if (abortFlag) break;
            if (step.type === 'buff') {
                console.log('[Combat] 播放增益动画:', step);
                await buffAnim(step);
            } else {
    ...
    async function resolveBattles(gameState, log, onComplete) {
        ...
        try {
            ...
            console.log('[Combat] 正在调用 settle-battle...');
            ...
            console.log('[Combat] settle-battle 返回:', data);
            if (!data.success) {
        ...
        console.log(`[Combat] buffEvents: ${buffEvents.length}, combatResults: ${combatResults.length}, 总步数: ${allSteps.length}`);
        ...
        if (remaining.length > 0) {
            ...
            console.warn('[Combat] 动画步骤为空，检查后端 buffEvents 和 combatResults');
        }
        ...
        if (data.updatedPlayers) {
            ...
            if (myId && gameState.players[myId]) {
                console.log('[Combat] 增益后我方卡牌:', gameState.players[myId].board.map(c => c ? { name: c.name, atk: c.atk, hp: c.hp } : null));
            }
        }
