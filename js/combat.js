...
window.YYCardCombat = (function() {
    ...
    function fight(board1, board2, logCallback = null) {
        ...
        for (let i = 0; i < 6; i++) {
            if (board1[i] && board1[i].hp > 0) {
                units1.push({ 
                    ...board1[i], 
                    position: i, 
                    instanceId: board1[i].instanceId || Math.random().toString() 
                });
            }
            if (board2[i] && board2[i].hp > 0) {
                units2.push({ 
                    ...board2[i], 
                    position: i, 
                    instanceId: board2[i].instanceId || Math.random().toString() 
                });
            }
        }
        ...
        // 随机先手：true 表示 board1 先行动，false 表示 board2 先行动
        const board1First = Math.random() >= 0.5;
        if (logCallback) {
            logCallback(`🎲 先手判定: ${board1First ? '我方' : '敌方'}先行动`);
        }
        
        // 合并所有单位并按位置排序（0→5），但行动顺序由先手决定
        const allUnits = [...units1, ...units2].sort((a, b) => a.position - b.position);
        
        let turn = 0;
        const maxTurns = 200; // 防止无限循环
        ...
        while (units1.length > 0 && units2.length > 0 && turn < maxTurns) {
            ...
            target.hp -= damage;
            if (logCallback) {
                logCallback(`  ⚡ ${attacker.name}(${attacker.position+1}) 攻击 ${target.name}(${target.position+1})，造成 ${damage} 伤害，剩余 ${target.hp}`);
            }
...
console.log('✅ combat.js 加载完成（完整战斗版）');
