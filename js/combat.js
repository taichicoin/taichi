...
window.YYCardCombat = (function() {
    ...
    async function init3D() {
        ...
        try {
            ...
            _swordGLB.scale.set(0.04, 0.04, 0.04);    // 可调整大小
    ...
    function spawnWeaponOnCard(cardEl, isEnemy) {
        ...
        // 我方剑尖朝上（+Y），敌方剑尖朝下（-Y），宽面始终朝相机（+Z）
        const upDir = new THREE.Vector3(0, isEnemy ? -1 : 1, 0);
        const cameraDir = new THREE.Vector3(0, 0, 1);   // 相机方向
        const rightDir = new THREE.Vector3().crossVectors(cameraDir, upDir).normalize();

        // 构造旋转矩阵（X=right, Y=upDir, Z=cameraDir）
        const rotationMatrix = new THREE.Matrix4().makeBasis(rightDir, upDir, cameraDir);
    ...
    function flyWeaponToTarget(weapon, startPos, defEl) {
        ...
        return new Promise(resolve => {
            ...
            const rotationMatrix = new THREE.Matrix4().makeBasis(rightDir, flyDir, camDir);
            weapon.quaternion.setFromRotationMatrix(rotationMatrix);

            const duration = 400;        // 飞行时长 0.4 秒
            const startTime = performance.now();
            const rotateSpeed = 0.015;   // 旋转速度（稍微调大一点让钻头感更明显）
    ...
    function attackAnim(a) {
        return new Promise(async resolve => {
            ...
            if (_3DReady) {
                ...
                // 将 isEnemyAttacker 传给飞行函数（用于钻头旋转方向）
