// ==================== 战斗模拟模块（完整版：山海经技能 + 轮流攻击 + 死亡移除 + 慢速动画 + 蠃鱼修复） ====================
window.YYCardCombat = (function() {
    ...
    panelLog('✅ combat.js 完整版 (蠃鱼修复+详细日志)');
    ...
    function applyBattleStartSkills(gameState) {
        ...
        for (const playerId in gameState.players) {
            ...
            panelLog(`🔍 开始处理玩家 ${playerId.slice(0,6)} 的棋盘技能`);
            
            for (let pos = 0; pos < 6; pos++) {
                const card = board[pos];
                if (!card) {
                    panelLog(`  🕳️ pos ${pos}: 空位`);
                    continue;
                }
                
                panelLog(`  🃏 pos ${pos}: ${card.name} (ID: ${card.id})`);
                
                const skill = parseSkill(card);
                if (!skill) {
                    panelLog(`    ↳ 无技能或解析失败`, true);
                    continue;
                }
                
                panelLog(`    ↳ skillId: ${skill.skillId}, type: ${skill.type}, trigger: ${skill.trigger}`);
                
                if (skill.trigger !== 'onBattleStart') {
                    panelLog(`    ↳ 触发时机不符 (需要 onBattleStart)`);
                    continue;
                }
    ...
    function processEnlightenment(card, skill, playerId, pos, board, buffEvents, gameState) {
        ...
        if (isJingwei) {
            ...
            panelLog(`  🌊 ${card.name} (${skill.skillId}) 悟道触发，固定增益 +${gainAtk}/+${gainHp}`);
        }
        
        const targets = getTargetCards(board, skill.scope, pos);
        panelLog(`  🎯 技能作用范围: ${skill.scope}，共 ${targets.length} 个目标`);
    ...
    function triggerReactionSkills(gameState, playerId, sourceCard, sourcePos, board, buffEvents) {
        panelLog(`  ⚡ 检查反应技能 (因 ${sourceCard.name} 悟道触发)`);
        for (let i = 0; i < 6; i++) {
            const card = board[i];
            if (!card) continue;
            const skill = parseSkill(card);
    ...
    function playBuffAnim(buff) {
        return new Promise(resolve => {
            ...
            if (!cardEl) {
                panelLog(`⏭️ 跳过增益动画，找不到卡牌 pos=${buff.position}`, true);
                return resolve();
            }
            const gainText = `+${buff.atkGain || 0}/+${buff.hpGain || 0}`;
            showFloatingText(cardEl, gainText, '#7bffb1', 1000); // 显示时长改为 1 秒，可自行调整
            setTimeout(resolve, 300); // 动画占用时间
        });
    }
        ...
        try {
            while (animQueue.length && !abortFlag) {
                ...
                try {
                    ...
                    await new Promise(r => setTimeout(r, 80)); // 步间间隔改为 80ms（可自行调整）
                } catch (e) {
    ...
    async function resolveBattles(gameState, log, updateCallback) {
        ...
        if (buffEvents.length > 0) {
            ...
            for (const ev of buffEvents) {
                ...
                await new Promise(r => setTimeout(r, 100)); // 增益间隔改为 100ms（可自行调整）
            }
