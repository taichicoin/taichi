...
window.YYCardCombat = (function() {
    ...
    // 使武器朝向：剑尖朝上/下（世界Y），宽面朝向相机（世界Z）
    function setWeaponFlatOrientation(weapon, isEnemy) {
        const THREE = _threeModule;
        const tipY = isEnemy ? -1 : 1;   // 剑尖方向：敌方朝下，我方朝上
        const tipAxis = new THREE.Vector3(0, tipY, 0);
        const faceAxis = new THREE.Vector3(0, 0, 1);  // 宽面朝向屏幕外
