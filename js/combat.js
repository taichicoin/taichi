    ...
    function buffAnim(buff) {
        return new Promise(async resolve => {
            ...
            if (totalAtkGain > 0 || totalHpGain > 0) {
                floatingText(el, `+${totalAtkGain}/+${totalHpGain}`, '#7bffb1', 1200);
            }
            resolve(); // 立即完成，间隔由外部playSteps控制
        });
    }

    // 合并动画：所有目标同时飘字，总间隔0.6秒
        ...
        return new Promise(async resolve => {
            ...
            setTimeout(() => {
                ...
                if (a.blocked) {
                    ...
                    shieldDiv.style.cssText = 'position:absolute; color:#ffbb33; font-size:24px; font-weight:bold; text-shadow:0 0 8px #000; z-index:200; left:50%; bottom:40%; transform:translate(-50%,0); animation:damageFloat 1s forwards; pointer-events:none;';
                    ...
                    dmgDiv.style.cssText = 'position:absolute; color:#f44; font-size:32px; font-weight:bold; text-shadow:0 0 8px #000; z-index:200; left:50%; bottom:40%; transform:translate(-50%,0); animation:damageFloat 1s forwards; pointer-events:none;';
                    defEl.style.position = 'relative';
