...
window.YYCardCombat = (function() {
    ...
    async function playSteps(steps, myId, oppId) {
        ...
        for (let i = 0; i < steps.length; i++) {
            ...
            // 群体增益（按角色位置合并，不检查数值差异）
            else if (step.type === 'mass_buff') {
                // 后端已提供 triggerPos，用 sourceCard + triggerPos 区分不同角色
                const uniqueKey = step.sourceCard + '|' + (step.triggerPos ?? 'none');
                const group = [step];
                let j = i + 1;
                while (j < steps.length && steps[j].type === 'mass_buff' &&
                       (steps[j].sourceCard + '|' + (steps[j].triggerPos ?? 'none')) === uniqueKey) {
                    group.push(steps[j]);
                    j++;
                }
                if (group.length > 1) {
                    utils.debugLog(`  ▶ merged ${step.sourceCard} (pos ${step.triggerPos}) ×${group.length}`);
                    await anims.mergedMassBuffAnim(group);
                    i = j - 1;
                } else {
                    utils.debugLog(`  ▶ mass_buff #${idx}: ${step.sourceCard} ${step.desc}`);
