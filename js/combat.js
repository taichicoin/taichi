...
window.YYCardCombat = (function() {
    ...
    function createSpiritCard(slotEl, spiritAtk, spiritHp, spiritImage, spiritName, star = 0, tempShield = 0) {
        ...
        if (tempShield > 0) {
            const shieldDiv = document.createElement('div');
            shieldDiv.className = 'card-shield';  // ★ 样式由 CSS 控制
        ...
        return new Promise(async resolve => {
            ...
            if (shieldGain > 0) {
                ...
                if (!shieldEl) {
                    ...
                    shieldEl.className = 'card-shield';  // ★ 样式由 battle.css 控制
                ...
                floatingText(el, `🛡️ +${shieldGain}`, '#00bfff', 1200);
            }
    ...
    // ★★★ 攻击动画（武器图片改用 object-fit: contain 完整显示，攻防显示?） ★★★
    ...
    async function massBuffAnim(step) {
        ...
        for (const { el } of elements) {
            floatingText(el, ` +${atkGain}+${hpGain}`, '#7bffb1', 1200);
        }

        await new Promise(r => setTimeout(r, 300));
    }

    // ★★★ 清理后的 massShieldAnim：样式交给 CSS ★★★
    ...
    function renderEnemyBoardFromData(oppId, oppBoardData) {
        ...
        if (window.YYCardRender && typeof window.YYCardRender.renderBoard === 'function') {
            ...
            // ★ 敌方视觉顺序重排：视觉0→数据3，视觉1→数据4，视觉2→数据5，
            //    视觉3→数据0，视觉4→数据1，视觉5→数据2
