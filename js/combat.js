...
window.YYCardCombat = (function() {
    ...
    function spawnWeaponOnCard(cardEl, isEnemy) {
        ...
        // 决定剑尖方向：我方朝上（+Y），敌方朝下（-Y），形成对称
        const dirY = isEnemy ? -1 : 1;
        const upTarget = pos.clone().add(new THREE.Vector3(0, dirY, 0));
        weapon.lookAt(upTarget);

        // 让剑身宽面朝向相机（+Z方向），调整旋转
        // 经测试，使用 rotateX(-Math.PI/2) 能让剑平躺且宽面朝相机
        weapon.rotateX(-Math.PI / 2);

        // 如果方向还是像一条线，可尝试以下替代（二选一）：
        // weapon.rotateZ(Math.PI / 2);
        // 或者调整正负号，如 rotateX(Math.PI/2)
    ...
    function flyWeaponToTarget(weapon, startPos, defEl) {
        ...
        return new Promise(resolve => {
            ...
            weapon.rotateZ(Math.PI / 2); // 维持宽面朝向，同时用于后续自转

            const duration = 400;        // 飞行时长 0.40 秒
            ...
            function fly(now) {
                ...
                weapon.rotateZ(rotateSpeed * dt);   // 绕飞行方向旋转
                if (t < 1) {
    ...
    function attackAnim(a) {
        return new Promise(async resolve => {
            ...
            const myId = window.YYCardAuth?.currentUser?.id;
            const isEnemy = a.attackerOwnerId !== myId;
            const attEl = await getCardElementRetry(a.attackerOwnerId, a.attackerPos, isEnemy);
            ...
            if (_3DReady) {
                // 1. 生成平放的武器，传入isEnemy以控制剑尖方向
                const startPos = domToWorld(attEl);
                const weapon = spawnWeaponOnCard(attEl, isEnemy);
