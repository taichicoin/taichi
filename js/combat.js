...
window.YYCardCombat = (function() {
    ...
    function getCardElement(playerId, dataPos, isEnemy) {
        ...
        // 2. 全局查找：遍历所有棋盘，找 data-board-index 匹配的卡牌（不限制玩家）
        const allBoards = document.querySelectorAll('.board');
        for (const b of allBoards) {
            let slot = b.querySelector(`.card-slot[data-board-index="${dataPos}"]`);
            if (slot) {
            ...
            if (isEnemy) {
                const v = ENEMY_DATA_TO_VISUAL[dataPos];
                if (v !== undefined) {
                    slot = b.querySelector(`.card-slot[data-board-index="${v}"]`);
                    if (slot) {
                        const card = slot.querySelector('.card:not(.empty-slot)');
                        if (card) return card;
                    }
        ...
        // 3. 最后尝试 data-slot-index 匹配
        for (const b of allBoards) {
            const slot = b.querySelector(`.card-slot[data-slot-index="${dataPos}"]`);
            if (slot) {
                const card = slot.querySelector('.card:not(.empty-slot)');
                if (card) return card;
            }
    ...
    async function playSteps(steps, myId, oppId) {
        ...
        for (let i = 0; i < steps.length; i++) {
            ...
            const isRelevant =
                ...
                step.attackerOwnerId === oppId ||
                step.defenderOwnerId === myId ||
                step.defenderOwnerId === oppId;
    ...
    function renderBoard(boardElement, playerId, boardData) {
        if (!boardElement) return;
        boardElement.setAttribute('data-player-id', playerId);
        ...
        for (let i = 0; i < 6; i++) {
            ...
            slot.setAttribute('data-board-index', i);
    ...
    function renderEnemyBoardFromData(oppId, oppBoardData) {
        ...
        renderBoard(enemyBoard, oppId, oppBoardData);  // 先普通渲染
        // 然后再翻转视觉位置
        const slots = enemyBoard.querySelectorAll('.card-slot');
        if (slots.length === 6) {
            const originalCards = [];
            slots.forEach(s => originalCards.push(s.innerHTML));
            // 重新排列：索引 0-2 与 3-5 交换
            for (let i = 0; i < 3; i++) {
                const temp = originalCards[i];
                originalCards[i] = originalCards[i + 3];
                originalCards[i + 3] = temp;
            }
            slots.forEach((s, i) => {
                s.innerHTML = originalCards[i];
                // 重新设置 data-board-index 以匹配数据索引
                const dataIndex = i < 3 ? i + 3 : i - 3;
                s.setAttribute('data-board-index', dataIndex);
            });
        }
    }
    ...
    async function resolveBattles(gameState, onComplete) {
        ...
        if (!myCombat && combatResults.length > 0) {
            myCombat = combatResults[0];
            oppId = myCombat.p2 === myId ? myCombat.p1 : myCombat.p2;
        }

        // 重新渲染己方棋盘
        const myBoardEl = document.getElementById('my-board') || document.querySelector('.board[data-player-id="' + myId + '"]');
        if (myBoardEl && gameState.players[myId]?.board) {
            renderBoard(myBoardEl, myId, gameState.players[myId].board);
            debugLog('🔧 己方棋盘已更新');
        } else {
            debugLog('⚠️ 无法更新己方棋盘，请检查HTML中己方棋盘id或data-player-id');
        }
        ...
        if (oppId && gameState.players[oppId]?.board) {
            ...
            if (!ready) debugLog('⚠️ 敌方棋盘未在 3 秒内就绪，部分动画可能缺失');
        } else {
            debugLog('⚠️ 未找到对手，可能为镜像模式');
        }
