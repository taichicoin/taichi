...
window.YYCardCombat = (function() {
    ...
    async function playSteps(steps, myId, oppId) {
        ...
        for (let i = 0; i < steps.length; i++) {
            ...
            // 群体增益（支持跨步骤收集同源 mass_buff 合并）
            else if (step.type === 'mass_buff') {
                // 收集所有后续同 sourceCard、同数值的 mass_buff（无论是否连续）
                const group = [step];
                const toRemove = [];
                for (let k = i + 1; k < steps.length; k++) {
                    const s = steps[k];
                    if (s.type === 'mass_buff' &&
                        s.sourceCard === step.sourceCard &&
                        s.atkGain === step.atkGain &&
                        s.hpGain === step.hpGain) {
                        group.push(s);
                        toRemove.push(k);
                    }
                }
                ...
                for (let r = toRemove.length - 1; r >= 0; r--) {
                    steps.splice(toRemove[r], 1);
                }
                if (group.length > 1) {
                    utils.debugLog(`  ▶ merged mass_buff #${idx}: ${step.sourceCard} ×${group.length}`);
