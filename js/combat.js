...
window.YYCardCombat = (function() {
    ...
    function attackAnim(a) {
        return new Promise(async resolve => {
            ...
            if (_3DReady) {
                ...
                applyDamageEffects(a, defEl, true); // 普通攻击播放音效
    ...
    function rangedAttackAnim(stepsArray) {
        return new Promise(async resolve => {
            ...
            const startPos3D = _3DReady ? domToWorld(slotEl) : null;
            ...
            // 并行处理所有目标
            const animPromises = stepsArray.map(async (step) => {
                if (abortFlag) return;
                ...
                let weapon = null;
                if (_3DReady && _daggerGLB && startPos3D) {
                    weapon = spawnWeaponOnCard(slotEl, isEnemyAttacker, _daggerGLB);
                    if (weapon) {
                        await flyWeaponToTarget(weapon, startPos3D, defEl);
                        if (weapon.parent) _scene.remove(weapon);
                    }
                }
