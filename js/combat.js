    ...
    function instantKillAnim(step) {
        return new Promise(resolve => {
            ...
            beam.style.cssText = `
                ...
                background: radial-gradient(circle, #fff, #f5d76e);
                border-radius: 50%;
                box-shadow: 0 0 20px #f5d76e, 0 0 40px #ffaa00;
                left: ${ar.left + ar.width/2 - 6}px;
                top: ${ar.top + ar.height/2 - 6}px;
            ...
            requestAnimationFrame(() => {
                beam.style.left = (dr.left + dr.width/2 - 6) + 'px';
                beam.style.top = (dr.top + dr.height/2 - 6) + 'px';
            });
            ...
            setTimeout(() => {
                ...
                // 目标卡牌坍缩动画
                defEl.style.transition = 'transform 0.8s ease-in, opacity 0.8s ease-in';
                ...
                if (currentPlayerId && step.playerId === currentPlayerId) {
                    ...
                    if (!hand) { debugLog(`  ⚠️ generate 失败：手牌数据不存在`); }
                        ...
                        if (validCount >= 15) { debugLog(`  ⚠️ 手牌已满，无法生成`); }
                            ...
                            } else { debugLog(`  ⚠️ 未找到${rarity}角色牌模板`); }
                        }
                    }
