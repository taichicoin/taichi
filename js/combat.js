...
window.YYCardCombat = (function() {
    ...
    function applyBattleStartSkills(gameState) {
        ...
        for (const playerId in gameState.players) {
            ...
            if (!board) continue;
            panelLog(`🔍 处理玩家 ${playerId.slice(0,6)} 棋盘技能`);
    ...
    function processEnlightenment(card, skill, playerId, pos, board, buffEvents, gameState) {
        ...
        if (skill.skillId === 'skill_xianshi_tianhai') {
            ...
            gainHp = gainAtk;
            panelLog(`  ✨ 精卫 Lv.${card.enlightenLevel} 增益 +${gainAtk}/+${gainHp}`);
        } else {
    ...
    function fightWithLog(board1, board2, owner1, owner2) {
        ...
        while (units1.length && units2.length && turn < 200) {
            ...
            if (curSide === 1) {
                ...
                for (let i = 0; i < units1.length; i++) {
                    const ci = (startIdx + i) % units1.length;
                ...
                for (let i = 0; i < units2.length; i++) {
                    const ci = (startIdx + i) % units2.length;
        ...
        panelLog(`🏆 战斗结束 side${units1.length ? 1 : 2} 回合${turn}`);
    ...
    function playBuffAnim(buff) {
        return new Promise(resolve => {
            ...
            if (!cardEl) { panelLog(`⏭️ 增益动画跳过 pos=${buff.position}`, true); return resolve(); }
    ...
    async function playLog(logs) {
        ...
        // 强制重置状态，防止因异常导致 isPlaying 卡死
        isPlaying = false;
        if (logs.length === 0) { panelLog('⚠️ 战斗日志为空，无动画'); return; }
        isPlaying = true;
        isAnimating = true;
        abortFlag = false;
        animQueue = [...logs];
    ...
    async function resolveBattles(gameState, log, updateCallback) {
        ...
        if (buffEvents.length > 0) {
            panelLog(`✨ 播放增益动画 ${buffEvents.length} 个`);
            // 独立播放增益，不通过 playLog，避免状态干扰
        ...
        for (const [p1, p2] of pairs) {
            ...
            // 重要：确保 combatLog 不为空再播放
            if (combatLog.length > 0) {
                await playLog(combatLog);
            } else {
                panelLog('⚠️ 没有攻击动作，跳过动画');
            }
        }
        panelLog('🏁 结算完成');
