...
window.YYCardCombat = (function() {
    ...
    function renderEnemyBoardFromData(oppId, oppBoardData) {
        ...
        // 敌方棋盘需要视觉翻转
        renderBoard(enemyBoard, oppId, oppBoardData); // 注意：renderBoard是直接按数据索引0~5，但敌方需要翻转
        // 但之前代码翻转了 displayBoard，我们重写直接翻转
    ...
    async function resolveBattles(gameState, onComplete) {
        ...
        // ★ 重新渲染己方棋盘，确保元素最新
        const myBoardEl = document.getElementById('my-board'); // 假设你的己方棋盘id为my-board
        if (myBoardEl && gameState.players[myId]?.board) {
            const myBoardData = gameState.players[myId].board;
            // 己方棋盘不需要翻转
            renderBoard(myBoardEl, myId, myBoardData);
            debugLog('🔧 己方棋盘已更新');
        } else {
            debugLog('⚠️ 无法更新己方棋盘，id="my-board"未找到或缺少数据');
        }
