// ==================== 战斗模拟模块（强制渲染敌方棋盘 + 防跳过 + 纯数字自适应 + 清理临时棋盘） ====================
window.YYCardCombat = (function() {
    ...
    // 在动画播放前临时创建敌方棋盘，播放完立刻删除
    function createTempEnemyBoards(gameState, combatResults, myId) {
        const allPlayerIds = new Set();
        combatResults.forEach(cr => {
            allPlayerIds.add(cr.p1);
            allPlayerIds.add(cr.p2);
        });
        allPlayerIds.delete(myId);

        const createdBoards = [];

        allPlayerIds.forEach(oppId => {
            if (!document.querySelector(`.board[data-player-id="${oppId}"]`)) {
                const oppPlayer = gameState.players[oppId];
                if (oppPlayer && oppPlayer.board) {
                    const myBoard = document.getElementById('my-board');
                    if (myBoard) {
                        const clone = myBoard.cloneNode(false);
                        clone.id = 'enemy-temp-' + oppId.slice(0,8);
                        clone.setAttribute('data-player-id', oppId);
                        clone.classList.add('temp-enemy-board');
                        const parent = myBoard.parentNode;
                        if (parent) {
                            parent.appendChild(clone);
                            for (let i = 0; i < 6; i++) {
                                const c = oppPlayer.board[i];
                            ...
                            createdBoards.push(clone);
                            debugLog(`🔧 临时渲染对手 ${oppId.slice(0,8)} 棋盘`);
                        }
                    }
                }
            }
    ...
    function removeTempEnemyBoards(boards) {
        boards.forEach(el => el.remove());
        if (boards.length > 0) {
            debugLog(`🧹 清理了 ${boards.length} 个临时棋盘`);
        }
    }
    ...
    async function resolveBattles(gameState, log, onComplete) {
        ...
        if (combatResults.length > 0) {
            tempBoards = createTempEnemyBoards(gameState, combatResults, myId);
        }
        ...
        // 动画播放完立刻删除临时棋盘
        removeTempEnemyBoards(tempBoards);
