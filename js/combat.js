...
window.YYCardCombat = (function() {
    ...
    function ensureLogPanel() {
        ...
        `;
        panel.innerHTML = '<div style="color:#0f0; text-align:center;">⚔️ 战斗日志 (可滑动) ⚔️</div>';
    ...
    async function resolveBattles(gameState, log, updateCallback) {
        ...
        for (const [p1Id, p2Id] of pairs) {
            ...
            logText += `A(${p1Id.slice(0,6)}): `;
            for (let i = 0; i < 6; i++) {
                const c = board1[i];
                logText += c ? `[${i+1}]${c.name}(${c.hp}/${c.atk}) ` : `[${i+1}]空 `;
            }
            logText += `\nB(${p2Id.slice(0,6)}): `;
            for (let i = 0; i < 6; i++) {
                const c = board2[i];
...
console.log('✅ combat.js 强制同步数据版 + UI强制刷新');
