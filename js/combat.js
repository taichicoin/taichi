// ==================== 战斗模拟模块（黑化弟AOE全体动画 + 生命转化攻击动画 + 零等待他人） ====================
window.YYCardCombat = (function() {
    ...
    async function aoeAttackAnim(attackerOwnerId, attackerPos, attackerName, targetList) {
        const myId = window.YYCardAuth?.currentUser?.id;
        const attEl = getCardElement(attackerOwnerId, attackerPos, attackerOwnerId !== myId);
        if (!attEl) return;

        // 攻击者先做一个冲击动作
        attEl.style.transition = 'transform 0.3s ease-out';
        attEl.style.transform = 'scale(1.15)';
        await new Promise(r => setTimeout(r, 300));
        attEl.style.transition = 'transform 0.3s ease-in';
        attEl.style.transform = 'scale(1.0)';

        // 同时对所有目标播放受击动画
        const promises = targetList.map(a => {
            return new Promise(resolve => {
                const defEl = getCardElement(a.defenderOwnerId, a.defenderPos, a.defenderOwnerId !== myId);
                if (!defEl) { resolve(); return; }
