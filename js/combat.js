// ==================== 战斗模拟模块（完整版 · 金币增加实时更新 + 动效 + 战后刷新UI + 优化合并播放） ====================
window.YYCardCombat = (function() {
    ...
    function buffAnim(buff) {
        return new Promise(async resolve => {
            ...
            if (shieldGain > 0) {
                ...
                floatingText(el, `🛡️ +${shieldGain}`, '#ffaa00', 500);
            }
    ...
    // ================== 步骤合并工具 ==================
    function mergeBuffSteps(steps) {
        if (!steps || steps.length === 0) return steps;
        const merged = [];
        let i = 0;
        while (i < steps.length) {
            const step = steps[i];
            // 只合并 type === 'buff' 且非金币事件
            if (step.type === 'buff' && step.goldGain === undefined) {
                const group = [step];
                let j = i + 1;
                while (j < steps.length &&
                       steps[j].type === 'buff' &&
                       steps[j].goldGain === undefined &&
                       steps[j].playerId === step.playerId &&
                       steps[j].sourceCard === step.sourceCard &&
                       steps[j].atkGain === step.atkGain &&
                       steps[j].hpGain === step.hpGain &&
                       steps[j].tempAtkGain === step.tempAtkGain &&
                       steps[j].tempHpGain === step.tempHpGain &&
                       steps[j].tempShieldGain === step.tempShieldGain &&
                       steps[j].desc === step.desc) {
                    group.push(steps[j]);
                    j++;
                }
                if (group.length > 1) {
                    ...
                    merged.push({
                        ...
                        desc: step.desc,
                        atkGain: step.atkGain,
                        hpGain: step.hpGain,
                        tempAtkGain: step.tempAtkGain,
                        tempHpGain: step.tempHpGain,
                        tempShieldGain: step.tempShieldGain,
                        positions: positions,
                        originalSteps: group
                    });
                    i = j;
    ...
    async function playSteps(steps) {
        ...
        // 第一步：合并同源连续Buff（精卫类全体增益）
        const mergedSteps = mergeBuffSteps(steps);
        debugLog(`  合并后步骤数: ${mergedSteps.length} (原始: ${steps.length})`);
        ...
        let idx = 0;
        const activeAnimations = [];  // 追踪已启动的并发动画
        ...
        for (let i = 0; i < mergedSteps.length; i++) {
            ...
            // ----- 全体Buff（合并后）-----
            if (step.type === 'multi-buff' && isMyEvent) {
                debugLog(`  ▶ multi-buff #${idx}: ${step.sourceCard} ${step.desc} → [${step.positions.join(',')}]`);
                // 并发更新所有目标卡牌，不阻塞
                const tasks = step.positions.map(async pos => {
                    const el = await getCardElementRetry(step.playerId, pos, step.playerId !== myId);
                    if (el) {
                        const totalAtkGain = (step.atkGain || 0) + (step.tempAtkGain || 0);
                ...
                activeAnimations.push(...tasks);
                await new Promise(r => setTimeout(r, 300));
                continue;
            }

            // ----- 普通Buff（单个）-----
            if (step.type === 'buff' && isMyEvent) {
                debugLog(`  ▶ buff #${idx}: ${step.sourceCard} ${step.desc} pos=${step.position}`);
                const task = buffAnim(step);
                activeAnimations.push(task);
                await new Promise(r => setTimeout(r, 300));
                continue;
            }

            // ----- 攻击事件（串行等待）-----
            if (step.type === 'attack' && isMyEvent) {
                debugLog(`  ▶ atk #${idx}: ${step.attackerName}→${step.defenderName} dmg=${step.damage}`);
                const aoeGroup = [step];
                let j = i + 1;
                while (j < mergedSteps.length && mergedSteps[j].type === 'attack' &&
                       mergedSteps[j].attackerOwnerId === step.attackerOwnerId &&
                       mergedSteps[j].attackerPos === step.attackerPos) {
                    aoeGroup.push(mergedSteps[j]);
                    j++;
                }
            ...
            // ----- 即死、跳过、生成等事件（保留原逻辑）-----
            if (step.type === 'instant_kill' && isMyEvent) {
        ...
        // 等待所有已启动的动画完成
        await Promise.allSettled(activeAnimations);
