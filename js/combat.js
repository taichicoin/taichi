...
window.YYCardCombat = (function() {
    ...
    async function resolveBattles(gameState, log, updateCallback) {
        ...
        if (!myId) return panelLog('❌ 无用户ID', true);
        const players = gameState.players;
        const pairs = pairPlayers(players);
        gameState.battlePairs = pairs;
        panelLog(`👥 ${pairs.length} 组 (真人优先配对)`);
        ...
        for (const [p1, p2] of pairs) {
            ...
            const p1IsSelf = p1 === myId;
            const map = {
                attacker: p1IsSelf ? {1:'my-board',2:'enemy-board'} : {1:'enemy-board',2:'my-board'},
                defender: p1IsSelf ? {1:'enemy-board',2:'my-board'} : {1:'my-board',2:'enemy-board'}
            };
            const mapped = combatLog.map(e => ({
                attackerBoard: map.attacker[e.attackerSide],
                defenderBoard: map.defender[e.defenderSide],
                attackerPos: e.attackerPos,
                defenderPos: e.defenderPos,
                damage: e.damage,
                defenderHpAfter: e.defenderHpAfter,
                isFatal: e.isFatal
            }));
            panelLog(`📊 ${p1.slice(0,6)} vs ${p2.slice(0,6)}: ${mapped.length}步`);
            log(`⚔️ 对战: ${p1.slice(0,6)} vs ${p2.slice(0,6)}`);
            await playLog(mapped);
        }
        panelLog('🏁 结束');
