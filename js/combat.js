// ==================== 战斗模拟模块（修复：双索引增益查找 + 敌方镜像自动适配） ====================
window.YYCardCombat = (function() {
    let isAnimating = false;
    const AVG_STEP_MS = 480;

    // 缓存“敌方数据索引 → 视觉索引”的映射关系
    const enemyVisualMap = [3,4,5,0,1,2]; // 数据索引0→视觉3, 1→视觉4... 数据索引3→视觉0, 4→视觉1...
    ...
    function getCardElement(playerId, dataPos, isEnemy) {
        ...
        // 如果是敌方，则按镜像视觉索引查找
        if (isEnemy) {
            const visualPos = enemyVisualMap[dataPos];  // 数据索引→视觉索引
            slot = board.querySelector(`.card-slot[data-board-index="${visualPos}"]`);
            if (slot) return slot.querySelector('.card:not(.empty-slot)');

            // 如果还找不到，尝试按 data-slot-index（视觉位置）
            slot = board.querySelector(`.card-slot[data-slot-index="${visualPos}"]`);
            if (slot) return slot.querySelector('.card:not(.empty-slot)');
        }

        // 我方也尝试 data-slot-index
    ...
    function buffAnim(buff) {
        return new Promise(resolve => {
            // 判断目标是否属于敌方棋盘（通过比较 playerId 与当前登录用户）
            ...
            console.log(`[Combat] buffAnim 查找 DOM: playerId=${buff.playerId.slice(0,8)} pos=${buff.position} isEnemy=${isEnemy} el=${el ? '✅找到' : '❌未找到'}`);
    ...
    function attackAnim(a) {
        return new Promise(resolve => {
            ...
            // 攻击动画按原有方式查找，也可以加上镜像适配（但攻击动画目前正常，暂不改动）
