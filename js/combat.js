...
window.YYCardCombat = (function() {
    ...
    async function init3D() {
        ...
        try {
            ...
            // 加载武器模型（相对路径，请确保 autumn_sword.glb 在网站根目录的 3d/ 文件夹下）
    ...
    function spawnWeaponOnCard(cardEl, isEnemy) {
        ...
        // 构建旋转矩阵：局部X=right, 局部Y=up, 局部Z=camera
        const rotMat = new THREE.Matrix4().makeBasis(rightDir, upDir, cameraDir);
    ...
    function flyWeaponToTarget(weapon, startPos, defEl) {
        ...
        return new Promise(resolve => {
            ...
            const cameraDir = new THREE.Vector3(0, 0, 1);
            const rightDir = new THREE.Vector3().crossVectors(cameraDir, flyDir).normalize();
            const rotMat = new THREE.Matrix4().makeBasis(rightDir, flyDir, cameraDir);
            weapon.quaternion.setFromRotationMatrix(rotMat);

            const duration = 600;   // 飞行时长 0.6 秒
