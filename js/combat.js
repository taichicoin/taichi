...
window.YYCardCombat = (function() {
    ...
    // ★ 修改：attackAnim 中调用 applyDamageEffects 时传入攻击音效
    ...
    // ★ 修改：multiHitAnim 中调用 applyDamageEffects 时传入攻击音效
    ...
    // ★★★ 修改：飞刀动画支持投石车（根据 step.isCatapult 使用 🪨） ★★★
        ...
        return new Promise(async resolve => {
            ...
            if (fastMode) {
                for (let i = 0; i < totalSteps; i++) {
                    ...
                    applyDamageEffects(step, defEl, false); // 飞刀动画中不在此处播放音效

                    // 音效：第一刀和每5刀播放一次，使用 zs.mp3
                    if (i === 0 || i % 5 === 0) {
                        try {
    ...
    async function playSteps(steps, myId, oppId) {
        ...
        for (let i = 0; i < steps.length; i++) {
            ...
            // ========= 生成事件（女娲造人 / 亡魂碎片 / 毕方消耗牌） =========
            else if (step.type === 'generate') {
                debugLog(`  ▶ generate #${idx}: ${step.sourceCard} ${step.desc}`);

                // ★★★ 毕方悟道：生成消耗牌 ★★★
                if (step.generator === 'bifang') {
                    if (myId && step.playerId === myId) {
                        // 只显示提示，不随机角色，手牌已由后端同步
                        floatingText(document.getElementById('hand-container'), `✨ 消耗牌`, '#ffd700', 1000);
                    }
                    await new Promise(r => setTimeout(r, 400));
                    continue; // 跳过后续随机角色逻辑
                }
                ...
                if (myId && step.playerId === myId) {
                    ...
                    if (step.generator === 'spirit_loot') {
                        ...
                        floatingText(document.getElementById('hand-container'), `✨ ${newCard.name} (${newCard.atk}/${newCard.hp})`, '#ffd700', 1000);
                        await new Promise(r => setTimeout(r, 100));
                    } 
                    // ★ 女娲造人：使用后端传递的真实卡牌信息
                        ...
                        if (window.YYCardShop?.renderHand) window.YYCardShop.renderHand();
                        floatingText(document.getElementById('hand-container'), `✨ ${newCard.name}`, '#ffd700', 1500);
                        await new Promise(r => setTimeout(r, 400));
                    }
                    // 兼容旧日志（无真实数据时回退随机）
                    else {
