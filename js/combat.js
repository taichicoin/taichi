...
window.YYCardCombat = (function() {
    ...
    function spawnWeaponOnCard(cardEl, isEnemy) {
        ...
        // 先让模型看向屏幕里面（-Z方向），这样宽面会朝向屏幕
        const screenTarget = pos.clone().add(new THREE.Vector3(0, 0, -1));
        weapon.lookAt(screenTarget);
        // 再绕Z轴旋转，使剑身平放
        weapon.rotateZ(Math.PI / 2);

        // 如果需要剑尖朝上（我方）或朝下（敌方），再绕X轴旋转
        const xRot = isEnemy ? -Math.PI / 2 : Math.PI / 2;
        weapon.rotateX(xRot);
