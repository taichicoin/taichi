...
window.YYCardCombat = (function() {
    ...
    // 可调参数
    const SWORD_TIP_AXIS = 'y';               // 模型剑尖的局部轴：'y' / '-y' / 'z' / '-z'
    const CORRECTION_ANGLES = {               // 平面内旋转修正（弧度），绕世界Z轴
        enemy: -Math.PI / 4,                  // 敌方逆时针45°
        ally:  Math.PI * 3 / 4                // 我方顺时针135°
    };
    ...
    async function init3D() {
        ...
        try {
            ...
            _swordGLB.scale.set(0.04, 0.04, 0.04);   // 大小
    ...
    // 根据剑尖轴获取基向量（tipDirection 剑尖方向，faceDirection 宽面方向）
    function getSwordAxes(tipDirection, faceDirection) {
        const THREE = _threeModule;
        const tip = tipDirection.clone().normalize();
        const face = faceDirection.clone().normalize();

        let tipAxis, faceAxis;
        if (SWORD_TIP_AXIS === 'y') {
            tipAxis = tip;
            faceAxis = face;
        } else if (SWORD_TIP_AXIS === '-y') {
            tipAxis = tip.clone().multiplyScalar(-1);
            faceAxis = face;
        } else if (SWORD_TIP_AXIS === 'z') {
            tipAxis = face;
            faceAxis = tip;
        } else if (SWORD_TIP_AXIS === '-z') {
            tipAxis = face.clone().multiplyScalar(-1);
            faceAxis = tip;
        } else {
            tipAxis = tip;
            faceAxis = face;
        ...
        const right = new THREE.Vector3().crossVectors(faceAxis, tipAxis).normalize();
        return { tip: tipAxis, face: faceAxis, right: right };
    }
    ...
    function spawnWeaponOnCard(cardEl, isEnemy) {
        ...
        // 基础朝向：剑尖朝上/下，宽面朝相机
        const upDir = new THREE.Vector3(0, isEnemy ? -1 : 1, 0);
        const camDir = new THREE.Vector3(0, 0, 1);
        const axes = getSwordAxes(upDir, camDir);

        const rotMat = new THREE.Matrix4().makeBasis(axes.right, axes.tip, axes.face);
        weapon.quaternion.setFromRotationMatrix(rotMat);

        // 额外修正：绕世界Z轴旋转，使剑尖方向正确
        const correctionAngle = isEnemy ? CORRECTION_ANGLES.enemy : CORRECTION_ANGLES.ally;
        weapon.rotateOnWorldAxis(new THREE.Vector3(0, 0, 1), correctionAngle);

        _scene.add(weapon);
        return weapon;
    }

    // 武器飞行（剑尖指向目标，宽面尽量朝相机，带旋转）
    function flyWeaponToTarget(weapon, startPos, defEl, isEnemyAttacker) {
        const THREE = _threeModule;
        ...
        return new Promise(resolve => {
            ...
            const camDir = new THREE.Vector3(0, 0, 1);
            const axes = getSwordAxes(flyDir, camDir);
            const rotMat = new THREE.Matrix4().makeBasis(axes.right, axes.tip, axes.face);
            weapon.quaternion.setFromRotationMatrix(rotMat);

            const duration = 400;                // 飞行时长 0.4 秒
            const startTime = performance.now();
            const rotateSpeed = 0.015;           // 旋转速度
            ...
            function fly(now) {
                ...
                // 绕自身剑尖方向旋转（钻头效果）
                weapon.rotateOnWorldAxis(new THREE.Vector3(0, isEnemyAttacker ? -1 : 1, 0), rotateSpeed * dt);
                if (t < 1) {
    ...
    function attackAnim(a) {
        return new Promise(async resolve => {
            ...
            if (_3DReady) {
                ...
                await flyWeaponToTarget(weapon, startPos, defEl, isEnemyAttacker);
                if (abortFlag) return resolve();
