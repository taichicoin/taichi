// ==================== 战斗模拟模块（最终版：后端结算 + 同步动画 + 快进重连） ====================
window.YYCardCombat = (function() {
    const config = window.YYCardConfig;
    let isAnimating = false;

    // 平均每步动画耗时（毫秒），用于重连快进计算
    const AVG_STEP_MS = 500; // 可根据实际动画总时长调整
    ...
    async function resolveBattles(gameState, log, updateCallback) {
        ...
        if (!roomId) {
            console.error('[Combat] 无房间ID，无法结算');
            return;
        }

        // 调用后端 settle-battle 接口
        let data;
        try {
