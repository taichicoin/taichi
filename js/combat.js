...
window.YYCardCombat = (function() {
    ...
    // ★ 如果剑身不是竖直而是斜的，修改下面这个角度（弧度制，绕世界Z轴旋转）
    // 正值顺时针，负值逆时针。例如 -Math.PI/4 逆时针45度， Math.PI/4 顺时针45度。
    const SWORD_ALIGN_ANGLE = -Math.PI / 4;   // 先试试 -Math.PI/4，如果不对就改成 Math.PI/4 或 0
    ...
    // 设置武器平放且竖直
    function setWeaponFlatOrientation(weapon, isEnemy) {
        const THREE = _threeModule;
        ...
        // 补偿模型自身的旋转，使剑身竖直而不是对角线
        weapon.rotateOnWorldAxis(new THREE.Vector3(0, 0, 1), SWORD_ALIGN_ANGLE);
    }
    ...
    function spawnWeaponOnCard(cardEl, isEnemy) {
        ...
        setWeaponFlatOrientation(weapon, isEnemy);
        _scene.add(weapon);
        return weapon;
    }
