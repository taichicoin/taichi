...
window.YYCardCombat = (function() {
    ...
    function pairPlayers(players) {
        ...
        for (let i = 0; i < allIds.length; i += 2) {
            pairs.push([allIds[i], allIds[i+1] || null]);
        }
        return pairs;
    }
    ...
    function findTarget(attackerPos, enemyUnits) {
        const tablePos = attackerPos + 1; // 0~5 → 1~6
        ...
        for (const targetPos of priority) {
            const targetIndex = targetPos - 1; // 1~6 → 0~5
    ...
    function fight(board1, board2, logCallback) {
        ...
        // 强制输出棋盘内容（便于诊断）
        console.log('🔍 战斗棋盘1:', board1.map(c => c ? `${c.name}(${c.hp})` : '空'));
        console.log('🔍 战斗棋盘2:', board2.map(c => c ? `${c.name}(${c.hp})` : '空'));
