...
window.YYCardCombat = (function() {
    ...
    function buffAnim(buff) {
        return new Promise(resolve => {
            ...
            let atkGain = buff.atkGain || 0;
            let hpGain = buff.hpGain || 0;
            let shieldGain = buff.shieldGain || 0;
            let tempHpGain = buff.tempHpGain || 0;
            let tempAtkGain = buff.tempAtkGain || 0;
            let tempShieldGain = buff.tempShieldGain || 0;
            ...
            if (tempHpGain > 0) {
                ...
                color = '#5cd6ff';
                icon = '💧';
            } else if (tempAtkGain > 0) {
                text = `+${tempAtkGain}`;
                color = '#ff944d';
                icon = '⚡';
            } else if (tempShieldGain > 0) {
                text = `🛡+${tempShieldGain}`;
                color = '#ffbb33';
                icon = '🟠';
            } else if (shieldGain > 0) {
                text = `🛡+${shieldGain}`;
                color = '#ffbb33';
                icon = '🔵';
            } else if (atkGain > 0 || hpGain > 0) {
                text = `+${atkGain}/+${hpGain}`;
                color = '#7bffb1';
                icon = '⬆️';
            }
            ...
            // 更新卡牌数值
            updateCardStats(el, atkGain + tempAtkGain, hpGain + tempHpGain, shieldGain + tempShieldGain);

            if (text) {
                floatingText(el, `${icon} ${text}`, color, 1200);
            }
