...
window.YYCardCombat = (function() {
    ...
    // 自动注入浮动动画关键帧（如果不存在）
    function ensureDamageFloatKeyframes() {
        if (document.getElementById('damage-float-keyframes')) return;
        const style = document.createElement('style');
        style.id = 'damage-float-keyframes';
        style.textContent = `
            @keyframes damageFloat {
                0%   { opacity: 1; transform: translate(-50%, 0); }
                100% { opacity: 0; transform: translate(-50%, -60px); }  /* 向上移动60px */
            }
        `;
        document.head.appendChild(style);
    }
    ensureDamageFloatKeyframes();
    ...
    function updateCardStats(el, atkGain, hpGain, shieldDelta = 0) {
        ...
        if (shieldDelta !== 0) {
            ...
            span.textContent = newShield;
            shieldEl.style.display = newShield > 0 ? 'flex' : 'none';
        }
    }

    // ★ 所有浮动文字从底部 0 处开始向上飘，持续时间默认1000ms
    function floatingText(el, text, color, duration = 1000) {
        if (!el) return;
    ...
    function buffAnim(buff) {
        return new Promise(async resolve => {
            ...
            if (shieldGain > 0) {
                floatingText(el, `🛡️ +${shieldGain}`, '#ffaa00', 1000);
            }
            if (totalAtkGain > 0 || totalHpGain > 0) {
                floatingText(el, `+${totalAtkGain}/+${totalHpGain}`, '#7bffb1', 1000);
            }
            resolve();
        });
    }

    // 合并动画间隔400ms，浮动文字持续1秒
    async function massBuffAnim(buffs, sampleBuff) {
        const myId = window.YYCardAuth?.currentUser?.id;
        const totalAtkGain = (sampleBuff.atkGain || 0) + (sampleBuff.tempAtkGain || 0);
        const totalHpGain = (sampleBuff.hpGain || 0) + (sampleBuff.tempHpGain || 0);
        const shieldGain = sampleBuff.tempShieldGain || 0;

        const targets = [];
        for (const b of buffs) {
            const el = await getCardElementRetry(b.playerId, b.position, b.playerId !== myId);
        ...
        return new Promise(async resolve => {
            ...
            setTimeout(() => {
                ...
                if (a.blocked) {
                    ...
                    floatingText(defEl, `${blockType} -1`, '#ffbb33', 1000);
                    ...
                    floatingText(defEl, `-${a.damage}`, '#f44', 1000);
        ...
        for (let i = 0; i < steps.length; i++) {
            ...
            if (step.goldGain !== undefined && step.goldGain !== 0 && isMyEvent) {
                ...
                if (my) {
                    ...
                    if (goldDisplay) {
                        floatingText(goldDisplay, `💰 +${step.goldGain}`, '#ffd966', 1000);
                    } else {
                        floatingText(document.body, `💰 +${step.goldGain}`, '#ffd966', 1000);
                    }
            ...
            if (step.type === 'buff' && step.playerId === myId) {
                const group = [step];
                let j = i + 1;
                while (j < steps.length &&
                       steps[j].type === 'buff' &&
                       steps[j].playerId === myId &&
                ...
                if (group.length > 1) {
                    debugLog(`  ▶ buff #${idx}-#${i + group.length}: ${step.sourceCard} ${step.desc} ×${group.length} (合并)`);
                    await massBuffAnim(group, step);
                    i = j - 1;
                } else {
                    ...
                    if (hand) {
                        ...
                        if (validCount < 15) {
                            ...
                            if (allCards.length > 0) {
                                ...
                                floatingText(document.getElementById('hand-container'), `✨ ${picked.name}`, '#ffd700', 1000);
                            }
                        }
                    }
                    await new Promise(r => setTimeout(r, 400));
