...
window.YYCardCombat = (function() {
    ...
    function fight(board1, board2, logCallback) {
        ...
        if (logCallback) {
            ...
            logCallback(`📊 我方: ${units1.map(u => `${u.name}(${u.position+1}) HP:${u.hp} ATK:${u.atk}`).join(' | ')}`);
            logCallback(`📊 敌方: ${units2.map(u => `${u.name}(${u.position+1}) HP:${u.hp} ATK:${u.atk}`).join(' | ')}`);
        }
