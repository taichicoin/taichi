...
window.YYCardCombat = (function() {
    ...
    function debugBoard(playerId, players) {
        ...
        if (!player) { debugLog(`玩家 ${playerId.slice(0,8)} 不存在`); return; }
        debugLog(`------ 玩家 ${playerId.slice(0,8)} 棋盘 ------`);
        debugLog(`斩妖除魔层数: ${player.demonSlayerLevel ?? 0}`);
        if (!player.board || !Array.isArray(player.board)) { debugLog('棋盘为空'); return; }
    ...
    function updateCardStats(el, atkGain, hpGain) {
        ...
        if (atkEl && atkGain) {
            ...
            atkEl.textContent = `⚔️${currentAtk + atkGain}`;
        }
        if (hpEl && hpGain) {
            const currentHp = parseInt(hpEl.textContent.replace(/\D/g, ''), 10) || 0;
            hpEl.textContent = `🛡️${currentHp + hpGain}`;
        }
        // 如果没有特定的类，尝试查找包含“⚔️”或“🛡️”的 span（兜底）
        if (!atkEl || !hpEl) {
            const spans = el.querySelectorAll('span');
            spans.forEach(span => {
                if (!atkEl && span.textContent.includes('⚔️') && atkGain) {
                    const currentAtk = parseInt(span.textContent.replace(/\D/g, ''), 10) || 0;
                    span.textContent = `⚔️${currentAtk + atkGain}`;
                }
                if (!hpEl && span.textContent.includes('🛡️') && hpGain) {
                    const currentHp = parseInt(span.textContent.replace(/\D/g, ''), 10) || 0;
                    span.textContent = `🛡️${currentHp + hpGain}`;
                }
            });
        }
    }
