...
window.YYCardCombat = (function() {
    ...
    // 修改后的单卡增益动画，不再显示箭头
    function buffAnim(buff, hideArrow = false) {
        return new Promise(async resolve => {
            ...
            if (totalAtkGain > 0 || totalHpGain > 0) {
                const text = hideArrow ? `+${totalAtkGain}/+${totalHpGain}` : `+${totalAtkGain}/+${totalHpGain}`;
    ...
    // ★ 并行合并动画：所有目标同时显示各自增益（无箭头），不显示“全体”
