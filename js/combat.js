...
window.YYCardCombat = (function() {
    ...
    function fight(board1, board2, logCallback) {
        ...
        if (logCallback) {
            ...
            logCallback(`📊 我方单位: ${units1.map(u => `${u.name}(${u.position+1}) HP:${u.hp} ATK:${u.atk}`).join(' | ')}`);
            logCallback(`📊 敌方单位: ${units2.map(u => `${u.name}(${u.position+1}) HP:${u.hp} ATK:${u.atk}`).join(' | ')}`);
        }
        ...
        while (units1.length > 0 && units2.length > 0 && turn < maxTurns) {
            ...
            target.hp -= damage;
            if (logCallback) {
                logCallback(`  ⚡ ${attacker.name}(${attacker.position+1}) → ${target.name}(${target.position+1}) 伤害 ${damage}，剩余 ${target.hp}`);
            }
    ...
    async function resolveBattles(gameState, log, updateCallback) {
        ...
        for (const [p1Id, p2Id] of pairs) {
            ...
            log(`💥 伤害 = 等级${level} + 存活单位额外${survivorBonus} = ${damage}，${loserPlayer === p1 ? p1Id : p2Id} 剩余血量 ${loserPlayer.health}`);
