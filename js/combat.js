// ==================== 战斗模拟模块（翻转武器展示起手动画 · 受击音效 · 遗言飞刀 · 刀尖朝向修复 · 曹操全体护盾优化 · 遗言金币处理 · 共工减半debuff动画 · 连续buff支持 · 亡魂召唤 · 内力连击动画 · 小乔飞刀加速 · 亡魂碎片快速生成 · 召唤护盾显示） ====================
window.YYCardCombat = (function() {
    ...
    function spawnWeaponOnCard(cardEl, isEnemy, model = null) {
        ...
        const edgeY = isEnemy ? rect.bottom : rect.top;   // 敌方底部，我方顶部
    ...
    // ★ 修改：增加 soundPath 参数，默认为 '/assets/mp3/zs.mp3'
    ...
    // ==================== 新起手动画：变大 + 翻转显示武器 ====================
    async function flipAttackStart(attEl, weaponImage) {
        if (abortFlag) return;
        const slot = attEl.parentNode;
        if (!slot) return;

        // 保存原始样式
        const origTransition = attEl.style.transition;
        const origTransform = attEl.style.transform;
        const origTransformStyle = attEl.style.transformStyle;
        const origBackface = attEl.style.backfaceVisibility;
        const origZIndex = attEl.style.zIndex;

        const origSlotPerspective = slot.style.perspective;
        const origSlotTransformStyle = slot.style.transformStyle;
        ...
        backEl.style.cssText = `
            ...
            background-image: url('${weaponImage}');
        ...
        try {
            ...
            // 阶段2：翻转 0.2s
            attEl.style.transition = 'transform 0.2s ease-in-out';
            attEl.style.transform = 'scale(1.25) rotateY(180deg)';
            ...
            // 阶段4：恢复翻转 0.2s
            attEl.style.transition = 'transform 0.2s ease-in-out';
            attEl.style.transform = 'scale(1) rotateY(0deg)';
            await new Promise(r => setTimeout(r, 200));
        } catch (e) {
            // abort 时立即清理
        } finally {
            // 清理
            if (backEl.parentNode) backEl.remove();
            attEl.style.transition = origTransition;
            attEl.style.transform = origTransform;
            attEl.style.transformStyle = origTransformStyle;
            attEl.style.backfaceVisibility = origBackface;
            attEl.style.zIndex = origZIndex;
            slot.style.perspective = origSlotPerspective;
            slot.style.transformStyle = origSlotTransformStyle;
        }
    }
    ...
    function attackAnim(a) {
        return new Promise(async resolve => {
            ...
            // 使用注入的武器图片
            const weaponImg = a._weaponImage || 'assets/default_weapon.png';
            await flipAttackStart(attEl, weaponImg);
            if (abortFlag) return resolve();
    ...
    function multiHitAnim(stepsList) {
        return new Promise(async resolve => {
            ...
            // 播放一次起手动画
            const weaponImg = first._weaponImage || 'assets/default_weapon.png';
            await flipAttackStart(attEl, weaponImg);
            if (abortFlag) return resolve();
    ...
    // ... massBuffAnim, massShieldAnim, debuffAnim 保持不变 ...
        ...
        for (let i = 0; i < steps.length; i++) {
            ...
            // ----- 攻击（近战、远程、AOE） -----
                ...
                if (step.isRanged) {
                    ...
                    // ★ 获取武器图片并注入到攻击组
                    const playerId = step.attackerOwnerId;
                    const pos = step.attackerPos;
                    const gs = getGameState();
                    let weaponImg = 'assets/default_weapon.png';
                    if (gs && gs.players && gs.players[playerId] && gs.players[playerId].board) {
                        const card = gs.players[playerId].board[pos];
                        if (card && card.weapon) {
                            weaponImg = card.weapon.image || `/assets/weapon/${card.weapon.card_id}.png`;
                        }
                    }
                    for (const s of attackGroup) s._weaponImage = weaponImg;
    ...
    // ... 其余函数 waitForEnemyBoard, renderEnemyBoardFromData, resolveBattles 保持不变 ...
