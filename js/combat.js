// ==================== 战斗模拟模块（新版：不创建临时棋盘，复用原有渲染，不跳过动画） ====================
window.YYCardCombat = (function() {
    ...
    async function resolveBattles(gameState, log, onComplete) {
        ...
        // 设置战斗配对信息，以便 renderEnemyBoard 能找到正确的对手
        if (data.combatResults) {
            const myBattle = data.combatResults.find(cr => cr.p1 === myId || cr.p2 === myId);
            if (myBattle) {
                const oppId = myBattle.p1 === myId ? myBattle.p2 : myBattle.p1;
                if (gameState.battlePairs) {
                    gameState.battlePairs = [[myId, oppId]];
                } else {
                    gameState.battlePairs = [[myId, oppId]];
                }
                debugLog(`对手ID: ${oppId.slice(0,8)}`);
            }
        }

        // 现在调用 shop.js 的全局刷新，它会用 gameState 中的最新数据重新绘制我方和敌方棋盘
        if (window.YYCardShop?.refreshAllUI) {
            window.YYCardShop.refreshAllUI();
            debugLog('✅ 已刷新界面（含敌方棋盘）');
        } else {
            debugLog('⚠️ refreshAllUI 不可用，尝试手动渲染敌方棋盘');
            // 如果 shop.js 未加载，只能手动插入（但很可能位置错误）
            // 但这种情况不应发生，所以尽量确保 shop.js 在 combat.js 之前加载
        }
        ...
        // 完全取消跳过逻辑，强制从头播放全部步骤，让你看清楚每一个动画
