...
window.YYCardCombat = (function() {
    ...
    // ★★★ 新增：创建亡魂卡片（淡银色边框）
    function createSpiritCard(slotEl, spiritAtk, spiritHp, spiritImage, spiritName) {
        slotEl.innerHTML = '';
        ...
        for (let i = 0; i < steps.length; i++) {
            ...
            if (step.type === 'summon_spirit') {
                ...
                if (step.summoned && step.position >= 0) {
                    const slotEl = getSlotElement(step.playerId, step.position, isEnemy);
                    if (slotEl) {
                        createSpiritCard(slotEl, step.spiritAtk, step.spiritHp,
                            '/assets/card/daodun1.png', '亡魂');
                    } else {
