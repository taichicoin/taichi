...
window.YYCardCombat = (function() {
    ...
    function playAttackAnim(action) {
        return new Promise(resolve => {
            ...
            if (!$attacker || !$defender) {
                panelLog(`⚠️ 跳过 (${action.attackerBoard}[数据${action.attackerPos}] → ${action.defenderBoard}[数据${action.defenderPos}])`, true);
                resolve();
                return;
            }

            panelLog(`⚔️ ${action.attackerBoard}[数据${action.attackerPos}] → ${action.defenderBoard}[数据${action.defenderPos}]`);
    ...
    async function resolveBattlesWithAnimation(gameState, log, updateCallback) {
        ...
        for (const [p1Id, p2Id] of pairs) {
            ...
            // 将原始攻击记录转换为带棋盘 ID 的格式
            const mappedLog = combatLog.map(entry => ({
                attackerBoard: entry.attackerSide === 1 ? attackerBoardMap[1] : attackerBoardMap[2],
                defenderBoard: entry.defenderSide === 1 ? attackerBoardMap[2] : attackerBoardMap[1], // defenderSide 是相反 side
