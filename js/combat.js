...
window.YYCardCombat = (function() {
    ...
    function getCardElement(playerId, dataPos, isEnemy) {
        ...
        if (!board) {
            debugLog(`[DOM] 找不到棋盘: player=${playerId.slice(0,8)}`);
            return null;
        }

        // 方法1：直接按 data-board-index 匹配
        ...
        // 方法4：遍历所有格子，匹配 data-board-index
    ...
    function buffAnim(buff) {
        return new Promise(resolve => {
            ...
            if (!el) {
                debugLog(`⚠️ 增益未找到: player=${buff.playerId.slice(0,8)} pos=${buff.position} card=${buff.sourceCard} ${buff.desc}`);
                return resolve();
            }
    ...
    function attackAnim(a) {
        return new Promise(resolve => {
            ...
            if (!attEl || !defEl) {
                debugLog(`⚠️ 攻击动画缺失: atk=${a.attackerName}(${a.attackerOwnerId.slice(0,8)} pos${a.attackerPos}) def=${a.defenderName}(${a.defenderOwnerId.slice(0,8)} pos${a.defenderPos}) attFound=${!!attEl} defFound=${!!defEl}`);
                return resolve();
            }
    ...
    async function playSteps(steps) {
        ...
        for (const step of steps) {
            ...
            if (step.type === 'buff') {
                ...
                debugLog(`  ▶ atk #${idx}: ${step.attackerName}→${step.defenderName} dmg=${step.damage} pos${step.attackerPos}→${step.defenderPos}`);
                await attackAnim(step);
            }
            await new Promise(r => setTimeout(r, 80));
        }
        debugLog(`  🏁 动画播放完毕，共${idx}步`);
        isAnimating = false;
    }
    ...
    async function resolveBattles(gameState, log, onComplete) {
        ...
        try {
            ...
            if (!data.success) { debugLog('[Combat] 后端结算失败: ' + data.error); if (onComplete) onComplete(); return; }
        } catch (err) {
