...
window.YYCardCombat = (function() {
    ...
    // ★ 播放黑化帝攻击动画视频
    async function playAttackVideo(attackerName) {
        if (!attackerName || !attackerName.includes('黑化帝')) return;
        ...
        return new Promise((resolve) => {
            ...
            video.play().catch(e => {
                utils.debugLog('⚠️ 黑化帝视频自动播放失败: ' + e);
                clearTimeout(timer);
                cleanup();
            });
        });
    ...
    async function playSteps(steps, myId, oppId) {
        ...
        for (let i = 0; i < steps.length; i++) {
            ...
            if (step.type === 'summon_spirit') {
                ...
                await new Promise(r => setTimeout(r, 200)); // 音效先行 0.2 秒
            ...
            // 攻击（包含远程、多段、AOE）
                ...
                if (!videoPlayedFor.has(attackerKey) && step.attackerName && step.attackerName.includes('黑化帝')) {
                    videoPlayedFor.add(attackerKey);
                    await playAttackVideo(step.attackerName);
                    if (_aborted) break;
                }
