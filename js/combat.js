            ...
            // 攻击（远程 / 多段 / 单体）
                ...
                if (step.isRanged) {
                    ...
                    if (step.isCatapult) {
                        const catapultGroup = [];
                        let j = i;
                        while (j < steps.length && steps[j].type === 'attack' && steps[j].isRanged && steps[j].isCatapult) {
                            catapultGroup.push(steps[j]);
                            j++;
                        }
                        utils.debugLog(`  ▶ catapult #${idx}: 投石车 ×${catapultGroup.length}`);
                        await anims.rangedAttackAnim(catapultGroup); // 会识别 isCatapult 使用 🪨
                        i = j - 1;
                    } else {
                        // 普通飞刀合并
                        const rangedGroup = [];
