...
window.YYCardCombat = (function() {
    ...
    panelLog('✅ combat.js 完整修正版 (应龙/蠃鱼/穷奇/白泽联动)');
    ...
    function getTargetCards(board, scope, sourcePos) {
        ...
        if (scope === 'self') {
            ...
            panelLog(`  📍 同排判断: sourcePos=${sourcePos}, rowStart=${rowStart}`);
            for (let i = rowStart; i < rowStart + 3; i++) {
                if (board[i]) {
                    panelLog(`    ↳ 找到同排卡牌: ${board[i].name} at pos ${i}`);
                    targets.push({ card: board[i], pos: i });
                }
            }
        }
        return targets;
    ...
    function processEnlightenment(card, skill, playerId, pos, board, buffEvents, gameState) {
        ...
        targets.forEach(({ card: targetCard, pos: targetPos }) => {
            ...
            buffEvents.push({
                type: 'buff', playerId, position: targetPos,
                atkGain: gainAtk, hpGain: gainHp, sourceCard: card.name
            });
            panelLog(`    ↳ ${targetCard.name} 获得 +${gainAtk}/+${gainHp}`);
        });
        // 触发反应技能
    ...
    function triggerReactionSkills(gameState, playerId, sourceCard, sourcePos, board, buffEvents) {
        panelLog(`  ⚡ 反应技能检查 (因 ${sourceCard.name} 悟道)`);
        for (let i = 0; i < 6; i++) {
            const card = board[i];
            if (!card) continue;
            const skill = parseSkill(card);
            if (!skill || skill.trigger !== 'onEnlightenmentTriggered') continue;
            if (skill.skillId === 'skill_chengshan_yange') {
            ...
            else if (skill.skillId === 'skill_jinglei') {
                const gainAtk = skill.effect.value?.atk || 2;
                const gainHp = skill.effect.value?.hp || 5;
                card.atk = (card.atk || 0) + gainAtk;
                card.hp = (card.hp || 0) + gainHp;
                buffEvents.push({ type: 'buff', playerId, position: i, atkGain: gainAtk, hpGain: gainHp, sourceCard: card.name });
                panelLog(`    ↳ 白泽 ${card.name} 惊雷 +${gainAtk}/+${gainHp}`);
            }
        }
    }
    ...
    async function playLog(logs) {
        ...
        if (isPlaying) { panelLog('⚠️ 动画已在播放', true); return; }
        isPlaying = true; isAnimating = true; abortFlag = false; animQueue = [...logs];
        try {
            while (animQueue.length && !abortFlag) {
                ...
                } catch (e) { panelLog(`❌ 单步异常: ${e.message}`, true); }
            }
        } catch (e) { panelLog(`❌ 主循环异常: ${e.message}`, true); }
        isPlaying = false; isAnimating = false;
        panelLog(`✅ 播放完毕`);
    }
    ...
    async function resolveBattles(gameState, log, updateCallback) {
        ...
        if (buffEvents.length > 0) {
            panelLog(`✨ 播放技能增益 (${buffEvents.length} 个)`);
