...
window.YYCardCombat = (function() {
    ...
    function spawnWeaponOnCard(cardEl, isEnemy, model = null) {
        ...
        const centerPos = domToWorld(cardEl); // 卡牌中心世界坐标
        ...
        const offset = edgeWorld.clone().sub(centerPos); // 从中心指向边缘
        // 武器中心向边缘反方向移动，使矛尖刚好在边缘上
        weapon.position.copy(centerPos).sub(offset);
    ...
    // ★ 修改：attackAnim 中调用 flyWeaponToTarget 时传入 isEnemyAttacker
    ...
    // ★ 修改：multiHitAnim 中调用 flyWeaponToTarget 时传入 isEnemyAttacker
    ...
    // ★★★ 飞刀动画支持投石车（根据 step.isCatapult 使用 🪨） ★★★
