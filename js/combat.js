...
window.YYCardCombat = (function() {
    ...
    // ★★★ 如果剑的方向不对，改下面两个数字（单位：弧度） ★★★
    // SWORD_ROTATE_Z：绕世界Z轴旋转，控制剑身宽面是否朝向屏幕
    const SWORD_ROTATE_Z = Math.PI / 2;   // 试试 0 或 Math.PI/2 或 -Math.PI/2
    // SWORD_ROTATE_X：绕世界X轴旋转，控制剑尖朝向（正数朝上，负数朝下）
    const SWORD_ROTATE_X = -Math.PI / 2;   // 试试 Math.PI/2 或 -Math.PI/2
    ...
    async function init3D() {
        ...
        try {
            ...
            _swordGLB.scale.set(0.04, 0.04, 0.04);
    ...
    // 给武器应用固定旋转补偿（在 lookAt 之后调用）
    function applySwordRotation(weapon, isEnemy) {
        const THREE = _threeModule;
        // 先绕世界Z轴旋转
        weapon.rotateOnWorldAxis(new THREE.Vector3(0, 0, 1), SWORD_ROTATE_Z);
        // 再绕世界X轴旋转（敌方可能需反向）
        const xAngle = isEnemy ? -SWORD_ROTATE_X : SWORD_ROTATE_X;
        weapon.rotateOnWorldAxis(new THREE.Vector3(1, 0, 0), xAngle);
    }
    ...
    function spawnWeaponOnCard(cardEl, isEnemy) {
        ...
        // 让武器面向正上方（世界+Y）作为初始方向，便于我们后面旋转修正
        const upTarget = pos.clone().add(new THREE.Vector3(0, 1, 0));
        weapon.lookAt(upTarget);

        // 应用修正旋转，让剑竖直、宽面朝屏幕
        applySwordRotation(weapon, isEnemy);
    ...
    function flyWeaponToTarget(weapon, startPos, defEl, isEnemyAttacker) {
        ...
        return new Promise(resolve => {
            ...
            // 先看向目标
            weapon.lookAt(endPos);
            // 应用同样的旋转补偿，保持剑身方向不变
            applySwordRotation(weapon, isEnemyAttacker);
