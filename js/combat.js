...
window.YYCardCombat = (function() {
    ...
    function insertWeaponIntoCard(defEl, isEnemy) {
        ...
        return new Promise(resolve => {
            const duration = 100;  // 插入动画时长 0.1 秒
    ...
    // ★★★ 单个增益动画（攻击/生命同时加时显示 +atk+hp，单个时只显示一个，上限 9999） ★★★
    function buffAnim(buff) {
        return new Promise(async resolve => {
            ...
            // 攻击/生命飘字（恢复为同时显示 +atk+hp 格式，去掉 +0）
            if (totalAtkGain > 0 && totalHpGain > 0) {
                floatingText(el, `+${clampDisplay(totalAtkGain)}+${clampDisplay(totalHpGain)}`, '#7bffb1', 1200);
            } else if (totalAtkGain > 0) {
                floatingText(el, `+${clampDisplay(totalAtkGain)}`, '#7bffb1', 1200, -30);
            } else if (totalHpGain > 0) {
                floatingText(el, `+${clampDisplay(totalHpGain)}`, '#7bffb1', 1200, -30);
            }
    ...
    function attackAnim(a) {
        return new Promise(async resolve => {
            ...
            if (_3DReady) {
                ...
                // 3) 武器图展示 0.4s，第 0.3s 开始插入长矛（插入动画 0.1s）
                const insertDelay = 300;        // 0.3s 后开始插矛
                const displayDuration = 400;    // 武器展示总时长 0.4s
    ...
    function multiHitAnim(stepsList) {
        return new Promise(async resolve => {
            ...
            if (_3DReady) {
                ...
                // 3) 武器展示 + 插入长矛（展示总长 0.4s，0.3s 后开始插入 0.1s 动画）
                const insertDelay = 300;
                const totalDisplay = 400;
                await new Promise(r => setTimeout(r, insertDelay));
                await insertWeaponIntoCard(defEl, isEnemyAttacker);
                const insertDuration = 100;
                const remaining = Math.max(0, totalDisplay - insertDelay - insertDuration);
    ...
    // ★★★ 群体增益动画（攻击/生命同时加时显示 +atk+hp，单个时只显示一个） ★★★
    async function massBuffAnim(step) {
        ...
        for (const { el } of elements) {
            if (atkGain > 0 && hpGain > 0) {
                floatingText(el, `+${clampDisplay(atkGain)}+${clampDisplay(hpGain)}`, '#7bffb1', 1200);
            } else if (atkGain > 0) {
                floatingText(el, `+${clampDisplay(atkGain)}`, '#7bffb1', 1200, -30);
            } else if (hpGain > 0) {
                floatingText(el, `+${clampDisplay(hpGain)}`, '#7bffb1', 1200, -30);
            }
        }
    ...
    // playSteps（修改了 summon_spirit 部分）
