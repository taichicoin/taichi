...
window.YYCardCombat = (function() {
    ...
    async function playSteps(steps, myId, oppId) {
        ...
        for (let i = 0; i < steps.length; i++) {
            ...
            // ★★★ 群体增益：收集连续区块，并将精卫的蠃鱼单体增益视为同一批次 ★★★
            else if (step.type === 'mass_buff') {
                const block = [];
                let j = i;
                // 收集连续的 mass_buff，以及中间夹着的精卫蠃鱼单体 buff
                while (j < steps.length) {
                    if (steps[j].type === 'mass_buff') {
                        block.push(steps[j]);
                        j++;
                    } else if (steps[j].type === 'buff' && steps[j].sourceCard === 'char_jingwei' && steps[j].desc && steps[j].desc.includes('蠃鱼')) {
                        // 快速处理精卫自身的单体增益：只更新属性 + 播放音效，不显示浮动
                        const bstep = steps[j];
                        const isEnemy3 = bstep.playerId !== myId;
                        const el3 = await utils.getCardElementRetry(bstep.playerId, bstep.position, isEnemy3);
                        if (el3) {
                            const totalAtk = (bstep.atkGain || 0) + (bstep.tempAtkGain || 0);
                            const totalHp = (bstep.hpGain || 0) + (bstep.tempHpGain || 0);
                            utils.updateCardStats(el3, totalAtk, totalHp);
                            try {
