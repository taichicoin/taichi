...
window.YYCardCombat = (function() {
    ...
    let _swordGLB = null;      // 普通攻击的剑模型
    let _daggerGLB = null;     // 遗言飞刀的匕首模型
    let _scene, _camera, _renderer;

    // ★ 剑尖方向补偿角度（弧度）
    const SWORD_Z_OFFSET = -1;
    ...
    async function init3D() {
        ...
        try {
            ...
            if (daggerGltf) {
                ...
                const daggerTargetHeight = 0.02; // 匕首小一点
    ...
    function rangedAttackAnim(stepsArray) {
        return new Promise(async resolve => {
            ...
            const myId = window.YYCardAuth?.currentUser?.id;
            // 所有飞刀源自同一个死亡单位，取第一个 step 的 attackerPos
            ...
            const startPos = domToWorld(slotEl); // 世界坐标
            ...
            for (let i = 0; i < stepsArray.length; i++) {
                ...
                let weapon = null;
                let useEmoji = false;
                if (_3DReady && _daggerGLB) {
                    weapon = spawnWeaponOnCard(slotEl, isEnemyAttacker, _daggerGLB);
                }
                if (!weapon) {
                    // 降级为表情符号
                    useEmoji = true;
                    ...
                    emoji.style.cssText = `
                        ...
                        left: ${slotEl.getBoundingClientRect().left + slotEl.getBoundingClientRect().width/2}px;
                        top: ${slotEl.getBoundingClientRect().top + slotEl.getBoundingClientRect().height/2}px;
                    ...
                    emoji.style.left = `${defRect.left + defRect.width/2}px`;
                    emoji.style.top = `${defRect.top + defRect.height/2}px`;
