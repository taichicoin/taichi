...
window.YYCardCombat = (function() {
    ...
    // ★ 如果剑的方向还不对，调整下面两个旋转角度（弧度制，正值顺时针）
    const MY_SWORD_ROTATION = -Math.PI / 4;      // 我方额外绕Z轴旋转角度
    const ENEMY_SWORD_ROTATION = -3 * Math.PI / 4; // 敌方额外绕Z轴旋转角度
    ...
    function spawnWeaponOnCard(cardEl, isEnemy) {
        ...
        // 基础朝向：宽面朝相机（+Z），剑尖暂时指向世界Y方向
        const upDir = new THREE.Vector3(0, isEnemy ? -1 : 1, 0);
        const camDir = new THREE.Vector3(0, 0, 1);
        const rightDir = new THREE.Vector3().crossVectors(camDir, upDir).normalize();
        const rotMat = new THREE.Matrix4().makeBasis(rightDir, upDir, camDir);
        weapon.quaternion.setFromRotationMatrix(rotMat);

        // 根据敌我添加额外旋转，校正剑身方向
        const extraAngle = isEnemy ? ENEMY_SWORD_ROTATION : MY_SWORD_ROTATION;
        weapon.rotateOnWorldAxis(new THREE.Vector3(0, 0, 1), extraAngle);
