...
window.YYCardCombat = (function() {
    ...
    // ★ 播放黑化帝攻击动画视频
    async function playAttackVideo(sourceCard) {
        // 用 sourceCard（如 char_heihuadi）或 attackerName 判断
        const isHeihua = (sourceCard && sourceCard.includes('heihua')) || 
                         (sourceCard && sourceCard.includes('黑化帝'));
        if (!isHeihua) return;
        
        utils.debugLog(`🎬 播放黑化帝攻击动画 (${sourceCard})`);
    ...
    async function playSteps(steps, myId, oppId) {
        ...
        let idx = 0;
        const videoPlayedFor = new Set(); // 记录已播放视频的攻击者，避免重复
        ...
        for (let i = 0; i < steps.length; i++) {
            ...
            // 攻击（包含远程、多段、AOE）
                ...
                // ★ 黑化帝攻击前播放视频（使用 sourceCard 或 attackerName 判断）
                const attackerId = step.sourceCard || step.attackerName || '';
                const attackerKey = step.attackerOwnerId + ':' + step.attackerPos;
                if (!videoPlayedFor.has(attackerKey) && attackerId.includes('heihua')) {
                    videoPlayedFor.add(attackerKey);
                    await playAttackVideo(attackerId);
                    if (_aborted) break;
                }
                ...
                if (step.isRanged) {
                    ...
                    utils.debugLog(`  ▶ ranged #${idx}: 飞刀 ×${rangedGroup.length} (来自 ${step.sourceCard || step.attackerName})`);
                    ...
                    if (allSameTarget) {
                        utils.debugLog(`  ▶ multi-hit #${idx}: ${step.sourceCard || step.attackerName}→${step.defenderName} ×${attackGroup.length}`);
                        await anims.multiHitAnim(attackGroup);
                        i = j - 1;
                    } else {
                        utils.debugLog(`  ▶ aoe #${idx}: ${step.sourceCard || step.attackerName}→${attackGroup.map(s => s.defenderName).join(',')}`);
                        await anims.aoeAttackAnim(step.attackerOwnerId, step.attackerPos, step.sourceCard || step.attackerName, attackGroup);
                        i = j - 1;
                    }
                    await new Promise(r => setTimeout(r, 300));
                }
