// ==================== 战斗模拟模块（3D 武器攻击版 · 受击音效 · 遗言飞刀 · 刀尖朝向修复 · 曹操全体护盾优化 · 遗言金币处理 · 共工减半debuff动画 · 连续buff支持 · 亡魂召唤 · 内力连击动画 · 小乔飞刀加速 · 亡魂碎片快速生成） ====================
window.YYCardCombat = (function() {
    ...
    function createSpiritCard(slotEl, spiritAtk, spiritHp, spiritImage, spiritName, star = 0) {
        slotEl.innerHTML = '';
    ...
    function multiHitAnim(stepsList) {
        return new Promise(async resolve => {
            ...
            for (let i = 0; i < stepsList.length; i++) {
                ...
                applyDamageEffects(step, defEl, true);
            ...
            if (step.type === 'summon_spirit') {
                ...
                if (step.summoned && step.position >= 0) {
                    ...
                    if (slotEl) {
                        ...
                        createSpiritCard(slotEl, step.spiritAtk, step.spiritHp, image, '亡魂', star);
                    } else {
                ...
                if (myId && step.playerId === myId) {
                    ...
                    if (step.generator === 'spirit_loot') {
                        ...
                        await new Promise(r => setTimeout(r, 100)); // 快速依次出现
                    } else {
                        // 原有女娲生成逻辑
        ...
        for (let i = 0; i < 6; i++) {
            ...
            if (c && typeof c === 'object' && (c.card_id || c.cardId) && (c.hp + (c.tempHp || 0)) > 0) {
                ...
                if (c.shield > 0) {
                    ...
                    shieldDiv.innerHTML = `<span>${c.shield}</span>`;
                    el.appendChild(shieldDiv);
                }
