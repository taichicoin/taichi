...
window.YYCardCombat = (function() {
    ...
    // 创建屏幕中央大字显示区域
    function createBigLogPanel() {
        const old = document.getElementById('combat-big-log');
        if (old) old.remove();
        const p = document.createElement('div');
        p.id = 'combat-big-log';
        p.style.cssText = `
            position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%);
            background: rgba(0,0,0,0.85); color: #ff0; font-size: 16px; padding: 20px;
            border-radius: 12px; border: 2px solid #f0f; z-index: 100001;
    ...
    function showBigLog(text) {
        const panel = document.getElementById('combat-big-log') || createBigLogPanel();
    ...
    async function resolveBattles(gameState, log, updateCallback) {
        ...
        // 构建大字显示内容
        let bigText = '══════ 本回合对战 ══════\n\n';
        bigText += `配对: ${pairs.map(p => p[1] ? `${p[0].slice(0,8)} vs ${p[1].slice(0,8)}` : `${p[0].slice(0,8)} 轮空`).join(' | ')}\n\n`;
        ...
        for (const [p1Id, p2Id] of pairs) {
            ...
            if (isHumanVsHuman) {
                ...
                bigText += `━━━ ${p1.isBot ? '人机' : '真人'} vs ${p2.isBot ? '人机' : '真人'} ━━━\n`;
            }

            bigText += `玩家A (${p1Id.slice(0,8)}):\n`;
            bigText += board1.map((c, i) => c ? `  [${i+1}] ${c.name} HP:${c.hp} ATK:${c.atk}` : `  [${i+1}] 空`).join('\n');
            bigText += `\n`;
            bigText += `玩家B (${p2Id.slice(0,8)}):\n`;
            bigText += board2.map((c, i) => c ? `  [${i+1}] ${c.name} HP:${c.hp} ATK:${c.atk}` : `  [${i+1}] 空`).join('\n');
            bigText += `\n\n`;
            ...
            bigText += `结果: ${winner === 1 ? '玩家A' : '玩家B'} 胜利\n`;
            bigText += `扣血: ${damage} (等级${level} + 额外${survivorBonus})\n`;
            bigText += `失败方剩余血量: ${loserPlayer.health}\n`;
        }
