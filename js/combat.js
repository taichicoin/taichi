...
window.YYCardCombat = (function() {
    ...
    function fight(board1, board2, logCallback) {
        ...
        let turn = 0;
        const maxTurns = 200;
        let currentAttackerSide = board1First ? 1 : 2; // 1=board1, 2=board2
        ...
        if (units1.length > 0 && units2.length === 0) {
            if (logCallback) logCallback(`🏆 我方胜利，剩余 ${units1.length} 个单位`);
            return 1;
        }
        if (units2.length > 0 && units1.length === 0) {
            if (logCallback) logCallback(`🏆 敌方胜利，剩余 ${units2.length} 个单位`);
            return 2;
        }
        if (logCallback) logCallback(`💔 同归于尽`);
        return 0;
    }
