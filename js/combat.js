...
window.YYCardCombat = (function() {
    ...
    const SWORD_Z_OFFSET = 1.6;   // 根据实际效果调整
    ...
    async function init3D() {
        ...
        try {
            ...
            // 自动计算合适的缩放大小（保持剑在屏幕上约 0.3 单位高）
            const box = new THREE.Box3().setFromObject(_swordGLB);
            const size = new THREE.Vector3();
            box.getSize(size);
            const targetHeight = 0.01;   // 你可以在 0.1 ~ 0.5 之间调整
            const scaleFactor = targetHeight / (size.y || 1);
            _swordGLB.scale.set(scaleFactor, scaleFactor, scaleFactor);

            _3DReady = true;
            debugLog('⚔️ 3D 武器系统就绪 (自动缩放 ' + scaleFactor.toFixed(4) + ')');
        } catch (e) {
