...
window.YYCardCombat = (function() {
    ...
    async function resolveBattles(gameState, onComplete) {
        ...
        if (oppId && gameState.players[oppId]?.board) {
            ...
            // 如果内存里没有对手棋盘，尝试从数据库拉取（重连场景）
            if (window.YYCardBattle?.fetchOpponentBoard) {
                const oppBoard = await window.YYCardBattle.fetchOpponentBoard(oppId);
                if (oppBoard) {
                    gameState.players[oppId].board = oppBoard;
                    renderEnemyBoardFromData(oppId, oppBoard);
