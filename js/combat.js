...
window.YYCardCombat = (function() {
    ...
    const SWORD_Z_OFFSET = 0;   // 3D 剑补偿（本次不使用）

    // ★ 表情飞刀刀尖方向补偿（可根据实际效果微调，范围 -360~360）
    ...
    // ★★★ 核心动画入口，适配新 settle-battle 的 _myCombatLog 和 _myOpponentId ★★★
    async function resolveBattles(gameState, onComplete) {
        ...
        // ★ 直接使用 battle.js 传入的分块数据，不再从 _combatResults 查找
        const buffEvents = gameState._buffEvents || [];
        const myCombatLog = gameState._myCombatLog || [];
        const oppId = gameState._myOpponentId;

        // ★ 不再做任何渲染，完全使用 battle.js 提前渲染好的双方棋盘
        // 如果需要确认棋盘存在，只做检查，不做渲染
        if (oppId) {
            const boardEl = document.querySelector(`.board[data-player-id="${oppId}"]`);
            if (!boardEl) {
                debugLog('⚠️ 敌方棋盘不存在，动画可能无法完整显示');
            }
        }

        // 合并步骤：buffEvents（可能为全局事件，通常已包含在日志中） + 我的战斗日志
