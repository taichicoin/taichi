// ==================== 战斗模拟模块（最终版：山海经技能联动修正 + 轮流攻击 + 死亡移除 + 慢速动画） ====================
window.YYCardCombat = (function() {
    ...
    panelLog('✅ combat.js 最终版 (应龙/蠃鱼/穷奇/白泽联动修正)');
    ...
    function applyBattleStartSkills(gameState) {
        ...
        for (const playerId in gameState.players) {
            ...
            for (let pos = 0; pos < 6; pos++) {
                ...
                if (skill.trigger !== 'onBattleStart') continue;
        ...
        panelLog(`🔍 技能处理完成，共生成 ${buffEvents.length} 个增益事件`);
        return buffEvents;
    }

    function processEnlightenment(card, skill, playerId, pos, board, buffEvents, gameState) {
        if (skill.skillId === 'skill_xianhuo_shoufa') return;
        ...
        if (isJingwei) {
            ...
            panelLog(`  ✨ 精卫悟道层数: ${level}，增益 +${gainAtk}/+${gainHp}`);
        } else {
            gainAtk = skill.effect.baseValue || 1;
            gainHp = skill.effect.baseValue || 1;
            panelLog(`  🌊 ${card.name} 悟道触发，固定增益 +${gainAtk}/+${gainHp}`);
        }
    ...
    function triggerReactionSkills(gameState, playerId, sourceCard, sourcePos, board, buffEvents) {
        for (let i = 0; i < 6; i++) {
            ...
            if (skill.skillId === 'skill_jinglei') {
                ...
                panelLog(`    ↳ 白泽 ${card.name} 惊雷触发，自身 +${gainAtk}/+${gainHp}`);
            }
                ...
                panelLog(`    ↳ 穷奇 ${card.name} 惩善扬恶触发，自身与${sourceCard.name}各 +${gainAtk}/+${gainHp}`);
            }
        }
    }
    ...
    function fightWithLog(board1, board2, owner1, owner2) {
        ...
        if (units1.length === 0 || units2.length === 0) {
            panelLog(`🏆 战斗结束 胜者: side${units1.length ? 1 : 2}  总回合: 0`);
        ...
        while (units1.length > 0 && units2.length > 0 && turn < 200) {
            ...
            if (curSide === 1) {
                ...
                for (let i = 0; i < units1.length; i++) {
                    ...
                    if (units1[checkIdx] && units1[checkIdx].hp > 0) { attacker = units1[checkIdx]; idx1 = (checkIdx + 1) % units1.length; break; }
                }
            } else {
                let startIdx = idx2 % units2.length;
                for (let i = 0; i < units2.length; i++) {
                    const checkIdx = (startIdx + i) % units2.length;
                    if (units2[checkIdx] && units2[checkIdx].hp > 0) { attacker = units2[checkIdx]; idx2 = (checkIdx + 1) % units2.length; break; }
                }
            }
            if (!attacker) break;
    ...
    function playBuffAnim(buff) {
        return new Promise(resolve => {
            ...
            if (!cardEl) { panelLog(`⏭️ 跳过增益动画，找不到卡牌 pos=${buff.position}`, true); return resolve(); }
    ...
    async function playLog(logs) {
        ...
        if (isPlaying) { panelLog('⚠️ 动画已在播放，忽略重复调用', true); return; }
        isPlaying = true;
        isAnimating = true;
        abortFlag = false;
        animQueue = [...logs];
        try {
            while (animQueue.length && !abortFlag) {
                ...
                } catch (e) { panelLog(`❌ 单步动画异常: ${e.message}`, true); }
            }
        } catch (e) { panelLog(`❌ 动画主循环异常: ${e.message}`, true); }
        isPlaying = false;
        isAnimating = false;
        panelLog(`✅ 播放完毕`);
    }
