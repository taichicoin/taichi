...
window.YYCardCombat = (function() {
    ...
    panelLog('🔥 combat.js v4 开始执行', false);

    if (!config) {
        panelLog('❌ YYCardConfig 未定义，模块无法工作', true);
        return {};
    }
    ...
    async function playCombatLog(logs) {
        panelLog(`▶️ 播放动画序列，共 ${logs.length} 步`);
        ...
        try {
            ...
            panelLog(`❌ 动画播放异常: ${err.message}`, true);
        }
        isAnimPlaying = false;
        panelLog(`✅ 动画播放完毕`);
    }
    ...
    async function resolveBattlesWithAnimation(gameState, log, updateCallback) {
        ...
        for (const [p1Id, p2Id] of pairs) {
            ...
            const board1 = p1.board || [];
            const board2 = p2.board || [];
        ...
        // 如果没有任何攻击步骤（例如双方都没上怪），则播放一段演示动画，确保能看到效果
        if (totalSteps === 0) {
