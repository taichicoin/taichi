...
window.YYCardCombat = (function() {
    ...
    async function massBuffAnim(buffs, sampleBuff) {
        ...
        targets.forEach(el => updateCardStats(el, totalAtkGain, totalHpGain, shieldGain));

        // 显示一条全体飘字
        if (targets.length > 0) {
            const displayText = totalAtkGain || totalHpGain ? `⬆️ 全体 +${totalAtkGain}/+${totalHpGain}` : `🛡️ 全体 +${shieldGain}`;
            floatingText(targets[0], displayText, '#7bffb1', 1500);
        }
    ...
    async function resolveBattles(gameState, log, onComplete) {
        ...
        // 确保敌方棋盘已经显示最终状态（已经在前面渲染过，但可能再次刷新）
        if (oppId && gameState.players[oppId]?.board) {
            renderEnemyBoardFromData(oppId, gameState.players[oppId].board);
        }
