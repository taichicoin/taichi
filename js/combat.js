...
window.YYCardCombat = (function() {
    ...
    // 原有的 spawnWeaponOnCard 和 flyWeaponToTarget 不再需要，但保留以防其他地方调用
    ...
    // ★★★ 新攻击动画：翻转0.2s → 显示武器0.5s (第0.3s插入) → 翻转回角色0.2s → 缩小0.1s → 伤害
    function attackAnim(a) {
        return new Promise(async resolve => {
            ...
            if (_3DReady) {
                ...
                // 1. 翻转显示武器 0.2s（无提前变大，翻转后即为 1.25 倍）
