...
window.YYCardCombat = (function() {
        ...
        cardConfigPromise = (async () => {
            try {
                ...
                if (res.ok) {
                    ...
                    // 同步给 3D 模块
                    if (combat3D) combat3D.setCardConfig(cardConfig);
                }
            } catch (e) {}
        })();
    ...
    // rangedAttackAnim（依赖 combat3D.EMOJI_DAGGER_ANGLE_OFFSET）
    ...
    async function resolveBattles(gameState, onComplete) {
        ...
        // 初始化 3D 模块（如果存在）
        if (combat3D && !combat3D.isReady()) {
            await combat3D.init().catch(e => debugLog('3D 初始化异常: ' + e));
        }
