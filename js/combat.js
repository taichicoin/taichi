...
window.YYCardCombat = (function() {
    ...
    const AVG_STEP_MS = 480; // 每步动画平均耗时，用于快进计算
    ...
    // 主入口：调用后端 settle-battle，获取日志并播放
    async function resolveBattles(gameState, log, onComplete) {
        ...
        if (!roomId) { console.error('[Combat] 无房间ID'); onComplete?.(); return; }
        ...
        // 用后端返回的数据更新 gameState（技能增益、血量等已更新）
        ...
        // 刷新 UI（此时 battle.js 的 isAnimPlaying 仍为 true，但回调执行完会置 false 并调 safeRefreshUI）
        // 这里主动刷新一次，确保后端数据已经应用到棋盘
