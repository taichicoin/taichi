...
window.YYCardCombat = (function() {
    ...
    // ★ 剑的朝向修正角度（弧度），根据视觉调整
    const CORRECTION_ANGLE = Math.PI / 4; // 45度，若要反向可改为负值
    ...
    // 生成平放在卡牌上的武器（剑尖方向由 isEnemy 决定，并应用修正旋转）
    function spawnWeaponOnCard(cardEl, isEnemy) {
        ...
        // 基础方向：剑尖朝向 upDir，宽面朝向相机
        const upDir = new THREE.Vector3(0, isEnemy ? -1 : 1, 0);
        const camDir = new THREE.Vector3(0, 0, 1);
        const right = new THREE.Vector3().crossVectors(camDir, upDir).normalize();
        const rotMat = new THREE.Matrix4().makeBasis(right, upDir, camDir);
        weapon.quaternion.setFromRotationMatrix(rotMat);

        // 应用视觉修正旋转：我方逆时针转，敌方顺时针转
        const offsetAngle = isEnemy ? -CORRECTION_ANGLE : CORRECTION_ANGLE;
        weapon.rotateZ(offsetAngle);

        _scene.add(weapon);
        return weapon;
    }

    // 武器飞行（剑尖指向飞行方向，宽面尽量朝相机，钻头旋转）
    function flyWeaponToTarget(weapon, startPos, defEl, isEnemyAttacker) {
        ...
        return new Promise(resolve => {
            ...
            const camDir = new THREE.Vector3(0, 0, 1);
            const right = new THREE.Vector3().crossVectors(camDir, flyDir).normalize();
            const rotMat = new THREE.Matrix4().makeBasis(right, flyDir, camDir);
            weapon.quaternion.setFromRotationMatrix(rotMat);

            // 飞行前也应用修正角度以保持显示一致
            const offsetAngle = isEnemyAttacker ? -CORRECTION_ANGLE : CORRECTION_ANGLE;
            weapon.rotateZ(offsetAngle);
