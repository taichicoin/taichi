...
window.YYCombatUtils = (function() {
    let _combatLogText = '';
    let _panelInsertScheduled = false;   // 防止动画中多次插入
    ...
    // ---------- 调试面板（仅战斗界面内显示，不影响动画）----------
    function _createDebugPanel(battleView) {
        const panel = document.createElement('div');
        panel.id = 'combat-debug-panel';
        // 使用 fixed 定位但挂载在 battle-view 内，确保只随战斗视图显示/隐藏
        panel.style.cssText = `
        ...
        battleView.appendChild(panel);
    }

    function ensureDebugPanel() {
        const battleView = document.getElementById('battle-view');
        const existingPanel = document.getElementById('combat-debug-panel');

        // 不在战斗界面：清理残留面板
        if (!battleView || battleView.style.display === 'none') {
            if (existingPanel) existingPanel.remove();
            _panelInsertScheduled = false;
            return;
        }

        // 已在战斗界面，但面板还没创建（且未安排延迟插入），安排一次安全的插入
        if (!existingPanel && !_panelInsertScheduled) {
            _panelInsertScheduled = true;
            // 用微任务确保不在当前动画帧中插入 DOM，避免影响布局
            Promise.resolve().then(() => {
                // 再次确认条件（防止在这微任务期间退出战斗）
                const bv = document.getElementById('battle-view');
                if (!bv || bv.style.display === 'none') {
                    _panelInsertScheduled = false;
                    return;
                }
                if (!document.getElementById('combat-debug-panel')) {
                    _createDebugPanel(bv);
                }
                _panelInsertScheduled = false;
            });
        }
    ...
    function debugLog(msg) {
        _combatLogText += msg + '\n';
        ensureDebugPanel();   // 只安排插入，不立即操作 DOM
        const c = document.getElementById('combat-debug-content');
        if (!c) return;      // 面板还未创建，日志已缓存，后续显示时会补上历史内容？这里简化：面板创建后历史不会补，但所有日志都通过 _combatLogText 保存，如需补录可复制
