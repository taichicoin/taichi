...
// ★ 调试面板默认隐藏，仅通过 showDebugPanel() 显示，通过 hideDebugPanel() 隐藏
window.YYCombatUtils = (function() {
    let _combatLogText = '';
    let _logBuffer = [];          // 缓存未显示时的日志
    let _panelVisible = false;    // 面板是否已显示
    ...
    // 暴露公共 API，新增 showDebugPanel / hideDebugPanel
    return {
        loadCardConfig,
        getCardDisplay,
        showDebugPanel,       // ★ 进入战斗时调用
        hideDebugPanel,       // ★ 退出战斗时调用
        debugLog,
        clearDebug,
        getSlotElement,
        getCardElement,
