...
window.YYCombatAnimations = (function() {
    ...
    // ★ 连续群体增益合并动画（全体弹出浮动，间隔0.2秒，强制显示0.8秒，0.3秒后放行，音效同步）
    async function mergedMassBuffAnim(stepsList) {
        ...
        // 记录每个位置累积的增益
        const cumulativeMap = new Map(); // key: pos, value: { atk, hp }
        ...
        for (const pos of allPositions) {
            ...
            if (el) {
                ...
                utils.floatingText(
                    el,
                    `+${utils.clampDisplay(total.atk)}+${utils.clampDisplay(total.hp)}`,
                    '#7bffb1',
                    800  // 强制显示 0.8 秒
                );
            }
        ...
        // 开始 0.3 秒后即可进行后续步骤（浮动文字仍会持续显示至 0.8 秒）
        await new Promise(r => setTimeout(r, 300));
    }
