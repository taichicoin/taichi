...
window.YYCombatAnimations = (function() {
    ...
    // ★ 合并群体增益动画
    // 逻辑：N 次同角色群体 buff → N 次快速底部数值增加（0.2s/次）+ 1 次合并浮动数字 + 同步播放 vbuff.wav
    async function mergedMassBuffAnim(stepsList) {
        ...
        // 记录每个位置的总增益
        const totalMap = new Map(); // key: pos, value: { atk, hp }
        const allPositions = new Set();

        // 快速执行 N 次底部数值增加（0.2秒/次），不弹浮动数字，不放音效
        for (const step of stepsList) {
            ...
            for (const pos of posList) {
                ...
                if (!totalMap.has(pos)) {
                    totalMap.set(pos, { atk: 0, hp: 0 });
                }
                const acc = totalMap.get(pos);
