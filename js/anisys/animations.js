// ==================== 动画效果库 (animations.js) ====================
// 所有音效音量已统一调至最大值 1.0
// ★ 修复：增益浮动文字改为 fixed 定位挂载到 body，避免被卡牌容器裁剪
window.YYCombatAnimations = (function() {
    ...
    // ===================== 增益浮动文字 =====================
    function showBuffFloat(el, atkGain, hpGain) {
        if (!el) return;
        const atk = atkGain || 0;
        const hp  = hpGain  || 0;
        if (atk === 0 && hp === 0) return;

        let text, leftPercent;
        if (atk > 0 && hp > 0) {
            text = `+${atk}+${hp}`;
            leftPercent = 50;
        } else if (atk > 0) {
        ...
        // 计算初始位置：卡牌内部 75% 高度处
        const startX = rect.left + rect.width * leftPercent / 100;
        ...
        // 停留 0.7 秒后向上飘动（飘到卡牌顶部约 10% 处）
        setTimeout(() => {
            d.style.top = (rect.top + rect.height * 0.1) + 'px';
    ...
    function buffAnim(buff) {
        return new Promise(async resolve => {
            try {
                if ((buff.tempShieldGain || 0) > 0) {
                    const shieldAudio = new Audio('/assets/wanv/shield.wav');
                    shieldAudio.volume = 1.0;
                    shieldAudio.play();
                } else {
                    const buffAudio = new Audio('/assets/wanv/vup.wav');
                    buffAudio.volume = 1.0;
                    buffAudio.play();
                }
            } catch (e) {}
            ...
            if (totalAtkGain > 0 || totalHpGain > 0) {
                showBuffFloat(el, totalAtkGain, totalHpGain);
            }
    ...
    async function massBuffAnim(step) {
        ...
        for (const { el } of elements) {
            showBuffFloat(el, atkGain, hpGain);
        }
    ...
    // ===================== 合并群体增益 =====================
    async function mergedMassBuffAnim(stepsList) {
        const myId = window.YYCardAuth?.currentUser?.id;
        const firstStep = stepsList[0];
        const isEnemy = firstStep.playerId !== myId;
        ...
        for (const step of stepsList) {
            ...
            for (const pos of posList) {
                const el = await utils.getCardElementRetry(step.playerId, pos, isEnemy);
                if (el) {
                    utils.updateCardStats(el, step.atkGain || 0, step.hpGain || 0);
                }
                if (!cumulativeMap.has(pos)) {
                    cumulativeMap.set(pos, { atk: 0, hp: 0 });
                }
                const acc = cumulativeMap.get(pos);
                acc.atk += step.atkGain || 0;
                acc.hp += step.hpGain || 0;
                allPositions.add(pos);
            }

            try {
                const audio = new Audio('/assets/wanv/vup.wav');
                audio.volume = 1.0;
                audio.play();
            } catch(e) {}
        ...
        try {
            const audio = new Audio('/assets/wanv/vbuff.wav');
            audio.volume = 1.0;
            audio.play();
        } catch(e) {}

        for (const pos of allPositions) {
            const el = await utils.getCardElementRetry(firstStep.playerId, pos, isEnemy);
            if (el) {
                const total = cumulativeMap.get(pos) || { atk: 0, hp: 0 };
                showBuffFloat(el, total.atk, total.hp);
            }
        }
    ...
    function rangedAttackAnim(stepsArray) {
        return new Promise(async resolve => {
            ...
            const isCatapultGroup = stepsArray[0].isCatapult === true;
            const rangedSound = isCatapultGroup ? '/assets/wanv/tsc.wav' : '/assets/wanv/feidao.wav';
            ...
            if (fastMode) {
                for (let i = 0; i < totalSteps; i++) {
                    ...
                    const emojiChar = step.isCatapult ? '🪨' : '🗡️';
                    const emoji = document.createElement('div');
                    emoji.textContent = emojiChar;
                    emoji.style.cssText = `
                        position: fixed; z-index: 1500; font-size: 31px; pointer-events: none;
                    ...
                    if (i === 0 || i % 5 === 0) {
                        try {
                            const hitAudio = new Audio(rangedSound);
                            hitAudio.volume = 1.0;
                            hitAudio.play();
                        } catch (e) {}
                    }
