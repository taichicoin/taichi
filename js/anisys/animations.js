...
window.YYCombatAnimations = (function() {
    ...
    async function mergedMassBuffAnim(stepsList) {
        ...
        for (const step of stepsList) {
            ...
            totalAtk += step.atkGain || 0;
            totalHp += step.hpGain || 0;

            await new Promise(r => setTimeout(r, 100));
        }

        // 取一个代表位置显示合并后的浮动数字
        const repPos = firstStep.targetPositions?.[0] ?? 0;
        const repEl = await utils.getCardElementRetry(firstStep.playerId, repPos, isEnemy);
        if (repEl) {
            utils.floatingText(
                repEl,
                `+${utils.clampDisplay(totalAtk)}+${utils.clampDisplay(totalHp)}`,
                '#7bffb1',
                800
            );
        }
