...
window.YYCardCombat = (function() {
    ...
    async function init3D() {
        ...
        try {
            ...
            _swordGLB.scale.set(0.01, 0.01, 0.01);
    ...
    function spawnWeaponOnCard(cardEl, isEnemy) {
        ...
        // 关键修正：模型剑尖是 Z 轴，宽面是 Y 轴，因此交换 Y 和 Z
        // 局部 X = right, 局部 Y = cameraDir (宽面), 局部 Z = upDir (剑尖)
        const rotMat = new THREE.Matrix4().makeBasis(rightDir, cameraDir, upDir);
