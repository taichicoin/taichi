...
window.YYCardCombat = (function() {
    ...
    // 3D 武器飞行（带旋转钻头效果）
    function attackAnim3D(attEl, defEl) {
        const THREE = _threeModule;
        if (!_3DReady || !_swordGLB || !THREE) return Promise.resolve();

        return new Promise(resolve => {
            const weapon = _swordGLB.clone();
            const start = domToWorld(attEl);
            ...
            const duration = 600;                // 飞行时长（毫秒）
            const startTime = performance.now();
            const rotateSpeed = 0.01;            // 旋转速度（弧度/毫秒），可根据需要调整
            ...
            function fly(now) {
                ...
                weapon.position.lerpVectors(start, end, t);
    ...
    // 应用攻击效果（伤害数字、血条、死亡动画）—— 从原 attackAnim 剥离而来
    ...
    function attackAnim(a) {
        return new Promise(async resolve => {
            ...
            if (_3DReady) {
                ...
                await new Promise(r => setTimeout(r, 400)); // 放大停留时间
                ...
                // 3D 武器飞行
                await attackAnim3D(attEl, defEl);
                if (abortFlag) return resolve();
