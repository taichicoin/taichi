...
window.YYCardShopRender = (function() {
    ...
    function renderShop() {
        ...
        // ★ 读取当前玩家的商店基础加成
        const bonusAtk = my.shopBonus?.atk || 0;
        const bonusHp  = my.shopBonus?.hp  || 0;
        ...
        for (let i = start; i < start + 3; i++) {
            ...
            if (render.isValidCard(card)) {
                ...
                // ★ 对角色牌（非武器/道具）叠加 shopBonus，避免修改原始数据
                let displayCard = card;
                if (card.type !== 'weapon' && card.type !== 'item' && (bonusAtk > 0 || bonusHp > 0)) {
                    displayCard = {
                        ...card,
    ...
    function getShopDisplayCards(player) {
        ...
        if (shop?.buffer && Array.isArray(shop.buffer)) {
            ...
            if (Array.isArray(group) && group.length >= 30) {
                const start = sub * 3;
                const bonusAtk = player.shopBonus?.atk || 0;
                const bonusHp  = player.shopBonus?.hp  || 0;
