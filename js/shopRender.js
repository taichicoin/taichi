...
window.YYCardShopRender = (function() {
    ...
    function injectShopStyles() {
        ...
        style.textContent = `
            .card[data-card-type="shop"] .card-stats { position: absolute !important; top: 65% !important; left: 0 !important; width: 100% !important; height: 0 !important; z-index: 2; pointer-events: none; }
            .card[data-card-type="shop"] .card-atk { position: absolute; left: 6%; top: 65%; transform: translateY(-50%); color: #ff4d4d; font-weight: bold; text-shadow: 0 0 4px #000; font-size: 1.1em; }
            .card[data-card-type="shop"] .card-hp { position: absolute; right: 6%; top: 65%; transform: translateY(-50%); color: #4dff4d; font-weight: bold; text-shadow: 0 0 4px #000; font-size: 1.1em; }
            .card-skill-desc { position: absolute; bottom: -3%; left: 5%; width: 90%; height: 30%; background: rgba(0, 0, 0, 0.75); color: #fff; font-weight: bold; display: flex; align-items: center; justify-content: flex-start; text-align: left; padding: 2px 4px; box-sizing: border-box; z-index: 1; border-radius: 0 0 4px 4px; line-height: 1.2; text-shadow: 1px 1px 2px black; pointer-events: none; word-break: break-word; overflow: hidden; }
            .card[data-card-type="shop"] .card-shield { z-index: 3; }
        `;
    ...
    function calcSkillFontSize(text) {
        ...
        if (len <= 5) return '0.8em'; if (len <= 6) return '0.75em'; if (len <= 7) return '0.7em';
        if (len <= 8) return '0.65em'; if (len <= 9) return '0.65em'; if (len <= 10) return '0.65em';
        if (len <= 15) return '0.6em'; if (len <= 20) return '0.55em'; if (len <= 25) return '0.5em';
        return '0.5em';
    }
    ...
    // ★ 渲染函数 —— 通过构造临时对象，确保 createCardElement 接收到加成后的数据
    function renderShop() {
        ...
        for (let i = start; i < start + 3; i++) {
            ...
            if (render.isValidCard(original)) {
                ...
                if (isCharacterCard(original) && (bonusAtk > 0 || bonusHp > 0)) {
                    // 取原始攻防（兼容字段差异）
                    const baseAtk = original.atk !== undefined ? original.atk : (original.base_atk || 0);
                    const baseHp  = original.hp  !== undefined ? original.hp  : (original.base_hp  || 0);

                    // 建立新对象，覆盖 atk/hp/base_atk/base_hp
                    displayCard = {
                        ...original,
                        atk: baseAtk + bonusAtk,
                        hp:  baseHp  + bonusHp,
                        base_atk: (original.base_atk || 0) + bonusAtk,
                        base_hp:  (original.base_hp  || 0) + bonusHp
                    };
                }
    ...
    // getShopDisplayCards 对外提供商店卡牌数据时也要叠加加成（姜子牙等会用到）
    function getShopDisplayCards(player) {
        ...
        return group.slice(start, start + 3).filter(render.isValidCard).map(card => {
            ...
            return {
                ...
                atk: baseAtk + bonusAtk,
                hp:  baseHp  + bonusHp,
                base_atk: (card.base_atk || 0) + bonusAtk,
                base_hp:  (card.base_hp  || 0) + bonusHp
            };
        });
    }
