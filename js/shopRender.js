...
window.YYCardShopRender = (function() {
    ...
    function injectShopStyles() {
        ...
        style.textContent = `
            .card[data-card-type="shop"] .card-stats {
                position: absolute !important; top: 65% !important;
                left: 0 !important; width: 100% !important; height: 0 !important;
                z-index: 2; pointer-events: none;
            }
            .card[data-card-type="shop"] .card-atk {
                position: absolute; left: 6%; top: 65%;
                transform: translateY(-50%); color: #ff4d4d;
                font-weight: bold; text-shadow: 0 0 4px #000; font-size: 1.1em;
            }
            .card[data-card-type="shop"] .card-hp {
                position: absolute; right: 6%; top: 65%;
                transform: translateY(-50%); color: #4dff4d;
                font-weight: bold; text-shadow: 0 0 4px #000; font-size: 1.1em;
            }
            .card-skill-desc {
                position: absolute; bottom: -3%; left: 5%; width: 90%; height: 30%;
                background: rgba(0, 0, 0, 0.75); color: #fff; font-weight: bold;
                display: flex; align-items: center; justify-content: flex-start;
                text-align: left; padding: 2px 4px; box-sizing: border-box;
                z-index: 1; border-radius: 0 0 4px 4px; line-height: 1.2;
    ...
    function calcSkillFontSize(text) {
        ...
        if (len <= 5) return '0.8em'; if (len <= 6) return '0.75em';
        if (len <= 7) return '0.7em'; if (len <= 8) return '0.65em';
        if (len <= 9) return '0.65em'; if (len <= 10) return '0.65em';
        if (len <= 15) return '0.6em'; if (len <= 20) return '0.55em';
        if (len <= 25) return '0.5em'; return '0.5em';
    }
    ...
    // ★ 渲染主函数：从玩家状态读取 shopBonus 并叠加到角色牌
    function renderShop() {
        ...
        for (let i = start; i < start + 3; i++) {
            ...
            addSkillDesc(el, original);      // 技能描述仍用原始数据
    ...
    // getShopDisplayCards 同样需要叠加加成（供姜子牙等外部使用）
