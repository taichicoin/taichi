...
window.YYCardShopRender = (function() {
    ...
    function renderShop() {
        ...
        // ★★★ 统一修改 DOM：遍历所有商店卡牌，强行覆盖数值 ★★★
        if (bonusAtk > 0 || bonusHp > 0) {
            const allShopCards = container.querySelectorAll('.card[data-card-type="shop"]');
            allShopCards.forEach(el => {
                const index = parseInt(el.getAttribute('data-shop-index'), 10);
                const originalCard = group[index];
                if (originalCard && isCharacterCard(originalCard)) {
                    const atkSpan = el.querySelector('.card-atk');
                    const hpSpan  = el.querySelector('.card-hp');
                    if (atkSpan) {
                        const baseAtk = originalCard.atk !== undefined ? originalCard.atk : (originalCard.base_atk || 0);
                        atkSpan.textContent = baseAtk + bonusAtk;
                    }
                    if (hpSpan) {
                        const baseHp = originalCard.hp !== undefined ? originalCard.hp : (originalCard.base_hp || 0);
                        hpSpan.textContent = baseHp + bonusHp;
                    }
                }
            });
        }
