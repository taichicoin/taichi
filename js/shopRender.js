...
window.YYCardShopRender = (function() {
    ...
    function isCharacter(card) {
        ...
        const t = card.type;
        if (t === 'weapon' || t === 'item' || t === 'consumable') return false;
    ...
    function renderShop() {
        ...
        for (let i = start; i < start + 3; i++) {
            ...
            // 构建最终用于渲染的卡牌对象（确保 atk/hp 字段存在且值为加后值）
            let displayCard = { ...original };
            if (isCharacter(displayCard) && (bonusAtk > 0 || bonusHp > 0)) {
                // 取原始攻防值
                const rawAtk = displayCard.atk !== undefined ? displayCard.atk : (displayCard.base_atk || 0);
                const rawHp  = displayCard.hp  !== undefined ? displayCard.hp  : (displayCard.base_hp  || 0);
                displayCard.atk = rawAtk + bonusAtk;
                displayCard.hp  = rawHp  + bonusHp;
                // 同时覆盖 base 字段，避免内部逻辑优先读取 base 值
                displayCard.base_atk = (displayCard.base_atk || 0) + bonusAtk;
                displayCard.base_hp  = (displayCard.base_hp  || 0) + bonusHp;
            }
    ...
    function getShopDisplayCards(player) {
        ...
        return group.slice(start, start + 3).filter(render.isValidCard).map(card => {
            if (!isCharacter(card)) return { ...card };
            const rawAtk = card.atk !== undefined ? card.atk : (card.base_atk || 0);
            const rawHp  = card.hp  !== undefined ? card.hp  : (card.base_hp  || 0);
            return {
                ...card,
                atk: rawAtk + bonusAtk,
                hp:  rawHp  + bonusHp,
