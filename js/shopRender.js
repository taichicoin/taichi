// ==================== 商店渲染子模块（内置价格表，叠加shopBonus到角色牌数值） ====================
window.YYCardShopRender = (function() {
    ...
    // ★ 核心改动：直接在这里读 shopBonus 并叠加到 atk/hp/base_atk/base_hp
    function renderShop() {
        ...
        for (let i = start; i < start + 3; i++) {
            ...
            if (isCharacterCard(original) && (bonusAtk > 0 || bonusHp > 0)) {
                displayCard.atk = (original.atk || original.base_atk || 0) + bonusAtk;
                displayCard.hp  = (original.hp  || original.base_hp  || 0) + bonusHp;
                // 同时覆盖 base 字段，防止 createCardElement 内部优先使用 base 值
                displayCard.base_atk = (original.base_atk || 0) + bonusAtk;
                displayCard.base_hp  = (original.base_hp  || 0) + bonusHp;
            }
    ...
    function getShopDisplayCards(player) {
        ...
        return group.slice(start, start + 3).filter(render.isValidCard).map(card => {
            if (!isCharacterCard(card) || (bonusAtk === 0 && bonusHp === 0)) return { ...card };
            return {
                ...card,
                atk: (card.atk || card.base_atk || 0) + bonusAtk,
                hp:  (card.hp  || card.base_hp  || 0) + bonusHp,
