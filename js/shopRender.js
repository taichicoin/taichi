// ==================== 商店渲染子模块（内置价格表，仅商店显示价格，显示时自动附加 shopBonus） ====================
    ...
    function renderShop() {
        ...
        for (let i = start; i < start + 3; i++) {
            ...
            if (render.isValidCard(card)) {
                hasAnyCard = true;
                // ★ 创建显示用的卡牌对象，附加商店加成
                const displayCard = Object.assign({}, card);
                displayCard.atk = (Number(card.atk) || 0) + bonusAtk;
                displayCard.hp  = (Number(card.hp)  || 0) + bonusHp;
                ...
                // 追加自适应技能描述
                addSkillDesc(el, card);           // 技能描述仍用原始 card
                // ★ 追加价格标签（使用内置价格表）
