...
window.YYCardShopRender = (function() {
    ...
    function renderShop() {
        ...
        for (let i = start; i < start + 3; i++) {
            ...
            // 构建用于渲染的卡牌数据（关键：显式设置 atk/hp，覆盖可能的 base_atk/base_hp）
            let displayCard = {
                ...original,
                atk: (original.atk !== undefined ? original.atk : (original.base_atk || 0)) + (canApplyShopBonus(original) ? bonusAtk : 0),
                hp:  (original.hp  !== undefined ? original.hp  : (original.base_hp  || 0)) + (canApplyShopBonus(original) ? bonusHp  : 0)
            };
    ...
    function getShopDisplayCards(player) {
        ...
        return group.slice(start, start + 3).filter(render.isValidCard).map(card => ({
            ...card,
            atk: (card.atk !== undefined ? card.atk : (card.base_atk || 0)) + (canApplyShopBonus(card) ? bonusAtk : 0),
            hp:  (card.hp  !== undefined ? card.hp  : (card.base_hp  || 0)) + (canApplyShopBonus(card) ? bonusHp  : 0)
        }));
    }
