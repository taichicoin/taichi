...
window.YYCardShopRender = (function() {
    ...
    function renderShop() {
        ...
        for (let i = start; i < start + 3; i++) {
            ...
            // 直接使用原始数据渲染，不加任何修改
            const el = render.createCardElement(original, 'shop');
            el.setAttribute('data-shop-index', i);
            el.setAttribute('data-card-type', 'shop');

            // ★ 如果是角色牌且有加成，直接改DOM里的数字
            if (isCharacterCard(original) && (bonusAtk > 0 || bonusHp > 0)) {
                const atkEl = el.querySelector('.card-atk');
                const hpEl  = el.querySelector('.card-hp');
                if (atkEl) {
                    const curAtk = parseInt(atkEl.textContent, 10) || 0;
                    atkEl.textContent = curAtk + bonusAtk;
                }
                if (hpEl) {
                    const curHp = parseInt(hpEl.textContent, 10) || 0;
                    hpEl.textContent = curHp + bonusHp;
                }
            }
    ...
    function getShopDisplayCards(player) {
        ...
        return group.slice(start, start + 3).filter(render.isValidCard).map(card => ({
            ...card,
            atk: (card.atk || card.base_atk || 0) + (isCharacterCard(card) ? bonusAtk : 0),
            hp:  (card.hp  || card.base_hp  || 0) + (isCharacterCard(card) ? bonusHp  : 0)
        }));
    }
