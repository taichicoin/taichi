...
window.YYCardShopRender = (function() {
    ...
    function renderShop() {
        ...
        for (let i = start; i < start + 3; i++) {
            ...
            // 如果是角色牌且有加成，直接改已生成的DOM数值
            if (isCharacterCard(card) && (bonusAtk > 0 || bonusHp > 0)) {
                const atkSpan = el.querySelector('.card-atk');
                const hpSpan  = el.querySelector('.card-hp');
                if (atkSpan) {
                    const curAtk = parseInt(atkSpan.textContent, 10) || 0;
                    atkSpan.textContent = curAtk + bonusAtk;
                }
                if (hpSpan) {
                    const curHp = parseInt(hpSpan.textContent, 10) || 0;
                    hpSpan.textContent = curHp + bonusHp;
                }
            }
    ...
    function getShopDisplayCards(player) {
        ...
        return group.slice(start, start + 3).filter(render.isValidCard).map(card => {
            ...
            return {
                ...card,
                atk: isCharacterCard(card) ? baseAtk + bonusAtk : baseAtk,
                hp:  isCharacterCard(card) ? baseHp  + bonusHp  : baseHp
            };
        });
    }
