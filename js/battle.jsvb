...
window.YYCardBattle = (function() {
    ...
    async function launchBattleAnimation(isReconnect = false) {
        ...
        try {
            // ★ 强制刷新最新全局状态，确保 round 最新
            await refreshGameState();

            // 重连检测：如果全局已结算过，直接跳过动画
            if (isReconnect || !isReconnect) { // 统一检查
                ...
                if (freshGlobal?.state) {
                    ...
                    const currentRound = freshGlobal.state.round || 1;
                    if (lastSettled >= currentRound) {
                        console.log('⚡ 重连或切换时发现已结算，跳过动画');
                        isAnimPlaying = false;
