...
window.YYCardBattle = (function() {
    ...
    function renderBoard(containerId, cards, isSelf) {
        ...
        if (!cont) return;
        cont.innerHTML = '';
        // 容器类名是 "board"，里面每个格子用 "card-slot"
        for (let i=0; i<6; i++) {
            const c = cards[i];
            const slot = document.createElement('div');
            slot.className = 'card-slot';   // 与HTML/CSS匹配
            if (c) {
...
console.log('✅ battle.js 加载完成（UI类名匹配HTML）');
