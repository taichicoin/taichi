...
window.YYCardConsumable = (function() {
    ...
    function forceRenderHandDirectly() {
        ...
        if (!handContainer) {
            console.warn('手牌容器 #hand-container 不存在');
            return;
        }
        ...
        const my = gs?.players?.[uid];
        if (!my || !my.hand) {
            handContainer.innerHTML = '<div style="color:#888;text-align:center;padding:20px;font-size:0.9rem;">手牌为空</div>';
            return;
        }
    ...
    function init(deps) {
        ...
        console.log('✅ consumable.js 已初始化 (最终修复版)');
    }
