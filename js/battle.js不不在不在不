...
window.YYCardBattle = (function() {
    ...
    function startPhaseTimer(phase, duration, skipStateUpdate = false) {
        ...
        if (!duration || duration <= 0) {
            duration = getPhaseDuration(phase, gameState?.round || 1);
            log(`⚠️ 计时器时长异常，已重置为 ${duration}秒`, true);
        }
...
console.log('✅ battle.js 加载完成【根治版状态机锁】');
