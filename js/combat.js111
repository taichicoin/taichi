// ==================== 战斗模拟模块（3D 武器攻击版 · 受击音效 · 遗言飞刀 · 刀尖朝向修复 · 曹操全体护盾优化 · 遗言金币处理） ====================
    ...
    async function playSteps(steps, myId, oppId) {
        ...
        for (let i = 0; i < steps.length; i++) {
            ...
            if (step.goldGain !== undefined && step.goldGain !== 0) {
                ...
                // ★ 纯金币事件（type: gold）跳过后续动画
                if (step.type === 'gold') {
                    debugLog(`  💰 遗言金币 #${idx}: ${step.sourceCard} +${step.goldGain}`);
                    continue;
                }
                // buff 类型继续执行动画（如击杀加金币同时可能有属性增益）
                // （原有的 if (step.type !== 'buff') continue; 已删除）
            }
