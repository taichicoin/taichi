// ==================== 世界杯预测子模块（含倒计时、赔率、双语、排序、upcoming禁用） ====================
window.YYCardHotBet_worldcup = (() => {
  ...
  async function loadWorldcupEvents() {
    try {
      const { data: events, error } = await supabase
        ...
        .order('start_time', { ascending: true }); // 原始时间排序
      ...
      // 先映射数据，再根据状态排序： live > upcoming > ended
      ...
      // 自定义排序：live 优先，然后 upcoming，最后 ended
  ...
  async function render(containerDiv, context) {
    ...
    containerDiv.querySelectorAll('.hotbet-option-bet-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        // 查找对应的事件和选项，检查是否是 upcoming 状态
        const event = events.find(ev => ev.id === eventId);
        if (event && event.status === 'upcoming') {
          // 显示 Toast 提示（使用 alert 代替，后续可改为自定义 Toast）
  ...
  function renderEventCard(event) {
    ...
    // 按钮禁用条件：已结束、已截止、或者状态为 upcoming（下注未开始）
    const disableBet = event.status === 'ended' || event.status === 'upcoming' || isBetOver;
