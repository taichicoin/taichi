// ==================== 世界杯预测子模块（含倒计时、赔率、双语、排序、按钮修正、事件/选项中英） ====================
window.YYCardHotBet_worldcup = (() => {
  ...
  function renderEventCard(event) {
    ...
    // 按钮禁用条件：非 live 状态或已过截止时间
    const disableBet = event.status !== 'live' || isBetOver;

    // 按钮文字
    let betBtnText = _t('event_bet_btn'); // 默认“下注”
    if (event.status === 'upcoming') {
      betBtnText = _t('event_bet_upcoming');   // “即将开始”
    } else if (event.status === 'ended' || isBetOver) {
      betBtnText = _t('event_bet_end');        // “已截止”
    }
