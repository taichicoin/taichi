// ==================== 加密货币预测子模块 (crypto.js) ====================
window.YYCardHotBet_crypto = (() => {
  const supabase = window.supabase;
  const _t = window._t || ((key) => key);
  let countdownTimer = null;

  // 根据当前语言选择中文或英文字段，若英文缺失则回退中文
  const _localText = window._localText || ((zhText, enText) => {
    const lang = (window.YYCardHotBetLang?.getLang?.() || window.YYCardLang?.getLang?.() || 'zh');
      ...
      const mapped = events.map(event => ({
        ...
        options: (options || [])
          ...
          .map(opt => ({
            ...
            name_en: opt.option_name_en,   // 英文选项名
            total_bet: (opt.total_bet != null ? opt.total_bet : (optionBets[opt.id] || 0))
          }))
      }));

      const statusOrder = { live: 1, upcoming: 2, ended: 3 };
      mapped.sort((a, b) => (statusOrder[a.status] || 4) - (statusOrder[b.status] || 4));
      return mapped;
    } catch (e) {
      console.error('加载加密货币事件失败:', e);
      return [];
    }
  }
  ...
  function updateCountdowns() {
    document.querySelectorAll('.bet-countdown').forEach(el => {
  ...
  async function render(containerDiv, context) {
    ...
    const events = await loadCryptoEvents();
  ...
  function renderEventCard(event) {
    ...
    const disableBet = event.status !== 'live' || isBetOver;

    let betBtnText = _t('event_bet_btn');
    if (event.status === 'upcoming') {
      betBtnText = _t('event_bet_upcoming');
    } else if (event.status === 'ended' || isBetOver) {
      betBtnText = _t('event_bet_end');
    }
      ...
      event.options.forEach(opt => {
        ...
        optionsHtml += `
          <div class="hotbet-option">
            <span class="hotbet-option-name">${_localText(opt.name, opt.name_en)}</span>
            ...
            <div style="display:flex;align-items:center;gap:8px;">
              <button class="hotbet-option-bet-btn"
                      ...
                      ${disableBet ? 'disabled' : ''}>${betBtnText}</button>
            </div>
          </div>
        `;
      });
      ...
      <div class="hotbet-event-card">
        <div style="display: flex; justify-content: space-between; align-items: flex-start;">
          <div class="hotbet-event-title" style="margin-bottom: 4px;">
            <span>${_localText(event.title, event.title_en)}</span>
          </div>
