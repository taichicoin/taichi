...
window.YYCardHotBet_mybets = (() => {
  ...
  async function loadMyBets(user, filterStatus) {
    // 查询时带上英文字段 title_en, option_name_en
    let query = supabase
      .from('hotbet_bets')
      .select(`
        id,
        ...
        event_id,
        option_id,
        hotbet_events!inner ( title, title_en, status ),
        hotbet_options!inner ( option_name, option_name_en )
      `)
      .eq('user_id', user.id)
  ...
  async function render(containerDiv, context) {
    ...
    const renderList = async () => {
      ...
      if (bets.length === 0) {
        ...
        bets.forEach(bet => {
          const eventTitle = _localText(
            bet.hotbet_events?.title || _t('mybets_unknown_event'),
            bet.hotbet_events?.title_en
          );
          const optionName = _localText(
            bet.hotbet_options?.option_name || _t('mybets_unknown_option'),
            bet.hotbet_options?.option_name_en
          );
