...
(function() {
  ...
  async function computeAddressByEmail(email) {
    ...
    throw new Error(t('all_nodes_unavailable', '所有RPC节点不可用，请稍后重试'));
  }
  ...
  async function getDepositAddress(uuid) {
    ...
    if (!coreInfo || !coreInfo.email || !coreInfo.deposit_address) {
      console.log(t('first_generation', '首次生成充值地址，调用后端...'));
      ...
      if (!coreInfo || !coreInfo.email || !coreInfo.deposit_address) {
        throw new Error(t('generation_success_but_db_not_updated', '后端生成成功但数据库未更新'));
      }
    }
    ...
    if (computedAddr.toLowerCase() !== coreInfo.deposit_address.toLowerCase()) {
      throw new Error(t('address_mismatch', '充值地址验证失败，数据库记录与合约计算不一致'));
    }
  ...
  function showModal(addr) {
    ...
    overlay.querySelector('#copy-addr-btn').onclick = () => {
      ...
      alert(t('address_copied', '地址已复制（仅限 BSC 链）'));
    };
  }
  ...
  function showErrorModal(message) {
    ...
    overlay.innerHTML = `
      <div class="deposit-modal" style="background:#450a0a; color:#fecaca;">
        <h3>❌ ${t('deposit_get_failed', '充值地址获取失败')}</h3>
        <p style="white-space: pre-wrap; text-align: left; font-size: 13px;">${message}</p>
        <button class="close-deposit" style="background:#ef4444; margin-top: 10px;">${t('close', '关闭')}</button>
      </div>
    `;
    document.body.appendChild(overlay);
    overlay.querySelector('.close-deposit').onclick = () => overlay.remove();
  ...
  window.openDepositPopup = async function() {
    ...
    if (!user?.id) {
      showErrorModal(t('please_login', '请先登录'));
      return;
    }
    if (!ready) {
      showErrorModal(t('system_initializing', '系统初始化中，请稍后点击'));
      return;
    }
    ...
    try {
      ...
      showErrorModal(e.message || t('unknown_error', '未知错误'));
    }
  };
