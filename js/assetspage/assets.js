    ...
    try {
      ...
      html += `
        <div class="assets-actions" style="display:flex; gap:8px; flex-wrap:wrap; justify-content:center;">
          <button class="assets-btn assets-btn-deposit" onclick="window.openDepositPopup()">${t('deposit', '充币')}</button>
          <button class="assets-btn assets-btn-withdraw" onclick="window.showWithdrawPopup()">${t('withdraw', '提币')}</button>
          <button class="assets-btn" style="background:#f59e0b; color:white;" onclick="window.showSelfDeposit()">${t('self_deposit', '手动上账')}</button>
          <button class="assets-btn" style="background:#e2e8f0; color:#1e293b; border:1px solid #cbd5e1;" onclick="window.showBillModal()">${t('bill', '账单')}</button>
        </div>
      `;
