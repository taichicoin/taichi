// js/assets.js (支持中英双语，所有静态中文已替换为翻译函数)
(function() {
  ...
  async function fetchBalances(userId) {
    ...
    try {
      ...
      clearTimeout(timer);
      if (e.name === 'AbortError') return { error: t('network_timeout', '请求超时，请检查网络后重试') };
      return { error: t('network_error', '网络请求失败') + ': ' + (e.message || '') };
    }
  }
  ...
  async function render() {
    ...
    try {
      // 确保语言包已初始化（如果有）
      if (window.YYCardAssetsLang && window.YYCardAssetsLang.init) {
        await window.YYCardAssetsLang.init();
      }

      const user = window.YYCardAuth?.currentUser;
      const userId = user?.id;
      if (!userId) {
        container.innerHTML = `<div style="padding:20px; text-align:center; color:#666;">${t('please_login', '请先登录')}</div>`;
        return;
      }
      ...
      container.innerHTML = `<div style="text-align:center; color:#666; padding-top:20px;">${t('loading', '加载中...')}</div>`;
      ...
      try {
        ...
        result = { error: t('render_error', '渲染异常') + ': ' + (e.message || '') };
      }
      ...
      html += `<div class="assets-title">${t('assets_title', '资产')}</div>`;

      // ★ 统一黄色按钮样式
      html += `
        <div class="assets-actions" style="display:flex; gap:8px; flex-wrap:wrap; justify-content:center;">
          <button class="assets-btn" style="background:#f59e0b; color:white; cursor:pointer;" onclick="window.openDepositPopup()">${t('deposit', '充币')}</button>
          <button class="assets-btn" style="background:#f59e0b; color:white; cursor:pointer;" onclick="window.showWithdrawPopup()">${t('withdraw', '提币')}</button>
          <button class="assets-btn" style="background:#f59e0b; color:white;" onclick="window.showSelfDeposit()">${t('self_deposit', '手动上账')}</button>
          <button class="assets-btn" style="background:#f59e0b; color:white;" onclick="window.showBillModal()">${t('bill', '账单')}</button>
        </div>
      `;
      ...
      if (isError) {
        html += `
          <div style="margin-top: 30px; padding: 16px; background: #fff3cd; border: 1px solid #ffc107; border-radius: 8px; font-size: 14px; color: #856404; word-break: break-all;">
            <strong>⚠️ ${t('balance_load_error', '余额加载失败')}</strong><br>
            ${result.error}
          </div>
        `;
      }
      ...
      if (watchStatus.watching) {
        statusHtml = `<div style="margin-top:20px;padding:8px 16px;background:#d4edda;border-radius:8px;font-size:12px;color:#155724;">🟢 ${t('watch_active', '自动监听已开启')}</div>`;
      } else if (watchStatus.error) {
        statusHtml = `<div style="margin-top:20px;padding:8px 16px;background:#f8d7da;border-radius:8px;font-size:12px;color:#721c24;">🔴 ${t('watch_inactive', '监听未启动')}: ${watchStatus.error}</div>`;
      } else {
        statusHtml = `<div style="margin-top:20px;padding:8px 16px;background:#fff3cd;border-radius:8px;font-size:12px;color:#856404;">⏳ ${t('watch_init', '监听初始化中...')}</div>`;
      }
      html += statusHtml;
      ...
      container.innerHTML = `
        <div style="padding:20px; text-align:center; color:#a00;">
          ❌ ${t('page_crash', '资产页面渲染失败')}: ${globalError.message || ''}
        </div>
      `;
    }
  }
