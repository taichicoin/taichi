// /js/assetspage/lang.js —— 资产页面独立语言包（最终完整版）
(function () {
  ...
  const translations = {
    ...
    system_initializing: { zh: '系统初始化中，请稍后点击', en: 'System initializing, please try again later' },
    ...
    // ========== 充值弹窗模块 (getdepositaddr.js) ==========
    first_generation: { zh: '首次生成充值地址，调用后端...', en: 'First time generating deposit address, calling backend...' },
    generation_success_but_db_not_updated: { zh: '后端生成成功但数据库未更新', en: 'Backend generation successful but database not updated' },
    address_mismatch: { zh: '充值地址验证失败，数据库记录与合约计算不一致', en: 'Deposit address verification failed, database record does not match contract computation' },
    deposit_modal_title: { zh: '充值 (BSC 测试网)', en: 'Deposit (BSC Testnet)' },
    deposit_modal_desc: { zh: '向此地址转币，系统自动加余额', en: 'Transfer tokens to this address, balance will be credited automatically' },
    deposit_note: { zh: '仅限 BSC 链，跨链转账无法找回', en: 'Only for BSC chain, cross-chain transfers cannot be recovered' },
    address_copied: { zh: '地址已复制（仅限 BSC 链）', en: 'Address copied (BSC chain only)' },
    deposit_get_failed: { zh: '充值地址获取失败', en: 'Failed to get deposit address' }
  };
