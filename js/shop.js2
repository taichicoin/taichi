...
window.YYCardShop = (function() {
    ...
    function renderHand() {
        ...
        my.hand.forEach((card, i) => {
            if (isValidCard(card)) {
                ...
                // ★ loot 牌和消耗牌一样，不绑定商店的拖拽事件？loot 牌需要拖拽，所以保留
    ...
    function createCardElement(card, cardType = 'board', isBoard = false) {
        ...
        // ★ 消耗牌保留原样（因为消耗牌通常没有带边框的抠图，需要用纯色+文字）
        if (card.type === 'consumable' || card.isConsumable) {
        ...
        // ========== ★ 普通卡牌（武器、道具、角色、Loot）全部直出图片 ==========
        const display = getCardDisplay(card);
        const imgPath = display.image;

        // 【重点】强制清除所有 HTML 和 CSS 带来的边框/背景/阴影
        d.style.cssText = `
            ...
            border-radius: 0 !important; /* 去掉CSS自带的圆角，完全信任你抠的图的边缘 */
        `;

        // ★【重点】将 contain 改为 fill，让图片完全撑满整个格子，不留下任何空隙！
        d.innerHTML = `
            <img src="${imgPath}" 
                 alt="${display.name}" 
    ...
    function onDragStart(e, type, card, index, element) {
        ...
        dragState = {
            ...
            pointerId: e.pointerId   // ★ 记录 pointerId
        };

        // ★★★ loot 牌拖拽播放 use.mp3，否则保持原有 grab 事件 ★★★
    ...
    function onDragEnd(e) {
        // ★ 只处理发起拖拽的那个指针（pointerup/pointercancel 都可能触发）
        if (!dragState.active || e.pointerId !== dragState.pointerId) return;
        e.preventDefault();
