...
window.YYCardProfileUI = {
    ...
    // 【核心修复】使用事件委托监听整个页面，100%能点开头像
    bindEvents() {
        this._injectStyles();

        // 只要页面有点击动作，我们直接通过 e.target 判断点到了头像没
        document.addEventListener('click', (e) => {
            const avatar = e.target.closest('#lobby-avatar');
            if (avatar) {
                e.stopPropagation();
    ...
    _showUsernameModal({ isFirstTime = false }) {
        ...
        if (isFirstTime) subtitle = '🔔 首次登录，请设置您的专属游戏ID';
        else if (!cooldown.canChange && !hasCard) { canChange = false; subtitle = `修改冷却中（剩余 ${cooldown.remainingDays} 天），暂无改名卡`; }
        else if (!cooldown.canChange && hasCard) { needCard = true; subtitle = `冷却中（剩余 ${cooldown.remainingDays} 天），本次将消耗 1 张改名卡`; }
        else subtitle = '修改后将重置冷却时间为 1 年';
...
console.log('✅ 全局事件委托修复版加载！头像100%可点');
