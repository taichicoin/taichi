...
window.YYCardProfileUI = {
    ...
    _toggleDropdown(avatarEl) {
        ...
        const items = [
            ...
            { action: 'change-avatar',   label: L.t('menu_change_avatar') }
        ];
        if (!isTelegram) {
    ...
    async _uploadAvatar(file) {
        ...
        if (!cooldown.canChange) { alert(L.t('avatar_cooldown_err')); return; }

        const ext = file.name.split('.').pop();
        const path = `${auth.currentUser.id}_${Date.now()}.${ext}`;
        const { error: upErr } = await supabase.storage.from('avatars').upload(path, file, { upsert: true });
        if (upErr) { alert(L.t('avatar_upload_fail', { message: upErr.message })); return; }

        const { data: urlData } = supabase.storage.from('avatars').getPublicUrl(path);
        await this._updateProfileField('avatar_url', urlData.publicUrl, L.t('avatar_update_success'));
