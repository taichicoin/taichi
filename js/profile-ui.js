...
window.YYCardProfileUI = {
    ...
    _toggleDropdown(avatarEl) {
        ...
        const items = [
            ...
            { action: 'show-email',      label: L.t('menu_show_email') }   // 替换原来的修改头像
        ];
        if (!isTelegram) {
        ...
        box.innerHTML = items.map(item =>
            `<div class="yy-dropdown-item" data-action="${item.action}">${item.label}</div>`
        ).join('');
        ...
        box.querySelector('[data-action="show-email"]').onclick = () => { this._handleShowEmail(); overlay.remove(); this._dropdown = null; };
        if (!isTelegram) {
            box.querySelector('[data-action="logout"]').onclick = async () => {
    ...
    _handleShowEmail() {
        const L = this.L;
        const auth = window.YYCardAuth;
        const email = auth.currentUser?.email || auth.currentProfile?.email || '';
        if (email) {
            if (navigator.clipboard && navigator.clipboard.writeText) {
                navigator.clipboard.writeText(email).then(() => {
                    alert(L.t('email_copied') + ': ' + email);
                }).catch(() => {
                    prompt(L.t('email_copy_manually'), email);
                });
            } else {
                prompt(L.t('email_copy_manually'), email);
            }
        } else {
            alert(L.t('email_not_available'));
        }
    },
    ...
    // 移除 _onChangeAvatar, _uploadAvatar 方法
...
console.log('✅ 头像菜单已双语化，头像修改已替换为邮箱显示');
