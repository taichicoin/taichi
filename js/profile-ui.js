...
window.YYCardProfileUI = {
    ...
    _toggleDropdown(avatarEl) {
        ...
        const L = this.L;
        const profile = window.YYCardAuth.currentProfile;  // ★ 获取当前档案
        ...
        // ★ 构建菜单项（增加邮箱展示）
        const items = [];
        // 如果有 google_email，则在最上方显示（只读）
        if (profile.google_email) {
            items.push({
                action: null,
                label: '📧 ' + profile.google_email,
                className: 'email-info'  // 用于应用特殊样式
            });
        }
        items.push(
...
console.log('✅ 头像菜单已更新：无修改头像功能，新增Google邮箱显示');
