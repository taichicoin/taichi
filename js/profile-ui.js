...
window.YYCardProfileUI = {
    ...
    _injectStyles() {
        ...
        s.textContent = `
            ...
            .yy-modal-box {
                background: #1a1a1e; border-radius: 0px !important; /* 强制定位直角 */
    ...
    update() {
        ...
        // 覆盖旧版按钮，使其不作响应（功能整合进头像菜单）
        this._disableLegacyButtons();
    ...
    // 隐藏/覆盖旧版大厅顶部独立按钮（⭐核心优化：保留占位，防止 Flex 布局错乱）
    _disableLegacyButtons() {
        const oldBtns = ['logout-btn', 'set-username-btn', 'bind-wallet-btn', 'change-avatar-btn'];
        oldBtns.forEach(id => {
            const el = document.getElementById(id);
            if (el) {
                el.onclick = (e) => { e.preventDefault(); };
                // 使用 visibility 代替 display: none，位置依然保留，防止布局错位
                el.style.visibility = 'hidden'; 
                el.style.pointerEvents = 'none'; // 禁止点击
            }
        });
    },
    ...
    _showUsernameModal({ isFirstTime = false }) {
        ...
        const closeHandler = () => {
            ...
            if (isFirstTime) auth.signOut(); // 首次登录强行关闭就退出
        };
...
console.log('✅ 无圆角黑金高级版 ProfileUI 已加载！');
