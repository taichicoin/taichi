...
window.YYCardProfileUI = {
    ...
    // 注入全局 CSS（包含直接隐藏原生按钮的代码，彻底解决闪现问题）
    _injectStyles() {
        ...
        s.textContent = `
            ...
            .yy-modal-box {
                background: #1a1a1e; border-radius: 0px !important; /* 强制直角 */
    ...
    update() {
        ...
        // ⭐ 强制设置用户名：只要没有用户名，且没有正在显示的弹窗，就自动弹出设置弹窗
    ...
    _showUsernameModal({ isFirstTime = false }) {
        ...
        document.getElementById('modal-username-save').onclick = async () => {
            ...
            modal.remove();
            this._currentModal = null;
            this.update(); // 刷新界面，此时用户名已存在，不会再弹窗，并启用匹配按钮
        };
    },

    // ==================== 2. 绑定钱包 定制弹窗（去除原生 prompt 的高级UI） ====================
...
console.log('✅ 已彻底删除旧按钮，功能完全集成于头像菜单！强制用户名设置已生效');
