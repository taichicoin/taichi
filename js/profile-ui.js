// ========== 大厅UI交互（中英双语版 · 无修改头像 · 显示当前邮箱 · 已移除用户名强制与改名卡） ==========
window.YYCardProfileUI = {
    ...
    update() {
        ...
        // 不再强制要求用户名，按钮始终可用
        btn.disabled = false;
        btn.textContent = L.t('match_btn_ready');

        // 移除强制设置用户名的弹窗
        // if (!profile.username && !this._currentModal) { this._showUsernameModal({ isFirstTime: true }); }
...
console.log('✅ 头像菜单已更新：无修改头像，显示当前登录邮箱（支持Google/电报假邮箱），已移除用户名强制与改名卡');
