...
window.YYCardBattle = (function() {
    ...
    async function tick() {
        ...
        // ========== 战斗结束检测：上一秒剩余 > 0 且当前剩余 <= 0，且上一秒是战斗阶段 ==========
        if (prevPhase === 'battle' && prevRemaining > 0 && remaining <= 0 && !isSettling) {
            log(`⚡ 战斗阶段结束（剩余时间从 ${prevRemaining} 变为 ${remaining}），触发结算`);
            await callSettlement();
            // 结算后，让计时器继续运行，下一轮 tick 会自然进入新回合
        }
...
console.log('✅ battle.js 加载完成（纯时间驱动，支持商店操作，已修复战斗结算触发）');
