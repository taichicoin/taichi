...
window.YYCardDrag = (function() {
    ...
    function onDragEnd(e, onDropCallback) {
        ...
        if (dropResult && onDropCallback) {
            // 业务层如果成功，会立即触发重绘，直接销毁掉 sourceElement 元素。
            // 业务层如果失败（如金币不足），会弹出 Toast 并中止，不触发重绘。
            onDropCallback(type, index, dropResult);
        }

        // ★【防残影与防消失终极方案】
        // 如果此时 sourceElement 依然存在于 DOM 中（意味着业务层失败了，没重绘），我们立刻把它恢复可见。
        // 如果业务层成功重绘了，sourceElement 已经被 DOM 删除了，下面的代码会自动跳过。
