...
window.YYCardDrag = (function() {
    ...
    // ========== ★ 核心改造：事件委托（取代逐个绑定） ★ ==========
    const DRAG_SELECTOR = '.card[data-card-type="hand"], .card[data-card-type="shop"], .card[data-card-type="board"]';

    function handlePointerDown(e) {
        // 从事件目标向上查找最近的符合选择器的卡片元素
        const cardEl = e.target.closest(DRAG_SELECTOR);
        if (!cardEl) return;

        const cardType = cardEl.getAttribute('data-card-type');
        const indexStr = cardEl.getAttribute('data-hand-index')
                      || cardEl.getAttribute('data-shop-index')
                      || cardEl.getAttribute('data-board-index');
        ...
        onDragStart(e, cardType, card, index, cardEl);
    }
    ...
    // 旧的 bindCards 保持为空操作，确保外部调用不会报错（兼容性）
    function bindCards() {
        // 已被事件委托完全替代，此函数不再需要做任何事
        // 如果外部仍有调用，不会出错
    }
    ...
    function init(dependencies) {
        ...
        bindDelegation(); // 只绑定一次，永不丢失
    }

    // 公开 API
    return {
        init,
        bindCards,       // 保留但空实现，外部调用无害
        cancelCurrentDrag,
        get isDragging() {
            return _isDragging;
        }
