// ==================== 商店与交互系统（手动模拟拖拽版：触碰即拿起，支持商店→手牌 + 缓冲期 + 镜像敌方棋盘） ====================
window.YYCardShop = (function() {
    ...
    // ===== 镜像敌方棋盘：实现两军对峙的视觉效果 =====
    function getMirroredBoard(board) {
        // 原始索引：0 1 2 (前排)  3 4 5 (后排)
        // 镜像规则：前排左右互换(0↔2,1不变)，后排左右互换(3↔5,4不变)
        const mirrored = new Array(6).fill(null);
        if (board[0] !== undefined) mirrored[2] = board[0];
        if (board[1] !== undefined) mirrored[1] = board[1];
        if (board[2] !== undefined) mirrored[0] = board[2];
        if (board[3] !== undefined) mirrored[5] = board[3];
        if (board[4] !== undefined) mirrored[4] = board[4];
        if (board[5] !== undefined) mirrored[3] = board[5];
        return mirrored;
    }

    // ===== 战斗阶段根据 battlePairs 精确找到对手，并应用镜像 =====
    function renderEnemyBoard() {
        ...
        if (oppId && gameState.players[oppId]) {
            ...
            // 应用镜像
            const mirroredBoard = getMirroredBoard(originalBoard);
            renderBoard('enemy-board', mirroredBoard, false);
            // 调试日志（可选）
            console.log(`[renderEnemyBoard] 对手 ${oppId} 原始棋盘: ${originalBoard.map(c=>c?c.name:'空').join(',')}`);
            console.log(`[renderEnemyBoard] 镜像后棋盘: ${mirroredBoard.map(c=>c?c.name:'空').join(',')}`);
        }
    }
    ...
    function injectStyles() {
        ...
        style.textContent = `
            ...
            /* 缓冲期禁止操作（由 setPhase 动态添加/移除 buffering-mode 类） */
            .buffering-mode .card,
            .buffering-mode .btn,
    ...
    function init() {
        ...
        bindUIEvents();
        refreshAllUI();
        log('✅ 商店交互模块已启动（手动模拟拖拽版，支持商店→手牌 + 缓冲期 + 镜像敌方棋盘）');
    }
...
console.log('✅ shop.js 加载完成（手动模拟拖拽版 + 战斗对手精确匹配 + 缓冲期 + 镜像敌方棋盘）');
