// ==================== 消耗牌系统模块 (consumable.js · instanceId + 索引双保险) ====================
window.YYCardConsumable = (function() {
    ...
    // ★ 双保险：优先 instanceId，兜底 data-board-index
    function getTargetUnderPoint(point) {
        ...
        for (const el of elements) {
            if (el.classList.contains('card') && !el.classList.contains('empty-slot') && !el.closest('#consumable-cards')) {
                ...
                // 优先用 instanceId 精确匹配
                const iid = el.getAttribute('data-instance-id');
                if (iid) {
                    const card = my.board.find(c => c && c.instanceId === iid);
                    if (card && card.card_id && !card.isConsumable && card.type !== 'consumable') return el;
                }

                // 兜底：用 data-board-index 按索引匹配
                const slot = el.closest('.card-slot');
                if (slot) {
                    const bi = slot.getAttribute('data-board-index');
                    if (bi !== null) {
                        const card = my.board[parseInt(bi)];
                        if (card && card.card_id && !card.isConsumable && card.type !== 'consumable') return el;
                    }
                }
            }
    ...
    async function useConsumableOnTarget(consumableCard, targetEl) {
        ...
        // 1) 优先通过 instanceId 找
        const iid = targetEl.getAttribute('data-instance-id');
        if (iid) {
            targetCard = my.board.find(c => c && c.instanceId === iid);
        }

        // 2) 兜底：通过 data-board-index 找
        if (!targetCard) {
            const slot = targetEl.closest('.card-slot');
            if (slot && slot.hasAttribute('data-board-index')) {
    ...
    function init(deps) {
        ...
        console.log('✅ consumable.js 已初始化 (instanceId + 索引双保险)');
    }
