// ==================== 消耗牌系统模块 (consumable.js · instanceId 匹配 · 合成兼容) ====================
window.YYCardConsumable = (function() {
    ...
    function getTargetUnderPoint(point) {
        ...
        for (const el of elements) {
            if (el.classList.contains('card') && !el.classList.contains('empty-slot') && !el.closest('#consumable-cards')) {
                ...
                if (!instanceId) continue; // 必须有 instanceId
                const uid = getCurrentUserId();
                const gs = getGameState();
                const my = gs?.players?.[uid];
                if (!my?.board) return null;
                const card = my.board.find(c => c && c.instanceId === instanceId);
    ...
    // ★ 纯 instanceId 定位目标，不再依赖 data-board-index
    ...
    function init(deps) {
        ...
        console.log('✅ consumable.js 已初始化 (instanceId 匹配)');
    }
