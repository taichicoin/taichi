// ==================== 消耗牌系统模块 (consumable.js · 后端持久化奖励版) ====================
window.YYCardConsumable = (function() {
    ...
    let selectedCardId = null;   // 当前选中的 card_id
    ...
    // 显示选择面板（从 pendingConsumables 读取第一组）
    ...
    async function confirmSelection() {
        ...
        try {
            ...
            // 调用 select-consumable Edge Function
    ...
    // ========== 拖拽使用逻辑（保持不变，仍调用 use_consumable RPC） ==========
    ...
    function init(deps) {
        ...
        // 监听游戏状态更新后刷新徽章
        const originalRefresh = window.YYCardBattle?.forceRefreshState;
        if (originalRefresh) {
            const wrapped = async function() {
                await originalRefresh.apply(window.YYCardBattle, arguments);
                updateRewardBadge();
            };
            window.YYCardBattle.forceRefreshState = wrapped;
        }
        console.log('✅ consumable.js 已初始化（持久化奖励版）');
    }
