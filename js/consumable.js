// ==================== 消耗牌系统模块 (consumable.js · 阶段限制 + 灵敏识别) ====================
window.YYCardConsumable = (function() {
    ...
    function formatEffectDescFromData(cardData) {
        ...
        if (ctype === 'other') {
            ...
            if (eff.effect_type === 'add_attack') return `攻击 +${eff.value}`;
            if (eff.effect_type === 'add_health') return `生命 +${eff.value}`;
            if (eff.effect_type === 'add_attack_health') return `攻击 +${eff.attack} 生命 +${eff.health}`;
            return '立即生效';
        }
    ...
    function onDragStart(e) {
        ...
        if (card.target_scope === 'all') {
            ...
            crosshair.classList.add('consumable-ui-element'); // 标记以便后面排除
            ...
            dashedLine.classList.add('consumable-ui-element');
    ...
    function updateDraggingVisuals() {
        ...
        if (dragCard && dragCard.target_scope === 'all') {
            if (dragClone) {
                dragClone.style.left = currentPoint.x + 'px';
                dragClone.style.top = currentPoint.y + 'px';
            }
            if (highlightTarget) {
    ...
    function getTargetUnderPoint(point) {
        ...
        for (const el of elements) {
            // 跳过我们自己的 UI 元素
            if (el.classList.contains('consumable-ui-element')) continue;
            if (el.classList.contains('consumable-allowed')) continue;
    ...
    function init(deps) {
        ...
        console.log('✅ consumable.js 已初始化（阶段限制 + 灵敏识别）');
    }
