...
window.YYCardConsumable = (function() {
        ...
        for (const el of elements) {
            if (el.classList.contains('card') && !el.classList.contains('empty-slot') && !el.closest('#consumable-cards')) {
                ...
                if (boardIndex !== null && my?.board) {
                    ...
                    // 必须是非消耗牌的有效角色/道具等
                    if (card && !card.isConsumable && card.type !== 'consumable') return el;
                }
            }
        }
        return null;
    ...
    // 全体消耗牌乐观更新：遍历己方棋盘所有卡牌，修改属性并刷新 UI
    function applyAllEffect(consumableCard) {
        ...
        for (let i = 0; i < board.length; i++) {
            ...
            if (!targetCard || !targetCard.card_id) continue; // 空位跳过
    ...
    async function useConsumableAll(consumableCard) {
        ...
        try {
            ...
            // 后端失败，需要回滚：将消耗牌放回手牌，并用服务器数据覆盖本地
    ...
    function init(deps) {
        ...
        console.log('✅ consumable.js 已初始化（最终流畅版）');
    }
