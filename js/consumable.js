// ==================== 消耗牌系统模块 (consumable.js · instanceId匹配修复版) ====================
window.YYCardConsumable = (function() {
    ...
    // ★★★ 修改点 1：用 instanceId 匹配，不依赖 data-board-index 索引
    function getTargetUnderPoint(point) {
        ...
        for (const el of elements) {
            if (el.classList.contains('card') && !el.classList.contains('empty-slot') && !el.closest('#consumable-cards')) {
                const cardInstanceId = el.getAttribute('data-instance-id');
                if (!cardInstanceId) continue;
                const uid = getCurrentUserId();
                const gs = getGameState();
                const my = gs?.players?.[uid];
                if (my?.board) {
                    const card = my.board.find(c => c && c.instanceId === cardInstanceId);
    ...
    // ★★★ 修改点 2：通过 instanceId 获取目标卡牌，不依赖 data-board-index
    ...
    function init(deps) {
        ...
        console.log('✅ consumable.js 已初始化（instanceId匹配版）');
    }
