// ==================== 消耗牌系统模块 (consumable.js · 最终坐标定位版 · 合成兼容) ====================
window.YYCardConsumable = (function() {
    ...
    // ★ 坐标定位目标格子，用 data-board-index 直接读 my.board
            ...
            if (point.x >= rect.left && point.x <= rect.right &&
                ...
                if (bi !== null) {
                    ...
                    if (card && card.card_id && !card.isConsumable && card.type !== 'consumable') {
                        const el = slot.querySelector('.card:not(.empty-slot)');
                        if (el) return el;
                    }
                }
            }
    ...
    // ★ 使用单体消耗牌，通过卡槽索引取目标，不依赖 instanceId
    ...
    function init(deps) {
        ...
        console.log('✅ consumable.js 已初始化 (坐标定位版)');
    }
