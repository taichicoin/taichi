// ==================== 消耗牌系统模块 (consumable.js · DOM直接刷新版) ====================
window.YYCardConsumable = (function() {
    ...
    // ★★★ 核心修改：直接调用 window.YYCardShop.renderHand() 并强制DOM刷新 ★★★
    async function confirmSelection() {
        ...
        try {
            ...
            // 方式2：直接调用全局 YYCardShop.renderHand（最可靠）
            if (window.YYCardShop?.renderHand) {
                window.YYCardShop.renderHand();
            }
            
            // 方式3：直接操作DOM重新渲染手牌（兜底方案）
            forceRenderHandDirectly();
    ...
    // ★★★ 直接操作DOM刷新手牌（不依赖任何外部函数） ★★★
    function forceRenderHandDirectly() {
        ...
        if (!hand || hand.length === 0) {
            ...
            emptyMsg.style.cssText = 'color: #888; text-align: center; width: 100%; padding: 20px; font-size: 0.9rem;';
            emptyMsg.textContent = '手牌为空';
            handContainer.appendChild(emptyMsg);
            return;
        }
        
        // 渲染手牌卡片（复用 YYCardShop 的渲染逻辑）
        if (window.YYCardShop?.renderHand) {
            // 如果 YYCardShop.renderHand 存在，直接调用它
            window.YYCardShop.renderHand();
            return;
        }
        ...
        hand.forEach((card, index) => {
            ...
            slot.setAttribute('data-instance-id', card.instanceId || '');
            
            const cardEl = document.createElement('div');
            cardEl.className = 'card';
            cardEl.setAttribute('data-instance-id', card.instanceId || '');
            cardEl.setAttribute('data-rarity', card.rarity || 'Common');
            cardEl.setAttribute('data-star', card.star || '0');
            
            // 卡牌图标
            const icon = document.createElement('div');
            icon.className = 'card-icon';
            const img = document.createElement('img');
            img.src = card.image_url || '/assets/default-card.png';
            img.alt = card.name || '卡牌';
            img.onerror = function() { this.src = '/assets/default-card.png'; };
            ...
            stats.innerHTML = `
                <span class="card-atk">⚔${card.atk || 0}</span>
                <span class="card-hp">❤${card.hp || 0}</span>
            `;
            ...
            if (card.type === 'consumable' || card.isConsumable) {
                ...
                mark.style.cssText = `
                    ...
                    border-radius: 50%; width: 3vw; height: 3vw;
                    display: flex; align-items: center; justify-content: center;
    ...
    function init(deps) {
        ...
        console.log('✅ consumable.js 已初始化 (DOM直接刷新版)');
    }
