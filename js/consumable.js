...
window.YYCardConsumable = (function() {
    ...
    /**
     * 终极视觉兜底：只要坐标落在卡槽内且卡槽里有非空卡牌，就认为目标有效
     */
    function getTargetUnderPoint(point) {
        ...
        for (const slot of slots) {
            ...
            if (point.x >= rect.left && point.x <= rect.right &&
                ...
                // 只要这个卡槽里有卡牌元素（不是 empty-slot），就返回它
    ...
    /**
     * 从视觉元素反查数据卡牌：通过卡槽索引或 instanceId
     */
    async function useConsumableOnTarget(consumableCard, targetEl) {
        ...
        // 1) 通过卡槽的 data-board-index 获取数据卡牌
        ...
        // 2) 兜底：通过元素上的 data-instance-id 匹配
        if (!targetCard) {
        ...
        // 3) 最终兜底：如果仍然没有，但目标元素确实是一个非空卡牌，直接以该元素在 board 中的位置强行取
        if (!targetCard && slot) {
            const idx = parseInt(slot.getAttribute('data-board-index'));
            if (!isNaN(idx)) {
                targetCard = my.board[idx]; // 可能是 undefined，但下面会判断
            }
        }

        if (!targetCard || !targetCard.card_id) return; // 数据无效则放弃
    ...
    function init(deps) {
        ...
        console.log('✅ consumable.js 已初始化 (视觉兜底版)');
    }
