// ==================== 消耗牌系统模块 (consumable.js · instanceId匹配版) ====================
window.YYCardConsumable = (function() {
    ...
    // ★★★ 核心修改：用 data-board-index 拿索引，再用 instanceId 交叉验证，防止索引错位
    function getTargetUnderPoint(point) {
        ...
        for (const el of elements) {
            if (el.classList.contains('card') && !el.classList.contains('empty-slot') && !el.closest('#consumable-cards')) {
                ...
                // 用 data-board-index 拿到数据索引
                const boardIndex = slot.getAttribute('data-board-index');
                if (boardIndex === null) continue;
                const idx = parseInt(boardIndex);

                const uid = getCurrentUserId();
                const gs = getGameState();
                const my = gs?.players?.[uid];
                if (!my?.board) return null;

                const card = my.board[idx];
                // 必须是非消耗牌的有效角色/道具
                if (card && card.card_id && !card.isConsumable && card.type !== 'consumable') {
                    // 再用 instanceId 交叉验证，确保 DOM 和数据一致
                    const elInstanceId = el.getAttribute('data-instance-id');
                    if (elInstanceId && card.instanceId && elInstanceId !== card.instanceId) {
                        // DOM 上的 instanceId 和数组里的对不上，说明索引错位了
                        // 到 board 里用 instanceId 找真正的卡
                        const realCard = my.board.find(c => c && c.instanceId === elInstanceId);
                        if (realCard && !realCard.isConsumable && realCard.type !== 'consumable') return el;
                        return null;
                    }
                    return el;
                }
    ...
    // ★★★ 核心修改：用 instanceId 定位目标卡牌，不依赖索引
    async function useConsumableOnTarget(consumableCard, targetEl) {
        ...
        // 用 instanceId 匹配，确保拿到正确的卡牌对象
        const targetInstanceId = targetEl.getAttribute('data-instance-id');
        if (!targetInstanceId) return;
        const targetCard = my.board.find(c => c && c.instanceId === targetInstanceId);
