// ==================== 消耗牌系统模块 (consumable.js · 完整版 · 准星+虚线拖拽) ====================
window.YYCardConsumable = (function() {
    ...
    // 拖拽相关变量
    let dragData = null;
    let crosshair = null;          // 准星元素
    let dashedLine = null;         // 虚线元素
    ...
    function highlightNearestTarget(point) {
        if (highlightTarget) {
            highlightTarget.classList.remove('drop-target', 'consumable-allowed');
    ...
    function onDragEnd(e) {
        ...
        if (card.target_scope === 'all') {
            ...
            const targetEl = getTargetUnderPoint(point);
            if (targetEl) {
                useConsumableOnTarget(card, targetEl);
            }
        }
    ...
    function init(deps) {
        ...
        console.log('✅ consumable.js 已初始化（准星+虚线版）');
    }
