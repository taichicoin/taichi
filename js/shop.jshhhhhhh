...
window.YYCardShop = (function() {
    ...
    // 通用：强制刷新UI（从数据库拉取最新状态）- 保留用于其他操作，但刷新商店不再使用
    ...
    async function refreshShopAction() {
        ...
        try {
            ...
            // 优化：如果后端返回了 updatedPlayer，直接合并到本地状态，无需重新拉取
            if (data.updatedPlayer) {
                const currentGameState = window.YYCardBattle.getGameState();
                const uid = getCurrentUserId();
                if (currentGameState && uid) {
                    Object.assign(currentGameState.players[uid], data.updatedPlayer);
                    refreshAllUI();  // 立即刷新界面
                }
            } else {
                // 兼容旧版后端：降级使用全量同步
...
console.log('✅ shop.js 加载完成（优化响应速度+移除刷新弹窗）');
