...
window.YYCardBattle = (function() {
    ...
    async function fetchGameState() {
        ...
        const { data, error } = await supabase.from('game_states').select('state').eq('room_id', currentRoomId).maybeSingle();
    ...
    function getBoardElements() {
        ...
        console.log("获取棋盘元素:", { leftCards, rightCards });
    ...
    async function playAttackEvent(event, boardElements) {
        return new Promise(resolve => {
            ...
            if (!attackerEl || !targetEl) {
                console.warn("攻击者或目标元素不存在", event);
                resolve();
                return;
            }
    ...
    async function playCombatAnimations(combatEvents) {
        ...
        for (const pair of combatEvents) {
            ...
            console.log(`播放战斗动画: ${pair.p1Id} vs ${pair.p2Id}, 事件数: ${pair.events.length}`);
    ...
    async function onPhaseEnd(phase) {
        ...
        try {
            if (phase === 'prepare') {
                ...
                await updatePlayersToDB(); // 简化，原updatePhaseToDB
                await applyUIMode(false);
                await startPhaseTimerFromEndTime(phaseEndTime);
                await simulateBattleWithAnimation(); // 调用带动画的战斗
                ...
                if (gameState.phaseEndTime) await startPhaseTimerFromEndTime(new Date(gameState.phaseEndTime).getTime());
                else {
                    const nowServer = await getServerTimeMs();
                    const prepareDur = getPrepareDuration(gameState.round);
                    await startPhaseTimerFromEndTime(nowServer + prepareDur * 1000);
                }
            }
    ...
    async function startPhaseTimerFromEndTime(phaseEndTimeMs) {
        ...
        phaseTimer = setTimeout(() => { onPhaseEnd(phase); }, remaining * 1000);
        safetyTimer = setTimeout(() => { if (phaseTimer) { log(`⚠️ 阶段 ${phase} 超时强制结束`, true); clearTimeout(phaseTimer); phaseTimer = null; clearInterval(timerInterval); timerInterval = null; onPhaseEnd(phase); } }, (remaining + 2) * 1000);
    }
    ...
    function subscribeGameState() {
        ...
        gameSubscription = supabase.channel(`game:${currentRoomId}`).on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'game_states', filter: `room_id=eq.${currentRoomId}` }, async (payload) => {
    ...
    function endGame(winnerId) {
        stopPolling(); isInPhaseTransition = false;
    ...
    async function fastForwardAndResume() { /* 简化，可暂时注释 */ return true; }
...
console.log('✅ battle.js 加载完成（完整版，含战斗动画）');
