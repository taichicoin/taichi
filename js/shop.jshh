...
window.YYCardShop = (function() {
    ...
    async function handleSell(type, index) {
        ...
        if (success) { refreshAllUI(); toast('出售成功'); }
        else toast('出售失败', true);
    }

    // ===== 按钮操作（核心修改：调用后端 Edge Function + 弹窗调试） =====
...
console.log('✅ shop.js 加载完成（后端刷新商店 + 弹窗调试）');
