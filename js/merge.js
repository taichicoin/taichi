...
window.mergeService = (function() {
    ...
    function canMerge(card) {
        ...
        const targetStar = card.star || 0;

        if (card.type === 'weapon') {
            const handWeapons = (my.hand || []).filter(c => c && isValidCard(c) && c.type === 'weapon' && c.card_id === targetId && (c.star || 0) === targetStar);
            const equippedWeapons = [];
            (my.board || []).forEach(c => {
                if (c && c.weapon && c.weapon.card_id === targetId && (c.weapon.star || 0) === targetStar) {
                    equippedWeapons.push(c.weapon);
                }
            });
            const total = handWeapons.length + equippedWeapons.length;
            if (total >= 3 && equippedWeapons.length === 1) return true;
            return false;
        } else {
            const allChars = [];
            (my.board || []).forEach(c => { if (c && isValidCard(c) && c.type !== 'weapon' && c.type !== 'item') allChars.push(c); });
            (my.hand || []).forEach(c => { if (c && isValidCard(c) && c.type !== 'weapon' && c.type !== 'item') allChars.push(c); });
            const sameGroup = allChars.filter(c => c.card_id === targetId && (c.star || 0) === targetStar);
    ...
    async function envokeMerge() {
        ...
        (my.board || []).forEach((c, idx) => {
            if (c && isValidCard(c) && c.type !== 'weapon' && c.type !== 'item') {
        ...
        allChars.forEach(({ card, location, index }) => {
            const key = (card.card_id || card.cardId) + '_' + (card.star || 0);
        ...
        for (const key in characterGroups) {
            ...
            const newStar = (mergeTargets[0].card.star || 0) + 1;
        ...
        const weaponGroups = {};
        (my.hand || []).forEach((c, idx) => {
            if (c && isValidCard(c) && c.type === 'weapon') {
                const key = (c.card_id || c.cardId) + '_' + (c.star || 0);
        ...
        (my.board || []).forEach((c, idx) => {
            if (c && c.weapon) {
                const w = c.weapon;
                const key = (w.card_id) + '_' + (w.star || 0);
                if (!weaponGroups[key]) weaponGroups[key] = { hand: [], equipped: [] };
        ...
        for (const key in weaponGroups) {
            ...
            const needed = 3;
            if (group.hand.length + group.equipped.length < needed) continue;
            const neededHand = needed - group.equipped.length;
            ...
            const newStar = (oldWeapon.star || 0) + 1;
    ...
    (function init() {
        ...
        window.addEventListener('merge-trigger', envokeMerge);
    })();
