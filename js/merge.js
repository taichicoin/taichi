// ==================== 自动合成模块 (instanceId 修复 + type 兜底 + 吕布万能合成 · 排除消耗牌) ====================
window.mergeService = (function() {
    ...
    function canMerge(card) {
        ...
        // ★ 排除消耗牌 / 亡魂碎片
        if (card.type === 'consumable' || card.isConsumable || card.type === 'loot') return false;
        ...
        // 角色合成：只统计棋盘和手牌，不统计商店，并排除消耗牌/loot
        const allChars = [];
        (my.board || []).forEach((c, idx) => {
            if (c && isValidCard(c) && c.type !== 'weapon' && c.type !== 'item' && c.type !== 'consumable' && c.type !== 'loot' && (c.star || 0) === 0) {
        ...
        if (isLvbu) {
            ...
            // 三个吕布不能合成
            if (lvbuCount >= 3 && otherCards.length === 0) return false;
            // 两个吕布 + 至少一个其他
            if (lvbuCount >= 2 && otherCards.length >= 1) return true;
            // 一个吕布 + 两个相同其他
            if (lvbuCount >= 1) {
                const otherGroups = {};
                ...
                for (const id in otherGroups) {
                    if (otherGroups[id].length >= 2) return true;
                }
            }
            return false;
        } else {
    ...
    function findMergePlan(my) {
        ...
        if (lvbuCount >= 2 && otherCards.length >= 1) {
            const targetOther = pickTarget(otherCards);
            const usedLvbu = lvbuCards.slice(0, 2);
        ...
        if (lvbuCount >= 1) {
            ...
            for (const id in otherGroups) {
                ...
                if (group.length >= 2) {
                    const targetOther = pickTarget(group);
                    const usedOther = group.filter(c => c !== targetOther).slice(0, 1);
                    const usedLvbu = lvbuCards.slice(0, 1);
    ...
    function updateMergeGlow() {
        ...
        document.querySelectorAll(selectors.join(',')).forEach(el => {
            ...
            // ★ 跳过消耗牌 / loot 的高亮
            if (card && (card.type === 'consumable' || card.isConsumable || card.type === 'loot')) {
                el.style.boxShadow = '';
