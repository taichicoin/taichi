// ==================== 自动合成模块 (完整光效修正：显眼内发光 + 保留外阴影 + 稀有度飞行动画) ====================
window.mergeService = (function() {
    ...
    // 稀有度对应的颜色配置
    const RARITY_COLORS = {
        'Common':    { rgba: '0,255,0',    hex: '#00ff00' },   // 绿色
        'Rare':      { rgba: '0,128,255',  hex: '#0080ff' },   // 蓝色
        'Epic':      { rgba: '255,136,0',  hex: '#ff8800' },   // 橙色
        'Legendary': { rgba: '255,34,34',  hex: '#ff2222' }    // 红色
    };

    // 默认卡牌外阴影（保证所有卡牌都有立体阴影，不会丢失）
    const DEFAULT_CARD_SHADOW = '0 2px 6px rgba(0,0,0,0.3)';
        ...
        for (const key in characterGroups) {
            ...
            if (group.length >= 3) {
                ...
                return {
                    ...
                    targetIndex,
                    newCard,
                    rarity: mergeTargets[0].card.rarity || 'Common'
                };
            }
        }
    ...
    // 飞行动画 - 颜色由调用者决定
    function playFlyAnimation(sourceElements, targetElement, colorHex, duration = 600) {
        return new Promise(resolve => {
            ...
            sourceElements.forEach(srcEl => {
                ...
                particle.style.cssText = `
                    ...
                    height: 36px;
                    border-radius: 50%;
                    background: radial-gradient(circle, rgba(255,255,255,0.8), ${colorHex});
                    box-shadow: 0 0 45px ${colorHex};
                    z-index: 100001;
            ...
            if (targetElement) {
                ...
                targetElement.style.boxShadow = `0 0 60px 20px ${colorHex}`;
    ...
    async function executeMergePlan(plan, my) {
        ...
        const rarityInfo = RARITY_COLORS[plan.rarity] || RARITY_COLORS['Common'];
        const colorHex = rarityInfo.hex;
        ...
        if (plan.type === 'character') {
            ...
            await playFlyAnimation(sourceEls, targetEl, colorHex, 600);
            ...
            await playFlyAnimation(handEls, boardEl, colorHex, 600);
    ...
    function updateMergeGlow() {
        ...
        document.querySelectorAll(selectors.join(',')).forEach(el => {
            ...
            if (card && canMerge(card)) {
                ...
                // 同时保留默认外阴影 + 内部铺满明显的光晕（不透明度30%，足够显眼）
                el.style.boxShadow = `${DEFAULT_CARD_SHADOW}, inset 0 0 0 1000em rgba(${colors.rgba}, 0.3)`;
            } else {
                // 恢复成默认外阴影
                el.style.boxShadow = DEFAULT_CARD_SHADOW;
            }
        });
    }

    // 初始化：确保所有卡牌默认都有外阴影（通过注入样式）
    (function init() {
        // 注入默认卡牌样式（保留原来的边框，只加一个基础阴影）
        const style = document.createElement('style');
        style.textContent = `
            .card {
                box-shadow: ${DEFAULT_CARD_SHADOW};
            }
        `;
