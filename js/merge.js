...
window.mergeService = (function() {
    ...
    function updateMergeGlow() {
        ...
        const rarityColors = {
            'Common':    'rgba(0, 255, 0, 0.2)',   // 绿色 20%透明
            'Rare':      'rgba(0, 128, 255, 0.2)', // 蓝色
            'Epic':      'rgba(255, 136, 0, 0.2)', // 橙色
            'Legendary': 'rgba(255, 34, 34, 0.2)'  // 红色
        };
        ...
        document.querySelectorAll(selectors.join(',')).forEach(el => {
            ...
            if (card && canMerge(card)) {
                ...
                // 用内阴影铺满整个卡牌内部
                el.style.boxShadow = `inset 0 0 60px 20px ${color}`;
            } else {
                el.style.boxShadow = '';
            }
        });
