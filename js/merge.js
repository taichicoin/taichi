...
window.mergeService = (function() {
    ...
    function canMerge(card) {
        ...
        // ★ 排除消耗牌
        if (card.type === 'consumable' || card.isConsumable) return false;
        ...
        (my.board || []).forEach((c, idx) => {
            if (c && isValidCard(c) && c.type !== 'weapon' && c.type !== 'item' && c.type !== 'consumable' && !c.isConsumable && (c.star || 0) === 0) {
