// ==================== 自动合成模块 (适配每组15张 + 不同稀有度光效 + 内铺光 + 颜色飞行) ====================
window.mergeService = (function() {
    ...
    // ★ 颜色映射（稀有度 → 发光颜色 / 飞行粒子颜色）
    function getRarityGlowColors(rarity) {
        switch(rarity) {
            case 'Legendary': return { fill: 'rgba(255,34,34,0.2)',  shadow: 'rgba(255,0,0,0.8)',   particle: '#ff3333' };
            case 'Epic':      return { fill: 'rgba(255,136,0,0.2)', shadow: 'rgba(255,136,0,0.8)', particle: '#ff8800' };
            case 'Rare':      return { fill: 'rgba(0,128,255,0.2)', shadow: 'rgba(0,128,255,0.8)', particle: '#0088ff' };
            default:          return { fill: 'rgba(0,255,0,0.2)',   shadow: 'rgba(0,255,0,0.8)',   particle: '#00ff00' };
        }
    }
    ...
    function playFlyAnimation(sourceElements, targetElement, rarity = 'Common', duration = 600) {
        return new Promise(resolve => {
            ...
            const colors = getRarityGlowColors(rarity);
            let targetRect;
            if (targetElement) {
            ...
            sourceElements.forEach(srcEl => {
                ...
                particle.style.cssText = `
                    ...
                    box-shadow: 0 0 45px ${colors.shadow};
                    z-index: 100001;
    ...
    function updateMergeGlow() {
        ...
        document.querySelectorAll(selectors.join(',')).forEach(el => {
            ...
            if (card && canMerge(card)) {
                const colors = getRarityGlowColors(card.rarity || 'Common');
                // 内阴影铺满整个卡片内部，透明度20%，保留原有边框等样式
                el.style.boxShadow = `inset 0 0 0 9999px ${colors.fill}`;
            } else {
                el.style.boxShadow = '';
            }
        });
