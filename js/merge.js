// ==================== 自动合成模块 (合成锁 + 伪元素内光 + 外发光 + 颜色飞行 + 卡牌锁定 + 卸下修复 + 目标位置装备保留) ====================
window.mergeService = (function() {
    ...
    let isMerging = false;
    let mergingCards = [];   // 存放正在合成的卡牌 instanceId
    ...
    async function executeMergePlan(plan, my) {
        ...
        if (plan.type === 'character') {
            ...
            // ★ 卸下非目标位置的棋盘卡牌的装备（目标位置的装备保留，已在 newCard 中继承）
