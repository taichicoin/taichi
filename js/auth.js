...
window.YYCardAuth = {
    ...
    async init() {
        ...
        if (isTelegram) {
            ...
            // 先清除本地缓存的旧会话，确保每次都用 initData 登录
            await supabase.auth.signOut({ scope: 'local' });
            this.log('🧹 已清除本地会话，强制走 initData 验证');

            // 强制通过 Telegram 用户信息进行自动登录
            this.log('🔄 通过 Telegram initData 自动登录...');
            ...
            try {
                const res = await fetch(`${window.YYCardConfig.SUPABASE_URL}/functions/v1/telegram-simple-auth`, {
                    ...
                    body: JSON.stringify({
                        initData: tg.initData   // 完整 initData 字符串，服务端验证签名
                    })
                });
                ...
                this.log('✅ Telegram 自动登录成功（已更新本地会话）');
            } catch (err) {
        ...
        if (profileError && profileError.code === 'PGRST116') {
            ...
            const { data: newProfile, error: createErr } = await supabase
                ...
                .insert({
                    ...
                    rename_card_count: 1,
                    telegram_id: user.user_metadata?.telegram_id || null   // 从 JWT 中获取电报 ID
                })
                .select('*')
                .single();
