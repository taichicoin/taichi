...
window.YYCardMatchmaking = (function() {
    ...
    async function cancel() {
        ...
        // 3. 遍历涉及的房间，检查并强制清理空房间
        for (const roomId of uniqueRoomIds) {
            // 查询该房间剩余的真人玩家数量
            const { data: realPlayers, error: countError } = await supabase
                .from('room_players')
                .select('player_id')
                .eq('room_id', roomId)
                .eq('is_bot', false);
            ...
            if (countError) {
                log(`❌ 查询房间 ${roomId} 真人失败: ${countError.message}`, true);
                continue;
            }
            ...
            if (!realPlayers || realPlayers.length === 0) {
                ...
                const { error: stateError } = await supabase
                    ...
                    .eq('room_id', roomId);
                if (stateError) {
                    log(`❌ 删除 game_states 失败: ${stateError.message}`, true);
                }
