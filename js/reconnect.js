...
window.YYCardReconnect = (function() {
    ...
    function showReconnectModal(roomId, uid, supabase) {
        ...
        modal.innerHTML = `
            <div style="
                background: linear-gradient(rgba(26, 31, 53, 0.8), rgba(26, 31, 53, 0.8)), 
                            url('/assets/recobg.png') center/cover no-repeat;
                border: 2px solid #f5d76e; border-radius: 16px;
                padding: 48px 24px;  /* 上下内边距加倍，高度翻倍 */
                max-width: 320px; 
                text-align: center; 
                color: #fff;
            ...
            ">
                ...
                <div style="font-size: 14px; color: #eee; margin-bottom: 20px;">
                    你有一场未结束的战斗，是否重新连接？
                </div>
                <button id="reconnect-yes" style="
                    display: block; width: 100%; padding: 12px; margin-bottom: 10px;
                    background: linear-gradient(145deg, #f5d76e, #f0b34b);
                    color: #0b0f1c; border: none; border-radius: 8px;
                    font-size: 16px; font-weight: bold; cursor: pointer;
                ">✅ 重新连接</button>
                <button id="reconnect-no" style="
                    display: block; width: 100%; padding: 12px;
                    background: #2e3f5e; color: #fff; border: 1px solid #555;
