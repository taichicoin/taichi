...
window.YYCardShop = (function() {
    ...
    function getCurrentRoomId() {
        return window.YYCardBattle?.getCurrentRoomId?.() || null;
    }
    ...
    // ===== 按钮操作（核心修改：刷新商店调用后端 Edge Function） =====
    async function refreshShopAction() {
        ...
        if (!roomId || !userId) {
            toast('房间或用户信息缺失', true);
            return;
        }

        // 调用后端 Edge Function
        try {
            // 获取 Supabase 客户端和 token（用于认证）
            ...
            // 后端已更新数据库，本地需要同步最新状态
            // 方法1：调用 battle 的强制刷新（如果有）
            if (window.YYCardBattle?.forceRefreshState) {
                await window.YYCardBattle.forceRefreshState();
            } else {
                // 方法2：手动合并返回的数据（更可靠）
                const localState = getGameState();
                if (localState && localState.players[userId]) {
                    localState.players[userId].gold = data.gold;
                    localState.players[userId].shopCards = data.shopCards;
                    // 将合并后的状态写回数据库（可选，因为后端已写，但保持本地一致）
...
console.log('✅ shop.js 加载完成（后端刷新商店版本）');
