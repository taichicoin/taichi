// ==================== 通用工具函数 + 真实卡池管理（修复攻击/生命显示 undefined） ====================
window.YYCardUtils = (function() {
    ...
    // 加载卡牌模板（失败时记录日志，但不抛出异常，让调用方自行处理）
    async function loadCardTemplates() {
        ...
        try {
            ...
            console.log(`✅ 卡牌模板加载完成，共 ${cardTemplates.length} 张，已建立索引`);
        } catch (err) {
    ...
    function generateShopCard(shopLevel) {
        if (cardTemplates.length === 0) {
            console.warn('⚠️ 卡牌模板为空，无法生成商店卡牌');
            return null;
        }
        ...
        return {
            instanceId: uuid(),
            cardId: template.card_id || template.id,        // 业务 id
            card_id: template.card_id || template.id,       // 原始 card_id（用作索引键）
            name: template.name,
            type: template.type || 'character',
            rarity: template.rarity,
            atk: template.base_atk || template.baseAtk || 0,    // ✅ 修复：使用正确的属性名
            hp: template.base_hp || template.baseHp || 0,       // ✅ 修复
            ...
            icon: imagePath,
            image: imagePath,
            skill: template.skill,         // 携带完整技能对象
            faction: template.faction,     // 携带阵营
    ...
    function getDefaultDeck() {
        ...
        return selected.map(t => {
            ...
            return {
                ...
                rarity: t.rarity,
                atk: t.base_atk || t.baseAtk || 0,     // ✅ 修复
                hp: t.base_hp || t.baseHp || 0,        // ✅ 修复
...
console.log('✅ utils.js 加载完成（已修复攻击/生命字段映射 + 模板索引）');
