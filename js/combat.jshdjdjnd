...
window.YYCardCombat = (function() {
    ...
    // 寻敌：使用 0~5 索引，优先前排（0,1,2）
    function findTarget(attackerPos, enemyUnits) {
        const tablePos = attackerPos + 1; // 转为 1~6 键名
        ...
        for (const targetPos of priority) {
            const targetIndex = targetPos - 1; // 转为 0~5
    ...
    function fightWithLog(board1, board2, owner1, owner2) {
        ...
        let currentSide = board1First ? 1 : 2; // 1=玩家1攻击，2=玩家2攻击
        let turn = 0;
        const maxTurns = 200;
    ...
    function playAttackAnim(action) {
        return new Promise(resolve => {
            ...
            if (!$attacker || !$defender) {
                panelLog(`⚠️ 跳过 (${aBoard}[${action.attacker.pos}] → ${dBoard}[${action.defender.pos}])`, true);
                resolve();
                return;
            }
