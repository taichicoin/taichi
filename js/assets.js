// js/assets.js (改造后，适配 coinlister 目录)
(function() {
  ...
  // 同步历史充值（调用 post-deposits Edge Function）
  async function syncDeposits(userId) {
    try {
      let token = '';
      try {
        ...
        console.warn('获取 session 失败，将不带 token 同步', e);
      }

      const headers = { 'Content-Type': 'application/json' };
      if (token) headers['Authorization'] = `Bearer ${token}`;

      const resp = await fetch(`${SUPABASE_URL}/functions/v1/post-deposits`, {
        method: 'POST',
        headers,
      ...
      if (!resp.ok) {
        ...
        console.warn('同步充值失败:', resp.status, errText);
      } else {
        const data = await resp.json();
        console.log('同步充值完成:', data);
      }
    } catch (e) {
      console.warn('同步充值网络错误:', e.message);
    }
  }
    ...
    try {
      ...
      html += `
        <div class="assets-actions">
          ...
          <button class="assets-btn" style="background:#f59e0b; color:white;" onclick="window.showManualDepositModal()">手动上账</button>
        </div>
      `;
