...
(function() {
  ...
  // 获取有效的 access_token（优先用 auth 模块，其次从 supabase 客户端取）
  async function getAccessToken() {
    // 方式1：从 auth 模块拿
    const session = window.YYCardAuth?.currentSession;
    if (session?.access_token) return session.access_token;
    // 方式2：通过 supabase 客户端拿
  ...
  async function fetchBalances(userId) {
    ...
    console.log('当前 token 是否存在:', !!token);
    
    const headers = {
      'apikey': SUPABASE_ANON_KEY,
      'Content-Type': 'application/json'
    };
    // ★ 如果拿到 token，带上 Authorization 头（关键！）
    if (token) {
      headers['Authorization'] = `Bearer ${token}`;
    } else {
      return { error: '无法获取登录凭据，请重新登录' };
    }
    ...
    try {
      ...
      return { error: '网络请求失败: ' + (e.message || '未知') };
    }
  }
