...
window.YYCardUtils = (function() {
    ...
    function generateShopCard(shopLevel) {
        ...
        return {
            ...
            card_id: template.card_id || template.cardId || template.id, // 保留原始 card_id
            name: template.name,
            ...
            icon: imagePath,
            image: imagePath,
            skill: template.skill,         // 直接复制完整的技能对象
            faction: template.faction,    // 传递阵营
    ...
    function getDefaultDeck() {
        ...
        return selected.map(t => {
            ...
            return {
                ...
                card_id: t.card_id || t.cardId || t.id,
                name: t.name,
                ...
                icon: imagePath,
                image: imagePath,
                skill: t.skill,         // 重要：携带技能
                faction: t.faction,    // 重要：携带阵营
                equipment: { weapon: null, items: [null, null] },
                enlightenmentCount: 0
            };
        });
...
console.log('✅ utils.js 加载完成（含模板索引，技能/阵营自动传递）');
