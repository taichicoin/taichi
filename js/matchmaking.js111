...
window.YYCardMatchmaking = (function() {
    ...
    async function start() {
        ...
        if (!isBeta) {
            log('⛔ 非内测用户，无法匹配', true);
            const msg = '您不是内测用户，暂无法匹配';
            if (window.YYCardShop?.toast) window.YYCardShop.toast(msg, true);
            else alert(msg);
            isProcessing = false;
            return;
        }
...
console.log('✅ matchmaking.js 加载完成（玩家独立行 + 全局行 + 内测限制）');
