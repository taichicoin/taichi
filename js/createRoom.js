// /js/createRoom.js (解耦版 matchmaking 配合 UI，带完整空值保护)
window.YYCardCreateRoom = (function() {
    ...
    function showRoom() {
        try {
            ...
            lockUI(false);
            if (startBtn) {
                const hasName = !!auth?.currentProfile?.username;
                startBtn.textContent = hasName ? '⚡ 开始匹配' : '请先设置游戏ID';
    ...
    function init() {
        ...
        bottomNav = document.getElementById('yy-nav');  // 修正为 nav.js 生成的 id
        ...
        if (!enterBtn) {
            console.error('❌ 找不到进入匹配房间的按钮，请检查 HTML 中是否存在 id="enter-match-room-btn"');
            return;
        }
        if (!backBtn || !startBtn || !cancelBtn) {
            console.warn('⚠️ 匹配房间部分按钮缺失，功能可能不完整');
        }
        ...
        if (backBtn) backBtn.onclick = hideRoom;
        if (startBtn) startBtn.onclick = startMatch;
        if (cancelBtn) cancelBtn.onclick = cancelMatch;
