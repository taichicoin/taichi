// ========== 大厅UI交互（强制设置用户名版，透明按钮菜单） ==========
window.YYCardProfileUI = {
    _dropdown: null,
    _currentModal: null,
    _fileInput: null,
    _avatarClickTarget: null, // 覆盖在头像上的透明按钮
    ...
    _toggleDropdown(avatarEl) {
        ...
        // 根据是否在 Telegram 中决定是否显示“登出”项
        const isTelegram = window.Telegram?.WebApp?.initData;
        const items = [
            { action: 'change-username', label: '🔤 修改游戏ID' },
            { action: 'bind-wallet', label: '💳 绑定钱包' },
            { action: 'change-avatar', label: '🖼️ 修改头像' }
        ];
        if (!isTelegram) {
            items.push({ action: 'logout', label: '🚪 登出' });
        }
...
console.log('✅ 头像菜单已修复，透明按钮覆盖，强制用户名设置已生效');
