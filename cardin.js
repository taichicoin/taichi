...
window.YYCardInspector = (function() {
    ...
    function showCardDetail(card, boardIndex, element, isOwn) {
        ...
        // 生成每个槽的HTML —— 缩小上下高度至约60%
        function renderSlot(slotKey, equip, label) {
            if (equip) {
                return `
                    <div class="equip-row" style="display:flex; justify-content:space-between; align-items:center; background:rgba(255,255,255,0.1); padding:3px 6px; border-radius:6px; margin:2px 0;">
                        <div style="display:flex; align-items:center; gap:6px;">
                            <img src="${equip.image || '/assets/default-avatar.png'}" style="width:22px; height:22px; border-radius:4px;" onerror="this.src='/assets/default-avatar.png'">
                            <div>
                        ...
                        ${isOwn ? `<button class="unequip-btn" data-slot="${slotKey}" style="background:#c44; color:#fff; border:none; border-radius:4px; padding:1px 8px; font-size:11px; cursor:pointer;">✕</button>` : ''}
                    </div>
                `;
            } else {
                return `
                    <div class="equip-row empty-slot" style="background:rgba(255,255,255,0.05); padding:3px 6px; border-radius:6px; margin:2px 0; color:#999; font-size:11px;">
                        ${label}待装配
                    </div>
                `;
            }
        ...
        Object.assign(popupEl.style, {
            ...
            padding: '12px',          // 整体内边距缩小（原20px → 12px）
            zIndex: '100002',
