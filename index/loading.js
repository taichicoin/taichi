window.YYCardLoading = (function() {
    ...
    async function start(supabase, online) {
        ...
        const MIN_DURATION = 5000; // 至少 5 秒加载动画
        let onlineCheckDone = false;
        let queueTipActive = false;
        let tipIndex = 0;

        // 进度条：根据已用时间平滑增长，5 秒正好到 100%
        ...
        // 淡出并跳转（★ 带上 loaded=1 标记，让 game.html 放行）
        overlay.style.transition = "0.8s";
        overlay.style.opacity = "0";

        await new Promise(r => setTimeout(r, 800));
        window.location.replace("game.html?loaded=1");
    }
